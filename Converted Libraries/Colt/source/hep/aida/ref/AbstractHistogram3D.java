...
abstract class AbstractHistogram3D extends Histogram implements IHistogram3D {
  ...
  public int allEntries(){
    int n=0;
    for (int i=xAxis.bins(); --i >= -2; )     for (int j=yAxis.bins(); --j >= -2; )     for (int k=zAxis.bins(); --k >= -2; ) {
      n+=binEntries(i,j,k);
    }
    return n;
  }
  ...
  public int entries(){
    int n=0;
    for (int i=0; i < xAxis.bins(); i++)     for (int j=0; j < yAxis.bins(); j++)     for (int k=0; k < zAxis.bins(); k++) {
      n+=binEntries(i,j,k);
    }
    return n;
  }
