 ...
 */
  public void decompose(  DoubleMatrix2D A){
    ...
    cern.colt.list.IntArrayList nonZeroIndexes=new cern.colt.list.IntArrayList();
    DoubleMatrix1D LUcolj=LU.viewColumn(0).like();
    cern.jet.math.Mult multFunction=cern.jet.math.Mult.mult(0);
    for (int j=0; j < n; j++) {
      LUcolj.assign(LU.viewColumn(j));
      int maxCardinality=m / CUT_OFF;
 ...
 */
  protected boolean isNonsingular(  DoubleMatrix2D matrix){
    ...
    double epsilon=algebra.property().tolerance();
 ...
 */
  public void solve(  DoubleMatrix2D B){
    ...
    cern.colt.list.IntArrayList nonZeroIndexes=new cern.colt.list.IntArrayList();
    DoubleMatrix1D Browk=cern.colt.matrix.DoubleFactory1D.dense.make(nx);
    for (int k=0; k < n; k++) {
      Browk.assign(Brows[k]);
      int maxCardinality=nx / CUT_OFF;
