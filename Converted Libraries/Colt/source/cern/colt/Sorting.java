 ...
 */
  ...
  private static void mergeSort1(  byte src[],  byte dest[],  int low,  int high){
    int length=high - low;
    if (length < SMALL) {
      for (int i=low; i < high; i++)       for (int j=i; j > low && dest[j - 1] > dest[j]; j--)       swap(dest,j,j - 1);
      return;
    }
    int mid=(low + high) / 2;
  ...
  private static void mergeSort2(  double a[],  int fromIndex,  int toIndex){
    ...
    if (numNegZeros != 0) {
      int j=new cern.colt.list.DoubleArrayList(a).binarySearchFromTo(0.0d,fromIndex,n - 1);
      do {
        j--;
      }
 while (j >= 0 && a[j] == 0.0d);
  ...
  private static void mergeSort2(  float a[],  int fromIndex,  int toIndex){
    ...
    mergeSort1(aux,a,fromIndex,n);
    if (numNegZeros != 0) {
      int j=new cern.colt.list.FloatArrayList(a).binarySearchFromTo(0.0f,fromIndex,n - 1);
      do {
        j--;
      }
 while (j >= 0 && a[j] == 0.0f);
 ...
 */
  private static void quickSort1(  Object x[],  int off,  int len){
    if (len < SMALL) {
      for (int i=off; i < len + off; i++)       for (int j=i; j > off && ((Comparable)x[j - 1]).compareTo((Comparable)x[j]) > 0; j--)       swap(x,j,j - 1);
      return;
    }
    int m=off + len / 2;
    if (len > SMALL) {
