...
public final class SampleSet implements Access1D<Double> {
  public static SampleSet make(  final RandomNumber aRndmNmbr,  final int aSize){
    final SimpleArray.Primitive retVal=SimpleArray.makePrimitive(aSize);
  ...
  @SuppressWarnings("unused") private SampleSet(){
    this(null);
  ...
  public double getCovariance(  final SampleSet aSet){
    ...
    final int tmpCount=Math.min(mySamples.size(),aSet.size());
 ...
 */
  ...
  public double getLast(){
    return mySamples.doubleValue(mySamples.size() - 1);
  }
  /** 
 * max(value)
 */
  ...
  public double getMedian(){
    if (Double.isNaN(myMedian)) {
      ...
      Arrays.sort(tmpCopy);
      myMedian=tmpCopy[mySamples.size() / 2];
    }
    return myMedian;
  }
  /** 
 ...
 */
  public double getSumOfSquares(){
    ...
    double tmpVal;
    final int tmpLimit=mySamples.size();
 ...
 */
  ...
  @Override public String toString(){
    return "Sample set size: " + this.size() + ", Mean: "+ this.getMean()+ ", Median: "+ this.getMedian()+ ", Variance: "+ this.getVariance()+ ", Standard Deviation: "+ this.getStandardDeviation()+ ", Minimum: "+ this.getMinimum()+ ", Maximum: "+ this.getMaximum();
  }
  Access1D<?> getSamples(){
    return mySamples;
  }
