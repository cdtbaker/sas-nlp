...
public class BigPolynomial extends AbstractPolynomial<BigDecimal> {
  ...
  @Override protected BigDecimal getDerivativeFactor(  final int aPower){
    final int tmpNextIndex=aPower + 1;
    return this.get(tmpNextIndex).multiply(new BigDecimal(tmpNextIndex));
  }
  @Override protected BigDecimal getPrimitiveFactor(  final int aPower){
    if (aPower <= 0) {
      return BigMath.ZERO;
    }
 else {
  ...
  @Override protected AbstractPolynomial<BigDecimal> makeInstance(  final int aSize){
