...
static final class Big extends TridiagonalDecomposition<BigDecimal> {
    ...
    @Override Array1D<BigDecimal> makeReal(    final DiagonalAccess<BigDecimal> aDiagonalAccessD){
      return null;
    }
  }
static final class Complex extends TridiagonalDecomposition<ComplexNumber> {
    ...
    @Override Array1D<ComplexNumber> makeReal(    final DiagonalAccess<ComplexNumber> aDiagonalAccessD){
      final Array1D<ComplexNumber> retVal=Array1D.COMPLEX.makeZero(aDiagonalAccessD.getMinDim());
      retVal.fillAll(ComplexNumber.ONE);
      final Array1D<ComplexNumber> tmpSubdiagonal=aDiagonalAccessD.subdiagonal;
...
static final class Primitive extends TridiagonalDecomposition<Double> {
    ...
    @Override Array1D<Double> makeReal(    final DiagonalAccess<Double> aDiagonalAccessD){
      return null;
    }
  }
  @SuppressWarnings("unchecked") public static final <N extends Number>Tridiagonal<N> make(  final Access2D<N> aTypical){
  ...
  public final boolean compute(  final Access2D<?> matrix){
    ...
    try {
      final int tmpRowDim=matrix.getRowDim();
      ...
      final Array1D<N> tmpSub=tmpArray2D.sliceDiagonal(1,0).copy();
  ...
  protected final DecompositionStore<N> makeQ(){
    ...
    final int tmpDim=retVal.getMinDim();
