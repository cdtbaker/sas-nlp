...
static final class Big extends BidiagonalDecomposition<BigDecimal> {
    ...
    @Override Array1D<BigDecimal>[] makeReal(){
      return null;
    }
  }
static final class Complex extends BidiagonalDecomposition<ComplexNumber> {
    ...
    @Override Array1D<ComplexNumber>[] makeReal(){
      ...
      if (tmpUpper) {
        ...
        final int tmpLimit=tmpSuperdiagonal.length;
        ...
        final int tmpLimit=tmpSubdiagonal.length;
        for (int i=0; i < tmpLimit; i++) {
          if (!tmpMainDiagonal.isReal(i)) {
            final ComplexNumber tmpSignum=tmpMainDiagonal.get(i).signum();
            tmpMainDiagonal.set(i,tmpMainDiagonal.get(i).divide(tmpSignum));
...
static final class Primitive extends BidiagonalDecomposition<Double> {
    ...
    @Override Array1D<Double>[] makeReal(){
      return null;
    }
  }
  @SuppressWarnings("unchecked") public static final <N extends Number>Bidiagonal<N> make(  final Access2D<N> aTypical){
    ...
    for (int j=0; j < tmpDim; j++) {
      ...
      if (TypeUtils.isZero(tmpSingular)) {
        ...
        for (int i=0; i < tmpLim; i++) {
          aMtrxV.set(i,j,((aMtrxSimilar.doubleValue(i,i) * aMtrxV.doubleValue(i,j)) + (aMtrxSimilar.doubleValue(i,i + 1) * aMtrxV.doubleValue(i + 1,j))) / tmpSingular);
        }
  ...
  private final DecompositionStore<N> solve2(  final PhysicalStore<N> aMtrxV,  final MatrixStore<N> aMtrxD,  final DiagonalAccess<N> aMtrxSimilar){
    ...
    for (int j=0; j < tmpDim; j++) {
      ...
      if (TypeUtils.isZero(tmpSingular)) {
        ...
        for (int i=0; i < tmpLim; i++) {
          retVal.set(i,j,((aMtrxSimilar.doubleValue(i,i) * aMtrxV.doubleValue(i,j)) + (aMtrxSimilar.doubleValue(i,i + 1) * aMtrxV.doubleValue(i + 1,j))) / tmpSingular);
        }
