...
public static final class RowColumn implements Comparable<RowColumn> {
    ...
    @SuppressWarnings("unused") private RowColumn(){
      this(-1,-1);
    }
  ...
  @SuppressWarnings("unused") private Expression(  final Expression entityToCopy){
    this(entityToCopy,null,false);
    ProgrammingError.throwForIllegalInvocation();
  }
  @SuppressWarnings("unused") private Expression(  final String aName){
    this(aName,null);
    ProgrammingError.throwForIllegalInvocation();
  }
 ...
 */
  ...
  public void setQuadraticFactors(  final List<Variable> variables,  final Access2D<?> factors){
    final int tmpLimit=variables.size();
    if ((factors.getRowDim() != tmpLimit) || (factors.getColDim() != tmpLimit)) {
      throw new IllegalArgumentException();
    }
  ...
  @Override protected int getAdjustmentExponent(){
 ...
 */
  boolean simplify(){
    ...
    if (!tmpInitiallyRedundant && (this.countQuadraticFactors() == 0) && (this.countLinearFactors() <= (tmpFixedVariables.size() + 1))) {
      final BigDecimal tmpFixedValue=this.calculateFixedValue(tmpFixedVariables);
