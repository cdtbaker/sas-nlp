...
public static final class Builder extends AbstractBuilder<LinearSolver.Builder,LinearSolver> {
    ...
    @Override public LinearSolver build(    final Optimisation.Options options){
      final ExpressionsBasedModel tmpModel=this.getModel();
      return new SimplexTableauSolver(tmpModel,options,this);
    }
    @Override public Builder equalities(    final MatrixStore<Double> AE,    final MatrixStore<Double> BE){
      return super.equalities(AE,BE);
    }
    @Override protected Builder objective(    final MatrixStore<Double> C){
  ...
  static void copy(  final ExpressionsBasedModel sourceModel,  final LinearSolver.Builder destinationBuilder){
    ...
    final Optimisation.Result tmpKickStarter=new Optimisation.Result(Optimisation.State.UNEXPLORED,Double.NaN,ZeroStore.makePrimitive(tmpTotalVarCount,1));
  ...
  protected LinearSolver(  final ExpressionsBasedModel aModel,  final Optimisation.Options solverOptions,  final BaseSolver.AbstractBuilder<LinearSolver.Builder,LinearSolver> matrices){
