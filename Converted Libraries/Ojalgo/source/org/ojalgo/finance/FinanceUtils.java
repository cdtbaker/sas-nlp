...
public abstract class FinanceUtils {
  ...
  public static GeometricBrownianMotion estimateExcessDiffusionProcess(  final CalendarDateSeries<?> aPriceSeries,  final CalendarDateSeries<?> aRiskFreeInterestRateSeries,  final CalendarDateUnit aTimeUnit){
  ...
  public static CalendarDateSeries<RandomNumber> forecast(  final CalendarDateSeries<? extends Number> aSeries,  final int aPointCount,  final CalendarDateUnit aTimeUnit,  final boolean includeOriginalSeries){
  ...
  public static CalendarDateSeries<BigDecimal> makeCalendarPriceSeries(  final double[] somePrices,  final Calendar aStartCalendar,  final CalendarDateUnit aResolution){
 ...
 * @return A sample set of price growth rates adjusted for risk free return
 */
  public static CalendarDateSeries<Double> makeNormalisedExcessPrice(  final CalendarDateSeries<?> aPriceSeries,  final CalendarDateSeries<?> aRiskFreeInterestRateSeries){
    if (aPriceSeries.size() != aRiskFreeInterestRateSeries.size()) {
      throw new IllegalArgumentException("The two series must have the same size (number of elements).");
    }
    if (!aPriceSeries.firstKey().equals(aRiskFreeInterestRateSeries.firstKey())) {
 ...
 */
  ...
  private static <K extends Comparable<K>>void copyValues(  final CalendarDateSeries<BigDecimal> aSeries,  final CalendarDate aFirstKey,  final double[] someValues){
