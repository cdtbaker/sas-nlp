 ...
 */
    void setPrivateKey(    RSAPrivateCrtKey key) throws InvalidKeyException, KeyStoreException {
      byte[] modulusBytes=key.getModulus().toByteArray();
      int keyBitLength=(modulusBytes[0] == 0) ? (modulusBytes.length - 1) * 8 : modulusBytes.length * 8;
      byte[] keyBlob=generatePrivateKeyBlob(keyBitLength,modulusBytes,key.getPublicExponent().toByteArray(),key.getPrivateExponent().toByteArray(),key.getPrimeP().toByteArray(),key.getPrimeQ().toByteArray(),key.getPrimeExponentP().toByteArray(),key.getPrimeExponentQ().toByteArray(),key.getCrtCoefficient().toByteArray());
      privateKey=storePrivateKey(keyBlob,"{" + UUID.randomUUID().toString() + "}",keyBitLength);
    }
    /** 
    ...
    void setCertificateChain(    X509Certificate[] chain) throws CertificateException, KeyStoreException {
      for (int i=0; i < chain.length; i++) {
        ...
        if (i == 0 && privateKey != null) {
          storeCertificate(getName(),alias,encoding,encoding.length,privateKey.getHCryptProvider(),privateKey.getHCryptKey());
        }
 else {
          storeCertificate(getName(),alias,encoding,encoding.length,0L,0L);
        }
      }
      certChain=chain;
    }
 ...
 */
  public java.security.Key engineGetKey(  String alias,  char[] password) throws NoSuchAlgorithmException, UnrecoverableKeyException {
    ...
    if (engineIsKeyEntry(alias) == false)     return null;
      ...
      if (alias.equals(entry.getAlias())) {
        ...
        if (certChain != null) {
          try {
            ...
            removeCertificate(getName(),alias,encoding,encoding.length);
          }
        ...
        if (privateKey != null) {
          destroyKeyContainer(Key.getContainerName(privateKey.getHCryptProvider()));
        }
 ...
 */
  private native void loadKeysOrCertificateChains(  String name,  Collection<KeyEntry> entries) throws KeyStoreException ;
  /** 
 ...
 */
  private native byte[] generatePrivateKeyBlob(  int keyBitLength,  byte[] modulus,  byte[] publicExponent,  byte[] privateExponent,  byte[] primeP,  byte[] primeQ,  byte[] exponentP,  byte[] exponentQ,  byte[] crtCoefficient) throws InvalidKeyException ;
  private native RSAPrivateKey storePrivateKey(  byte[] keyBlob,  String keyContainerName,  int keySize) throws KeyStoreException ;
}
