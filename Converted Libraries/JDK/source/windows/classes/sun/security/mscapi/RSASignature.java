...
public static final class Raw extends RSASignature {
    ...
    @Override protected void engineUpdate(    ByteBuffer byteBuffer){
    ...
    @Override protected void resetDigest(){
      offset=0;
    }
    @Override protected byte[] getDigestValue() throws SignatureException {
...
public static final class MD2 extends RSASignature {
  ...
  protected void engineInitVerify(  PublicKey key) throws InvalidKeyException {
    ...
    if ((key instanceof sun.security.mscapi.RSAPublicKey) == false) {
      ...
      byte[] keyBlob=generatePublicKeyBlob(keyBitLength,modulusBytes,exponentBytes);
      try {
  ...
  protected void engineInitSign(  PrivateKey key) throws InvalidKeyException {
    ...
    RSAKeyFactory.checkKeyLengths(((privateKey.bitLength() + 7) & ~7),null,RSAKeyPairGenerator.KEY_SIZE_MIN,RSAKeyPairGenerator.KEY_SIZE_MAX);
    this.publicKey=null;
    resetDigest();
  }
  /** 
 ...
 */
  private byte[] convertEndianArray(  byte[] byteArray){
    if (byteArray == null || byteArray.length == 0)     return byteArray;
    byte[] retval=new byte[byteArray.length];
    for (int i=0; i < byteArray.length; i++)     retval[i]=byteArray[byteArray.length - i - 1];
    return retval;
  }
  /** 
 ...
 */
  protected boolean engineVerify(  byte[] sigBytes) throws SignatureException {
    byte[] hash=getDigestValue();
    return verifySignedHash(hash,hash.length,messageDigestAlgorithm,convertEndianArray(sigBytes),sigBytes.length,publicKey.getHCryptProvider(),publicKey.getHCryptKey());
  }
  /** 
 ...
 * @deprecated Replaced by {@link #engineSetParameter(java.security.spec.AlgorithmParameterSpec)engineSetParameter}.
 */
 ...
 */
  static native byte[] generatePublicKeyBlob(  int keyBitLength,  byte[] modulus,  byte[] publicExponent) throws InvalidKeyException ;
  /** 
 * Imports a public-key BLOB.
 */
  static native RSAPublicKey importPublicKey(  byte[] keyBlob,  int keySize) throws KeyStoreException ;
}
