...
class KinitOptions {
    ...
    for (int i=0; i < args.length; i++) {
      if (args[i].equals("-f")) {
        ...
        if (principal.getRealm() == null) {
          String realm=Config.getInstance().getDefault("default_realm","libdefaults");
          if (realm != null) {
            principal.setRealm(realm);
          }
 else           throw new IllegalArgumentException("invalid " + "Realm name");
        }
      }
  ...
  PrincipalName getDefaultPrincipal(){
    ...
    try {
      ...
      System.out.println("Can not get default realm " + e.getMessage());
      e.printStackTrace();
      return null;
    }
    try {
      CCacheInputStream cis=new CCacheInputStream(new FileInputStream(cachename));
      int version;
      if ((version=cis.readVersion()) == FileCCacheConstants.KRB5_FCC_FVNO_4) {
        cis.readTag();
      }
 else {
        if (version == FileCCacheConstants.KRB5_FCC_FVNO_1 || version == FileCCacheConstants.KRB5_FCC_FVNO_2) {
    ...
    if (realm != null) {
      try {
        ...
        if (p.getRealm() == null)         p.setRealm(realm);
        return p;
      }
 catch (      RealmException e) {
        if (DEBUG) {
