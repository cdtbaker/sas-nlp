  ...
  static WindowsFileAttributes get(  WindowsPath path,  boolean followLinks) throws WindowsException {
    if (!ensureAccurateMetadata) {
      ...
      try {
        ...
        int fileAttrs=unsafe.getInt(address + OFFSETOF_FILE_ATTRIBUTE_DATA_ATTRIBUTES);
        if (!isReparsePoint(fileAttrs))         return fromFileAttributeData(address,0);
      }
 catch (      WindowsException x) {
        if (x.lastError() != ERROR_SHARING_VIOLATION)         throw x;
        firstException=x;
      }
 finally {
        buffer.release();
      ...
      if (firstException != null) {
        ...
        if (last == ':' || last == '\\')         throw firstException;
        ...
        try {
          ...
          if (attrs.isReparsePoint())           throw firstException;
          return attrs;
        }
 ...
 */
  ...
  int volSerialNumber(){
    if (volSerialNumber == 0)     throw new AssertionError("Should not get here");
