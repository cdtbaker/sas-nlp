 ...
 */
  private static URI toUri(  String path,  boolean isUnc,  boolean addSlash){
    ...
    try {
      ...
      if (!isUnc)       throw new AssertionError(x);
    }
    uriPath="//" + path.replace('\\','/');
    if (addSlash)     uriPath+="/";
    try {
 ...
 */
  static WindowsPath fromUri(  WindowsFileSystem fs,  URI uri){
    ...
    if (path.equals(""))     throw new IllegalArgumentException("URI path component is empty");
    String auth=uri.getAuthority();
    if (auth != null && !auth.equals("")) {
      String host=uri.getHost();
      if (host == null)       throw new IllegalArgumentException("URI authority component has undefined host");
      if (uri.getUserInfo() != null)       throw new IllegalArgumentException("URI authority component has user-info");
      if (uri.getPort() != -1)       throw new IllegalArgumentException("URI authority component has port number");
      if (host.startsWith("[")) {
