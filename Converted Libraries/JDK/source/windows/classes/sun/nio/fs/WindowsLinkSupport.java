 ...
 */
  private static String getFinalPath(  WindowsPath input) throws IOException {
    ...
    try {
      ...
      if (x.lastError() != ERROR_INVALID_LEVEL)       x.rethrowAsIOException(input);
    }
 finally {
      CloseHandle(h);
    }
 ...
 */
  static String getFinalPath(  WindowsPath input,  boolean followLinks) throws IOException {
    ...
    try {
      if (!followLinks || !fs.supportsLinks())       return input.getPathForWin32Calls();
    ...
    do {
      ...
      WindowsPath link=WindowsPath.createFromNormalizedPath(fs,readLink(target));
      ...
      if (parent == null) {
        ...
        target=AccessController.doPrivileged(new PrivilegedAction<WindowsPath>(){
          @Override public WindowsPath run(){
 ...
 */
  static String getRealPath(  WindowsPath input,  boolean resolveLinks) throws IOException {
    WindowsFileSystem fs=input.getFileSystem();
    if (resolveLinks && !fs.supportsLinks())     resolveLinks=false;
    String path=null;
    try {
    ...
    while (curr < path.length()) {
      ...
      try {
        ...
        if (resolveLinks && WindowsFileAttributes.isReparsePoint(fileData.attributes())) {
          String result=getFinalPath(input);
          if (result == null) {
            WindowsPath resolved=resolveAllLinks(WindowsPath.createFromNormalizedPath(fs,path));
  ...
  private static String readLinkImpl(  long handle) throws IOException {
    ...
    try {
      try {
        ...
        if (x.lastError() == ERROR_NOT_A_REPARSE_POINT)         throw new NotLinkException(null,null,x.errorString());
      ...
      if ((nameLengthInBytes % 2) != 0)       throw new FileSystemException(null,null,"Symbolic link corrupted");
 ...
 */
  private static WindowsPath resolveAllLinks(  WindowsPath path) throws IOException {
    ...
    while (elem < path.getNameCount()) {
      ...
      if (attrs.isSymbolicLink()) {
        linkCount++;
        if (linkCount > 32)         throw new IOException("Too many links");
        WindowsPath target=WindowsPath.createFromNormalizedPath(fs,readLink(current));
