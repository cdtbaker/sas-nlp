...
class WindowsFileSystem extends FileSystem {
  ...
  WindowsFileSystem(  WindowsFileSystemProvider provider,  String dir){
    ...
    if ((result.type() != WindowsPathType.ABSOLUTE) && (result.type() != WindowsPathType.UNC))     throw new AssertionError("Default directory is not an absolute path");
...
private static class LookupService {
    static final UserPrincipalLookupService instance=new UserPrincipalLookupService(){
      ...
      @Override public GroupPrincipal lookupPrincipalByGroupName(      String group) throws IOException {
        ...
        if (!(user instanceof GroupPrincipal))         throw new UserPrincipalNotFoundException(group);
