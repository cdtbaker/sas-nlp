...
private static class WindowsPathWithAttributes extends WindowsPath implements BasicFileAttributesHolder {
    ...
    @Override public BasicFileAttributes get(){
      return ref.get();
    }
  ...
  String getPathForWin32Calls() throws WindowsException {
    if (isAbsolute() && path.length() <= MAX_PATH)     return path;
...
synchronized (path) {
  ...
  private String getAbsolutePath() throws WindowsException {
    ...
    if (type == WindowsPathType.RELATIVE) {
      ...
      if (isEmpty())       return defaultDirectory;
      if (defaultDirectory.endsWith("\\")) {
        return defaultDirectory + path;
      }
 else {
        StringBuilder sb=new StringBuilder(defaultDirectory.length() + path.length() + 1);
    ...
    if (isSameDrive(root,getFileSystem().defaultRoot())) {
      ...
      try {
        ...
        if (dt == DRIVE_UNKNOWN || dt == DRIVE_NO_ROOT_DIR)         throw new WindowsException("");
      ...
      if (wd.endsWith("\\")) {
        ...
        if (path.length() > root.length())         result+="\\" + path.substring(root.length());
      }
      return result;
    }
  }
  ...
  @Override public WindowsFileSystem getFileSystem(){
    return fs;
  }
  ...
  @Override public Path getFileName(){
    ...
    if (root.length() == len)     return null;
    int off=path.lastIndexOf('\\');
    if (off < root.length())     off=root.length();
 else     off++;
    return new WindowsPath(getFileSystem(),WindowsPathType.RELATIVE,"",path.substring(off));
  }
  @Override public WindowsPath getParent(){
    if (root.length() == path.length())     return null;
    int off=path.lastIndexOf('\\');
    if (off < root.length())     return getRoot();
 else     return new WindowsPath(getFileSystem(),type,root,path.substring(0,off));
  }
  @Override public WindowsPath getRoot(){
  ...
  @Override public WindowsPath relativize(  Path obj){
    WindowsPath other=toWindowsPath(obj);
    if (this.equals(other))     return emptyPath();
    if (this.type != other.type)     throw new IllegalArgumentException("'other' is different type of Path");
    if (!this.root.equalsIgnoreCase(other.root))     throw new IllegalArgumentException("'other' has different root");
 ...
 while (prevRemaining > remaining);
  ...
  @Override public WindowsPath resolve(  Path obj){
    WindowsPath other=toWindowsPath(obj);
...
case DRIVE_RELATIVE:
{
    if (!root.endsWith("\\"))     return other;
    String thisRoot=root.substring(0,root.length() - 1);
    if (!thisRoot.equalsIgnoreCase(other.root))     return other;
...
if (start != off) list.add(start);
}
synchronized (this) {
if (offsets == null) offsets=list.toArray(new Integer[list.size()]);
}
}
}
...
initOffsets();
if (i == (offsets.length - 1)) return path.substring(offsets[i]);
return path.substring(offsets[i],offsets[i + 1] - 1);
}
@Override public WindowsPath getName(int index){
initOffsets();
...
@Override public WindowsPath subpath(int beginIndex,int endIndex){
initOffsets();
if (beginIndex < 0) throw new IllegalArgumentException();
if (beginIndex >= offsets.length) throw new IllegalArgumentException();
if (endIndex > offsets.length) throw new IllegalArgumentException();
...
@Override public boolean startsWith(Path obj){
if (!(Objects.requireNonNull(obj) instanceof WindowsPath)) return false;
...
if (!thisElement.equalsIgnoreCase(otherElement)) return false;
}
return true;
}
return false;
}
@Override public boolean endsWith(Path obj){
if (!(Objects.requireNonNull(obj) instanceof WindowsPath)) return false;
WindowsPath other=(WindowsPath)obj;
...
if (otherCount < thisCount) return false;
if (!this.root.equalsIgnoreCase(other.root)) return false;
}
...
@Override public int compareTo(Path obj){
...
if (!followLinks && getFileSystem().supportsLinks()) flags|=FILE_FLAG_OPEN_REPARSE_POINT;
return CreateFile(getPathForWin32Calls(),FILE_READ_ATTRIBUTES,(FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE),0L,OPEN_EXISTING,flags);
}
void checkRead(){
...
@Override public WindowsPath toAbsolutePath(){
...
@Override public WindowsPath toRealPath(LinkOption... options) throws IOException {
checkRead();
...
if (watcher == null) throw new NullPointerException();
if (!(watcher instanceof WindowsWatchService)) throw new ProviderMismatchException();
SecurityManager sm=System.getSecurityManager();
if (sm != null) {
...
sm.checkRead(s);
if (watchSubtree) sm.checkRead(s + "\\-");
}
