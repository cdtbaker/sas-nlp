...
class WindowsException extends Exception {
  ...
  private IOException translateToIOException(  String file,  String other){
    if (lastError() == 0)     return new IOException(errorString());
    if (lastError() == ERROR_FILE_NOT_FOUND || lastError() == ERROR_PATH_NOT_FOUND)     return new NoSuchFileException(file,other,null);
    if (lastError() == ERROR_FILE_EXISTS || lastError() == ERROR_ALREADY_EXISTS)     return new FileAlreadyExistsException(file,other,null);
    if (lastError() == ERROR_ACCESS_DENIED)     return new AccessDeniedException(file,other,null);
