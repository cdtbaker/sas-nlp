 ...
 */
  ...
  /** 
 * Parses the given input as a Windows path.
 * @param requireToNormalizeIndicates if the path requires to be normalized
 */
 ...
 */
  ...
  private static final int nextSlash(  String path,  int off,  int end){
    ...
    while (off < end && !isSlash(c=path.charAt(off))) {
      if (isInvalidPathChar(c))       throw new InvalidPathException(path,"Illegal character [" + c + "] in path",off);
      off++;
    }
    return off;
  }
