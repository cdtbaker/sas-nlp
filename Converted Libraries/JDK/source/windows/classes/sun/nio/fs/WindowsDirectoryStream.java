...
private class WindowsDirectoryIterator implements Iterator<Path> {
    ...
    private Path acceptEntry(    String s,    BasicFileAttributes attrs){
      ...
      Path entry=WindowsPath.createFromNormalizedPath(dir.getFileSystem(),s,attrs);
      try {
        if (filter.accept(entry))         return entry;
      }
    ...
    private Path readNextEntry(){
      if (first != null) {
        ...
        first=null;
        if (nextEntry != null)         return nextEntry;
      }
      for (; ; ) {
        String name=null;
...
synchronized (closeLock) {
          ...
          attrs=WindowsFileAttributes.fromFindData(findDataBuffer.address());
        }
