 ...
 */
  ...
  private void checkAccess(  WindowsPath file,  boolean checkRead,  boolean checkWrite){
    ...
    if (sm != null) {
      if (checkRead)       sm.checkRead(file.getPathForPermissionCheck());
      if (checkWrite)       sm.checkWrite(file.getPathForPermissionCheck());
  ...
  @Override public UserPrincipal getOwner() throws IOException {
    ...
    try {
      ...
      if (sidAddress == 0L)       throw new IOException("no owner");
  ...
  @Override public void setOwner(  UserPrincipal obj) throws IOException {
    if (obj == null)     throw new NullPointerException("'owner' is null");
    if (!(obj instanceof WindowsUserPrincipals.User))     throw new ProviderMismatchException();
