...
class WindowsUserDefinedFileAttributeView extends AbstractUserDefinedFileAttributeView {
  ...
  private List<String> listUsingBackupRead() throws IOException {
    ...
    try {
      ...
      handle=CreateFile(file.getPathForWin32Calls(),GENERIC_READ,FILE_SHARE_READ,OPEN_EXISTING,flags);
    }
 catch (    WindowsException x) {
      x.rethrowAsIOException(file);
    }
    ...
    try {
      ...
      try {
        for (; ; ) {
          BackupResult result=BackupRead(handle,address,SIZEOF_STREAM_HEADER,false,context);
          context=result.context();
          if (result.bytesTransferred() == 0)           break;
          ...
          if (nameSize > 0) {
            ...
            if (result.bytesTransferred() != nameSize)             break;
          }
          if (streamId == BACKUP_ALTERNATE_DATA) {
            ...
            if (segs.length == 3)             list.add(segs[1]);
          }
  ...
  @Override public int size(  String name) throws IOException {
    ...
    try {
      ...
      opts.add(READ);
      if (!followLinks)       opts.add(WindowsChannelFactory.OPEN_REPARSE_POINT);
      fc=WindowsChannelFactory.newFileChannel(join(file,name),null,opts,0L);
    }
 catch (    WindowsException x) {
    ...
    try {
      long size=fc.size();
      if (size > Integer.MAX_VALUE)       throw new ArithmeticException("Stream too large");
      return (int)size;
    }
  finally {
      fc.close();
  ...
  @Override public int read(  String name,  ByteBuffer dst) throws IOException {
    ...
    try {
      if (fc.size() > dst.remaining())       throw new IOException("Stream too large");
      int total=0;
  ...
  @Override public int write(  String name,  ByteBuffer src) throws IOException {
    ...
    try {
      ...
      try {
        named=WindowsChannelFactory.newFileChannel(join(file,name),null,opts,0L);
      }
 catch (      WindowsException x) {
        x.rethrowAsIOException(join(file.getPathForPermissionCheck(),name));
      }
