...
synchronized (this) {
      if (closed)       throw new AssertionError("Should not get here");
      long ov;
  ...
  @SuppressWarnings("unchecked") <V,A>PendingFuture<V,A> remove(  long overlapped){
synchronized (this) {
...
synchronized (this) {
      if (closed)       return;
      if (!pendingIoMap.isEmpty())       clearPendingIoMap();
  ...
  private void clearPendingIoMap(){
    ...
    closePending=false;
    if (pendingIoMap.isEmpty())     return;
