...
private final static class FdMap extends HashMap<Integer,MapEntry> {
    ...
    private MapEntry remove(    SelectionKeyImpl ski){
      ...
      MapEntry x=get(fd);
      if ((x != null) && (x.ski.channel == ski.channel))       return remove(fd);
      return null;
    }
  }
private final static class MapEntry {
  ...
  protected int doSelect(  long timeout) throws IOException {
    if (channelArray == null)     throw new ClosedSelectorException();
    ...
    try {
      ...
      if (threads.size() > 0)       finishLock.waitForHelperThreads();
    }
  finally {
      end();
    }
...
private final class FinishLock {
    ...
    private void checkForException() throws IOException {
      ...
      StringBuffer message=new StringBuffer("An exception occured" + " during the execution of select(): \n");
...
private final class SubSelector {
    ...
    private int processSelectedKeys(    long updateCount){
      ...
      numKeysUpdated+=processFDSet(updateCount,readFds,PollArrayWrapper.POLLIN,false);
      numKeysUpdated+=processFDSet(updateCount,writeFds,PollArrayWrapper.POLLCONN | PollArrayWrapper.POLLOUT,false);
      numKeysUpdated+=processFDSet(updateCount,exceptFds,PollArrayWrapper.POLLIN | PollArrayWrapper.POLLCONN | PollArrayWrapper.POLLOUT,true);
...
private final class SelectThread extends Thread {
    ...
    public void run(){
      while (true) {
        if (startLock.waitForStart(this))         return;
        try {
  ...
  private void adjustThreadsCount(){
    if (threadsCount > threads.size()) {
      ...
      for (int i=threads.size() - 1; i >= threadsCount; i--)       threads.remove(i).makeZombie();
    }
  }
...
synchronized (interruptLock) {
      if (interruptTriggered == false)       return;
