 ...
 */
  ...
  @Override Future<AsynchronousSocketChannel> implAccept(  Object attachment,  final CompletionHandler<AsynchronousSocketChannel,Object> handler){
    if (!isOpen()) {
    ...
    if (isAcceptKilled())     throw new RuntimeException("Accept not allowed due to cancellation");
    if (localAddress == null)     throw new NotYetBoundException();
    ...
    if (ioe != null) {
      if (handler == null)       return CompletedFuture.withFailure(ioe);
    ...
    if (!accepting.compareAndSet(false,true))     throw new AcceptPendingException();
