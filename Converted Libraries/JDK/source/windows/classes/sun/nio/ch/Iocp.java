...
synchronized (staleIoSet) {
  ...
  @Override final Object attachForeignChannel(  final Channel channel,  FileDescriptor fdObj) throws IOException {
...
,0L);
  ...
  @Override void closeAllChannels(){
 ...
 while (count > 0);
  ...
  @Override void executeOnHandlerTask(  Runnable task){
synchronized (this) {
  ...
  @Override void shutdownHandlerTasks(){
 ...
 */
  void disassociate(  int key){
    ...
    try {
      ...
      if (keyToChannel.isEmpty())       checkForShutdown=true;
    }
  finally {
      keyToChannelLock.writeLock().unlock();
    }
 ...
 */
  ...
  private static IOException translateErrorToIOException(  int error){
    ...
    if (msg == null)     msg="Unknown error: 0x0" + Integer.toHexString(error);
...
private class EventHandlerTask implements Runnable {
    public void run(){
      ...
      try {
        for (; ; ) {
          if (myGroupAndInvokeCount != null)           myGroupAndInvokeCount.resetInvokeCount();
          replaceMe=false;
          try {
