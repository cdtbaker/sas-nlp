 ...
 */
  ...
  private native boolean initFlipBackbuffer(  long pData,  long pPeerData,  int numbuffers,  int swapEffect,  int syncType);
...
static {
  ...
  protected D3DSurfaceData(  WComponentPeer peer,  D3DGraphicsConfig gc,  int width,  int height,  Image image,  ColorModel cm,  int numBackBuffers,  int swapEffect,  VSyncType vSyncType,  int type){
  ...
  @Override public SurfaceDataProxy makeProxyFor(  SurfaceData srcData){
    return D3DSurfaceDataProxy.createProxy(srcData,(D3DGraphicsConfig)graphicsDevice.getDefaultConfiguration());
  }
  /** 
 ...
 */
  public static D3DSurfaceData createData(  D3DGraphicsConfig gc,  int width,  int height,  ColorModel cm,  Image image,  int type){
    ...
    try {
      ret=new D3DSurfaceData(null,gc,width,height,image,cm,0,SWAP_DISCARD,VSYNC_DEFAULT,type);
    }
...
return graphicsDevice.isCapPresent(CAPS_LCD_SHADER) && sg2d.compositeState <= SunGraphics2D.COMP_ISCOPY && sg2d.paintState <= SunGraphics2D.PAINT_OPAQUECOLOR && sg2d.surfaceData.getTransparency() == Transparency.OPAQUE;
}
/** 
...
if ((sg2d.compositeState <= sg2d.COMP_ISCOPY && sg2d.paintState <= sg2d.PAINT_ALPHACOLOR) || (sg2d.compositeState == sg2d.COMP_ALPHA && sg2d.paintState <= sg2d.PAINT_ALPHACOLOR && (((AlphaComposite)sg2d.composite).getRule() == AlphaComposite.SRC_OVER)) || (sg2d.compositeState == sg2d.COMP_XOR && sg2d.paintState <= sg2d.PAINT_ALPHACOLOR)) {
...
@Override protected MaskFill getMaskFill(SunGraphics2D sg2d){
if (sg2d.paintState > sg2d.PAINT_ALPHACOLOR) {
...
@Override public boolean copyArea(SunGraphics2D sg2d,int x,int y,int w,int h,int dx,int dy){
...
static void swapBuffers(D3DSurfaceData sd,final int x1,final int y1,final int x2,final int y2){
 ...
 */
@Override public void setSurfaceLost(boolean lost){
...
super(peer,gc,peer.getBounds().width,peer.getBounds().height,null,peer.getColorModel(),1,SWAP_COPY,VSYNC_DEFAULT,WINDOW);
 ...
 */
@Override public SurfaceData getReplacement(){
 ...
 */
@Override public Object getDestination(){
