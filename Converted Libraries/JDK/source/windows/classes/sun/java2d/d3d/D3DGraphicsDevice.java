 ...
 */
  ...
  @Override protected void enterFullScreenExclusive(  final int screen,  WindowPeer wp){
  ...
  @Override protected void exitFullScreenExclusive(  final int screen,  WindowPeer w){
    if (fsStatus) {
      D3DRenderQueue rq=D3DRenderQueue.getInstance();
      rq.lock();
      try {
...
private static class D3DFSWindowAdapter extends WindowAdapter {
  ...
  @Override protected void addFSWindowListener(  Window w){
  ...
  @Override protected void removeFSWindowListener(  Window w){
  ...
  @Override protected DisplayMode getCurrentDisplayMode(  final int screen){
    D3DRenderQueue rq=D3DRenderQueue.getInstance();
    rq.lock();
    try {
class Result {
  ...
  @Override protected void configDisplayMode(  final int screen,  final WindowPeer w,  final int width,  final int height,  final int bitDepth,  final int refreshRate){
    if (!fsStatus) {
  ...
  private static native void enumDisplayModesNative(  int screen,  ArrayList modes);
  @Override protected void enumDisplayModes(  final int screen,  final ArrayList modes){
    D3DRenderQueue rq=D3DRenderQueue.getInstance();
    rq.lock();
    try {
      rq.flushAndInvokeNow(new Runnable(){
  ...
  @Override public int getAvailableAcceleratedMemory(){
    D3DRenderQueue rq=D3DRenderQueue.getInstance();
    rq.lock();
    try {
class Result {
  ...
  @Override public GraphicsConfiguration[] getConfigurations(){
    if (configs == null) {
  ...
  @Override public GraphicsConfiguration getDefaultConfiguration(){
    if (defaultConfig == null) {
      if (isD3DEnabledOnDevice()) {
...
class Result {
  ...
  @Override protected void invalidate(  int defaultScreen){
