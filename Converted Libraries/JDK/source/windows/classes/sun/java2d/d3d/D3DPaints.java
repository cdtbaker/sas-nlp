 ...
 */
    @Override boolean isPaintValid(    SunGraphics2D sg2d){
 ...
 * texture object
 */
    @Override public boolean isPaintValid(    SunGraphics2D sg2d){
...
private static class LinearGradient extends MultiGradient {
    ...
    @Override boolean isPaintValid(    SunGraphics2D sg2d){
      ...
      if (paint.getFractions().length == 2 && paint.getCycleMethod() != CycleMethod.REPEAT && paint.getColorSpace() != ColorSpaceType.LINEAR_RGB) {
        D3DSurfaceData dstData=(D3DSurfaceData)sg2d.surfaceData;
        D3DGraphicsDevice gd=(D3DGraphicsDevice)dstData.getDeviceConfiguration().getDevice();
        if (gd.isCapPresent(CAPS_LCD_SHADER)) {
          return true;
