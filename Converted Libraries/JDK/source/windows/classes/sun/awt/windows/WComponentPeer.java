 ...
 */
  ...
  private native int[] createPrintedPixels(  int srcX,  int srcY,  int srcW,  int srcH,  int alpha);
  public void print(  Graphics g){
    ...
    for (int startY=0; startY < totalH; startY+=hInc) {
      ...
      int[] pix=createPrintedPixels(0,startY,totalW,h,bgColor == null ? 255 : bgColor.getAlpha());
      if (pix != null) {
        BufferedImage bim=new BufferedImage(totalW,h,BufferedImage.TYPE_INT_ARGB);
...
if (target == e.getSource() && !((Component)target).isFocusOwner() && WKeyboardFocusManagerPeer.shouldFocusOnClick((Component)target)) {
...
int id=e.getID();
if ((e instanceof InputEvent) && !((InputEvent)e).isConsumed() && ((Component)target).isEnabled()) {
...
if (WKeyboardFocusManagerPeer.processSynchronousLightweightTransfer((Component)target,lightweightChild,temporary,focusedWindowChangeAllowed,time)) {
return true;
}
int result=WKeyboardFocusManagerPeer.shouldNativelyFocusHeavyweight((Component)target,lightweightChild,temporary,focusedWindowChangeAllowed,time,cause);
switch (result) {
...
return WKeyboardFocusManagerPeer.deliverFocus(lightweightChild,(Component)target,temporary,focusedWindowChangeAllowed,time,cause);
...
PaintEvent event=PaintEventDispatcher.getPaintEventDispatcher().createPaintEvent((Component)target,x,y,w,h);
if (event != null) {
postEvent(event);
}
}
...
postEvent(new PaintEvent((Component)target,PaintEvent.PAINT,new Rectangle()));
}
isLayouting=false;
}
 ...
 */
@Override public void createBuffers(int numBuffers,BufferCapabilities caps) throws AWTException {
Win32GraphicsConfig gc=(Win32GraphicsConfig)getGraphicsConfiguration();
...
@Override public void flip(int x1,int y1,int x2,int y2,BufferCapabilities.FlipContents flipAction){
...
@Override public synchronized Image getBackBuffer(){
...
boolean isTranslucent=SunToolkit.isContainingTopLevelTranslucent((Component)target);
...
native void setRectangularShape(int lox,int loy,int hix,int hiy,Region region);
...
if (shape != null) {
setRectangularShape(shape.getLoX(),shape.getLoY(),shape.getHiX(),shape.getHiY(),(shape.isRectangular() ? null : shape));
}
 else {
