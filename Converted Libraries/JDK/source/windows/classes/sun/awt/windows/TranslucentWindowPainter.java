...
private static class BIWindowPainter extends TranslucentWindowPainter {
    ...
    @Override protected Image getBackBuffer(    boolean clear){
    ...
    @Override protected boolean update(    Image bb){
...
private static class VIWindowPainter extends BIWindowPainter {
    ...
    @Override protected Image getBackBuffer(    boolean clear){
      ...
      int h=window.getHeight();
      GraphicsConfiguration gc=peer.getGraphicsConfiguration();
      if (viBB == null || viBB.getWidth() != w || viBB.getHeight() != h || viBB.validate(gc) == IMAGE_INCOMPATIBLE) {
        flush();
        if (gc instanceof AccelGraphicsConfig) {
          AccelGraphicsConfig agc=((AccelGraphicsConfig)gc);
          viBB=agc.createCompatibleVolatileImage(w,h,TRANSLUCENT,RT_PLAIN);
        }
        if (viBB == null) {
...
private static class VIOptD3DWindowPainter extends VIOptWindowPainter {
    ...
    @Override protected boolean updateWindowAccel(    long psdops,    int w,    int h){
      return sun.java2d.d3d.D3DSurfaceData.updateWindowAccelImpl(psdops,peer.getData(),w,h);
    }
  }
private static class VIOptWGLWindowPainter extends VIOptWindowPainter {
    ...
    @Override protected boolean updateWindowAccel(    long psdops,    int w,    int h){
      return sun.java2d.opengl.WGLSurfaceData.updateWindowAccelImpl(psdops,peer,w,h);
    }
  }
}
