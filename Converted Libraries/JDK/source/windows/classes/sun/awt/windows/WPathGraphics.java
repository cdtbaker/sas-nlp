...
static {
    String textLayoutStr=(String)java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction("sun.java2d.print.enableGDITextLayout"));
 ...
 */
  ...
  @Override protected int platformFontCount(  Font font,  String str){
  ...
  private boolean strNeedsTextLayout(  String str,  Font font){
    ...
    if (!isComplex) {
      ...
      if (preferGDITextLayout || (isXP() && FontUtilities.textLayoutIsCompatible(font))) {
        return false;
      }
 else {
        return true;
 ...
 * @see #setClip
 */
  @Override public void drawString(  String str,  float x,  float y,  Font font,  FontRenderContext frc,  float targetW){
    if (str.length() == 0) {
      return;
    }
    ...
    if (font2D instanceof TrueTypeFont) {
      textOut(str,font,(TrueTypeFont)font2D,frc,scaledFontSizeY,iangle,awScale,deviceTransform,scaleFactorX,x,y,devpos.x,devpos.y,targetW);
    }
      ...
      while (endChar < len) {
        ...
        textOut(substr,font,slotFont,frc,scaledFontSizeY,iangle,awScale,deviceTransform,scaleFactorX,userx,usery,devx,devy,0f);
 ...
 * this glyphvector
 */
  @Override protected boolean printGlyphVector(  GlyphVector gv,  float x,  float y){
    ...
    AffineTransform advanceTransform=new AffineTransform(deviceTransform);
  ...
  private void textOut(  String str,  Font font,  PhysicalFont font2D,  FontRenderContext frc,  float deviceSize,  int rotation,  float awScale,  AffineTransform deviceTransform,  double scaleFactorX,  float userx,  float usery,  float devx,  float devy,  float targetW){
    String family=font2D.getFamilyName(null);
    int style=font.getStyle() | font2D.getStyle();
    WPrinterJob wPrinterJob=(WPrinterJob)getPrinterJob();
