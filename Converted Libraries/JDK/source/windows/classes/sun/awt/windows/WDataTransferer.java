 ...
 */
  ...
  private static final DirectColorModel directColorModel=new DirectColorModel(24,0x00FF0000,0x0000FF00,0x000000FF);
  private static final int[] bandmasks=new int[]{directColorModel.getRedMask(),directColorModel.getGreenMask(),directColorModel.getBlueMask()};
  /** 
...
synchronized (WDataTransferer.class) {
  ...
  protected Object translateBytesOrStream(  InputStream str,  byte[] bytes,  DataFlavor flavor,  long format,  Transferable localeTransferable) throws IOException {
    ...
    return super.translateBytesOrStream(str,bytes,flavor,format,localeTransferable);
  }
  ...
  protected String getNativeForFormat(  long format){
    return (format < predefinedClipboardNames.length) ? predefinedClipboardNames[(int)format] : getClipboardFormatName(format);
  }
 ...
 */
  ...
  public boolean isImageFormat(  long format){
    return format == CF_DIB || format == CF_ENHMETAFILE || format == CF_METAFILEPICT || format == CF_PNG || format == CF_JFIF;
  }
 ...
 */
  private native int[] platformImageBytesToImageData(  byte[] bytes,  long format) throws IOException ;
...
enum EHTMLReadMode {HTML_READ_ALL, HTML_READ_FRAGMENT, HTML_READ_SELECTION}
/** 
    ...
    String htmlSuffix="";
{
    ...
    int nStartHTML=VERSION.length() + VERSION_NUM.length() + EOLN.length()+ START_HTML.length()+ PADDED_WIDTH+ EOLN.length()+ END_HTML.length()+ PADDED_WIDTH+ EOLN.length()+ START_FRAGMENT.length()+ PADDED_WIDTH+ EOLN.length()+ END_FRAGMENT.length()+ PADDED_WIDTH+ EOLN.length()+ SOURCE_URL.length()+ stBaseUrl.length()+ EOLN.length();
  ...
  private long iHTMLStart, iHTMLEnd, iFragStart, iFragEnd, iSelStart, iSelEnd;
  private String stBaseURL;
  private String stVersion;
  private long iStartOffset, iEndOffset, iReadCount;
 ...
 */
  private void parseDescription() throws IOException {
    stBaseURL=null;
    stVersion=null;
    iHTMLEnd=iHTMLStart=iFragEnd=iFragStart=iSelEnd=iSelStart=-1;
    ...
    BufferedReader bufferedReader=new BufferedReader(new InputStreamReader(bufferedStream,ENCODING),CHAR_BUFFER_LEN);
    long iHeadSize=0;
...
if (-1 == iHTMLStart) iHTMLStart=iHeadSize;
if (-1 == iFragStart) iFragStart=iHTMLStart;
if (-1 == iFragEnd) iFragEnd=iHTMLEnd;
if (-1 == iSelStart) iSelStart=iFragStart;
if (-1 == iSelEnd) iSelEnd=iFragEnd;
switch (readMode) {
case HTML_READ_ALL:
