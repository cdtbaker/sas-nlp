...
class WListPeer extends WComponentPeer implements ListPeer {
  ...
  public Dimension minimumSize(  int v){
    return new Dimension(20 + fm.stringWidth("0123456789abcde"),(fm.getHeight() * v) + 4);
  }
  ...
  void handleAction(  final int index,  final long when,  final int modifiers){
    ...
    WToolkit.executeOnEventHandlerThread(l,new Runnable(){
      public void run(){
        l.select(index);
        postEvent(new ActionEvent(target,ActionEvent.ACTION_PERFORMED,l.getItem(index),when,modifiers));
      }
    }
);
  }
  void handleListChanged(  final int index){
    ...
    WToolkit.executeOnEventHandlerThread(l,new Runnable(){
      public void run(){
        postEvent(new ItemEvent(l,ItemEvent.ITEM_STATE_CHANGED,Integer.valueOf(index),isSelected(index) ? ItemEvent.SELECTED : ItemEvent.DESELECTED));
      }
    }
);
  }
