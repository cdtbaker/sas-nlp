...
static {
  ...
  private static String printScale=(String)java.security.AccessController.doPrivileged(new GetPropertyAction("sun.java2d.print.pluginscalefactor"));
 ...
 */
  @Deprecated public WEmbeddedFrame(  int handle){
    this((long)handle);
  }
  ...
  void print(  long hdc){
    ...
    if (bandImage == null) {
      ...
      bandImage=new BufferedImage(imgWid,imgHgt,BufferedImage.TYPE_3BYTE_BGR);
    }
    ...
    for (int bandTop=0; bandTop < frameHeight; bandTop+=bandHeight) {
      ...
      printBand(hdc,data,imageOffset,0,0,imgWid,currImgHeight,0,bandTop,bandWidth,currBandHeight);
      g2d.translate(0,-bandHeight);
    }
  }
  protected static int getPrintScaleFactor(){
    if (pScale != 0)     return pScale;
    if (printScale == null) {
      printScale=(String)java.security.AccessController.doPrivileged(new java.security.PrivilegedAction(){
        public Object run(){
  ...
  protected native void printBand(  long hdc,  byte[] data,  int offset,  int sx,  int sy,  int swidth,  int sheight,  int dx,  int dy,  int dwidth,  int dheight);
  /** 
