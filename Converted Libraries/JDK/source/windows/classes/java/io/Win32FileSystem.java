...
class Win32FileSystem extends FileSystem {
  ...
  public int prefixLength(  String path){
    ...
    if (isLetter(c0) && (c1 == ':')) {
      if ((n > 2) && (path.charAt(2) == slash))       return 3;
      return 2;
    }
    return 0;
  }
  ...
  protected native String canonicalizeWithPrefix0(  String canonicalPrefix,  String pathWithCanonicalPrefix) throws IOException ;
  ...
  public File[] listRoots(){
    ...
    for (int i=0; i < 26; i++) {
      if (((ds >> i) & 1) != 0) {
        if (!access((char)('A' + i) + ":" + slash))         ds&=~(1 << i);
 else         n++;
      }
    }
    File[] fs=new File[n];
