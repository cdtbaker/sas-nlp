...
static {
  ...
  static Color makeAlphaColor(  Color opaque,  int alpha){
    try {
      opaque=new Color(opaque.getRed(),opaque.getGreen(),opaque.getBlue(),alpha);
    }
  ...
  public static void init(){
    ...
    animList=new Option.IntList(groptroot,"anim","Movement of rendering position",new int[]{0,1,2},new String[]{"static","slide","bounce"},new String[]{"No movement","Shift horizontal alignment","Bounce around window"},0x4);
    sizeList=new Option.IntList(groptroot,"sizes","Size of Operations to perform",new int[]{1,20,100,250,1000},new String[]{"1x1","20x20","100x100","250x250","1000x1000"},new String[]{"Tiny Shapes (1x1)","Small Shapes (20x20)","Medium Shapes (100x100)","Large Shapes (250x250)","X-Large Shapes (1000x1000)"},0xa);
    if (hasGraphics2D) {
      String rulenames[]={"Clear","Src","Dst","SrcOver","DstOver","SrcIn","DstIn","SrcOut","DstOut","SrcAtop","DstAtop","Xor"};
      ...
      compRules=new Option.ObjectList(groptroot,"alpharule","AlphaComposite Rule",j,rulenames,rules,rulenames,ruledescs,(1 << defrule));
      ((Option.ObjectList)compRules).setNumRows(4);
      Transform xforms[]={Identity.instance,FTranslate.instance,Scale2x2.instance,Rotate15.instance,ShearX.instance,ShearY.instance};
      ...
      transforms=new Option.ObjectList(groptroot,"transform","Affine Transform",xforms.length,xformnames,xforms,xformnames,xformdescs,0x1);
      ((Option.ObjectList)transforms).setNumRows(3);
      doExtraAlpha=new Option.Toggle(groptroot,"extraalpha","Render with an \"extra alpha\" of 0.125",Option.Toggle.Off);
      doXor=new Option.Toggle(groptroot,"xormode","Render in XOR mode",Option.Toggle.Off);
      doClipping=new Option.Toggle(groptroot,"clip","Render through a complex clip shape",Option.Toggle.Off);
      String rhintnames[]={"Default","Speed","Quality"};
      renderHint=new Option.ObjectList(groptroot,"renderhint","Rendering Hint",rhintnames,new Object[]{RenderingHints.VALUE_RENDER_DEFAULT,RenderingHints.VALUE_RENDER_SPEED,RenderingHints.VALUE_RENDER_QUALITY},rhintnames,rhintnames,1);
    }
  }
...
public static double scaleForPoint(AffineTransform at,double xorig,double yorig,double x,double y,int w,int h){
...
public static Dimension scaleForTransform(AffineTransform at,Dimension dim){
int w=dim.width;
int h=dim.height;
...
return new Dimension((int)Math.floor(w * scalex),(int)Math.floor(h * scaley));
}
}
