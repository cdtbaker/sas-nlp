...
abstract class InputImageTests extends InputTests {
  ...
  public static void init(){
    ...
    toolkitReadFormatList=new Option.ObjectList(toolkitOptRoot,"format","Image Format",tkFormats,tkFormats,tkFormats,tkFormats,0x0);
    ...
    if (hasImageIO) {
      ...
      initIIOReadFormats();
      imageioReadFormatList=new Option.ObjectList(imageioOptRoot,"format","Image Format",imageioReadFormatShortNames,imageioReaderSpis,imageioReadFormatShortNames,imageioReadFormatShortNames,0x0);
      imageioTestRoot=new Group(imageioRoot,"tests","Image I/O Tests");
      new ImageIORead();
      ...
      seekForwardOnlyTog=new Option.Toggle(imageReaderOptRoot,"seekForwardOnly","Seek Forward Only",Option.Toggle.On);
      ignoreMetadataTog=new Option.Toggle(imageReaderOptRoot,"ignoreMetadata","Ignore Metadata",Option.Toggle.On);
      installListenerTog=new Option.Toggle(imageReaderOptRoot,"installListener","Install Progress Listener",Option.Toggle.Off);
...
private static class Context extends InputTests.Context {
    ...
    Context(    TestEnvironment env,    Result result,    int testType){
      ...
      if (testType == TEST_IMAGEIO || testType == TEST_IMAGEREADER) {
        ImageReaderSpi readerspi=(ImageReaderSpi)env.getModifier(imageioReadFormatList);
        ...
        if (format.equals("wbmp")) {
          BufferedImage newimg=new BufferedImage(size,size,BufferedImage.TYPE_BYTE_BINARY);
...
super(imageReaderTestRoot,"read","ImageReader.read()");
...
super(imageReaderTestRoot,"getImageMetadata","ImageReader.getImageMetadata()");
addDependency(generalSourceRoot);
addDependencies(imageioGeneralOptRoot,true);
addDependencies(imageioOptRoot,true);
addDependencies(imageReaderOptRoot,true);
