...
public abstract class PixelTests extends Test {
  ...
  public static void init(){
    ...
    doRenderTo=new Option.Toggle(pixeloptroot,"renderto","Render to Image before test",Option.Toggle.Off);
    doRenderFrom=new Option.Toggle(pixeloptroot,"renderfrom","Render from Image before test",Option.Toggle.Off);
...
public static class BufImg extends Option.Enable {
    public static int rgbvals[]={0x00000000,0xff0000ff,0x8000ff00,0xffffffff};
    static int cmap[]={0xff000000,0xffffffff,0x00000000,0x80ffffff,0x00ffffff,0x80000000,0xff333333,0xff666666,0xff999999,0xffcccccc,0xff0000ff,0xff00ff00,0xff00ffff,0xffff0000,0xffff00ff,0xffffff00};
    int type;
    int nbits;
    public BufImg(    int type){
      super(bufimgsrcroot,Destinations.BufImg.ShortNames[type],Destinations.BufImg.Descriptions[type],false);
      this.type=type;
      this.nbits=0;
    }
    public BufImg(    int type,    int nbits){
      super(bufimgsrcroot,nbits + "BitBinary",nbits + "-bit Binary Image",false);
      this.type=type;
      this.nbits=nbits;
    }
    ...
    public BufferedImage getImage(){
      ...
      if (nbits == 0) {
        ...
        IndexColorModel icm=new IndexColorModel(nbits,(1 << nbits),cmap,0,(nbits > 1),-1,DataBuffer.TYPE_BYTE);
        WritableRaster wr=icm.createCompatibleWritableRaster(64 / nbits,1);
