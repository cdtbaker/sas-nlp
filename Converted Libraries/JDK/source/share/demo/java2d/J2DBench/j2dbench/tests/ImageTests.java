...
static {
  ...
  static String transNodeNames[]={null,"opaque","bitmask","translucent"};
  static String transDescriptions[]={null,"Opaque","Bitmask","Translucent"};
  public static void init(){
    ...
    doTouchSrc=new Option.Toggle(imgoptionsroot,"touchsrc","Touch src image before every operation",Option.Toggle.Off);
    ...
    if (hasGraphics2D) {
      ...
      imageOpRoot=new Group(imageroot,"imageops","Image Op Benchmarks");
      imageOpOptRoot=new Group(imageOpRoot,"opts","Options");
      imageOpTestRoot=new Group(imageOpRoot,"tests","Tests");
      graphicsTestRoot=new Group(imageOpTestRoot,"graphics2d","Graphics2D Tests");
      bufImgOpTestRoot=new Group(imageOpTestRoot,"bufimgop","BufferedImageOp Tests");
      rasterOpTestRoot=new Group(imageOpTestRoot,"rasterop","RasterOp Tests");
      ...
      opList=new Option.ObjectList(imageOpOptRoot,"op","Operation",opStrArr,opStrArr,opStrArr,opDescArr,0x1);
...
public abstract static class TriStateImageType extends Group {
    ...
    public Image getImage(    TestEnvironment env,    int w,    int h){
      if (theImage == null || theImage.getWidth(null) != w || theImage.getHeight(null) != h) {
...
public static class CompatImg extends TriStateImageType {
    ...
    public CompatImg(    int transparency){
      super(imgsrcroot,Destinations.CompatImg.ShortNames[transparency],Destinations.CompatImg.LongDescriptions[transparency],transparency);
...
public static class BufImg extends TriStateImageType {
    int type;
    boolean unmanaged;
    static int Transparencies[]={Transparency.TRANSLUCENT,Transparency.OPAQUE,Transparency.TRANSLUCENT,Transparency.TRANSLUCENT,Transparency.OPAQUE,Transparency.OPAQUE,Transparency.TRANSLUCENT,Transparency.TRANSLUCENT,Transparency.OPAQUE,Transparency.OPAQUE,Transparency.OPAQUE,Transparency.OPAQUE,Transparency.OPAQUE,Transparency.OPAQUE};
    public BufImg(    int type){
      this(type,false);
    }
    public BufImg(    int type,    boolean unmanaged){
      super(bufimgsrcroot,(unmanaged ? "unmanaged" : "") + Destinations.BufImg.ShortNames[type],(unmanaged ? "Unmanaged " : "") + Destinations.BufImg.Descriptions[type],Transparencies[type]);
      this.type=type;
...
public static class BmByteIndexBufImg extends TriStateImageType {
    ...
    public BmByteIndexBufImg(){
      super(bufimgsrcroot,"ByteIndexedBm","8-bit Transparent Indexed Image",Transparency.BITMASK);
    }
    public Image makeImage(    TestEnvironment env,    int w,    int h){
      if (icm == null) {
        ...
        icm=new IndexColorModel(8,256,cmap,0,true,255,DataBuffer.TYPE_BYTE);
      }
...
public static class DrawableImage extends Option.Enable {
    ...
    static Color colorsets[][]=new Color[][]{null,{Color.blue,Color.red,Color.green,Color.yellow,Color.blue},{transparentBlack,Color.red,Color.green,transparentBlack,transparentBlack},{Color.blue,translucentRed,translucentGreen,translucentYellow,translucentRed}};
...
public static class DrawImageScale extends ImageTests {
    ...
    public DrawImageScale(    String dir,    float scale){
      super(imgtestroot,"drawimagescale" + dir,"drawImage(img, x, y, w*" + scale + ", h*"+ scale+ ", obs);");
      this.scale=scale;
    }
    ...
    ImageOpTests(    Group parent,    String nodeName,    String desc){
      super(parent,nodeName,desc,new Modifier.Filter(){
        public boolean isCompatible(        Object val){
          ...
          return !(imgtype instanceof VolatileImg) && !(imgtype instanceof OffScreen);
        }
      }
);
    ...
    public void initContext(    TestEnvironment env,    GraphicsTests.Context ctx){
      ...
      if (op.startsWith("convolve")) {
        ...
        if (op.startsWith("convolve3x3")) {
          float[] data={0.1f,0.1f,0.1f,0.1f,0.2f,0.1f,0.1f,0.1f,0.1f};
          kernel=new Kernel(3,3,data);
        }
 else {
          float[] data={-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,24.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f,-1.0f};
          kernel=new Kernel(5,5,data);
        }
        int edge=op.endsWith("zero") ? ConvolveOp.EDGE_ZERO_FILL : ConvolveOp.EDGE_NO_OP;
...
private static class BufImgOpFilter extends ImageOpTests {
    ...
    BufImgOpFilter(    boolean cached){
      super(bufImgOpTestRoot,"filter" + (cached ? "cached" : "null"),"op.filter(srcBufImg, " + (cached ? "cachedCompatibleDestImg" : "null") + ");");
      this.cached=cached;
    }
    public void initContext(    TestEnvironment env,    GraphicsTests.Context ctx){
      super.initContext(env,ctx);
...
private static class RasterOpFilter extends ImageOpTests {
    ...
    RasterOpFilter(    boolean cached){
      super(rasterOpTestRoot,"filter" + (cached ? "cached" : "null"),"op.filter(srcRaster, " + (cached ? "cachedCompatibleDestRaster" : "null") + ");");
      this.cached=cached;
    }
    public void initContext(    TestEnvironment env,    GraphicsTests.Context ctx){
      super.initContext(env,ctx);
