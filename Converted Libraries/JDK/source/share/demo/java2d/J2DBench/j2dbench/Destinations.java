...
public abstract class Destinations extends Option.Enable {
  ...
  public static void init(){
    destroot=new Group.EnableSet(TestEnvironment.globaloptroot,"dest","Output Destination Options");
    new Screen();
    new OffScreen();
    if (GraphicsTests.hasGraphics2D) {
      if (ImageTests.hasCompatImage) {
        compatimgdestroot=new Group.EnableSet(destroot,"compatimg","Compatible Image Destinations");
  ...
  public Destinations(  Group parent,  String nodename,  String description,  boolean defenabled){
...
public static class CompatImg extends Destinations {
    int transparency;
    public static String ShortNames[]={"compatimg","opqcompatimg","bmcompatimg","transcompatimg"};
    public static String ShortDescriptions[]={"Default","Opaque","Bitmask","Translucent"};
    public static String LongDescriptions[]={"Default Compatible Image","Opaque Compatible Image","Bitmask Compatible Image","Translucent Compatible Image"};
    public static String ModifierNames[]={"CompatImage()","CompatImage(Opaque)","CompatImage(Bitmask)","CompatImage(Translucent)"};
    public CompatImg(){
      this(0);
    }
    public CompatImg(    int transparency){
      super(compatimgdestroot,ShortNames[transparency],ShortDescriptions[transparency],false);
...
public static class CustomImg extends Destinations {
    ...
    public void setDestination(    TestEnvironment env){
      if (img == null) {
        ...
        WritableRaster raster=cm.createCompatibleWritableRaster(env.getWidth(),env.getHeight());
