...
abstract class OutputImageTests extends OutputTests {
  ...
  public static void init(){
    ...
    if (hasImageIO) {
      ...
      initIIOWriteFormats();
      imageioWriteFormatList=new Option.ObjectList(imageioOptRoot,"format","Image Format",imageioWriteFormatShortNames,imageioWriterSpis,imageioWriteFormatShortNames,imageioWriteFormatShortNames,0x0);
      ...
      installListenerTog=new Option.Toggle(imageWriterOptRoot,"installListener","Install Progress Listener",Option.Toggle.Off);
...
private static class Context extends OutputTests.Context {
    ...
    Context(    TestEnvironment env,    Result result,    int testType){
      ...
      if (testType == TEST_IMAGEIO || testType == TEST_IMAGEWRITER) {
        ImageWriterSpi writerspi=(ImageWriterSpi)env.getModifier(imageioWriteFormatList);
...
super(imageWriterTestRoot,"write","ImageWriter.write()");
