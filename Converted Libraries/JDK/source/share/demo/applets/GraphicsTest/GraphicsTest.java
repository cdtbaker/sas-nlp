...
@SuppressWarnings("serial") class GraphicsPanel extends Panel {
  ActionListener al;
  ItemListener il;
...
@SuppressWarnings("serial") public class GraphicsTest extends Applet implements ActionListener, ItemListener {
...
@SuppressWarnings("serial") class GraphicsCards extends Panel {
...
@SuppressWarnings("serial") class ArcCard extends Panel {
  public ArcCard(){
...
@SuppressWarnings("serial") class ArcDegreePanel extends Panel {
  boolean filled;
  ...
  @Override public void paint(  Graphics g){
    ...
    arcSteps(g,2,r.width / 4,r.height / 4,r.width / 2,r.height / 2,Color.yellow,Color.green);
    arcSteps(g,1,(r.width * 3) / 8,(r.height * 3) / 8,r.width / 4,r.height / 4,Color.magenta,Color.white);
  }
}
@SuppressWarnings("serial") class ArcPanel extends Panel {
  boolean filled;
...
class RectShape extends Shape {
  @Override void draw(  Graphics g,  int x,  int y,  int w,  int h){
    g.drawRect(x,y,w,h);
  }
  @Override void fill(  Graphics g,  int x,  int y,  int w,  int h){
    g.fillRect(x,y,w,h);
  }
}
...
class PolygonShape extends Shape {
  ...
  void scalePolygon(  float w,  float h){
    ...
    for (int i=0; i < pBase.npoints; ++i) {
      p.addPoint((int)(pBase.xpoints[i] * w),(int)(pBase.ypoints[i] * h));
    }
  }
  @Override void draw(  Graphics g,  int x,  int y,  int w,  int h){
...
@SuppressWarnings("serial") class ShapeTest extends Panel {
  Shape shape;
  int step;
  ...
  @Override public void paint(  Graphics g){
    ...
    int cx, cy, cw, ch;
    Color color;
    for (color=Color.red, cx=bounds.x, cy=bounds.y, cw=bounds.width / 2, ch=bounds.height; cw > 0 && ch > 0; cx+=step, cy+=step, cw-=(step * 2), ch-=(step * 2), color=ColorUtils.darker(color,0.9)) {
      g.setColor(color);
      shape.draw(g,cx,cy,cw,ch);
    }
    for (cx=bounds.x + bounds.width / 2, cy=bounds.y, cw=bounds.width / 2, ch=bounds.height; cw > 0 && ch > 0; cx+=step, cy+=step, cw-=(step * 2), ch-=(step * 2)) {
...
class ColorUtils {
  static Color brighter(  Color c,  double factor){
    return new Color(Math.min((int)(c.getRed() * (1 / factor)),255),Math.min((int)(c.getGreen() * (1 / factor)),255),Math.min((int)(c.getBlue() * (1 / factor)),255));
  }
