 ...
 */
@SuppressWarnings("serial") public class BarChart extends java.applet.Applet {
  ...
  private void paintHorizontal(  Graphics g){
    ...
    for (int i=0; i < columns; i++) {
      int widthOfItems=maxLabelWidth + 3 + (maxValue * scale)+ 5+ metrics.stringWidth(Integer.toString(maxValue));
      cx=Math.max((getSize().width - widthOfItems) / 2,0);
      cy=getSize().height - metrics.getDescent() - metrics.getHeight()- barSpacing- ((columns - i - 1) * (barSpacing + barHeight));
      g.setColor(Color.black);
  ...
  private void paintVertical(  Graphics g){
    ...
    for (int i=0; i < columns; i++) {
      ...
      int cy=getSize().height - metrics.getHeight() - metrics.getDescent()- 4;
      g.setColor(Color.black);
      g.drawString(labels[i],cx,cy);
      cy-=metrics.getHeight() - 3;
      g.fillRect(cx + 4,cy - (values[i] * scale) - 4,barWidth,(values[i] * scale));
      g.setColor(colors[i]);
      if (styles[i] == STRIPED) {
        for (int k=0; k <= values[i] * scale; k+=2) {
          g.drawLine(cx,cy - k,cx + barWidth,cy - k);
        }
      }
 else {
        g.fillRect(cx,cy - (values[i] * scale),barWidth + 1,(values[i] * scale) + 1);
      }
      cy-=(values[i] * scale) + 5;
      g.setColor(g.getColor().darker());
      g.drawString(Integer.toString(values[i]),cx,cy);
