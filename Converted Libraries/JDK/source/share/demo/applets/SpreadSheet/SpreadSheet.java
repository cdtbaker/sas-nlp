...
import java.io.*;
import java.net.*;
@SuppressWarnings("serial") public class SpreadSheet extends Applet implements MouseListener, KeyListener {
  String title;
  Font titleFont;
  Color cellColor;
  Color inputColor;
  ...
  @Override public synchronized void init(){
    ...
    for (int i=0; i < rows; i++) {
      for (int j=0; j < columns; j++) {
        cells[i][j]=new Cell(this,Color.lightGray,Color.black,cellColor,cellWidth - 2,cellHeight - 2);
        l[0]=(char)((int)'a' + j);
    ...
    Dimension d=getSize();
    inputArea=new SpreadSheetInput(null,this,d.width - 2,cellHeight - 1,inputColor,Color.white);
    resize(columns * cellWidth + rowLabelWidth,(rows + 3) * cellHeight + titleHeight);
...
String[][] info={{"title","string","The title of the spread sheet.  Default is 'Spreadsheet'"},{"rows","int","The number of rows.  Default is 9."},{"columns","int","The number of columns.  Default is 5."}};
return info;
}
}
...
try {
dataStream=new URL(target.app.getDocumentBase(),target.getValueString()).openStream();
...
if (subformula == null ? restFormula != null : !subformula.equals(restFormula)) {
left=new Node(node);
node.left=left;
node.right=right;
node.op=op;
