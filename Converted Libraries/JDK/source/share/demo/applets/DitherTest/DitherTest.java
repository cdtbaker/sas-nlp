...
enum DitherMethod {NOOP, RED, GREEN, BLUE, ALPHA, SATURATION}
@SuppressWarnings("serial") public class DitherTest extends Applet implements Runnable {
...
String[][] info={{"xaxis","{RED, GREEN, BLUE, ALPHA, SATURATION}","The color of the Y axis.  Default is RED."},{"yaxis","{RED, GREEN, BLUE, ALPHA, SATURATION}","The color of the X axis.  Default is BLUE."}};
return info;
}
}
@SuppressWarnings("serial") class DitherCanvas extends Canvas {
private Image img;
...
@SuppressWarnings("serial") class DitherControls extends Panel implements ActionListener {
...
for (DitherMethod m : DitherMethod.values()) {
choice.addItem(m.toString().substring(0,1) + m.toString().substring(1).toLowerCase());
}
...
@SuppressWarnings("serial") class CardinalTextField extends TextField {
String oldText=null;
...
@Override protected void processEvent(AWTEvent evt){
int id=evt.getID();
...
@Override protected void processTextEvent(TextEvent te){
