...
public class Demo {
  static enum Action {  rename,   movein,   moveout,   copy,   copyin,   copyin_attrs,   copyout,   copyout_attrs,   zzmove,   zzcopy,   attrs,   attrsspace,   setmtime,   setatime,   setctime,   lsdir,   mkdir,   mkdirs,   rmdirs,   list,   tlist,   vlist,   walk,   twalk,   extract,   update,   delete,   add,   create,   attrs2,   prof}
  public static void main(  String[] args) throws Throwable {
    ...
    if (action == Action.create)     env.put("create","true");
    try {
      Path path, src, dst;
switch (action) {
case rename:
...
path=fs.getPath(args[i]);
Files.setAttribute(path,"lastModifiedTime",FileTime.fromMillis(newDatetime.getTime()));
System.out.println(Files.readAttributes(path,BasicFileAttributes.class).toString());
}
break;
case setctime:
...
for (int i=3; i < args.length; i++) {
path=fs.getPath(args[i]);
Files.setAttribute(path,"creationTime",FileTime.fromMillis(newDatetime.getTime()));
System.out.println(Files.readAttributes(path,BasicFileAttributes.class).toString());
}
break;
case setatime:
...
for (int i=3; i < args.length; i++) {
path=fs.getPath(args[i]);
Files.setAttribute(path,"lastAccessTime",FileTime.fromMillis(newDatetime.getTime()));
System.out.println(Files.readAttributes(path,BasicFileAttributes.class).toString());
}
break;
case attrsspace:
...
System.out.printf("    totalSpace: %d%n",(Long)fstore.getAttribute("totalSpace"));
System.out.printf("   usableSpace: %d%n",(Long)fstore.getAttribute("usableSpace"));
System.out.printf("  unallocSpace: %d%n",(Long)fstore.getAttribute("unallocatedSpace"));
break;
case list:
case tlist:
if (args.length < 3) list(fs.getPath("/"),false);
 else list(fs.getPath(args[2]),false);
break;
case vlist:
if (args.length < 3) list(fs.getPath("/"),true);
 else list(fs.getPath(args[2]),true);
break;
case twalk:
case walk:
...
break;
case delete:
for (int i=2; i < args.length; i++) Files.delete(fs.getPath(args[i]));
break;
case create:
case add:
case update:
...
@SuppressWarnings("unused") private static byte[] getBytes(String name){
return name.getBytes();
}
@SuppressWarnings("unused") private static String getString(byte[] name){
...
@Override public FileVisitResult postVisitDirectory(Path dir,IOException ioe){
indent-=2;
return FileVisitResult.CONTINUE;
}
}
...
try {
for (Path child : ds) update(fs,child.toString());
}
 }
 else {
Path dst=fs.getPath(path);
Path parent=dst.getParent();
if (parent != null && Files.notExists(parent)) mkdirs(parent);
...
Path parent=dst.getParent();
if (Files.notExists(parent)) mkdirs(parent);
Files.copy(src,dst,REPLACE_EXISTING);
}
}
private static void z2zcopy(FileSystem src,FileSystem dst,String path) throws IOException {
...
dst=dstPath.resolve(dst);
try {
if (Files.notExists(dst)) mkdirs(dst);
}
 catch (IOException x) {
x.printStackTrace();
}
...
@SuppressWarnings("unused") private static void rmdirs(Path path) throws IOException {
...
if (verbose) System.out.println(Files.readAttributes(path,BasicFileAttributes.class).toString());
}
if (Files.notExists(path)) return;
if (Files.isDirectory(path)) {
try {
for (Path child : ds) list(child,verbose);
}
 }
}
@SuppressWarnings("unused") private static void checkEqual(Path src,Path dst) throws IOException {
...
System.out.printf("src[%s].size=%d, dst[%s].size=%d%n",chSrc.toString(),chSrc.size(),chDst.toString(),chDst.size());
throw new RuntimeException("CHECK FAILED!");
}
