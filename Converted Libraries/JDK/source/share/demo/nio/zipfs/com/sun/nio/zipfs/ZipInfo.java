...
public class ZipInfo {
  public static void main(  String[] args) throws Throwable {
    if (args.length < 1) {
      ...
      while (pos + CENHDR < cen.length) {
        ...
        if (zfs.readFullyAt(buf,0,len,locoff(cen,pos)) != len)         zfs.zerror("read loc header failed");
  ...
  static void printLOC(  byte[] loc){
    ...
    if (LOCEXT(loc) != 0)     printExtra(loc,LOCHDR + LOCNAM(loc),LOCEXT(loc));
  }
  static void printCEN(  byte[] cen,  int off){
    ...
    if (CENEXT(cen,off) != 0)     printExtra(cen,off + CENHDR + CENNAM(cen,off),CENEXT(cen,off));
  ...
  static long locoff(  byte[] cen,  int pos){
    ...
    if (locoff == ZIP64_MINVAL) {
      ...
      while (off + 4 < end) {
        ...
        off+=4;
        if (CENLEN(cen,pos) == ZIP64_MINVAL)         off+=8;
        if (CENSIZ(cen,pos) == ZIP64_MINVAL)         off+=8;
        return LL(cen,off);
      }
    }
    return locoff;
  ...
  static void printExtra(  byte[] extra,  int off,  int len){
    ...
    while (off + 4 <= end) {
      ...
      off+=4;
      for (int i=0; i < sz; i++)       print("%02x ",extra[off + i]);
      print("]%n");
switch (tag) {
case EXTID_ZIP64:
        print("         ->ZIP64: ");
...
case EXTID_NTFS:
print("         ->PKWare NTFS%n");
if (SH(extra,off + 4) != 0x0001 || SH(extra,off + 6) != 24) print("    Error: Invalid NTFS sub-tag or subsz");
