...
public class ZipFileSystem extends FileSystem {
  ...
  ZipFileSystem(  ZipFileSystemProvider provider,  Path zfpath,  Map<String,?> env) throws IOException {
    ...
    if (this.defaultDir.charAt(0) != '/')     throw new IllegalArgumentException("default dir should be absolute");
    ...
    if (!Files.isWritable(zfpath))     this.readOnly=true;
  ...
  @Override public ZipPath getPath(  String first,  String... more){
    String path;
    if (more.length == 0) {
      path=first;
    }
...
synchronized (tmppaths) {
  ...
  void setTimes(  byte[] path,  FileTime mtime,  FileTime atime,  FileTime ctime) throws IOException {
    ...
    try {
      ...
      if (e == null)       throw new NoSuchFileException(getString(path));
      if (e.type == Entry.CEN)       e.type=Entry.COPY;
      if (mtime != null)       e.mtime=mtime.toMillis();
      if (atime != null)       e.atime=atime.toMillis();
      if (ctime != null)       e.ctime=ctime.toMillis();
      update(e);
    }
  finally {
      endWrite();
  ...
  Iterator<Path> iteratorOf(  byte[] path,  DirectoryStream.Filter<? super Path> filter) throws IOException {
    ...
    try {
      ensureOpen();
      IndexNode inode=getInode(path);
      if (inode == null)       throw new NotDirectoryException(getString(path));
      List<Path> list=new ArrayList<>();
      IndexNode child=inode.child;
      while (child != null) {
        ZipPath zp=toZipPath(child.name);
        if (filter == null || filter.accept(zp))         list.add(zp);
        child=child.sibling;
      }
  ...
  void createDirectory(  byte[] dir,  FileAttribute<?>... attrs) throws IOException {
    ...
    try {
      ensureOpen();
      if (dir.length == 0 || exists(dir))       throw new FileAlreadyExistsException(getString(dir));
      checkParents(dir);
  ...
  void copyFile(  boolean deletesrc,  byte[] src,  byte[] dst,  CopyOption... options) throws IOException {
    checkWritable();
    if (Arrays.equals(src,dst))     return;
    beginWrite();
    try {
      ensureOpen();
      Entry eSrc=getEntry0(src);
      if (eSrc == null)       throw new NoSuchFileException(getString(src));
      if (eSrc.isDir()) {
        createDirectory(dst);
        return;
      }
      ...
      for (      CopyOption opt : options) {
        if (opt == REPLACE_EXISTING)         hasReplace=true;
 else         if (opt == COPY_ATTRIBUTES)         hasCopyAttrs=true;
      }
      Entry eDst=getEntry0(dst);
      if (eDst != null) {
        if (!hasReplace)         throw new FileAlreadyExistsException(getString(dst));
      }
 else {
        checkParents(dst);
      }
      ...
      if (eSrc.type == Entry.NEW || eSrc.type == Entry.FILECH) {
        u.type=eSrc.type;
        if (!deletesrc) {
          if (eSrc.bytes != null)           u.bytes=Arrays.copyOf(eSrc.bytes,eSrc.bytes.length);
      ...
      if (!hasCopyAttrs)       u.mtime=u.atime=u.ctime=System.currentTimeMillis();
      update(u);
      if (deletesrc)       updateDelete(eSrc);
    }
  finally {
      endWrite();
    }
  ...
  OutputStream newOutputStream(  byte[] path,  OpenOption... options) throws IOException {
    ...
    for (    OpenOption opt : options) {
      if (opt == READ)       throw new IllegalArgumentException("READ not allowed");
      if (opt == CREATE_NEW)       hasCreateNew=true;
      if (opt == CREATE)       hasCreate=true;
      if (opt == APPEND)       hasAppend=true;
    }
    beginRead();
    try {
      ensureOpen();
      Entry e=getEntry0(path);
      if (e != null) {
        if (e.isDir() || hasCreateNew)         throw new FileAlreadyExistsException(getString(path));
        if (hasAppend) {
        ...
        if (!hasCreate && !hasCreateNew)         throw new NoSuchFileException(getString(path));
        checkParents(path);
  ...
  InputStream newInputStream(  byte[] path) throws IOException {
    ...
    try {
      ...
      Entry e=getEntry0(path);
      if (e == null)       throw new NoSuchFileException(getString(path));
      if (e.isDir())       throw new FileSystemException(getString(path),"is a directory",null);
  ...
  private void checkOptions(  Set<? extends OpenOption> options){
    for (    OpenOption option : options) {
      ...
      if (!(option instanceof StandardOpenOption))       throw new IllegalArgumentException();
    }
  }
    ...
    if (options.contains(StandardOpenOption.WRITE) || options.contains(StandardOpenOption.APPEND)) {
      ...
      try {
        ...
        if (options.contains(StandardOpenOption.APPEND)) {
          Entry e=getEntry0(path);
          if (e != null && e.size >= 0)           leftover=e.size;
        }
...
;
      ...
      try {
        ensureOpen();
        Entry e=getEntry0(path);
        if (e == null || e.isDir())         throw new NoSuchFileException(getString(path));
  ...
  FileChannel newFileChannel(  byte[] path,  Set<? extends OpenOption> options,  FileAttribute<?>... attrs) throws IOException {
    ...
    try {
      ...
      if (forWrite) {
        checkWritable();
        if (e == null) {
          if (!options.contains(StandardOpenOption.CREATE_NEW))           throw new NoSuchFileException(getString(path));
        }
 else {
          if (options.contains(StandardOpenOption.CREATE_NEW))           throw new FileAlreadyExistsException(getString(path));
          if (e.isDir())           throw new FileAlreadyExistsException("directory <" + getString(path) + "> exists");
        }
      ...
      return new FileChannel(){
        ...
        protected void implCloseChannel() throws IOException {
          ...
          if (forWrite) {
            ...
            if (!isFCH)             removeTempPathForEntry(tmpfile);
          }
        }
      }
;
  ...
  private static byte[] getParent(  byte[] path){
    ...
    if (off > 0 && path[off] == '/')     off--;
    while (off > 0 && path[off] != '/') {
      off--;
    }
    if (off <= 0)     return ROOTPATH;
  ...
  private long getDataPos(  Entry e) throws IOException {
    if (e.locoff == -1) {
      ...
      if (e2 == null)       throw new ZipException("invalid loc for entry <" + e.name + ">");
      e.locoff=e2.locoff;
    }
    byte[] buf=new byte[LOCHDR];
    if (readFullyAt(buf,0,buf.length,e.locoff) != buf.length)     throw new ZipException("invalid loc for entry <" + e.name + ">");
...
synchronized (ch) {
  ...
  private END findEND() throws IOException {
    ...
    for (long pos=ziplen - buf.length; pos >= minPos; pos-=(buf.length - ENDHDR)) {
      ...
      if (readFullyAt(buf,off,len,pos + off) != len)       zerror("zip END header not found");
  ...
  private byte[] initCEN() throws IOException {
    ...
    if (end.cenlen > end.endpos)     zerror("invalid END header (bad central directory size)");
    long cenpos=end.endpos - end.cenlen;
    locpos=cenpos - end.cenoff;
    if (locpos < 0)     zerror("invalid END header (bad central directory offset)");
    ...
    while (pos < limit) {
      if (CENSIG(cen,pos) != CENSIG)       zerror("invalid CEN header (bad signature)");
      int method=CENHOW(cen,pos);
      int nlen=CENNAM(cen,pos);
      int elen=CENEXT(cen,pos);
      int clen=CENCOM(cen,pos);
      if ((CENFLG(cen,pos) & 1) != 0)       zerror("invalid CEN header (encrypted entry)");
      if (method != METHOD_STORED && method != METHOD_DEFLATED)       zerror("invalid CEN header (unsupported compression method: " + method + ")");
      if (pos + CENHDR + nlen > limit)       zerror("invalid CEN header (bad header size)");
  ...
  private void ensureOpen() throws IOException {
    if (!isOpen)     throw new ClosedFileSystemException();
  }
  ...
  private long copyLOCEntry(  Entry e,  boolean updateHeader,  OutputStream os,  long written,  byte[] buf) throws IOException {
    ...
    if ((e.flag & FLAG_DATADESCR) != 0) {
      if (e.size >= ZIP64_MINVAL || e.csize >= ZIP64_MINVAL)       size=24;
 else       size=16;
    }
    if (readFullyAt(buf,0,LOCHDR,locoff) != LOCHDR)     throw new ZipException("loc: reading failed");
    if (updateHeader) {
              ...
              try {
                ...
                if (e.type == Entry.NEW) {
                  ...
                  try {
                   ...
                   written+=e.csize;
                  if ((e.flag & FLAG_DATADESCR) != 0)                   written+=e.writeEXT(os);
                }
              }
               Files.delete(e.file);
  ...
  private Entry getEntry0(  byte[] path) throws IOException {
    IndexNode inode=getInode(path);
    if (inode instanceof Entry)     return (Entry)inode;
    if (inode == null || inode.pos == -1)     return null;
  ...
  public void deleteFile(  byte[] path,  boolean failIfNotExists) throws IOException {
    ...
    IndexNode inode=getInode(path);
    if (inode == null) {
      if (path != null && path.length == 0)       throw new ZipException("root directory </> can't not be delete");
      if (failIfNotExists)       throw new NoSuchFileException(getString(path));
    }
 else {
      if (inode.isDir() && inode.child != null)       throw new DirectoryNotEmptyException(getString(path));
      updateDelete(inode);
    }
  }
  ...
  private OutputStream getOutputStream(  Entry e) throws IOException {
    if (e.mtime == -1)     e.mtime=System.currentTimeMillis();
    if (e.method == -1)     e.method=METHOD_DEFLATED;
    e.flag=0;
    if (zc.isUTF8())     e.flag|=FLAG_EFS;
    OutputStream os;
    if (useTempFile) {
  ...
  private InputStream getInputStream(  Entry e) throws IOException {
    ...
    if (e.type == Entry.NEW) {
      if (e.bytes != null)       eis=new ByteArrayInputStream(e.bytes);
 else       if (e.file != null)       eis=Files.newInputStream(e.file);
 else       throw new ZipException("update entry data is missing");
    }
 else     if (e.type == Entry.FILECH) {
...
class EntryOutputStream extends DeflaterOutputStream {
    ...
    EntryOutputStream(    Entry e,    OutputStream os) throws IOException {
      ...
      if (e == null)       throw new NullPointerException("Zip entry is null");
      this.e=e;
      crc=new CRC32();
    }
    @Override public void write(    byte b[],    int off,    int len) throws IOException {
      if (e.type != Entry.FILECH)       ensureOpen();
...
if (out instanceof ByteArrayOutputStream) e.bytes=((ByteArrayOutputStream)out).toByteArray();
...
if (method == METHOD_DEFLATED) return 20;
 else if (method == METHOD_STORED) return 10;
...
if (elen64 != 0) elen64+=4;
if (atime != -1) {
if (isWindows) elenNTFS=36;
 else elenEXTT=9;
}
writeInt(os,CENSIG);
if (elen64 != 0) {
...
if (size0 == ZIP64_MINVAL) writeLong(os,size);
if (csize0 == ZIP64_MINVAL) writeLong(os,csize);
if (locoff0 == ZIP64_MINVAL) writeLong(os,locoff);
}
if (elenNTFS != 0) {
...
if (ctime == -1) os.write(0x3);
 else os.write(0x7);
writeInt(os,javaToUnixTime(mtime));
}
if (extra != null) writeBytes(os,extra);
if (comment != null) writeBytes(os,comment);
...
if (zipfs.readFullyAt(buf,0,LOCHDR,pos) != LOCHDR) throw new ZipException("loc: reading failed");
if (LOCSIG(buf) != LOCSIG) throw new ZipException("loc: wrong sig ->" + Long.toString(LOCSIG(buf),16));
version=LOCVER(buf);
flag=LOCFLG(buf);
method=LOCHOW(buf);
...
if (elen > 0) {
extra=new byte[elen];
if (zipfs.readFullyAt(extra,0,elen,pos + LOCHDR + nlen) != elen) {
...
if (e == null) throw new ZipException("loc: name not found in cen");
size=e.size;
csize=e.csize;
pos+=(method == METHOD_STORED ? size : csize);
if (size >= ZIP64_MINVAL || csize >= ZIP64_MINVAL) pos+=24;
 else pos+=16;
}
 else {
...
if (ctime != -1) writeInt(os,javaToUnixTime(ctime));
}
if (extra != null) {
writeBytes(os,extra);
}
...
case EXTID_EXTT:
byte[] buf=new byte[LOCHDR];
if (zipfs.readFullyAt(buf,0,buf.length,locoff) != buf.length) throw new ZipException("loc: reading failed");
if (LOCSIG(buf) != LOCSIG) throw new ZipException("loc: wrong sig ->" + Long.toString(LOCSIG(buf),16));
int locElen=LOCEXT(buf);
if (locElen < 9) break;
int locNlen=LOCNAM(buf);
buf=new byte[locElen];
if (zipfs.readFullyAt(buf,0,buf.length,locoff + LOCHDR + locNlen) != buf.length) throw new ZipException("loc extra: reading failed");
int locPos=0;
...
if (newOff != 0 && newOff != extra.length) extra=Arrays.copyOf(extra,newOff);
 else extra=null;
}
}
...
parent.child=inode;
if (name[name.length - 1] == '/') dirs.add(inode);
}
