 ...
 */
@SuppressWarnings("serial") class Notepad extends JPanel {
...
static {
  ...
  @SuppressWarnings("OverridableMethodCallInConstructor") Notepad(){
    super(true);
    try {
    ...
    try {
      ...
      port.setScrollMode(bs.booleanValue() ? JViewport.BACKINGSTORE_SCROLL_MODE : JViewport.BLIT_SCROLL_MODE);
    }
 ...
 */
  private Action[] defaultActions={new NewAction(),new OpenAction(),new SaveAction(),new ExitAction(),new ShowElementTreeAction(),undoAction,redoAction};
...
class OpenAction extends NewAction {
    ...
    @Override public void actionPerformed(    ActionEvent e){
      ...
      if (f.isFile() && f.canRead()) {
        ...
        JOptionPane.showMessageDialog(getFrame(),"Could not open file: " + f,"Error opening file",JOptionPane.ERROR_MESSAGE);
      }
    }
  }
    ...
    public void actionPerformed(    ActionEvent e){
      if (elementTreeFrame == null) {
        ...
        elementTreeFrame.addWindowListener(new WindowAdapter(){
          @Override public void windowClosing(          WindowEvent weeee){
    ...
    @Override public void run(){
      try {
        ...
        SwingUtilities.invokeLater(new Runnable(){
          public void run(){
            JOptionPane.showMessageDialog(getFrame(),"Could not open file: " + msg,"Error opening file",JOptionPane.ERROR_MESSAGE);
          }
        }
);
      }
...
class FileSaver extends Thread {
    ...
    @Override @SuppressWarnings("SleepWhileHoldingLock") public void run(){
      try {
        status.removeAll();
        JProgressBar progress=new JProgressBar();
        progress.setMinimum(0);
        ...
        SwingUtilities.invokeLater(new Runnable(){
          public void run(){
            JOptionPane.showMessageDialog(getFrame(),"Could not save file: " + msg,"Error saving file",JOptionPane.ERROR_MESSAGE);
          }
        }
);
      }
