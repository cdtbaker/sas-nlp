 ...
 */
public final class Font2DTest extends JPanel implements ActionListener, ItemListener, ChangeListener {
  ...
  public Font2DTest(  JFrame f,  boolean isApplet){
    ...
    contrastSlider=new JSlider(JSlider.HORIZONTAL,100,250,FontPanel.getDefaultLCDContrast().intValue());
  ...
  private void setupMenu(  boolean isApplet){
    ...
    fileMenu.addSeparator();
    if (!isApplet)     fileMenu.add(new MenuItemV2("Exit",this));
 else     fileMenu.add(new MenuItemV2("Close",this));
    ...
    for (int i=0; i < fontList.length; i++)     fontMenu.addItem(fontList[i]);
  ...
  private void setupDialog(  boolean isApplet){
    if (!isApplet)     filePromptDialog=new JFileChooser();
 else     filePromptDialog=null;
  ...
  public void fireRangeChanged(){
    ...
    if (showFontInfoCBMI.getState())     fireUpdateFontInfo();
  }
  public void fireChangeStatus(  String message,  boolean error){
    statusBar.setText(message);
    if (error)     fp.showingError=true;
 else     fp.showingError=false;
  }
  public void fireUpdateFontInfo(){
    if (showFontInfoCBMI.getState()) {
      String infos[]=fp.getFontInfo();
      for (int i=0; i < fontInfos.length; i++)       fontInfos[i].setText(infos[i]);
      fontInfoDialog.pack();
    }
  }
  private void setupFontList(  int rangeStart,  int rangeEnd){
    ...
    for (int i=0; i < listCount; i++) {
      ...
      if ((rm.getSelectedIndex() != RangeMenu.SURROGATES_AREA_INDEX) && canDisplayRange(f,rangeStart,rangeEnd)) {
  ...
  private String promptFile(  boolean isSave,  String initFileName){
    int retVal;
    String str;
    if (filePromptDialog == null)     return null;
    if (isSave) {
    ...
    if (initFileName != null)     filePromptDialog.setSelectedFile(new File(initFileName));
  ...
  private String[] parseUserText(  String orig){
    ...
    while (perLine.hasMoreElements()) {
      ...
      do {
        ...
        nextEscape=(nextBMPEscape < 0) ? ((nextSupEscape < 0) ? -1 : nextSupEscape) : ((nextSupEscape < 0) ? nextBMPEscape : Math.min(nextBMPEscape,nextSupEscape));
        if (nextEscape != -1) {
          if (prevEscapeEnd < nextEscape)           converted.append(oneLine.substring(prevEscapeEnd,nextEscape));
 ...
 while (nextEscape != -1);
      if (prevEscapeEnd < lineLength)       converted.append(oneLine.substring(prevEscapeEnd,lineLength));
  ...
  private void readTextFile(  String fileName){
    try {
      ...
      bis.close();
      if (numBytes >= 2 && ((byteData[0] == (byte)0xFF && byteData[1] == (byte)0xFE) || (byteData[0] == (byte)0xFE && byteData[1] == (byte)0xFF)))       fileText=new String(byteData,"UTF-16");
 else       fileText=new String(byteData);
 ...
 else           perLine=new StringTokenizer(fileText,"\r");
          break;
        }
 else         if (iTh == '\n')         break;
      }
  ...
  private void updateGUI(){
    ...
    if (selectedText == fp.USER_TEXT)     userTextDialog.show();
 else     userTextDialog.hide();
    printModeCBs[fp.ONE_PAGE].setSelected(true);
    if (selectedText == fp.FILE_TEXT || selectedText == fp.USER_TEXT) {
      if (selectedText == fp.FILE_TEXT)       methodsMenu.setSelectedItem("TextLayout.draw");
      ...
      if (selectedText == fp.ALL_GLYPHS)       methodsMenu.setSelectedItem("drawGlyphVector");
  ...
  private void loadOptions(  String fileName){
    try {
      ...
      bis.read(byteData,0,numBytes);
      bis.close();
      if (numBytes < 2 || (byteData[0] != (byte)0xFE || byteData[1] != (byte)0xFF))       throw new Exception("Not a Font2DTest options file");
      ...
      if (!title.equals("Font2DTest Option File"))       throw new Exception("Not a Font2DTest options file");
      ...
      fp.loadOptions(displayGridOpt,force16ColsOpt,rangeStartOpt,rangeEndOpt,fontNameOpt,fontSizeOpt,fontStyleOpt,fontTransformOpt,g2TransformOpt,textToUseOpt,drawMethodOpt,antialiasOpt,fractionalOpt,lcdContrast,userTextOpt);
      if (showFontInfoOpt) {
        fireUpdateFontInfo();
        fontInfoDialog.show();
      }
 else       fontInfoDialog.hide();
    }
 catch (    Exception ex) {
  ...
  public void actionPerformed(  ActionEvent e){
    ...
    if (source instanceof JMenuItem) {
      ...
      if (itemName.equals("Save Selected Options...")) {
        ...
        if (fileName != null)         writeCurrentOptions(fileName);
      }
 else       if (itemName.equals("Load Options...")) {
        String fileName=promptFile(false,"options.txt");
        if (fileName != null)         loadOptions(fileName);
      }
 else       if (itemName.equals("Save as PNG...")) {
        String fileName=promptFile(true,fontMenu.getSelectedItem() + ".png");
        if (fileName != null)         fp.doSavePNG(fileName);
      }
 else       if (itemName.equals("Load PNG File to Compare...")) {
        String fileName=promptFile(false,null);
        if (fileName != null)         loadComparisonPNG(fileName);
      }
 else       if (itemName.equals("Page Setup..."))       fp.doPageSetup();
 else       if (itemName.equals("Print..."))       printDialog.show();
 else       if (itemName.equals("Close"))       parent.dispose();
 else       if (itemName.equals("Exit"))       System.exit(0);
    }
      ...
      if (tf == sizeField)       fp.setFontParams(fontMenu.getSelectedItem(),sz,styleMenu.getSelectedIndex(),transformMenu.getSelectedIndex());
    }
 else     if (source instanceof JButton) {
      String itemName=((JButton)source).getText();
      if (itemName.equals("Print")) {
        for (int i=0; i < printModeCBs.length; i++)         if (printModeCBs[i].isSelected()) {
          printDialog.hide();
          fp.doPrint(i);
        }
      }
 else       if (itemName.equals("Cancel"))       printDialog.hide();
 else       if (itemName.equals("Update"))       fp.setTextToDraw(fp.USER_TEXT,null,parseUserText(userTextArea.getText()),null);
    }
      ...
      if (c == fontMenu || c == styleMenu || c == transformMenu) {
        ...
        fp.setFontParams(fontMenu.getSelectedItem(),sz,styleMenu.getSelectedIndex(),transformMenu.getSelectedIndex());
      }
 else       if (c == methodsMenu)       fp.setDrawMethod(methodsMenu.getSelectedIndex());
 else       if (c == textMenu) {
        if (canDisplayCheck) {
          fireRangeChanged();
        }
        int selected=textMenu.getSelectedIndex();
        if (selected == fp.RANGE_TEXT)         fp.setTextToDraw(fp.RANGE_TEXT,rm.getSelectedRange(),null,null);
 else         if (selected == fp.USER_TEXT)         fp.setTextToDraw(fp.USER_TEXT,null,parseUserText(userTextArea.getText()),null);
 ...
 else         if (selected == fp.ALL_GLYPHS)         fp.setTextToDraw(fp.ALL_GLYPHS,null,null,null);
        updateGUI();
        ...
        fp.setRenderingHints(antiAliasMenu.getSelectedItem(),fracMetricsMenu.getSelectedItem(),contrastSlider.getValue());
      }
    }
  }
  ...
  public void itemStateChanged(  ItemEvent e){
    ...
    if (source instanceof JCheckBoxMenuItem) {
      ...
      if (cbmi == displayGridCBMI)       fp.setGridDisplay(displayGridCBMI.getState());
 else       if (cbmi == force16ColsCBMI)       fp.setForce16Columns(force16ColsCBMI.getState());
...
private final class ChoiceV2 extends JComboBox {
    ...
    public String getToolTipText(){
      ...
      if (fontName != null && (textMenu.getSelectedIndex() == fp.RANGE_TEXT)) {
        if (getBit(index)) {
          return "Font \"" + fontName + "\" can display some characters in \""+ rm.getSelectedItem()+ "\" range";
        }
 else {
          return "Font \"" + fontName + "\" cannot display any characters in \""+ rm.getSelectedItem()+ "\" range";
        }
      }
...
private final class ChoiceV2Renderer extends DefaultListCellRenderer {
    ...
    public ChoiceV2Renderer(    ChoiceV2 choice){
      BufferedImage yes=new BufferedImage(10,10,BufferedImage.TYPE_INT_ARGB);
