...
synchronized (RequestServicer.class) {
 ...
 while (!cio.doHandshake());
  ...
  Reply build(  Request rq) throws IOException {
    ...
    if ((action != Request.Action.GET) && (action != Request.Action.HEAD))     rp=new Reply(Reply.Code.METHOD_NOT_ALLOWED,new StringContent(rq.toString()));
 else     rp=new Reply(Reply.Code.OK,new FileContent(rq.uri()),action);
    try {
      rp.prepare();
    }
 catch (    IOException x) {
