 ...
 */
  static MulticastAddress parse(  String s){
    ...
    if (components.length > 2)     throw new IllegalArgumentException("At most one '@' expected");
    String target=components[0];
    int len=components[0].length();
    int colon=components[0].lastIndexOf(':');
    if ((colon < 1) || (colon > (len - 2)))     throw new IllegalArgumentException("group:port expected");
    ...
    if (groupString.charAt(0) == '[') {
      ...
      if (groupString.charAt(len - 1) != ']')       throw new IllegalArgumentException("missing ']'");
      groupString=groupString.substring(1,len - 1);
      if (groupString.length() == 0)       throw new IllegalArgumentException("missing IPv6 address");
    }
