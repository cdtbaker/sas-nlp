 ...
 */
  public byte[] encrypt(  byte[] baseKey,  int usage,  byte[] ivec,  byte[] new_ivec,  byte[] plaintext,  int start,  int len) throws GeneralSecurityException, KrbCryptoException {
    ...
    try {
      ...
      if (debug) {
        System.err.println("confounder = " + confounder.length + "; plaintext = "+ len+ "; padding = "+ (plainSize - confounder.length - len)+ "; total = "+ plainSize);
 ...
 */
  ...
  static byte[] nfold(  byte[] in,  int outbits){
    ...
    for (i=lcm - 1; i >= 0; i--) {
      ...
      if (debug) {
        System.err.println("msbit[" + i + "] = "+ msbit+ "\tbval="+ Integer.toHexString(bval)+ "\toval="+ Integer.toHexString(oval)+ "\tsum = "+ Integer.toHexString(thisbyte));
      }
      thisbyte>>>=8;
      if (debug) {
