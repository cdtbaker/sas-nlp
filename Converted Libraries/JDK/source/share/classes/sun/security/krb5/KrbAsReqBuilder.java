...
public final class KrbAsReqBuilder {
  ...
  private static enum State {  INIT,   REQ_OK,   DESTROYED}
  private State state;
  ...
  private KrbAsReqBuilder send() throws KrbException, IOException {
    ...
    while (true) {
      try {
        ...
        if (!preAuthFailedOnce && (ke.returnCode() == Krb5.KDC_ERR_PREAUTH_FAILED || ke.returnCode() == Krb5.KDC_ERR_PREAUTH_REQUIRED)) {
          if (Krb5.DEBUG) {
          ...
          if (password == null) {
            ...
            if (kerr.getEType() == 0) {
              pakey=EncryptionKey.acquireSecretKey(password,snp.salt == null ? cname.getSalt() : snp.salt,EType.getDefaults("default_tkt_enctypes")[0],null);
            }
 else {
              pakey=EncryptionKey.acquireSecretKey(password,snp.salt == null ? cname.getSalt() : snp.salt,kerr.getEType(),snp.params);
            }
          }
          paList=kerr.getPA();
        }
