 ...
 */
  ...
  public boolean equals(  Object o){
    if (o instanceof PrincipalName)     return equals((PrincipalName)o);
 else     return false;
  }
  ...
  boolean equalsWithoutRealm(  PrincipalName other){
    if (nameType != KRB_NT_UNKNOWN && other.nameType != KRB_NT_UNKNOWN && nameType != other.nameType)     return false;
    if ((nameStrings != null && other.nameStrings == null) || (nameStrings == null && other.nameStrings != null))     return false;
    if (nameStrings != null && other.nameStrings != null) {
      if (nameStrings.length != other.nameStrings.length)       return false;
      for (int i=0; i < nameStrings.length; i++)       if (!nameStrings[i].equals(other.nameStrings[i]))       return false;
    }
    return true;
  }
  /** 
 ...
 */
  public static PrincipalName parse(  DerInputStream data,  byte explicitTag,  boolean optional) throws Asn1Exception, IOException {
    ...
    if (explicitTag != (der.getTag() & (byte)0x1F))     throw new Asn1Exception(Krb5.ASN1_BAD_ID);
 else {
      DerValue subDer=der.getData().getDerValue();
  ...
  protected static String[] parseName(  String name){
    ...
    if (i == temp.length())     if (componentStart < i) {
      component=temp.substring(componentStart,i);
      tempStrings.addElement(component);
    }
...
if ((result=c.getDefault(name,"domain_realm")) != null) return result;
 else {
