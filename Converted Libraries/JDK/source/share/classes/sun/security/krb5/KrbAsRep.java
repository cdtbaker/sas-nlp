 ...
 */
  void decryptUsingKeyTab(  KeyTab ktab,  KrbAsReq asReq,  PrincipalName cname) throws KrbException, Asn1Exception, IOException {
    ...
    if (dkey == null) {
      throw new KrbException(Krb5.API_INVALID_ARG,"Cannot find key for type/kvno to decrypt AS REP - " + EType.toString(encPartKeyType) + "/"+ encPartKvno);
    }
    decrypt(dkey,asReq);
  }
  /** 
 ...
 */
  void decryptUsingPassword(  char[] password,  KrbAsReq asReq,  PrincipalName cname) throws KrbException, Asn1Exception, IOException {
    ...
    EncryptionKey dkey=null;
    dkey=EncryptionKey.acquireSecretKey(password,snp.salt == null ? cname.getSalt() : snp.salt,encPartKeyType,snp.params);
    decrypt(dkey,asReq);
  }
  /** 
 ...
 */
  private void decrypt(  EncryptionKey dkey,  KrbAsReq asReq) throws KrbException, Asn1Exception, IOException {
    ...
    check(req,rep);
    creds=new Credentials(rep.ticket,req.reqBody.cname,rep.ticket.sname,enc_part.key,enc_part.flags,enc_part.authtime,enc_part.starttime,enc_part.endtime,enc_part.renewTill,enc_part.caddr);
    if (DEBUG) {
