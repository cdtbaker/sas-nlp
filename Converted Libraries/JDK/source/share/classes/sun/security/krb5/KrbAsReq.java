 ...
 */
  public KrbAsReq(  EncryptionKey pakey,  KDCOptions options,  PrincipalName cname,  PrincipalName sname,  KerberosTime from,  KerberosTime till,  KerberosTime rtime,  int[] eTypes,  HostAddresses addresses) throws KrbException, IOException {
    ...
    if (sname == null) {
      sname=new PrincipalName("krbtgt" + PrincipalName.NAME_COMPONENT_SEPARATOR + cname.getRealmAsString(),PrincipalName.KRB_NT_SRV_INST);
    }
    if (till == null) {
      till=new KerberosTime(0);
    }
    KDCReqBody kdc_req_body=new KDCReqBody(options,cname,cname.getRealm(),sname,from,till,rtime,Nonce.value(),eTypes,addresses,null,null);
