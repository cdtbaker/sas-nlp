...
case K_KRB5:
case K_KRB5_EXPORT:
preMasterSecret=this.clientKeyExchange(new KerberosClientKeyExchange(protocolVersion,clientRequestedVersion,sslContext.getSecureRandom(),input,kerberosKeys));
break;
case K_DHE_RSA:
case K_DHE_DSS:
case K_DH_ANON:
...
case K_DHE_RSA:
if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
preferableSignatureAlgorithm=SignatureAndHashAlgorithm.getPreferableAlgorithm(supportedSignAlgs,"RSA");
...
kerberosKeys=AccessController.doPrivileged(new PrivilegedExceptionAction<SecretKey[]>(){
...
kerberosKeys=null;
if (debug != null && Debug.isOn("handshake")) System.out.println("Permission to access Kerberos" + " secret key denied");
return false;
}
}
