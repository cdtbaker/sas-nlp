 ...
 */
  public ObjectIdentifier(  DerInputStream in) throws IOException {
    ...
    if (type_id != DerValue.tag_ObjectId)     throw new IOException("ObjectIdentifier() -- data isn't an object ID" + " (tag = " + type_id + ")");
  ...
  private void init(  int[] components,  int length){
    ...
    if (components[1] < Integer.MAX_VALUE - components[0] * 40)     pos+=pack7Oid(components[0] * 40 + components[1],tmp,pos);
 else {
 ...
 */
  private static byte[] pack(  byte[] in,  int ioffset,  int ilength,  int iw,  int ow){
    ...
    while (ipos < bits) {
      ...
      out[opos / ow]|=(((in[ioffset + ipos / iw] + 256) >> (iw - ipos % iw - count)) & ((1 << (count)) - 1)) << (ow - opos % ow - count);
      ipos+=count;
      opos+=count;
    }
    return out;
