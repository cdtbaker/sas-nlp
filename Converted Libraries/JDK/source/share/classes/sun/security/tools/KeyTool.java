 ...
 */
  void doCommands(  PrintStream out) throws Exception {
    ...
    if (isKeyStoreRelated(command)) {
      ...
      if (!nullStream) {
        try {
          ...
          if (command != GENKEYPAIR && command != GENSECKEY && command != IDENTITYDB && command != IMPORTCERT && command != IMPORTKEYSTORE && command != PRINTCRL) {
    ...
    if (nullStream && storePass != null) {
      ...
      if (!protectedPath && !KeyStoreUtil.isWindowsKeyStore(storetype) && (command == CERTREQ || command == DELETE || command == GENKEYPAIR || command == GENSECKEY || command == IMPORTCERT || command == IMPORTKEYSTORE || command == KEYCLONE || command == CHANGEALIAS || command == SELFCERT || command == STOREPASSWD || command == KEYPASSWD || command == IDENTITYDB)) {
        int count=0;
        do {
 ...
 */
  private void doGenCert(  String alias,  String sigAlgName,  InputStream in,  PrintStream out) throws Exception {
    ...
    info.set(X509CertInfo.SUBJECT,new CertificateSubjectName(dname == null ? req.getSubjectName() : new X500Name(dname)));
 ...
 */
  private void printX509Cert(  X509Certificate cert,  PrintStream out) throws Exception {
    ...
    Object[] source={cert.getSubjectDN().toString(),cert.getIssuerDN().toString(),cert.getSerialNumber().toString(16),cert.getNotBefore().toString(),cert.getNotAfter().toString(),getCertFingerPrint("MD5",cert),getCertFingerPrint("SHA1",cert),getCertFingerPrint("SHA-256",cert),cert.getSigAlgName(),cert.getVersion()};
    out.println(form.format(source));
