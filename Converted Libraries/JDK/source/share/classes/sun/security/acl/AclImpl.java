 ...
 */
  ...
  private Hashtable<Principal,AclEntry> findTable(  AclEntry entry){
    ...
    if (p instanceof Group) {
      if (entry.isNegative())       aclTable=deniedGroupsTable;
 else       aclTable=allowedGroupsTable;
    }
 else {
      if (entry.isNegative())       aclTable=deniedUsersTable;
 else       aclTable=allowedUsersTable;
    }
    return aclTable;
  }
...
final class AclEnumerator implements Enumeration<AclEntry> {
  ...
  AclEnumerator(  Acl acl,  Hashtable<?,AclEntry> u1,  Hashtable<?,AclEntry> g1,  Hashtable<?,AclEntry> u2,  Hashtable<?,AclEntry> g2){
    this.acl=acl;
    this.u1=u1.elements();
    this.u2=u2.elements();
    this.g1=g1.elements();
