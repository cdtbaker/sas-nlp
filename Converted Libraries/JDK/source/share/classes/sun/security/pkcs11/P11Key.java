...
abstract class P11Key implements Key {
  ...
  static SecretKey secretKey(  Session session,  long keyID,  String algorithm,  int keyLength,  CK_ATTRIBUTE[] attributes){
    attributes=getAttributes(session,keyID,attributes,new CK_ATTRIBUTE[]{new CK_ATTRIBUTE(CKA_TOKEN),new CK_ATTRIBUTE(CKA_SENSITIVE),new CK_ATTRIBUTE(CKA_EXTRACTABLE)});
...
private static final class P11DSAPublicKey extends P11Key implements DSAPublicKey {
    ...
    private synchronized void fetchValues(){
      ...
      CK_ATTRIBUTE[] attributes=new CK_ATTRIBUTE[]{new CK_ATTRIBUTE(CKA_VALUE),new CK_ATTRIBUTE(CKA_PRIME),new CK_ATTRIBUTE(CKA_SUBPRIME),new CK_ATTRIBUTE(CKA_BASE)};
      fetchAttributes(attributes);
      y=attributes[0].getBigInteger();
      params=new DSAParameterSpec(attributes[1].getBigInteger(),attributes[2].getBigInteger(),attributes[3].getBigInteger());
    }
    public String getFormat(){
      token.ensureValid();
      return "X.509";
...
private static final class P11DHPrivateKey extends P11Key implements DHPrivateKey {
    ...
    private synchronized void fetchValues(){
      ...
      CK_ATTRIBUTE[] attributes=new CK_ATTRIBUTE[]{new CK_ATTRIBUTE(CKA_VALUE),new CK_ATTRIBUTE(CKA_PRIME),new CK_ATTRIBUTE(CKA_BASE)};
      fetchAttributes(attributes);
      x=attributes[0].getBigInteger();
...
private static final class P11ECPrivateKey extends P11Key implements ECPrivateKey {
    ...
    private synchronized void fetchValues(){
      ...
      CK_ATTRIBUTE[] attributes=new CK_ATTRIBUTE[]{new CK_ATTRIBUTE(CKA_VALUE),new CK_ATTRIBUTE(CKA_EC_PARAMS,params)};
      fetchAttributes(attributes);
...
private static final class P11ECPublicKey extends P11Key implements ECPublicKey {
    ...
    private synchronized void fetchValues(){
      ...
      CK_ATTRIBUTE[] attributes=new CK_ATTRIBUTE[]{new CK_ATTRIBUTE(CKA_EC_POINT),new CK_ATTRIBUTE(CKA_EC_PARAMS)};
      fetchAttributes(attributes);
      try {
        params=P11ECKeyFactory.decodeParameters(attributes[1].getByteArray());
        ...
        try {
          ...
          if (wECPoint.getTag() != DerValue.tag_OctetString)           throw new IOException("Unexpected tag: " + wECPoint.getTag());
