 ...
 */
  public synchronized KeyStore.Entry engineGetEntry(  String alias,  KeyStore.ProtectionParameter protParam) throws KeyStoreException, NoSuchAlgorithmException, UnrecoverableEntryException {
    token.ensureValid();
    if (protParam != null && protParam instanceof KeyStore.PasswordProtection && ((KeyStore.PasswordProtection)protParam).getPassword() != null && !token.config.getKeyStoreCompatibilityMode()) {
      ...
      if (entry instanceof KeyStore.PrivateKeyEntry) {
        ...
        try {
          ...
          for (          String oldAlias : aliases) {
            ...
            if (aliasInfo.type == ATTR_CLASS_PKEY && aliasInfo.cert.getPublicKey().equals(chain[0].getPublicKey())) {
              updatePkey(alias,aliasInfo.id,(X509Certificate[])chain,!aliasInfo.cert.equals(chain[0]));
              updatedAlias=true;
              break;
            }
          }
 ...
 */
  ...
  private X509Certificate[] loadChain(  Session session,  X509Certificate endCert) throws PKCS11Exception, CertificateException {
    ...
    while (true) {
      CK_ATTRIBUTE[] attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_CERT,new CK_ATTRIBUTE(CKA_SUBJECT,next.getIssuerX500Principal().getEncoded())};
 ...
 */
  private void updatePkey(  String alias,  byte[] cka_id,  X509Certificate[] chain,  boolean replaceCert) throws KeyStoreException, CertificateException, PKCS11Exception {
    ...
    try {
      ...
      if (h.type == ATTR_CLASS_PKEY) {
        ...
        throw new KeyStoreException("expected but could not find private key " + "with CKA_ID " + getID(cka_id));
      }
      h=getTokenObject(session,ATTR_CLASS_CERT,cka_id,null);
      if (h.type != ATTR_CLASS_CERT) {
        throw new KeyStoreException("expected but could not find certificate " + "with CKA_ID " + getID(cka_id));
      }
 else {
        if (replaceCert) {
          destroyChain(cka_id);
          ...
          CK_ATTRIBUTE[] attrs=new CK_ATTRIBUTE[]{new CK_ATTRIBUTE(CKA_LABEL,alias),new CK_ATTRIBUTE(CKA_ID,alias)};
  ...
  private void storeCert(  String alias,  X509Certificate cert) throws PKCS11Exception, CertificateException {
    ...
    if (alias != null) {
      ...
      attrList.add(new CK_ATTRIBUTE(CKA_ID,getID(cert.getSubjectX500Principal().getName(X500Principal.CANONICAL),cert)));
    }
    Session session=null;
    try {
      session=token.getOpSession();
  ...
  private void storeSkey(  String alias,  KeyStore.SecretKeyEntry ske) throws PKCS11Exception, KeyStoreException {
    SecretKey skey=ske.getSecretKey();
    CK_ATTRIBUTE[] attrs=new CK_ATTRIBUTE[]{ATTR_SKEY_TOKEN_TRUE,ATTR_PRIVATE_TRUE,new CK_ATTRIBUTE(CKA_LABEL,alias)};
    try {
  ...
  private void storePkey(  String alias,  KeyStore.PrivateKeyEntry pke) throws PKCS11Exception, CertificateException, KeyStoreException {
    ...
    if (key instanceof RSAPrivateKey) {
      ...
      attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_PKEY,ATTR_PRIVATE_TRUE,new CK_ATTRIBUTE(CKA_KEY_TYPE,CKK_DSA),idAttrs[0],new CK_ATTRIBUTE(CKA_PRIME,dsaKey.getParams().getP()),new CK_ATTRIBUTE(CKA_SUBPRIME,dsaKey.getParams().getQ()),new CK_ATTRIBUTE(CKA_BASE,dsaKey.getParams().getG()),new CK_ATTRIBUTE(CKA_VALUE,dsaKey.getX())};
      if (idAttrs[1] != null) {
      ...
      attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_PKEY,ATTR_PRIVATE_TRUE,new CK_ATTRIBUTE(CKA_KEY_TYPE,CKK_DH),idAttrs[0],new CK_ATTRIBUTE(CKA_PRIME,dhKey.getParams().getP()),new CK_ATTRIBUTE(CKA_BASE,dhKey.getParams().getG()),new CK_ATTRIBUTE(CKA_VALUE,dhKey.getX())};
      if (idAttrs[1] != null) {
        attrs=addAttribute(attrs,idAttrs[1]);
      }
      ...
      attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_PKEY,ATTR_PRIVATE_TRUE,new CK_ATTRIBUTE(CKA_KEY_TYPE,CKK_EC),idAttrs[0],new CK_ATTRIBUTE(CKA_VALUE,ecKey.getS()),new CK_ATTRIBUTE(CKA_EC_PARAMS,encodedParams)};
      if (idAttrs[1] != null) {
        attrs=addAttribute(attrs,idAttrs[1]);
      }
  ...
  private CK_ATTRIBUTE[] getRsaPrivKeyAttrs(  String alias,  RSAPrivateKey key,  X500Principal subject) throws PKCS11Exception {
    ...
    if (key instanceof RSAPrivateCrtKey) {
      ...
      attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_PKEY,ATTR_PRIVATE_TRUE,new CK_ATTRIBUTE(CKA_KEY_TYPE,CKK_RSA),new CK_ATTRIBUTE(CKA_ID,alias),new CK_ATTRIBUTE(CKA_MODULUS,rsaKey.getModulus()),new CK_ATTRIBUTE(CKA_PRIVATE_EXPONENT,rsaKey.getPrivateExponent()),new CK_ATTRIBUTE(CKA_PUBLIC_EXPONENT,rsaKey.getPublicExponent()),new CK_ATTRIBUTE(CKA_PRIME_1,rsaKey.getPrimeP()),new CK_ATTRIBUTE(CKA_PRIME_2,rsaKey.getPrimeQ()),new CK_ATTRIBUTE(CKA_EXPONENT_1,rsaKey.getPrimeExponentP()),new CK_ATTRIBUTE(CKA_EXPONENT_2,rsaKey.getPrimeExponentQ()),new CK_ATTRIBUTE(CKA_COEFFICIENT,rsaKey.getCrtCoefficient())};
      ...
      attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_PKEY,ATTR_PRIVATE_TRUE,new CK_ATTRIBUTE(CKA_KEY_TYPE,CKK_RSA),new CK_ATTRIBUTE(CKA_ID,alias),new CK_ATTRIBUTE(CKA_MODULUS,rsaKey.getModulus()),new CK_ATTRIBUTE(CKA_PRIVATE_EXPONENT,rsaKey.getPrivateExponent())};
      attrs=token.getAttributes(TemplateManager.O_IMPORT,CKO_PRIVATE_KEY,CKK_RSA,attrs);
    }
    return attrs;
  }
    ...
    try {
      ...
      while (true) {
        ...
        if (ch == null || ch.length == 0) {
          ...
          next=loadCert(session,ch[0]);
          attrs=new CK_ATTRIBUTE[]{ATTR_TOKEN_TRUE,ATTR_CLASS_CERT,new CK_ATTRIBUTE(CKA_ISSUER,next.getSubjectX500Principal().getEncoded())};
  ...
  private THandle getTokenObject(  Session session,  CK_ATTRIBUTE type,  byte[] cka_id,  String cka_label) throws PKCS11Exception, KeyStoreException {
    ...
    if (h.length == 0) {
      ...
      if (type == ATTR_CLASS_SKEY) {
        ...
        throw new KeyStoreException("invalid KeyStore state: " + "found " + h.length + " certificates sharing CKA_ID "+ getID(cka_id));
      }
 else {
        throw new KeyStoreException("invalid KeyStore state: " + "found " + h.length + " private keys sharing CKA_ID "+ getID(cka_id));
      }
    }
 ...
 */
  private ArrayList<AliasInfo> mapPrivateKeys(  ArrayList<byte[]> pkeyIDs,  HashMap<String,HashSet<AliasInfo>> certMap) throws PKCS11Exception, CertificateException {
    ...
    for (    byte[] pkeyID : pkeyIDs) {
      ...
      if (!foundMatch) {
        if (debug != null) {
          debug.println("did not find match for private key with CKA_ID [" + getID(pkeyID) + "] (ignoring entry)");
        }
      }
    }
    return matchedCerts;
