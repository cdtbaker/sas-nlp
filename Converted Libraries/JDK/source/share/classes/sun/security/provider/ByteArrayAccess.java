...
static {
  ...
  private static boolean unaligned(){
    String arch=java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction("os.arch",""));
 ...
 */
  static void b2lBig(  byte[] in,  int inOfs,  long[] out,  int outOfs,  int len){
    if (littleEndianUnaligned) {
      ...
      len+=inOfs;
      while (inOfs < len) {
        out[outOfs++]=((long)unsafe.getInt(in,(long)inOfs) << 32) | (unsafe.getInt(in,(long)(inOfs + 4)) & 0xffffffffL);
        inOfs+=8;
      }
    }
 else {
      ...
      while (inOfs < len) {
        ...
        inOfs+=4;
        int i2=((in[inOfs + 3] & 0xff)) | ((in[inOfs + 2] & 0xff) << 8) | ((in[inOfs + 1] & 0xff) << 16)| ((in[inOfs]) << 24);
