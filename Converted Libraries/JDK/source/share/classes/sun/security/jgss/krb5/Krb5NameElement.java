 ...
 */
  static Krb5NameElement getInstance(  String gssNameStr,  Oid gssNameType) throws GSSException {
    if (gssNameType == null)     gssNameType=Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL;
 else     if (!gssNameType.equals(GSSName.NT_USER_NAME) && !gssNameType.equals(GSSName.NT_HOSTBASED_SERVICE) && !gssNameType.equals(Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL)&& !gssNameType.equals(GSSName.NT_EXPORT_NAME))     throw new GSSException(GSSException.BAD_NAMETYPE,-1,gssNameType.toString() + " is an unsupported nametype");
    ...
    try {
      if (gssNameType.equals(GSSName.NT_EXPORT_NAME) || gssNameType.equals(Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL)) {
        ...
        if (gssNameType.equals(GSSName.NT_USER_NAME))         principalName=new PrincipalName(gssNameStr,PrincipalName.KRB_NT_PRINCIPAL);
 else {
