 ...
 */
  public int getRemainingLifetime() throws GSSException {
    ...
    for (Enumeration<SearchKey> e=hashtable.keys(); e.hasMoreElements(); ) {
      ...
      if (tempKey.getUsage() == INITIATE_ONLY)       tempLife=tempCred.getInitLifetime();
 else       if (tempKey.getUsage() == ACCEPT_ONLY)       tempLife=tempCred.getAcceptLifetime();
 else {
 ...
 */
  public int getUsage() throws GSSException {
    ...
    for (Enumeration<SearchKey> e=hashtable.keys(); e.hasMoreElements(); ) {
      tempKey=e.nextElement();
      if (tempKey.getUsage() == INITIATE_ONLY)       initiate=true;
 else       if (tempKey.getUsage() == ACCEPT_ONLY)       accept=true;
 else       return INITIATE_AND_ACCEPT;
    }
    if (initiate) {
      if (accept)       return INITIATE_AND_ACCEPT;
 else       return INITIATE_ONLY;
    }
  ...
  public int getUsage(  Oid mech) throws GSSException {
    ...
    if (initiate && accept)     return GSSCredential.INITIATE_AND_ACCEPT;
 else     if (initiate)     return GSSCredential.INITIATE_ONLY;
 else     if (accept)     return GSSCredential.ACCEPT_ONLY;
 else {
      throw new GSSExceptionImpl(GSSException.BAD_MECH,mech);
    }
  }
  ...
  public void add(  GSSName name,  int initLifetime,  int acceptLifetime,  Oid mech,  int usage) throws GSSException {
    ...
    if (hashtable.containsKey(key)) {
      throw new GSSExceptionImpl(GSSException.DUPLICATE_ELEMENT,"Duplicate element found: " + getElementStr(mech,usage));
    }
    ...
    if (tempCred != null) {
      if (usage == GSSCredential.INITIATE_AND_ACCEPT && (!tempCred.isAcceptorCredential() || !tempCred.isInitiatorCredential())) {
        int currentUsage;
        int desiredUsage;
        ...
        hashtable.put(key,tempCred);
        tempCred=gssManager.getCredentialElement(nameElement,initLifetime,acceptLifetime,mech,desiredUsage);
 ...
 */
  public GSSCredentialSpi getElement(  Oid mechOid,  boolean initiate) throws GSSException {
    ...
    if (mechOid == null) {
      ...
      if (initiate)       key=new SearchKey(mechOid,INITIATE_ONLY);
 else       key=new SearchKey(mechOid,ACCEPT_ONLY);
      element=hashtable.get(key);
      if (element == null) {
        key=new SearchKey(mechOid,INITIATE_AND_ACCEPT);
        element=hashtable.get(key);
    ...
    if (element == null)     throw new GSSExceptionImpl(GSSException.NO_CRED,"No credential found for: " + mechOid + getElementStr(mechOid,initiate ? INITIATE_ONLY : ACCEPT_ONLY));
    return element;
  }
