  ...
  private MechanismFactory getMechFactory(  PreferencesEntry e,  Oid mechOid) throws GSSException {
    ...
    if (retVal == null) {
      ...
      if (className != null) {
        ...
        if (e.getOid() != null) {
          throw new GSSExceptionImpl(GSSException.BAD_MECH,"Provider " + p.getName() + " does not support mechanism "+ mechOid);
        }
      }
    }
    return retVal;
 ...
 */
  private static MechanismFactory getMechFactoryImpl(  Provider p,  String className,  Oid mechOid,  GSSCaller caller) throws GSSException {
    try {
  ...
  synchronized public void addProviderAtFront(  Provider p,  Oid mechOid) throws GSSException {
    ...
    if (mechOid == null) {
      ...
      if (p.getProperty(PROV_PROP_PREFIX + oidStr) == null)       throw new GSSExceptionImpl(GSSException.BAD_MECH,"Provider " + p.getName() + " does not support "+ oidStr);
      mechs.add(mechOid);
      foundSomeMech=true;
    }
    if (foundSomeMech) {
  ...
  synchronized public void addProviderAtEnd(  Provider p,  Oid mechOid) throws GSSException {
    ...
    if (mechOid == null)     foundSomeMech=addAllMechsFromProvider(p);
 else {
      String oidStr=mechOid.toString();
      if (p.getProperty(PROV_PROP_PREFIX + oidStr) == null)       throw new GSSExceptionImpl(GSSException.BAD_MECH,"Provider " + p.getName() + " does not support "+ oidStr);
      mechs.add(mechOid);
