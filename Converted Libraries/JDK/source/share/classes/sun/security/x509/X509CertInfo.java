...
if (issuerUniqueId == null) return null;
 else return (issuerUniqueId.get(suffix));
}
case (ATTR_SUBJECT_ID):
if (suffix == null) {
...
if (subjectUniqueId == null) return null;
 else return (subjectUniqueId.get(suffix));
}
}
return null;
}
...
private void verifyCert(CertificateSubjectName subject,CertificateExtensions extensions) throws CertificateParsingException, IOException {
X500Name subjectDN=(X500Name)subject.get(CertificateSubjectName.DN_NAME);
...
try {
subjectAltNameExt=(SubjectAlternativeNameExtension)extensions.get(SubjectAlternativeNameExtension.NAME);
...
algId.encode(tmp);
if ((version.compare(CertificateVersion.V1) == 0) && (issuer.toString() == null)) throw new CertificateParsingException("Null issuer DN not allowed in v1 certificate");
issuer.encode(tmp);
interval.encode(tmp);
if ((version.compare(CertificateVersion.V1) == 0) && (subject.toString() == null)) throw new CertificateParsingException("Null subject DN not allowed in v1 certificate");
subject.encode(tmp);
pubKey.encode(tmp);
if (issuerUniqueId != null) {
