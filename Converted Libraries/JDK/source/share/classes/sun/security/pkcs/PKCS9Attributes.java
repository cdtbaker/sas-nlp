 ...
 * @param permittedAttributesArray of attribute OIDs that will be accepted.
 * @param inthe contents of the DER encoding of the attribute set.
 * @exception IOExceptionon i/o error, encoding syntax error, unacceptable or
 ...
 */
  public PKCS9Attributes(  ObjectIdentifier[] permittedAttributes,  DerInputStream in) throws IOException {
    if (permittedAttributes != null) {
      ...
      for (int i=0; i < permittedAttributes.length; i++)       this.permittedAttributes.put(permittedAttributes[i],permittedAttributes[i]);
    }
 else {
 ...
 * @exception IOExceptionon i/o error, encoding syntax error, or unsupported or
 * duplicate attribute.
 * @see PKCS9Attribute
 */
 ...
 */
  public PKCS9Attributes(  PKCS9Attribute[] attribs) throws IllegalArgumentException, IOException {
    ...
    for (int i=0; i < attribs.length; i++) {
      ...
      if (attributes.containsKey(oid))       throw new IllegalArgumentException("PKCSAttribute " + attribs[i].getOID() + " duplicated while constructing "+ "PKCS9Attributes.");
