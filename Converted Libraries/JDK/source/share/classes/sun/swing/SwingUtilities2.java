    ...
    if (isPrinting(g)) {
      ...
      if (g2d != null) {
        ...
        if (frc != null && !isFontRenderContextPrintCompatible(deviceFontRenderContext,frc)) {
          TextLayout layout=createTextLayout(c,new String(data,offset,length),g2d.getFont(),deviceFontRenderContext);
 ...
 */
  private static synchronized boolean inputEvent_canAccessSystemClipboard(  InputEvent ie){
    if (inputEvent_CanAccessSystemClipboard_Field == null) {
      inputEvent_CanAccessSystemClipboard_Field=AccessController.doPrivileged(new java.security.PrivilegedAction<Field>(){
        public Field run(){
          try {
 ...
 * @throws NullPointerException if {@code rect} or {@code p} are{@code null}
 */
