...
public static abstract class ProcessHandler implements EndSubPathHandler {
    ...
    public abstract void processFixedLine(    int x1,    int y1,    int x2,    int y2,    int[] pixelInfo,    boolean checkBounds,    boolean endSubPath);
  }
...
private static class DrawProcessHandler extends ProcessHandler {
    ...
    void PROCESS_LINE(    int fX0,    int fY0,    int fX1,    int fY1,    boolean checkBounds,    int[] pixelInfo){
      ...
      if (!checkBounds || (dhnd.yMin <= Y0 && dhnd.yMax > Y0 && dhnd.xMin <= X0 && dhnd.xMax > X0)) {
        if (pixelInfo[0] == 1 && ((pixelInfo[1] == X0 && pixelInfo[2] == Y0) || (pixelInfo[3] == X0 && pixelInfo[4] == Y0))) {
          dhnd.drawPixel(X0,Y0);
        }
      }
  ...
  private static void ProcessMonotonicQuad(  ProcessHandler hnd,  float[] coords,  int[] pixelInfo){
    ...
    if (xMax - xMin > MAX_QUAD_SIZE || yMax - yMin > MAX_QUAD_SIZE) {
      ...
      DrawMonotonicQuad(hnd,coords,hnd.dhnd.xMinf >= xMin || hnd.dhnd.xMaxf <= xMax || hnd.dhnd.yMinf >= yMin || hnd.dhnd.yMaxf <= yMax,pixelInfo);
    }
  }
...
if (param > 0) {
ProcessFirstMonotonicPartOfQuad(hnd,coords,pixelInfo,(float)(param / (1.0 - params[0])));
}
break;
}
ProcessMonotonicQuad(hnd,coords,pixelInfo);
...
DrawMonotonicCubic(hnd,coords,hnd.dhnd.xMinf > xMin || hnd.dhnd.xMaxf < xMax || hnd.dhnd.yMinf > yMin || hnd.dhnd.yMaxf < yMax,pixelInfo);
}
}
...
if (param > 0) {
ProcessFirstMonotonicPartOfCubic(hnd,coords,pixelInfo,(float)(param / (1.0 - params[i - 1])));
}
}
}
ProcessMonotonicCubic(hnd,coords,pixelInfo);
