...
public class DrawImage implements DrawImagePipe {
  ...
  protected void transformImage(  SunGraphics2D sg,  Image img,  int x,  int y,  AffineTransform extraAT,  int interpType){
    ...
    if (sg.transformState <= sg.TRANSFORM_ANY_TRANSLATE && (txtype == AffineTransform.TYPE_IDENTITY || txtype == AffineTransform.TYPE_TRANSLATION)) {
      ...
      if (interpType == AffineTransformOp.TYPE_NEAREST_NEIGHBOR || (closeToInteger(itx,tx) && closeToInteger(ity,ty))) {
        renderImageCopy(sg,img,null,x + itx,y + ity,0,0,imgw,imgh);
        return;
      }
      checkfinalxform=false;
    }
 else     if (sg.transformState <= sg.TRANSFORM_TRANSLATESCALE && ((txtype & (AffineTransform.TYPE_FLIP | AffineTransform.TYPE_MASK_ROTATION | AffineTransform.TYPE_GENERAL_TRANSFORM)) == 0)) {
  ...
  protected void transformImage(  SunGraphics2D sg,  Image img,  AffineTransform tx,  int interpType,  int sx1,  int sy1,  int sx2,  int sy2,  Color bgColor){
    ...
    if (Math.abs(coords[0] - coords[4]) < MAX_TX_ERROR && Math.abs(coords[3] - coords[5]) < MAX_TX_ERROR && tryCopyOrScale(sg,img,sx1,sy1,sx2,sy2,bgColor,interpType,coords)) {
      return;
    }
  ...
  protected boolean tryCopyOrScale(  SunGraphics2D sg,  Image img,  int sx1,  int sy1,  int sx2,  int sy2,  Color bgColor,  int interpType,  double coords[]){
    ...
    if (closeToInteger(sx2 - sx1,dw) && closeToInteger(sy2 - sy1,dh)) {
      ...
      if (interpType == AffineTransformOp.TYPE_NEAREST_NEIGHBOR || (closeToInteger(idx,dx) && closeToInteger(idy,dy))) {
        renderImageCopy(sg,img,bgColor,idx,idy,sx1,sy1,sx2 - sx1,sy2 - sy1);
        return true;
      }
    }
    if (dw > 0 && dh > 0) {
      if (renderImageScale(sg,img,bgColor,interpType,sx1,sy1,sx2,sy2,coords[0],coords[1],coords[2],coords[3])) {
        return true;
      }
    }
    return false;
  ...
  protected void renderImageXform(  SunGraphics2D sg,  Image img,  AffineTransform tx,  int interpType,  int sx1,  int sy1,  int sx2,  int sy2,  Color bgColor){
    ...
    helper.Transform(tmpmaskblit,srcData,tmpData,AlphaComposite.Src,null,itx,interpType,sx1,sy1,sx2,sy2,0,0,dx2 - dx1,dy2 - dy1,edges,dx1,dy1);
    int index=2;
    for (int y=edges[0]; y < edges[1]; y++) {
      ...
      if (maskblit != null) {
        maskblit.MaskBlit(tmpData,dstData,sg.composite,clip,relx1,y,dx1 + relx1,dy1 + y,relx2 - relx1,1,null,0,0);
      }
 else {
