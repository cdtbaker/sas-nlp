 ...
 */
  boolean diagnoseMismatch(  Environment env,  Expression args[],  Type argTypes[]) throws ClassNotFound {
    ...
    while (start < argTypes.length) {
      ...
      if (castOK)       env.error(args[i].where,"explicit.cast.needed",opName,argTypes[i],ttype);
 else       env.error(args[i].where,"incompatible.type",opName,argTypes[i],ttype);
      saidSomething=true;
      start=i + 1;
    }
 ...
 */
  ...
  public Expression inlineValue(  Environment env,  Context ctx){
    ...
    try {
      ...
      ClassDefinition ctxClass=ctx.field.getClassDefinition();
      if (env.opt() && field.isInlineable(env,clazz.isFinal()) && ((right == null) || (right.op == THIS) || field.isStatic())&& ctxClass.permitInlinedAccess(env,field.getClassDeclaration())&& ctxClass.permitInlinedAccess(env,field)&& (right == null || ctxClass.permitInlinedAccess(env,env.getClassDeclaration(right.type)))&& (!ctx.field.isInitializer())&& ctx.field.isMethod()&& (ctx.getInlineMemberContext(field) == null)) {
        Statement s=(Statement)field.getValue(env);
        if ((s == null) || (s.costInline(MAXINLINECOST,env,ctx) < MAXINLINECOST)) {
