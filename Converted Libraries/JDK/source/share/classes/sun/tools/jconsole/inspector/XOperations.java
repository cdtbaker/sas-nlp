      ...
      @Override protected void done(){
        try {
          ...
          if (entryIf.getReturnType() != null && !entryIf.getReturnType().equals(Void.TYPE.getName()) && !entryIf.getReturnType().equals(Void.class.getName())) {
            ...
            new ThreadDialog(button,Resources.getText("Method successfully invoked"),Resources.getText("Info"),JOptionPane.INFORMATION_MESSAGE).run();
          }
        }
 catch (        Throwable t) {
          ...
          new ThreadDialog(button,Resources.getText("Problem invoking") + " " + button.getText()+ " : "+ t.toString(),Resources.getText("Error"),JOptionPane.ERROR_MESSAGE).run();
        }
      }
    }
.execute();
