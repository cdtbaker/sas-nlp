...
@SuppressWarnings("serial") public class XMBeanAttributes extends XTable {
  ...
  private WeakHashMap<XMBean,HashMap<String,ZoomedCell>> viewersCache=new WeakHashMap<XMBean,HashMap<String,ZoomedCell>>();
  ...
  @Override public synchronized Component prepareRenderer(  TableCellRenderer renderer,  int row,  int column){
  ...
  void updateRowHeight(  Object obj,  int row){
    ...
    if (obj instanceof ZoomedCell) {
      cell=(ZoomedCell)obj;
      if (cell.isInited())       setRowHeight(row,cell.getHeight());
 else       if (rowMinHeight != -1)       setRowHeight(row,rowMinHeight);
    }
 else     if (rowMinHeight != -1)     setRowHeight(row,rowMinHeight);
  }
  @Override public synchronized TableCellRenderer getCellRenderer(  int row,  int column){
    if (row >= getRowCount()) {
      return null;
    }
 else {
  ...
  @Override public final boolean editCellAt(  final int row,  final int column,  EventObject e){
    if (LOGGER.isLoggable(Level.FINER)) {
  ...
  @Override public void setValueAt(  Object value,  int row,  int column){
    if (!isCellError(row,column) && isColumnEditable(column) && isWritable(row)&& Utils.isEditableType(getClassName(row))) {
      if (JConsole.isDebug()) {
        System.err.println("validating [row=" + row + ", column="+ column+ "]: "+ getValueName(row)+ "="+ value);
      }
  ...
  @Override public String getToolTip(  int row,  int column){
    if (isCellError(row,column)) {
    ...
    if (isColumnEditable(column)) {
      ...
      if (value != null) {
        tip=value.toString();
        if (isAttributeViewable(row,VALUE_COLUMN))         tip=Resources.getText("Double click to expand/collapse") + ". " + tip;
      }
      return tip;
    }
 ...
 */
  @Override public synchronized int getRowCount(){
  ...
  public void loadAttributes(  final XMBean mbean,  final MBeanInfo mbeanInfo){
    final SwingWorker<Runnable,Void> load=new SwingWorker<Runnable,Void>(){
      @Override protected Runnable doInBackground() throws Exception {
...
;
  ...
  @Override void sortRequested(  int column){
    if (isEditing())     stopCellEditing();
    super.sortRequested(column);
  }
  @Override public synchronized void emptyTable(){
...
synchronized (viewersCache) {
      ...
      if (viewers.containsKey(attribute)) {
        ...
        if (cell.isMaximized() && cell.getType() != XDataViewer.NUMERIC) {
          Component comp=mbeansTab.getDataViewer().createAttributeViewer(value,mbean,attribute,XMBeanAttributes.this);
  ...
  private void updateColumnWidth(  int col1Width,  int col2Width){
    ...
    if (col1Width + col2Width < (int)getPreferredScrollableViewportSize().getWidth())     col2Width=(int)getPreferredScrollableViewportSize().getWidth() - col1Width;
...
@SuppressWarnings("serial") class ValueCellEditor extends XTextFieldEditor {
...
@SuppressWarnings("serial") class MaximizedCellRenderer extends DefaultTableCellRenderer {
    Component comp;
...
class ZoomedCell {
 ...
 else       return (int)maxRenderer.getComponent().getPreferredSize().getHeight();
    }
    int getMinHeight(){
      return minHeight;
    }
...
class AttributesListener implements TableModelListener {
    ...
    public void tableChanged(    final TableModelEvent e){
      if (isColumnEditable(e.getColumn())) {
        ...
        if (LOGGER.isLoggable(Level.FINER)) {
          LOGGER.finer("tableChanged: firstRow=" + e.getFirstRow() + ", lastRow="+ e.getLastRow()+ ", column="+ e.getColumn()+ ", value="+ tableValue);
        }
        if (tableValue instanceof String) {
          try {
            tableValue=Utils.createObjectFromString(getClassName(e.getFirstRow()),(String)tableValue);
          }
 catch (          Throwable ex) {
