...
@SuppressWarnings("serial") public class XOpenTypeViewer extends JPanel implements ActionListener {
...
class XOpenTypeDataListener extends MouseAdapter {
    ...
    private XOpenTypeData getSelectedViewedOpenType(){
      ...
      if (elem instanceof XOpenTypeData)       return (XOpenTypeData)elem;
 else       return null;
    }
  }
...
static abstract class XOpenTypeData extends JTable {
    ...
    public String getToolTip(    int row,    int col){
      if (col == 1) {
        ...
        if (value != null) {
          if (isClickableElement(value))           return Resources.getText("Double click to visualize") + ". " + value.toString();
...
static class TabularDataComparator implements Comparator<CompositeData> {
    ...
    @SuppressWarnings("unchecked") public int compare(    CompositeData o1,    CompositeData o2){
...
static class XTabularData extends XCompositeData implements XViewedTabularData {
    ...
    @SuppressWarnings("unchecked") public XTabularData(    XOpenTypeData parent,    TabularData tabular){
    ...
    public boolean canIncrement(){
      if (size == 0 || currentIndex == size - 1)       return false;
 else       return true;
    }
...
static class XArrayData extends XCompositeData implements XViewedArrayData {
    ...
    public void viewed(    XOpenTypeViewer viewer) throws Exception {
      ...
      if (dimension > 1)       throw new Exception(Resources.getText("Dimension is not " + "supported:") + dimension);
      super.viewed(viewer);
    }
    public int getElementCount(){
      return size;
    ...
    private void load(){
      ...
      for (int i=0; i < size; i++) {
        rowData[0]=isTabularType ? new XTabularData(this,(TabularData)Array.get(val,i)) : Array.get(val,i);
  ...
  void setOpenType(  XOpenTypeData data){
    ...
    if (!(data instanceof XViewedTabularData)) {
      ...
      if (hasMoreThanOneElement) {
        tabularLabel.setText(Resources.getText("MBeansTab.tabularNavigationMultiple",String.format("%d",tabular.getSelectedElementIndex() + 1),String.format("%d",tabular.getElementCount())));
      }
 else {
        tabularLabel.setText(tabularNavigationSingle);
      }
    ...
    if (!(data instanceof XViewedArrayData)) {
      ...
      if (hasMoreThanOneElement) {
        compositeLabel.setText(Resources.getText("MBeansTab.compositeNavigationMultiple",String.format("%d",array.getSelectedElementIndex() + 1),String.format("%d",array.getElementCount())));
      }
 else {
        compositeLabel.setText(compositeNavigationSingle);
      }
