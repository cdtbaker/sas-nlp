...
@SuppressWarnings("serial") class MemoryTab extends Tab implements ActionListener, ItemListener {
  ...
  public MemoryTab(  VMPanel vmPanel){
    ...
    bottomPanel.setBorder(new CompoundBorder(new TitledBorder(getText("Details")),new EmptyBorder(10,10,10,10)));
  ...
  private String formatDetails(){
    ...
    try {
      ...
      for (      GarbageCollectorMXBean garbageCollectorMBean : garbageCollectors) {
        ...
        String str=getText("GC time details",justify(formatTime(gcTime),14),gcName,String.format("%,d",gcCount));
        if (!descPrinted) {
...
private class PoolChart extends BorderedComponent implements Accessible, MouseListener {
    ...
    public void paintComponent(    Graphics g){
      ...
      for (      int pc : new int[]{0,25,50,75,100}) {
        String str=pc + "% --";
        g.drawString(str,leftMargin - fm.stringWidth(str) - 4,height - bottomMargin - (pc * barHeight / 100) + fontDescent + 1);
      }
...
private static class MemoryOverviewPanel extends OverviewPanel {
    ...
    private void updateMemoryInfo(    long used,    long committed,    long max){
      getInfoLabel().setText(getText(infoLabelFormat,formatBytes(used,true),formatBytes(committed,true),formatBytes(max,true)));
    }
  }
}
