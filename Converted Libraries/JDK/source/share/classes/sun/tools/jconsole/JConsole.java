...
@SuppressWarnings("serial") public class JConsole extends JFrame implements ActionListener, InternalFrameListener {
...
static {
  ...
  public JConsole(  boolean hotspot){
    ...
    if (hotspot) {
      ...
      hotspotMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_H,InputEvent.CTRL_MASK));
...
.start();
  ...
  private static void mainInit(  final List<String> urls,  final List<String> hostNames,  final List<Integer> ports,  final List<LocalVirtualMachine> vmids,  final ProxyClient proxyClient,  final boolean noTile,  final boolean hotspot){
    SwingUtilities.invokeLater(new Runnable(){
      public void run(){
