...
@Override public void approveSelection(){
  ...
  if (file != null) {
    ...
    if (file.exists()) {
      ...
      int ret=JOptionPane.showOptionDialog(this,getText("FileChooser.fileExists.message",file.getName()),getText("FileChooser.fileExists.title"),JOptionPane.OK_CANCEL_OPTION,JOptionPane.WARNING_MESSAGE,null,new Object[]{okStr,cancelStr},okStr);
      ...
      if (seq.isPlotted) {
        ...
        if (seq.size > 0) {
          if (unit == Unit.BYTES) {
            ...
            value=getFormattedValue(seq.value(seq.size - 1),false) + ((unit == Unit.PERCENT) ? "%" : "");
          }
        }
