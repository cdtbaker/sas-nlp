  ...
  public Object evaluate(  Expression e) throws MonitorException {
    ...
    if (e instanceof Identifier) {
      ...
      if (m.getVariability() == Variability.CONSTANT) {
        if (debug) {
          System.out.println("Converting constant " + id.getName() + " to literal with value "+ m.getValue());
        }
    ...
    if (l != null && r != null) {
      if ((l instanceof Literal) && (r instanceof Literal)) {
        ...
        if (debug) {
          System.out.println("Converting expression " + e + " (left = "+ ln.doubleValue()+ ")"+ " (right = "+ rn.doubleValue()+ ")"+ " to literal value "+ result);
        }
