 ...
 */
  ...
  public Environment(  Environment env,  Object source){
    if (env != null && env.env != null && env.getClass() == this.getClass())     env=env.env;
    this.env=env;
    this.source=source;
  }
  public Environment(){
 ...
 */
  public final ClassDefinition getClassDefinition(  Identifier nm) throws ClassNotFound {
    if (nm.isInner()) {
      ...
      walkTail:       while (tail.isQualified()) {
        tail=tail.getTail();
...
if (f.isInnerClass()) {
  ...
  if ((!isExtends && !ctxClass.canAccess(env,fdecl)) || (isExtends && !ctxClass.extendsCanAccess(env,fdecl))) {
