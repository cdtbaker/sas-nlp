...
case opc_invokespecial:
case opc_invokeinterface:
return ((MemberDefinition)value).getType().getReturnType().stackSize() - (((MemberDefinition)value).getType().stackSize() + 1);
case opc_invokestatic:
return ((MemberDefinition)value).getType().getReturnType().stackSize() - (((MemberDefinition)value).getType().stackSize());
}
...
case opc_nop:
if ((value != null) && !(value instanceof Integer)) return 2;
 else return 1;
}
return 1;
}
...
case opc_iinc:
out.writeByte(opc);
out.writeByte(((int[])value)[0]);
out.writeByte(((int[])value)[1]);
break;
case opc_iinc + 256:
out.writeByte(opc_wide);
out.writeByte(opc - 256);
...
case opc_nop:
if (value != null) {
if (value instanceof Integer) out.writeByte(((Integer)value).intValue());
 else out.writeShort(tab.index(value));
return;
}
default :
out.writeByte(opc);
