...
public class ByteToCharJISAutoDetect extends ByteToCharConverter {
  ...
  public int flush(  char[] output,  int outStart,  int outEnd) throws MalformedInputException, ConversionBufferFullException {
    badInputLength=0;
    if (detectedConv != null)     return detectedConv.flush(output,outStart,outEnd);
 else     return defaultConv.flush(output,outStart,outEnd);
  }
  /** 
    ...
    try {
      if (detectedConv == null) {
        ...
        for (cnt=inOff; cnt < inEnd; cnt++) {
          ...
          if (firstmask != 0) {
            if (cnt + 1 < inEnd) {
              ...
              if ((mask == SJIS2B_MASK) || (mask == SJIS1B_MASK) || (JISAutoDetect.canBeSJIS1B(firstmask) && secondmask == 0)) {
                convName=SJISName;
                break;
              }
        ...
        if (nonAsciiFound && (convName == null)) {
          if ((euckana > 1) || (ss2count > 1))           convName=EUCJPName;
 else           convName=SJISName;
        }
        if (convName != null) {
          try {
