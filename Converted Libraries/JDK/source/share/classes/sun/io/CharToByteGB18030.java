...
public class CharToByteGB18030 extends CharToByteConverter {
  ...
  public boolean canConvert(  char c){
    if ((c >= 0xd800 && c <= 0xdfff) || (c >= 0xfffe))     return false;
 else     return true;
  }
  /** 
  ...
  public int convert(  char[] input,  int inOff,  int inEnd,  byte[] output,  int outOff,  int outEnd) throws UnknownCharacterException, MalformedInputException, ConversionBufferFullException {
    ...
    while (charOff < inEnd) {
      ...
      if (inputChar >= '\uD800' && inputChar <= '\uDBFF') {
        ...
        if (inputChar >= 0xE000 && inputChar < 0xF900)         linearDiffValue+=0x82BD;
 else         if (inputChar >= 0xF900)         linearDiffValue+=0x93A9;
        if (hiByte > 0x80)         currentState=GB18030_DOUBLE_BYTE;
 else         currentState=GB18030_FOUR_BYTE;
      }
