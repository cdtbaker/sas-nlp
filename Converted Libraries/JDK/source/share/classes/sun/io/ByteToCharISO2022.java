...
public abstract class ByteToCharISO2022 extends ByteToCharConverter {
  ...
  private byte[] savetyGetSrc(  byte[] input,  int inOff,  int inEnd,  int nbytes){
    int i;
    byte tmp[];
    if (inOff <= (inEnd - nbytes + 1))     tmp=new byte[nbytes];
 else     tmp=new byte[inEnd - inOff];
...
public final int convert(byte[] input,int inOff,int inEnd,char[] output,int outOff,int outEnd) throws ConversionBufferFullException, MalformedInputException {
int i;
...
for (i=0; i < remainIndex - DesignatorLength; i++) remainByte[i]=remainByte[DesignatorLength + 1 + i];
remainIndex=i - 1;
}
 else {
remainIndex=-1;
