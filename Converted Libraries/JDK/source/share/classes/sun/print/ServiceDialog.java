...
private class PrintServicePanel extends JPanel implements ActionListener, ItemListener, PopupMenuListener {
    ...
    public void actionPerformed(    ActionEvent e){
      ...
      if (source == btnProperties) {
        if (uiFactory != null) {
          JDialog dialog=(JDialog)uiFactory.getUI(ServiceUIFactory.MAIN_UIROLE,ServiceUIFactory.JDIALOG_UI);
          if (dialog != null) {
            dialog.show();
          }
 else {
      ...
      if (e.getStateChange() == ItemEvent.SELECTED) {
        ...
        if ((index >= 0) && (index < services.length)) {
          if (!services[index].equals(psCurrent)) {
            ...
            if ((dest != null || isPrintToFileSelected()) && psCurrent.isAttributeCategorySupported(Destination.class)) {
              if (dest != null) {
                asCurrent.add(dest);
              }
 else {
                dest=(Destination)psCurrent.getDefaultAttributeValue(Destination.class);
                if (dest == null) {
                  try {
...
private class ValidatingFileChooser extends JFileChooser {
    public void approveSelection(){
      ...
      if (exists) {
        int val;
        val=JOptionPane.showConfirmDialog(this,getMsg("dialog.overwrite"),getMsg("dialog.owtitle"),JOptionPane.YES_NO_OPTION);
      ...
      try {
        ...
        JOptionPane.showMessageDialog(this,getMsg("dialog.writeerror") + " " + selected,getMsg("dialog.owtitle"),JOptionPane.WARNING_MESSAGE);
        return;
      }
