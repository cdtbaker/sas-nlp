...
@Override public void visitUTF8(int index,byte tag,String utf8){
putUTF8(index,utf8Map.get(utf8));
if (usedUtf8Keys != null) usedUtf8Keys.add(utf8);
}
@Override public void visitConstantValue(int index,byte tag,Object value){
putConstantValue(index,tag,valueMap.get(value));
if (usedValueKeys != null) usedValueKeys.add(value);
}
@Override public void visitConstantString(int index,byte tag,String name,int nameIndex){
...
@Override public void visitConstantString(int index,byte tag,String value,int j){
show(index,tag,value);
}
@Override public void visitMemberRef(int index,byte tag,String className,String memberName,String signature,int j,int k){
show(index,tag,new String[]{className,memberName,signature});
}
@Override public void visitDescriptor(int index,byte tag,String memberName,String signature,int j,int k){
...
if (classForTag(tag) != value.getClass()) throw new IllegalArgumentException("invalid constant value" + (tag == CONSTANT_None ? "" : " for tag " + tagName(tag)) + " of class "+ value.getClass());
}
private void checkTag(int index,byte putTag){
byte tag=outer.tags[index];
if (tag != putTag) throw new IllegalArgumentException("invalid put operation" + " for " + tagName(putTag) + " at index "+ index+ " found "+ tagName(tag));
}
private void checkTagMask(int index,int tagBitMask){
byte tag=outer.tags[index];
int tagBit=((tag & 0x1F) == tag) ? (1 << tag) : 0;
if ((tagBit & tagBitMask) == 0) throw new IllegalArgumentException("invalid put operation" + " at index " + index + " found "+ tagName(tag));
}
private static void checkMemberName(String memberName){
if (memberName.indexOf(';') >= 0) throw new IllegalArgumentException("memberName " + memberName + " contains a ';'");
}
/** 
 * Set the entry of the constant pool indexed by index to
 * a new string.
 * @param index an index to a constant pool entry containing a{@link ConstantPoolVisitor#CONSTANT_Utf8} value.
 * @param utf8 a string
 ...
 * @param tag one of the following values:{@link ConstantPoolVisitor#CONSTANT_Integer},{@link ConstantPoolVisitor#CONSTANT_Float},{@link ConstantPoolVisitor#CONSTANT_Long},{@link ConstantPoolVisitor#CONSTANT_Double},{@link ConstantPoolVisitor#CONSTANT_String}, or{@link ConstantPoolVisitor#CONSTANT_Class}
 ...
 * @param index an index to a constant pool entry containing a{@link ConstantPoolVisitor#CONSTANT_NameAndType} value.
...
checkMemberName(memberName);
if (signature.startsWith("(") == (tag == CONSTANT_Fieldref)) throw new IllegalArgumentException("bad signature: " + signature);
patchArray[index]=addSemis(className,memberName,signature);
}
static private final int CONSTANT_MemberRef_MASK=CONSTANT_Fieldref | CONSTANT_Methodref | CONSTANT_InterfaceMethodref;
...
static {
Object[][] values={{Integer.class,CONSTANT_Integer},{Long.class,CONSTANT_Long},{Float.class,CONSTANT_Float},{Double.class,CONSTANT_Double},{String.class,CONSTANT_String},{Class.class,CONSTANT_Class}};
...
if (className.indexOf('/') >= 0 || className.indexOf(';') >= 0) throw new IllegalArgumentException("invalid class name " + className);
}
