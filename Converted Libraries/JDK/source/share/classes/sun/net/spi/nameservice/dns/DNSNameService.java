...
private static class ThreadContext {
  ...
  private DirContext getTemporaryContext() throws NamingException {
    ...
    if (thrCtxt == null) {
      ...
      DirContext dirCtxt;
      try {
        dirCtxt=java.security.AccessController.doPrivileged(new java.security.PrivilegedExceptionAction<DirContext>(){
 ...
 */
  private ArrayList<String> resolve(  final DirContext ctx,  final String name,  final String[] ids,  int depth) throws UnknownHostException {
    ...
    Attributes attrs;
    try {
      attrs=java.security.AccessController.doPrivileged(new java.security.PrivilegedExceptionAction<Attributes>(){
  ...
  public DNSNameService() throws Exception {
    ...
    String nameservers=AccessController.doPrivileged(new GetPropertyAction("sun.net.spi.nameservice.nameservers"));
  ...
  public String getHostByAddr(  byte[] addr) throws UnknownHostException {
    ...
    try {
      ...
      if (addr.length == 4) {
        ...
        for (int i=addr.length - 1; i >= 0; i--) {
          literalip+=Integer.toHexString((addr[i] & 0x0f)) + "." + Integer.toHexString((addr[i] & 0xf0) >> 4)+ ".";
        }
