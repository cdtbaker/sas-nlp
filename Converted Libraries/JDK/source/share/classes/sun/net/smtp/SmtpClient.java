 ...
 */
  ...
  void issueCommand(  String cmd,  int expect) throws IOException {
    sendServer(cmd);
    int reply;
    while ((reply=readServerResponse()) != expect)     if (reply != 220) {
  ...
  public void to(  String s) throws IOException {
    ...
    while (pos < limit) {
      ...
      if (parendepth > 0) {
        ...
        if (parendepth == 0)         if (lastnonsp > st)         ignore=true;
 else         st=pos + 1;
      }
 else       if (c == '(')       parendepth++;
 ...
 */
  public SmtpClient(  String host) throws IOException {
    ...
    try {
      String s;
      mailhost=java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction("mail.host"));
      if (mailhost != null) {
        openServer(mailhost);
        return;
      }
