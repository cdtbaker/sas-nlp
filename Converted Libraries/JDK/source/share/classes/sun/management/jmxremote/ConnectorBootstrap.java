...
private static class AccessFileCheckerAuthenticator implements JMXAuthenticator {
    ...
    private void checkAccessFileEntries(    Subject subject){
      ...
      throw new SecurityException("Access denied! No entries found in the access file [" + accessFile + "] for any of the authenticated identities "+ principalsStr);
    }
 ...
 */
  public static synchronized JMXConnectorServer initialize(  String portStr,  Properties props){
    ...
    try {
      final JMXConnectorServerData data=exportMBeanServer(mbs,port,useSsl,useRegistrySsl,sslConfigFileName,enabledCipherSuitesList,enabledProtocolsList,sslNeedClientAuth,useAuthentication,loginConfigName,passwordFileName,accessFileName);
    ...
    try {
      if (fs.supportsFileSecurity(file)) {
        if (!fs.isAccessUserOnly(file)) {
          final String msg=Agent.getText("jmxremote.ConnectorBootstrap.initialize.password.readonly",passwordFileName);
  ...
  private static void checkRestrictedFile(  String restrictedFileName){
    ...
    try {
      if (fs.supportsFileSecurity(file)) {
        if (!fs.isAccessUserOnly(file)) {
          final String msg=Agent.getText("jmxremote.ConnectorBootstrap.initialize.file.readonly",restrictedFileName);
          log.config("initialize",msg);
 ...
 */
  ...
  private static JMXConnectorServerData exportMBeanServer(  MBeanServer mbs,  int port,  boolean useSsl,  boolean useRegistrySsl,  String sslConfigFileName,  String[] enabledCipherSuites,  String[] enabledProtocols,  boolean sslNeedClientAuth,  boolean useAuthentication,  String loginConfigName,  String passwordFileName,  String accessFileName) throws IOException, MalformedURLException {
    System.setProperty("java.rmi.server.randomIDs","true");
    ...
    if (useSsl || useRegistrySsl) {
      ...
      ssf=createSslRMIServerSocketFactory(sslConfigFileName,enabledCipherSuites,enabledProtocols,sslNeedClientAuth);
    }
    if (useSsl) {
    ...
    JMXServiceURL remoteURL=new JMXServiceURL("service:jmx:rmi:///jndi/rmi://" + url.getHost() + ":"+ ((UnicastRef)((RemoteObject)registry).getRef()).getLiveRef().getPort()+ "/jmxrmi");
