      ...
      try {
        while (src.hasRemaining() && dst.hasRemaining()) {
          ...
          if (c == UNMAPPABLE_DECODING) {
            ...
            int b2=src.get() & 0xff;
            if (b2 < b2Min || b2 > b2Max || (c=b2c[b1][b2 - b2Min]) == UNMAPPABLE_DECODING)             return crMalformedOrUnmappable(b1);
            inSize++;
          }
          dst.put(c);
          mark+=inSize;
...
public static class Decoder_EBCDIC extends Decoder {
    ...
    private static boolean isDoubleByte(    int b1,    int b2){
      return (0x41 <= b1 && b1 <= 0xfe && 0x41 <= b2 && b2 <= 0xfe) || (b1 == 0x40 && b2 == 0x40);
    }
    protected CoderResult decodeArrayLoop(    ByteBuffer src,    CharBuffer dst){
      byte[] sa=src.array();
      int sp=src.arrayOffset() + src.position();
      ...
      if (b2cNR != null) {
        ...
        while (j < b2cNR.length()) {
          ...
          if (b < 0x100 && b2cSB_ca != null) {
            ...
            if (b2c_ca[b >> 8][(b & 0xff) - b2Min] == c)             b2c_ca[b >> 8][(b & 0xff) - b2Min]=UNMAPPABLE_DECODING;
          }
        }
      }
      if (b2cSB_ca != null) {
