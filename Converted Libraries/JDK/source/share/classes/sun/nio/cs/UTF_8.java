...
da[dp++]=(char)((b1 << 12) ^ (b2 << 6) ^ (b3 ^ (((byte)0xE0 << 12) ^ ((byte)0x80 << 6) ^ ((byte)0x80 << 0))));
sp+=3;
}
 else if ((b1 >> 3) == -2) {
...
int b1=src.get();
if (b1 >= 0) {
if (dst.remaining() < 1) return xflow(src,mark,1);
dst.put((char)b1);
mark++;
}
 else if ((b1 >> 5) == -2) {
...
dst.put((char)((b1 << 12) ^ (b2 << 6) ^ (b3 ^ (((byte)0xE0 << 12) ^ ((byte)0x80 << 6) ^ ((byte)0x80 << 0)))));
mark+=3;
}
 else if ((b1 >> 3) == -2) {
