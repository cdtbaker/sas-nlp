...
private static final class StandardUnderline extends Underline {
    ...
    private BasicStroke createStroke(    float lineThickness){
      if (dashPattern == null) {
        ...
        return new BasicStroke(lineThickness,BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10.0f,dashPattern,0);
      }
    }
...
private static class IMGrayUnderline extends Underline {
    ...
    IMGrayUnderline(){
      stroke=new BasicStroke(DEFAULT_THICKNESS,BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10.0f,new float[]{1,1},0);
    }
    void drawUnderline(    Graphics2D g2d,    float thickness,    float x1,    float x2,    float y){
      Stroke saveStroke=g2d.getStroke();
