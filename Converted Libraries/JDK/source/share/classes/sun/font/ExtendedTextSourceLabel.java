 ...
 */
  ...
  public Rectangle2D getLogicalBounds(  float x,  float y){
    ...
    return new Rectangle2D.Float((float)(lb.getX() + x),(float)(lb.getY() + y),(float)lb.getWidth(),(float)lb.getHeight());
  }
  ...
  public Rectangle2D getVisualBounds(  float x,  float y){
    ...
    return new Rectangle2D.Float((float)(vb.getX() + x),(float)(vb.getY() + y),(float)vb.getWidth(),(float)vb.getHeight());
  }
  public Rectangle2D getAlignBounds(  float x,  float y){
    if (ab == null) {
      ab=createAlignBounds();
    }
    return new Rectangle2D.Float((float)(ab.getX() + x),(float)(ab.getY() + y),(float)ab.getWidth(),(float)ab.getHeight());
  }
  public Rectangle2D getItalicBounds(  float x,  float y){
    if (ib == null) {
      ib=createItalicBounds();
    }
    return new Rectangle2D.Float((float)(ib.getX() + x),(float)(ib.getY() + y),(float)ib.getWidth(),(float)ib.getHeight());
  }
 ...
 */
  ...
  public Rectangle2D handleGetCharVisualBounds(  int index){
    ...
    return new Rectangle2D.Float(charinfo[index + visx],charinfo[index + visy],charinfo[index + visw],charinfo[index + vish]);
  }
  ...
  protected float[] createCharinfo(){
    ...
    if (DEBUG) {
      ...
      for (int i=0; i < numGlyphs; ++i) {
        System.err.println("g: " + i + ", x: "+ glyphinfo[i * numvals + posx]+ ", a: "+ glyphinfo[i * numvals + advx]+ ", n: "+ indices[i]);
      }
    }
    ...
    while (gx != gxlimit) {
      ...
      gx+=xdelta;
      gp+=pdelta;
      while (gx != gxlimit && ((glyphinfo[gp + advx] == 0) || (minIndex != nextMin) || (indices[gx] <= maxIndex)|| (maxIndex - minIndex > clusterExtraGlyphs))) {
        if (!haveCopy) {
          int gps=gp - pdelta;
    ...
    if (DEBUG) {
      ...
      for (int i=0; i < length * numvals; ) {
        System.out.println(" ch: " + Integer.toHexString(chars[start + v2l(i / numvals)]) + " x: "+ glyphinfo[i++]+ " y: "+ glyphinfo[i++]+ " xa: "+ glyphinfo[i++]+ " ya: "+ glyphinfo[i++]+ " l: "+ glyphinfo[i++]+ " t: "+ glyphinfo[i++]+ " w: "+ glyphinfo[i++]+ " h: "+ glyphinfo[i++]);
      }
    }
    return glyphinfo;
  }
 ...
 */
  ...
  public void getJustificationInfos(  GlyphJustificationInfo[] infos,  int infoStart,  int charStart,  int charLimit){
    ...
    GlyphJustificationInfo nullInfo=new GlyphJustificationInfo(0,false,GlyphJustificationInfo.PRIORITY_NONE,0,0,false,GlyphJustificationInfo.PRIORITY_NONE,0,0);
    GlyphJustificationInfo spaceInfo=new GlyphJustificationInfo(size,true,GlyphJustificationInfo.PRIORITY_WHITESPACE,0,size,true,GlyphJustificationInfo.PRIORITY_WHITESPACE,0,size / 4f);
    GlyphJustificationInfo kanjiInfo=new GlyphJustificationInfo(size,true,GlyphJustificationInfo.PRIORITY_INTERCHAR,size,size,false,GlyphJustificationInfo.PRIORITY_NONE,0,0);
    char[] chars=source.getChars();
 ...
 else           if (c >= 0x4e00 && (c < 0xa000) || (c >= 0xac00 && c < 0xd7b0) || (c >= 0xf900 && c < 0xfb00)) {
            info=kanjiInfo;
          }
 else {
            info=nullInfo;
