 ...
 */
    public void layout(    FontStrikeDesc sd,    float[] mat,    int gmask,    int baseIndex,    TextRecord text,    int typo_flags,    Point2D.Float pt,    GVData data);
  }
  /** 
    ...
    public static SDCache get(    Font font,    FontRenderContext frc){
      if (frc.isTransformed()) {
        ...
        if (transform.getTranslateX() != 0 || transform.getTranslateY() != 0) {
          transform=new AffineTransform(transform.getScaleX(),transform.getShearY(),transform.getShearX(),transform.getScaleY(),0,0);
...
private final class EngineRecord {
    ...
    void init(    int start,    int limit,    Font2D font,    int script,    int lang,    int gmask){
      ...
      for (int i=start; i < limit; ++i) {
        ...
        int gc=getType(ch);
        if (gc == NON_SPACING_MARK || gc == ENCLOSING_MARK || gc == COMBINING_SPACING_MARK) {
          this.eflags=0x4;
          break;
        }
      }
