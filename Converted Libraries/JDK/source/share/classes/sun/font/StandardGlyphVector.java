  ...
  public static StandardGlyphVector getStandardGV(  GlyphVector gv,  FontInfo info){
    if (info.aaHint == SunHints.INTVAL_TEXT_ANTIALIAS_ON) {
      ...
      if (aaHint != VALUE_TEXT_ANTIALIAS_ON && aaHint != VALUE_TEXT_ANTIALIAS_GASP) {
        ...
        frc=new FontRenderContext(frc.getTransform(),VALUE_TEXT_ANTIALIAS_ON,frc.getFractionalMetricsHint());
...
static final class GlyphTransformInfo {
    ...
    AffineTransform getGlyphTransform(    int ix){
      ...
      int x=(index - 1) * 6;
      return new AffineTransform(transforms[x + 0],transforms[x + 1],transforms[x + 2],transforms[x + 3],transforms[x + 4],transforms[x + 5]);
    }
    int transformCount(){
      if (transforms == null) {
        return 0;
    ...
    private GlyphStrike getStrikeAtIndex(    GlyphStrike[] strikes,    int strikeIndex){
      ...
      if (strike == null) {
        if (strikeIndex == 0) {
          ...
          AffineTransform gtx=new AffineTransform(transforms[ix],transforms[ix + 1],transforms[ix + 2],transforms[ix + 3],transforms[ix + 4],transforms[ix + 5]);
...
public static final class GlyphStrike {
    ...
    Rectangle2D getGlyphOutlineBounds(    int glyphID,    float x,    float y){
      ...
      if (!result.isEmpty()) {
        result.setRect(result.getMinX() + x + dx,result.getMinY() + y + dy,result.getWidth(),result.getHeight());
      }
      return result;
    }
