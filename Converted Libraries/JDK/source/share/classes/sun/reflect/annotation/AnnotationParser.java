 ...
 */
  ...
  private static Map<Class<? extends Annotation>,Annotation> parseAnnotations2(  byte[] rawAnnotations,  ConstantPool constPool,  Class<?> container){
    ...
    for (int i=0; i < numAnnotations; i++) {
      ...
      if (a != null) {
        ...
        if (type.retention() == RetentionPolicy.RUNTIME)         if (result.put(klass,a) != null)         throw new AnnotationFormatError("Duplicate annotation for class: " + klass + ": "+ a);
      }
    }
    return result;
  }
 ...
 */
  private static Annotation parseAnnotation(  ByteBuffer buf,  ConstantPool constPool,  Class<?> container,  boolean exceptionOnMissingAnnotationClass){
    ...
    try {
      ...
      if (exceptionOnMissingAnnotationClass)       throw new TypeNotPresentException(sig,e);
    ...
    for (int i=0; i < numMembers; i++) {
      ...
      if (memberType == null) {
        ...
        if (value instanceof AnnotationTypeMismatchExceptionProxy)         ((AnnotationTypeMismatchExceptionProxy)value).setMember(type.members().get(memberName));
...
if (!enumType.getName().equals(typeName)) return new AnnotationTypeMismatchExceptionProxy(typeName + "." + constName);
}
