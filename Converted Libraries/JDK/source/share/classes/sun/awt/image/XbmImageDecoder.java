...
public class XbmImageDecoder extends ImageDecoder {
  ...
  private static int XbmHints=(ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES | ImageConsumer.SINGLEPASS| ImageConsumer.SINGLEFRAME);
  ...
  public void produceImage() throws IOException, ImageFormatException {
    ...
    while (!aborted && (c=input.read()) != -1) {
      if ('a' <= c && c <= 'z' || 'A' <= c && c <= 'Z' || '0' <= c && c <= '9' || c == '#' || c == '_') {
        ...
        if (nm[nc - 1] == 'h')         state=1;
 else         if (nm[nc - 1] == 't' && nc > 1 && nm[nc - 2] == 'h')         state=2;
          ...
          int n=0;
          for (int p=0; p < nc; p++)           if ('0' <= (c=nm[p]) && c <= '9')           n=n * 10 + c - '0';
 else {
            n=-1;
            break;
          }
