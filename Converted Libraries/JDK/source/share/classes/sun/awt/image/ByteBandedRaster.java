 ...
 */
  public ByteBandedRaster(  SampleModel sampleModel,  DataBuffer dataBuffer,  Rectangle aRegion,  Point origin,  ByteBandedRaster parent){
 ...
 * @exception RasterFormatExceptionif the specified bounding box is outside of the parent raster.
 */
  ...
  private void verify(  boolean strictCheck){
    ...
    if (data.length == 1) {
      if (data[0].length < maxSize * numDataElements) {
        throw new RasterFormatException("Data array too small " + "(it is " + data[0].length + " and should be "+ (maxSize * numDataElements)+ " )");
      }
    }
 else {
      for (int i=0; i < numDataElements; i++) {
        if (data[i].length < maxSize) {
          throw new RasterFormatException("Data array too small " + "(it is " + data[i].length + " and should be "+ maxSize+ " )");
        }
      }
    }
  }
