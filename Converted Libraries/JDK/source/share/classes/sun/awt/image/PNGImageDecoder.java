...
transparentPixel=((getShort(st + 0) & 0xFF) << 16) | ((getShort(st + 2) & 0xFF) << 8) | ((getShort(st + 4) & 0xFF));
}
break;
case GRAY:
case GRAY | ALPHA:
...
try {
for (int i=0; i < signature.length; i++) if ((signature[i] & 0xFF) != underlyingInputStream.read()) throw new PNGException("Chunk signature mismatch");
...
if (alpha_map == null) cm=new IndexColorModel(bitDepth,red_map.length,red_map,green_map,blue_map);
 else cm=new IndexColorModel(bitDepth,red_map.length,red_map,green_map,blue_map,alpha_map);
...
cm=new IndexColorModel(bitDepth,ramp.length,ramp,ramp,ramp,(transparentPixel & 0xFF));
}
greyModels[llog]=cm;
}
}
...
setColorModel(cm);
int flags=(interlaceMethod != 0 ? ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES : ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES | ImageConsumer.SINGLEPASS| ImageConsumer.SINGLEFRAME);
setHints(flags);
headerComplete();
int samplesPerPixel=((colorType & PALETTE) != 0 ? 1 : ((colorType & COLOR) != 0 ? 3 : 1) + ((colorType & ALPHA) != 0 ? 1 : 0));
...
filterRow(rowByteBuffer,firstRow ? null : prevRowByteBuffer,rowFilter,rowByteWidth,bytesPerPixel);
int col=sCol;
int spos=0;
int pixel=0;
while (col < width) {
...
isTransparent&=(rowByteBuffer[spos + i] & 0xFF) == (transparentPixel_16[i] & 0xFF);
}
if (!isTransparent) {
pixel|=0xff000000;
}
...
break;
case 4:
bPixels[col + rowOffset]=(byte)((rowByteBuffer[spos >> 1] >> ((1 - (spos & 1)) * 4)) & 15);
spos++;
break;
case 8:
...
if (interlaceMethod == 0) if (wPixels != null) {
...
break;
case 1:
for (x=bytesPerSample; x < rowByteWidth; x++) rowByteBuffer[x]+=rowByteBuffer[x - bytesPerSample];
break;
case 2:
...
for (; x < rowByteWidth; x++) rowByteBuffer[x]+=((prevRow[x] & 0xFF) + (rowByteBuffer[x - bytesPerSample] & 0xFF)) >> 1;
}
 else for (x=bytesPerSample; x < rowByteWidth; x++) rowByteBuffer[x]+=(rowByteBuffer[x - bytesPerSample] & 0xFF) >> 1;
break;
case 4:
if (prevRow != null) {
...
rowByteBuffer[x]+=(pa <= pb) && (pa <= pc) ? a : pb <= pc ? b : c;
 ...
 else for (x=bytesPerSample; x < rowByteWidth; x++) rowByteBuffer[x]+=rowByteBuffer[x - bytesPerSample];
break;
default :
...
return ((inbuf[pos] & 0xFF) << 24) | ((inbuf[pos + 1] & 0xFF) << 16) | ((inbuf[pos + 2] & 0xFF) << 8)| ((inbuf[pos + 3] & 0xFF));
}
...
while (chunkLength == 0 && getChunk()) if (handleChunk(chunkKey,inbuf,chunkStart,chunkLength)) chunkLength=0;
...
for (int n=0; n < 256; n++) {
int c=n;
for (int k=0; k < 8; k++) if ((c & 1) != 0) c=0xedb88320 ^ (c >>> 1);
 else c=c >>> 1;
crc_table[n]=c;
}
}
