 ...
 * @author Rosanna Lee
 */
final public class LdapCtx extends ComponentDirContext implements EventDirContext, LdapContext {
...
static {
  ...
  LdapCtx(  LdapCtx existing,  String newDN) throws NamingException {
    ...
    clnt=existing.clnt;
    clnt.incRefCount();
    parentIsLdapCtx=((newDN == null || newDN.equals(existing.currentDN)) ? existing.parentIsLdapCtx : true);
  ...
  protected void c_bind(  Name name,  Object obj,  Attributes attrs,  Continuation cont) throws NamingException {
    ...
    try {
      ...
      if (obj == null) {
        ...
        attrs=Obj.determineBindAttrs(addrEncodingSeparator,obj,attrs,false,name,this,envprops);
      }
  ...
  protected void c_rebind(  Name name,  Object obj,  Attributes attrs,  Continuation cont) throws NamingException {
    ...
    try {
      ...
      if (obj != null) {
        attrs=Obj.determineBindAttrs(addrEncodingSeparator,obj,attrs,inputAttrs != attrs,name,this,envprops);
      }
      String newDN=fullyQualifiedName(name);
      ...
      if ((addEx != null && !(addEx instanceof LdapReferralException)) || answer.status != LdapClient.LDAP_SUCCESS) {
  ...
  protected DirContext c_createSubcontext(  Name name,  Attributes attrs,  Continuation cont) throws NamingException {
    ...
    try {
      ensureOpen();
      if (attrs == null) {
        Attribute oc=new BasicAttribute(Obj.JAVA_ATTRIBUTES[Obj.OBJECT_CLASS],Obj.JAVA_OBJECT_CLASSES[Obj.STRUCTURAL]);
        oc.add("top");
 ...
 */
  ...
  private static boolean containsIgnoreCase(  NamingEnumeration enumStr,  String str) throws NamingException {
    String strEntry;
...
cont.setError(this,name);
try {
Attribute objectClassAttr=c_getAttributes(name,new String[]{"objectclass"},cont).get("objectclass");
if (objectClassAttr == null || objectClassAttr.size() == 0) {
return EMPTY_SCHEMA;
}
Context ocSchema=(Context)c_getSchema(name,cont).lookup(LdapSchemaParser.OBJECTCLASS_DEFINITION_NAME);
HierMemDirCtx objectClassCtx=new HierMemDirCtx();
DirContext objectClassDef;
String objectClassName;
for (Enumeration objectClasses=objectClassAttr.getAll(); objectClasses.hasMoreElements(); ) {
...
SearchControls constraints=new SearchControls(SearchControls.OBJECT_SCOPE,0,0,SCHEMA_ATTRIBUTES,true,false);
Name sse=(new CompositeName()).add(subschemasubentry);
NamingEnumeration results=searchAux(sse,"(objectClass=subschema)",constraints,false,true,new Continuation());
if (!results.hasMore()) {
...
return LdapSchemaCtx.createSchemaTree(envprops,subschemasubentry,(LdapCtx)obj,result.getAttributes(),netscapeSchemaBug);
}
private String getSchemaEntry(Name name,boolean relative) throws NamingException {
SearchControls constraints=new SearchControls(SearchControls.OBJECT_SCOPE,0,0,new String[]{"subschemasubentry"},false,false);
...
protected NamingEnumeration c_search(Name name,Attributes matchingAttributes,Continuation cont) throws NamingException {
...
NamingEnumeration searchAux(Name name,String filter,SearchControls cons,boolean relative,boolean waitForReply,Continuation cont) throws NamingException {
LdapResult answer=null;
...
LdapSearchEnumeration res=new LdapSearchEnumeration(this,answer,fullyQualifiedName(name),args,cont);
...
LdapResult answer=clnt.search(nm,scope,derefAliases,(int)cons.getCountLimit(),secLimit,cons.getReturningObjFlag() ? false : typesOnly,retattrs,filter,batchSize,reqCtls,binaryAttrs,waitForReply,replyQueueSize);
respCtls=answer.resControls;
return answer;
}
 catch (IOException e) {
...
port_number=DEFAULT_PORT;
 ...
 else if (propName.equals(VERSION) || propName.equals(SOCKET_FACTORY)) {
closeConnection(SOFT_CLOSE);
}
 else if (propName.equals(Context.SECURITY_AUTHENTICATION) || propName.equals(Context.SECURITY_PRINCIPAL) || propName.equals(Context.SECURITY_CREDENTIALS)) {
sharable=false;
}
...
throw new IllegalArgumentException("Illegal value for " + Context.REFERRAL + " property.");
}
}
 else {
...
StringTokenizer tokens=new StringTokenizer(attrIds.toLowerCase()," ");
...
clnt=LdapClient.getInstance(usePool,hostname,port_number,socketFactory,connectTimeout,readTimeout,trace,ldapVersion,authMechanism,bindCtls,secProtocol,user,passwd,envprops);
...
LdapResult answer=clnt.authenticate(initial,user,passwd,ldapVersion,authMechanism,bindCtls,envprops);
respCtls=answer.resControls;
if (answer.status != LdapClient.LDAP_SUCCESS) {
if (initial) {
...
processReturnCode(answer,(new CompositeName()).add(currentDN),this,remainName,envprops,fullyQualifiedName(remainName));
}
...
r=new LdapReferralException(resolvedName,resolvedObj,remainName,msg,envprops,fullDN,handleReferrals,reqCtls);
...
if (((res.entries == null) || (res.entries.size() == 0)) && (res.referrals.size() == 1)) {
...
ensureOpen(startTLS);
try {
LdapResult answer=clnt.extendedOp(request.getID(),request.getEncodedValue(),reqCtls,startTLS);
respCtls=answer.resControls;
if (answer.status != LdapClient.LDAP_SUCCESS) {
processReturnCode(answer,new CompositeName());
}
int len=(answer.extensionValue == null) ? 0 : answer.extensionValue.length;
ExtendedResponse er=request.createExtendedResponse(answer.extensionId,answer.extensionValue,0,len);
...
controls[i]=myResponseControlFactory.getControlInstance((Control)ctls.elementAt(i));
if (controls[i] == null) {
controls[i]=ControlFactory.getControlInstance((Control)ctls.elementAt(i),this,envprops);
}
}
return controls;
}
...
if (eventSupport == null) eventSupport=new EventSupport(this);
eventSupport.addNamingListener(getTargetName(new CompositeName(nm)),filter,cloneSearchControls(ctls),l);
if (l instanceof UnsolicitedNotificationListener && !unsolicited) {
addUnsolicited();
}
}
