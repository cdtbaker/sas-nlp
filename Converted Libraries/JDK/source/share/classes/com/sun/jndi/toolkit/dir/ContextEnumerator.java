...
public class ContextEnumerator implements NamingEnumeration {
  ...
  public ContextEnumerator(  Context context,  int scope) throws NamingException {
    this(context,scope,"",scope != SearchControls.ONELEVEL_SCOPE);
  }
  protected ContextEnumerator(  Context context,  int scope,  String contextName,  boolean returnSelf) throws NamingException {
    if (context == null) {
  ...
  public boolean hasMore() throws NamingException {
    return !rootProcessed || (scope != SearchControls.OBJECT_SCOPE && hasMoreDescendants());
  }
  ...
  private Binding getNextChild() throws NamingException {
    ...
    if (oldBinding.isRelative() && !contextName.equals("")) {
      ...
      newBinding=new Binding(newName.toString(),oldBinding.getClassName(),oldBinding.getObject(),oldBinding.isRelative());
    }
 else {
      if (debug) {
  ...
  private void prepNextChild() throws NamingException {
    ...
    if (scope == SearchControls.SUBTREE_SCOPE && currentChild.getObject() instanceof Context) {
