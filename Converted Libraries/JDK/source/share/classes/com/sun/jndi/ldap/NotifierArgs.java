...
final class NotifierArgs {
  ...
  public boolean equals(  Object obj){
    if (obj instanceof NotifierArgs) {
      ...
      return mask == target.mask && name.equals(target.name) && filter.equals(target.filter) && checkControls(target.controls);
    }
    return false;
  }
  ...
  private int sum=-1;
  public int hashCode(){
    if (sum == -1)     sum=mask + name.hashCode() + filter.hashCode()+ controlsCode();
    return sum;
  }
  private int controlsCode(){
    if (controls == null)     return 0;
    int total=(int)controls.getTimeLimit() + (int)controls.getCountLimit() + (controls.getDerefLinkFlag() ? 1 : 0)+ (controls.getReturningObjFlag() ? 1 : 0);
