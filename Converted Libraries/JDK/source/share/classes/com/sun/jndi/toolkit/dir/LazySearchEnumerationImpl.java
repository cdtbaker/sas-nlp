        ...
        if (obj instanceof DirContext) {
          ...
          if (filter.check(targetAttrs)) {
            if (!cons.getReturningObjFlag()) {
              ...
              try {
                Name nm=(context != null ? new CompositeName(next.getName()) : null);
            ...
            answer=new SearchResult(next.getName(),next.getClassName(),obj,SearchFilter.selectAttributes(targetAttrs,cons.getReturningAttributes()),true);
            if (!remove)             nextMatch=answer;
            return answer;
          }
        }
      }
