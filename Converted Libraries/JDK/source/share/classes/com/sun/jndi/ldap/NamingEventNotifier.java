...
final class NamingEventNotifier implements Runnable {
  ...
  NamingEventNotifier(  EventSupport support,  LdapCtx ctx,  NotifierArgs info,  NamingListener firstListener) throws NamingException {
    ...
    Control psearch;
    try {
      psearch=new PersistentSearchControl(info.mask,true,true,Control.CRITICAL);
    }
 ...
 */
  public void run(){
    try {
      ...
      Name nm=(info.name == null || info.name.equals("")) ? new CompositeName() : new CompositeName().add(info.name);
      results=context.searchAux(nm,info.filter,info.controls,true,false,cont);
      ...
      while (results.hasMore()) {
        ...
        respctls=(si instanceof HasControls) ? ((HasControls)si).getControls() : null;
        if (debug) {
...
NamingEvent e=new NamingEvent(eventSrc,NamingEvent.OBJECT_ADDED,newBd,null,new Long(changeID));
