 ...
 */
  private int parseIntWithTag(  int tag) throws DecodeException {
    if (parseByte() != tag) {
      throw new DecodeException("Encountered ASN.1 tag " + Integer.toString(buf[offset - 1] & 0xff) + " (expected tag "+ Integer.toString(tag)+ ")");
    }
    int len=parseLength();
    if (len > 4) {
 ...
 */
  public String parseStringWithTag(  int tag,  boolean decodeUTF8,  int rlen[]) throws DecodeException {
    ...
    if ((st=parseByte()) != tag) {
      throw new DecodeException("Encountered ASN.1 tag " + Integer.toString((byte)st) + " (expected tag "+ tag+ ")");
    }
    int len=parseLength();
 ...
 */
  public byte[] parseOctetString(  int tag,  int rlen[]) throws DecodeException {
    ...
    int st;
    if ((st=parseByte()) != tag) {
      throw new DecodeException("Encountered ASN.1 tag " + Integer.toString(st) + " (expected tag "+ Integer.toString(tag)+ ")");
    }
    int len=parseLength();
    if (len > bufsize - offset) {
      throw new DecodeException("Insufficient data");
