...
interface StringFilter extends AttrFilter {
  ...
  public boolean check(  Attributes targetAttrs) throws NamingException {
    if (targetAttrs == null)     return false;
  ...
  protected void normalizeFilter(){
    ...
    if (debug) {
      System.out.println("SearchFilter: normalized filter:" + filter);
    }
  }
  ...
  protected StringFilter createNextFilter() throws InvalidSearchFilterException {
    ...
    try {
      if (getCurrentChar() != BEGIN_FILTER_TOKEN) {
        throw new InvalidSearchFilterException("expected \"" + BEGIN_FILTER_TOKEN + "\" at position "+ pos);
      }
      this.consumeChar();
      skipWhiteSpace();
...
case AND_TOKEN:
        if (debug) {
          System.out.println("SearchFilter: creating AND");
        }
...
case OR_TOKEN:
if (debug) {
  System.out.println("SearchFilter: creating OR");
}
...
default :
if (debug) {
System.out.println("SearchFilter: creating SIMPLE");
}
...
throw new InvalidSearchFilterException("expected \"" + END_FILTER_TOKEN + "\" at position "+ pos);
}
this.consumeChar();
}
...
throw new InvalidSearchFilterException("Unable to parse " + "character " + pos + " in \""+ this.filter+ "\"");
}
return filter;
}
...
if (debug) {
System.out.println("relSubString: " + beginIndex + " "+ endIndex);
}
...
if (debug) {
System.out.println("CompoundFilter: adding");
}
...
if (debug) {
System.out.println("AtomicFilter: = at " + i);
}
int qualifier=SearchFilter.this.relCharAt(i - 1);
switch (qualifier) {
case APPROX_TOKEN:
if (debug) {
System.out.println("Atomic: APPROX found");
}
...
case GREATER_TOKEN:
if (debug) {
System.out.println("Atomic: GREATER found");
}
...
case LESS_TOKEN:
if (debug) {
System.out.println("Atomic: LESS found");
}
...
case EXTEND_TOKEN:
if (debug) {
System.out.println("Atomic: EXTEND found");
}
throw new OperationNotSupportedException("Extensible match not supported");
default :
if (debug) {
System.out.println("Atomic: EQUAL found");
}
...
if (debug) {
System.out.println("AtomicFilter: " + attrID + "="+ value);
}
}
...
if (debug) {
System.out.println("AtomicFilter: should never " + "here");
}
return false;
}
...
if (debug) {
System.out.println("Atomic: comparing: " + val);
}
switch (matchType) {
case APPROX_MATCH:
case EQUAL_MATCH:
...
if (debug) {
System.out.println("Atomic: EQUAL match");
}
return true;
}
break;
case GREATER_MATCH:
if (debug) {
System.out.println("Atomic: GREATER match");
}
...
case LESS_MATCH:
if (debug) {
System.out.println("Atomic: LESS match");
}
if (val.compareTo(this.value) <= 0) {
return true;
}
...
default :
if (debug) {
System.out.println("AtomicFilter: unkown " + "matchType");
}
}
}
return false;
...
if (debug) {
System.out.println("simple presence assertion");
}
return true;
}
...
if (debug) {
System.out.println("doing substring comparison");
}
int currentPos=0;
StringTokenizer subStrs=new StringTokenizer(proto,"*",false);
if (proto.charAt(0) != WILDCARD_TOKEN && !value.toString().toLowerCase().startsWith(subStrs.nextToken().toLowerCase())) {
if (debug) {
...
if (debug) {
System.out.println("looking for \"" + currentStr + "\"");
}
currentPos=value.toLowerCase().indexOf(currentStr.toLowerCase(),currentPos);
if (currentPos == -1) {
return false;
}
currentPos+=currentStr.length();
}
if (proto.charAt(proto.length() - 1) != WILDCARD_TOKEN && currentPos != value.length()) {
if (debug) {
System.out.println("faild final test");
}
return false;
}
return true;
...
if (where == start || where == -1 || val.charAt(where - 1) != '\\') return where;
start=where + 1;
}
return -1;
}
