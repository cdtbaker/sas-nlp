...
if (useFactory) {
try {
return DirectoryManager.getObjectInstance(target,name,this,myEnv,(target instanceof HierMemDirCtx) ? ((HierMemDirCtx)target).attrs : null);
}
...
if (useFactory) {
DirStateFactory.Result res=DirectoryManager.getStateToBind(obj,name,this,myEnv,attrs);
obj=res.getObject();
...
HierMemDirCtx ctx=(HierMemDirCtx)doLookup(getInternalName(newname),false);
...
protected NamingEnumeration doListBindings(boolean useFactory) throws NamingException {
...
case ADD_ATTRIBUTE:
if (debug) {
System.out.println("HierMemDSCtx: adding " + mod.getAttribute().toString());
}
...
return search(myParser.parse(name),matchingAttributes,attributesToReturn);
}
...
return new LazySearchEnumerationImpl(target.doListBindings(false),new ContainmentFilter(matchingAttributes),cons,this,myEnv,false);
}
public NamingEnumeration search(Name name,String filter,SearchControls cons) throws NamingException {
DirContext target=(DirContext)doLookup(name,false);
SearchFilter stringfilter=new SearchFilter(filter);
return new LazySearchEnumerationImpl(new HierContextEnumerator(target,(cons != null) ? cons.getSearchScope() : SearchControls.ONELEVEL_SCOPE),stringfilter,cons,this,myEnv,alwaysUseFactory);
}
...
try {
answer=DirectoryManager.getObjectInstance(obj,name,HierMemDirCtx.this,env,attrs);
}
 catch (NamingException e) {
throw e;
}
...
protected NamingEnumeration getImmediateChildren(Context ctx) throws NamingException {
