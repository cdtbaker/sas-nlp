 ...
 */
  ...
  private void initFromEnvironment() throws InvalidAttributeIdentifierException {
    ...
    authoritative="true".equalsIgnoreCase((String)environment.get(Context.AUTHORITATIVE));
    String val=(String)environment.get(RECURSION);
    recursion=((val == null) || "true".equalsIgnoreCase(val));
    val=(String)environment.get(INIT_TIMEOUT);
    timeout=(val == null) ? DEFAULT_INIT_TIMEOUT : Integer.parseInt(val);
    val=(String)environment.get(RETRIES);
    retries=(val == null) ? DEFAULT_RETRIES : Integer.parseInt(val);
  }
  private CT getLookupCT(  String attrId) throws InvalidAttributeIdentifierException {
    return (attrId == null) ? new CT(ResourceRecord.CLASS_INTERNET,ResourceRecord.TYPE_TXT) : fromAttrId(attrId);
  }
  public Object c_lookup(  Name name,  Continuation cont) throws NamingException {
    ...
    try {
      ...
      ResourceRecords rrs=getResolver().query(fqdn,lookupCT.rrclass,lookupCT.rrtype,recursion,authoritative);
      Attributes attrs=rrsToAttrs(rrs,null);
      DnsContext ctx=new DnsContext(this,fqdn);
      return DirectoryManager.getObjectInstance(ctx,name,this,environment,attrs);
    }
  ...
  public Attributes c_getAttributes(  Name name,  String[] attrIds,  Continuation cont) throws NamingException {
    ...
    try {
      ...
      ResourceRecords rrs=getResolver().query(fqdn,ct.rrclass,ct.rrtype,recursion,authoritative);
  ...
  public Name composeName(  Name name,  Name prefix) throws NamingException {
    ...
    result=(prefix == prefixC) ? (CompositeName)prefixC.clone() : prefixC;
    result.addAll(nameC);
    if (parentIsDns) {
  ...
  DnsName fullyQualify(  Name name) throws NamingException {
    ...
    if (dnsName.hasRootLabel()) {
      if (domain.size() == 1) {
        ...
        throw new InvalidNameException("DNS name " + dnsName + " not relative to "+ domain);
      }
    }
  ...
  private static boolean classAndTypeMatch(  int rrclass,  int rrtype,  CT[] cts){
    ...
    for (int i=0; i < cts.length; i++) {
      CT ct=cts[i];
      boolean classMatch=(ct.rrclass == ANY) || (ct.rrclass == rrclass);
      boolean typeMatch=(ct.rrtype == ANY) || (ct.rrtype == rrtype);
...
synchronized (zoneTree) {
    ...
    dprint("Deepest related zone in zone tree: " + ((znode != null) ? znode.getLabel() : "[none]"));
    NameNode topOfZone;
    NameNode nnode;
    if (znode != null) {
synchronized (znode) {
...
synchronized (znode) {
    ...
    zone=getResolver().findZoneName(fqdn,ResourceRecord.CLASS_INTERNET,recursion);
    dprint("Node's zone is " + zone);
synchronized (zoneTree) {
      znode=(ZoneNode)zoneTree.add(zone,1);
    }
synchronized (znode) {
      topOfZone=znode.isPopulated() ? znode.getContents() : populateZone(znode,zone);
    }
  ...
  private NameNode populateZone(  ZoneNode znode,  DnsName zone) throws NamingException {
    ...
    ResourceRecords rrs=getResolver().queryZone(zone,ResourceRecord.CLASS_INTERNET,recursion);
  ...
  private boolean isZoneCurrent(  ZoneNode znode,  DnsName zone) throws NamingException {
    ...
    ResourceRecord soa=getResolver().findSoa(zone,ResourceRecord.CLASS_INTERNET,recursion);
synchronized (znode) {
      if (soa == null) {
        znode.depopulate();
      }
...
class NameClassPairEnumeration implements NamingEnumeration {
  ...
  public Object nextElement(){
    try {
      ...
      throw (new java.util.NoSuchElementException("javax.naming.NamingException was thrown: " + e.getMessage()));
    }
  }
}
class BindingEnumeration extends NameClassPairEnumeration {
  ...
  public Object next() throws NamingException {
    ...
    try {
      Object obj=DirectoryManager.getObjectInstance(child,cname,ctx,child.environment,null);
