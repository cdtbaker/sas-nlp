 ...
 */
  CNCtx(  ORB orb,  OrbReuseTracker tracker,  NamingContext nctx,  Hashtable env,  NameComponent[] name) throws NamingException {
    if (orb == null || nctx == null)     throw new ConfigurationException("Must supply ORB or NamingContext");
    _orb=orb;
    orbTracker=tracker;
    if (orbTracker != null) {
 ...
 */
  ...
  private void setOrbAndRootContext(  ORB orb,  org.omg.CORBA.Object ncRef) throws NamingException {
    _orb=orb;
    try {
      _nc=NamingContextHelper.narrow(ncRef);
      if (_nc == null) {
  ...
  private String getStringifiedIor(  String url) throws NamingException {
    if (url.startsWith("IOR:") || url.startsWith("corbaloc:")) {
      ...
      try {
        ...
        NamingException ne=new ConfigurationException("Invalid URL: " + url);
        ne.setRootCause(e);
        throw ne;
      }
 finally {
  ...
  java.lang.Object callResolve(  NameComponent[] path) throws NamingException {
    try {
      ...
      try {
        ...
        if (nc != null) {
          return new CNCtx(_orb,orbTracker,nc,_env,makeFullName(path));
        }
 else {
          return obj;
        }
 ...
 */
  ...
  static private javax.naming.Context getContinuationContext(  CannotProceedException cpe) throws NamingException {
    try {
 ...
 */
  ...
  private boolean leafNotFound(  NotFound e,  NameComponent leaf){
    NameComponent rest;
    return e.why.value() == NotFoundReason._missing_node && e.rest_of_name.length == 1 && (rest=e.rest_of_name[0]).id.equals(leaf.id) && (rest.kind == leaf.kind || (rest.kind != null && rest.kind.equals(leaf.kind)));
  }
  /** 
 * Converts the "String" name into a CompositeName object and
 ...
 */
  public void unbind(  Name name) throws NamingException {
    if (name.size() == 0)     throw new InvalidNameException("Name is empty");
    NameComponent[] path=CNNameParser.nameToCosName(name);
    try {
      callUnbind(path);
    }
 ...
 */
  public void rename(  Name oldName,  Name newName) throws NamingException {
    if (_nc == null)     throw new ConfigurationException("Context does not have a corresponding NamingContext");
    if (oldName.size() == 0 || newName.size() == 0)     throw new InvalidNameException("One or both names empty");
 ...
 */
  private javax.naming.Context callBindNewContext(  NameComponent[] path) throws NamingException {
    ...
    try {
      ...
      return new CNCtx(_orb,orbTracker,nctx,_env,makeFullName(path));
    }
 catch (    Exception e) {
      throw ExceptionMapper.mapException(e,this,path);
    }
 ...
 */
  ...
  synchronized public void decEnumCount() throws NamingException {
    if (orbTracker == null) {
      return;
    }
    enumCount--;
    if (debug) {
      System.out.println("decEnumCount, new count:" + enumCount + "    isCloseCalled:"+ isCloseCalled);
    }
