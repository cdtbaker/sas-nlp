...
final class LdapReferralContext implements DirContext, LdapContext {
  ...
  LdapReferralContext(  LdapReferralException ex,  Hashtable env,  Control[] connCtls,  Control[] reqCtls,  String nextName,  boolean skipThisReferral,  int handleReferrals) throws NamingException {
    ...
    while (true) {
      ...
      if (obj instanceof DirContext) {
        ...
        NamingException ne=new NotContextException("Cannot create context for: " + referral);
  ...
  private void initDefaults(  String referral,  String nextName) throws NamingException {
    ...
    if (urlString == null) {
      ...
      urlName=urlString.equals("") ? new CompositeName() : new CompositeName().add(urlString);
    }
  }
  ...
  public NamingEnumeration search(  String name,  Attributes matchingAttributes) throws NamingException {
    return search(toName(name),SearchFilter.format(matchingAttributes),new SearchControls());
  }
  public NamingEnumeration search(  Name name,  Attributes matchingAttributes) throws NamingException {
    return search(name,SearchFilter.format(matchingAttributes),new SearchControls());
  }
  public NamingEnumeration search(  String name,  Attributes matchingAttributes,  String[] attributesToReturn) throws NamingException {
    SearchControls cons=new SearchControls();
    cons.setReturningAttributes(attributesToReturn);
    return search(toName(name),SearchFilter.format(matchingAttributes),cons);
  }
  ...
  public NamingEnumeration search(  Name name,  String filter,  SearchControls cons) throws NamingException {
    ...
    try {
      NamingEnumeration se=refCtx.search(overrideName(name),overrideFilter(filter),overrideAttributesAndScope(cons));
      refEx.setNameResolved(true);
  ...
  public NamingEnumeration search(  Name name,  String filterExpr,  Object[] filterArgs,  SearchControls cons) throws NamingException {
    ...
    try {
      ...
      if (urlFilter != null) {
        se=refCtx.search(overrideName(name),urlFilter,overrideAttributesAndScope(cons));
      }
 else {
        se=refCtx.search(overrideName(name),filterExpr,filterArgs,overrideAttributesAndScope(cons));
      }
      refEx.setNameResolved(true);
      ((ReferralEnumeration)se).appendUnprocessedReferrals(refEx);
      return (se);
  ...
  private SearchControls overrideAttributesAndScope(  SearchControls cons){
    ...
    if ((urlScope != null) || (urlAttrs != null)) {
      urlCons=new SearchControls(cons.getSearchScope(),cons.getCountLimit(),cons.getTimeLimit(),cons.getReturningAttributes(),cons.getReturningObjFlag(),cons.getDerefLinkFlag());
      if (urlScope != null) {
