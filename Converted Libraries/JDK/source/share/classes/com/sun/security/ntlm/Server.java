 ...
 */
  public String[] verify(  byte[] type3,  byte[] nonce) throws NTLMException {
    ...
    if (password == null) {
      throw new NTLMException(NTLMException.USER_UNKNOWN,"Unknown user");
    }
    ...
    if (!verified && (allVersion || v == Version.NTLMv2)) {
      ...
      byte[] nthash=calcNTHash(pw2);
      if (incomingLM.length > 0) {
        byte[] clientNonce=Arrays.copyOfRange(incomingLM,16,incomingLM.length);
        byte[] lmresponse=calcV2(nthash,username.toUpperCase(Locale.US) + incomingDomain,clientNonce,nonce);
        if (Arrays.equals(lmresponse,incomingLM)) {
          verified=true;
        }
      }
      if (incomingNTLM.length > 0) {
        ...
        byte[] ntlmresponse=calcV2(nthash,username.toUpperCase(Locale.US) + incomingDomain,clientBlob,nonce);
    ...
    if (!verified) {
      throw new NTLMException(NTLMException.AUTH_FAILED,"None of LM and NTLM verified");
    }
