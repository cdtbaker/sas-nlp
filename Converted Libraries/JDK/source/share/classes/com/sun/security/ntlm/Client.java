 ...
 */
  public byte[] type3(  byte[] type2,  byte[] nonce) throws NTLMException {
    ...
    if (domain == null) {
      throw new NTLMException(NTLMException.NO_DOMAIN_INFO,"No domain info");
    }
    ...
    if (v == Version.NTLM) {
      ...
      byte[] nthash=calcNTHash(pw2);
      if (writeLM)       lm=calcV2(nthash,username.toUpperCase(Locale.US) + domain,nonce,challenge);
      if (writeNTLM) {
        byte[] alist=type2.length > 48 ? r.readSecurityBuffer(40) : new byte[0];
        byte[] blob=new byte[32 + alist.length];
        System.arraycopy(new byte[]{1,1,0,0,0,0,0,0},0,blob,0,8);
        byte[] time=BigInteger.valueOf(new Date().getTime()).add(new BigInteger("11644473600000")).multiply(BigInteger.valueOf(10000)).toByteArray();
        ...
        ntlm=calcV2(nthash,username.toUpperCase(Locale.US) + domain,blob,challenge);
      }
    }
