 ...
 */
  DigestMD5Client(  String authzid,  String protocol,  String serverName,  Map props,  CallbackHandler cbh) throws SaslException {
 ...
 */
  public byte[] evaluateChallenge(  byte[] challengeData) throws SaslException {
    if (challengeData.length > MAX_CHALLENGE_LENGTH) {
      throw new SaslException("DIGEST-MD5: Invalid digest-challenge length. Got:  " + challengeData.length + " Expected < "+ MAX_CHALLENGE_LENGTH);
    }
    byte[][] challengeVal;
switch (step) {
case 2:
      List<byte[]> realmChoices=new ArrayList<byte[]>(3);
    challengeVal=parseDirectives(challengeData,DIRECTIVE_KEY,realmChoices,REALM);
  try {
...
case 3:
try {
  challengeVal=parseDirectives(challengeData,DIRECTIVE_KEY,null,REALM);
...
byte[] serverQop=parseQop(qopOptions,serverQopTokens,true);
...
digestResp.write(("username=\"" + quotedStringValue(username) + "\",").getBytes(encoding));
if (negotiatedRealm.length() > 0) {
digestResp.write(("realm=\"" + quotedStringValue(negotiatedRealm) + "\",").getBytes(encoding));
}
...
digestResp.write(("nc=" + nonceCountToHex(nonceCount) + ",").getBytes(encoding));
...
digestResp.write(generateResponseValue("AUTHENTICATE",digestUri,negotiatedQop,username,negotiatedRealm,passwd,nonce,cnonce,nonceCount,authzidBytes));
digestResp.write(',');
}
 catch (Exception e) {
...
try {
byte[] expected=generateResponseValue("",digestUri,negotiatedQop,username,negotiatedRealm,passwd,nonce,cnonce,nonceCount,authzidBytes);
