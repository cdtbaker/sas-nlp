 ...
 * @deprecated As of JDK&nbsp;1.4, replaced by{@link sun.security.provider.PolicyParser}.
 * This class is entirely deprecated.
 * @author Roland Schemers
 * @since 1.2
 */
@Deprecated class PolicyParser {
  private static final java.util.ResourceBundle rb=java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<java.util.ResourceBundle>(){
  ...
  private String expand(  String value) throws PropertyExpander.ExpandException {
    if (expandProp)     return PropertyExpander.expand(value);
 else     return value;
  }
  /** 
 ...
 */
  private void parseKeyStoreEntry() throws ParsingException, IOException {
    ...
    if (peek("\"")) {
      ...
      throw new ParsingException(st.lineno(),rb.getString("expected.keystore.type"));
    }
  }
  /** 
  ...
  private GrantEntry parseGrantEntry() throws ParsingException, IOException {
    ...
    while (!peek("{")) {
      if (peekAndMatch("Codebase")) {
        ...
        if (principalClass.equals(PrincipalEntry.WILDCARD_CLASS) && !principalName.equals(PrincipalEntry.WILDCARD_NAME)) {
          ...
          throw new ParsingException(st.lineno(),rb.getString("can.not.specify.Principal.with.a." + "wildcard.class.without.a.wildcard.name"));
        }
        try {
        ...
        throw new ParsingException(st.lineno(),rb.getString("expected.codeBase.or.SignedBy"));
      }
    }
    if (principals == null) {
      throw new ParsingException(st.lineno(),rb.getString("only.Principal.based.grant.entries.permitted"));
    }
    ...
    while (!peek("}")) {
      if (peek("Permission")) {
        ...
        throw new ParsingException(st.lineno(),rb.getString("expected.permission.entry"));
      }
    }
    match("}");
    try {
...
throw new ParsingException(st.lineno(),expect,rb.getString("number.") + String.valueOf(st.nval));
...
switch (lookahead) {
case StreamTokenizer.TT_NUMBER:
throw new ParsingException(st.lineno(),";",rb.getString("number.") + String.valueOf(st.nval));
case StreamTokenizer.TT_EOF:
...
out.print(pe.principalClass + " \"" + pe.principalName+ "\"");
