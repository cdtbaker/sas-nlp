...
break;
default :
throw new NTLMException(NTLMException.BAD_VERSION,"Unknown version " + version);
}
try {
...
try {
return internal[offset] & 0xff + (internal[offset + 1] & 0xff << 8) + (internal[offset + 2] & 0xff << 16)+ (internal[offset + 3] & 0xff << 24);
}
 catch (ArrayIndexOutOfBoundsException ex) {
throw new NTLMException(NTLMException.PACKET_READ_ERROR,"Input message incorrect size");
}
}
int readShort(int offset) throws NTLMException {
try {
return internal[offset] & 0xff + (internal[offset + 1] & 0xff << 8);
}
 catch (ArrayIndexOutOfBoundsException ex) {
throw new NTLMException(NTLMException.PACKET_READ_ERROR,"Input message incorrect size");
}
...
try {
return Arrays.copyOfRange(internal,pos,pos + readShort(offset));
}
 catch (ArrayIndexOutOfBoundsException ex) {
throw new NTLMException(NTLMException.PACKET_READ_ERROR,"Input message incorrect size");
}
...
throw new NTLMException(NTLMException.PACKET_READ_ERROR,"Invalid input encoding");
}
}
}
/** 
...
current=len;
System.arraycopy(new byte[]{'N','T','L','M','S','S','P',0,(byte)type},0,internal,0,9);
}
...
try {
writeSecurityBuffer(offset,str == null ? null : str.getBytes(unicode ? "UnicodeLittleUnmarked" : "ISO8859_1"));
}
 catch (UnsupportedEncodingException ex) {
assert false;
}
...
try {
byte[] ntlmv2hash=hmacMD5(nthash,text.getBytes("UnicodeLittleUnmarked"));
