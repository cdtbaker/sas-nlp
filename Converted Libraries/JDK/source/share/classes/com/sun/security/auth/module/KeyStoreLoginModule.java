...
public class KeyStoreLoginModule implements LoginModule {
  ...
  private static final TextOutputCallback bannerCallback=new TextOutputCallback(TextOutputCallback.INFORMATION,rb.getString("Please.enter.keystore.information"));
  private final ConfirmationCallback confirmationCallback=new ConfirmationCallback(ConfirmationCallback.INFORMATION,ConfirmationCallback.OK_CANCEL_OPTION,ConfirmationCallback.OK);
...
case AUTHENTICATED:
    if (token && !nullStream) {
      throw new LoginException("if keyStoreType is " + P11KEYSTORE + " then keyStoreURL must be "+ NONE);
    }
  if (token && privateKeyPasswordURL != null) {
    throw new LoginException("if keyStoreType is " + P11KEYSTORE + " then privateKeyPasswordURL must not be specified");
  }
...
break;
case NORMAL:
keyPassCallback=new PasswordCallback(rb.getString("Private.key.password.optional."),false);
case TOKEN:
...
LoginException le=new LoginException("Problem accessing keystore password \"" + keyStorePasswordURL + "\"");
le.initCause(e);
throw le;
}
 finally {
...
LoginException le=new LoginException("Problem accessing private key password \"" + privateKeyPasswordURL + "\"");
le.initCause(e);
throw le;
}
 finally {
...
LoginException le=new LoginException("Problem closing the private key password stream");
le.initCause(ioe);
throw le;
}
}
...
if (debug) debugPrint("Entering logout " + status);
switch (status) {
case UNINITIALIZED:
throw new LoginException("The login module is not initialized");
case INITIALIZED:
...
if (debug) debugPrint("Destroyed private credential, " + obj.getClass().getName());
break;
}
