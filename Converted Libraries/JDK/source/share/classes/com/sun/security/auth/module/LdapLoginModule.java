 ...
 */
  public boolean abort() throws LoginException {
    if (debug)     System.out.println("\t\t[LdapLoginModule] " + "aborted authentication");
    if (succeeded == false) {
      return false;
    }
 ...
 */
  private String findUserDN(  LdapContext ctx) throws LoginException {
    ...
    try {
      NamingEnumeration<SearchResult> results=ctx.search("",replaceUsernameToken(filterMatcher,userFilter),constraints);
      if (results.hasMore()) {
 ...
 */
  private void getUsernamePassword(  boolean getPasswdFromSharedState) throws LoginException {
    ...
    if (callbackHandler == null)     throw new LoginException("No CallbackHandler available " + "to acquire authentication information from the user");
