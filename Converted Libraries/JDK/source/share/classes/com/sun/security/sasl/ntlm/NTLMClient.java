...
final class NTLMClient implements SaslClient {
  ...
  private final static String NTLM_DOMAIN="com.sun.security.sasl.ntlm.domain";
  private final static String NTLM_HOSTNAME="com.sun.security.sasl.ntlm.hostname";
 ...
 */
  NTLMClient(  String mech,  String authzid,  String protocol,  String serverName,  Map props,  CallbackHandler cbh) throws SaslException {
    ...
    try {
      client=new Client(version,hostname,ncb.getName(),dcb.getText(),pcb.getPassword());
    }
  ...
  @Override public Object getNegotiatedProperty(  String propName){
  ...
  @Override public boolean hasInitialResponse(){
    return true;
  }
  @Override public byte[] evaluateChallenge(  byte[] challenge) throws SaslException {
    step++;
    if (step == 1) {
      return client.type1();
    }
