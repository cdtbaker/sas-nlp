 ...
 */
  void setMode(  String mode) throws NoSuchAlgorithmException {
    if (mode == null)     throw new NoSuchAlgorithmException("null mode");
 ...
 */
  void setPadding(  String paddingScheme) throws NoSuchPaddingException {
    ...
    if ((padding != null) && ((cipherMode == CTR_MODE) || (cipherMode == CTS_MODE))) {
      padding=null;
      throw new NoSuchPaddingException((cipherMode == CTR_MODE ? "CTR" : "CTS") + " mode must be used with NoPadding");
    }
  }
  /** 
 ...
 */
  int getOutputSize(  int inputLen){
    ...
    if (padding == null)     return totalLen;
    if (decrypting)     return totalLen;
    if (unitBytes != blockSize) {
      if (totalLen < diffBlocksize)       return diffBlocksize;
 else       return (totalLen + blockSize - ((totalLen - diffBlocksize) % blockSize));
    }
 else {
 ...
 */
  int update(  byte[] input,  int inputOffset,  int inputLen,  byte[] output,  int outputOffset) throws ShortBufferException {
    ...
    if (len != 0) {
      ...
      if (unitBytes != blockSize) {
        if (len < diffBlocksize)         diffBlocksize-=len;
 else         diffBlocksize=blockSize - ((len - diffBlocksize) % blockSize);
      }
 ...
 */
  byte[] doFinal(  byte[] input,  int inputOffset,  int inputLen) throws IllegalBlockSizeException, BadPaddingException {
    ...
    try {
      ...
      if (len < output.length) {
        out=new byte[len];
        if (len != 0)         System.arraycopy(output,0,out,0,len);
      }
 else {
        out=output;
      }
 ...
 */
  int doFinal(  byte[] input,  int inputOffset,  int inputLen,  byte[] output,  int outputOffset) throws IllegalBlockSizeException, ShortBufferException, BadPaddingException {
    ...
    if ((paddingLen > 0) && (paddingLen != blockSize) && (padding != null)&& decrypting) {
      throw new IllegalBlockSizeException("Input length must be multiple of " + blockSize + " when decrypting with padded cipher");
    }
    if (!decrypting && padding != null)     paddedLen+=paddingLen;
    if (output == null) {
      throw new ShortBufferException("Output buffer is null");
    }
    int outputCapacity=output.length - outputOffset;
    if (((!decrypting) || (padding == null)) && (outputCapacity < paddedLen) || (decrypting && (outputCapacity < (paddedLen - blockSize)))) {
  ...
  private int finalNoPadding(  byte[] in,  int inOff,  byte[] out,  int outOff,  int len) throws IllegalBlockSizeException {
    ...
    if ((cipherMode != CFB_MODE) && (cipherMode != OFB_MODE) && ((len % unitBytes) != 0)&& (cipherMode != CTS_MODE)) {
      if (padding != null) {
        throw new IllegalBlockSizeException("Input length (with padding) not multiple of " + unitBytes + " bytes");
      }
 else {
        throw new IllegalBlockSizeException("Input length not multiple of " + unitBytes + " bytes");
      }
    }
    if (decrypting) {
