 ...
 * @see BlowfishCipher
 */
final class BlowfishCrypt extends SymmetricCipher implements BlowfishConstants {
 ...
 */
  private void cipherBlock(  byte[] in,  int inOffset,  byte[] out,  int outOffset){
    temp[0]=((in[inOffset]) << 24) | ((in[inOffset + 1] & 0xff) << 16) | ((in[inOffset + 2] & 0xff) << 8)| ((in[inOffset + 3] & 0xff));
    temp[1]=((in[inOffset + 4]) << 24) | ((in[inOffset + 5] & 0xff) << 16) | ((in[inOffset + 6] & 0xff) << 8)| ((in[inOffset + 7] & 0xff));
    if (decrypting) {
      decryptBlock(temp);
    }
 else {
 ...
 */
  private int F(  int v){
    return ((s0[v >>> 24] + s1[(v >> 16) & 0xff]) ^ s2[(v >> 8) & 0xff]) + s3[v & 0xff];
  }
