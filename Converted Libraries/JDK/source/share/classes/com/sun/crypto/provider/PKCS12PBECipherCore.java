...
final class PKCS12PBECipherCore {
  ...
  private static void concat(  byte[] src,  byte[] dst,  int start,  int len){
    ...
    int off, i;
    for (i=0, off=0; i < loop; i++, off+=src.length)     System.arraycopy(src,0,dst,off + start,src.length);
  ...
  PKCS12PBECipherCore(  String symmCipherAlg,  int defKeySize) throws NoSuchAlgorithmException {
    ...
    if (algo.equals("DESede")) {
      ...
      throw new NoSuchAlgorithmException("No Cipher implementation " + "for PBEWithSHA1And" + algo);
    }
  ...
  AlgorithmParameters implGetParameters(){
    ...
    try {
      params=AlgorithmParameters.getInstance("PBEWithSHA1And" + (algo.equalsIgnoreCase("RC2") ? "RC2_40" : algo),"SunJCE");
    }
