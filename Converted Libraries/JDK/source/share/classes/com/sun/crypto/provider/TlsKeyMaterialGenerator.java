...
public final class TlsKeyMaterialGenerator extends KeyGeneratorSpi {
  private final static String MSG="TlsKeyMaterialGenerator must be " + "initialized using a TlsKeyMaterialParameterSpec";
  ...
  private SecretKey engineGenerateKey0() throws GeneralSecurityException {
    ...
    if (protocolVersion >= 0x0303) {
      ...
      keyBlock=doTLS12PRF(masterSecret,LABEL_KEY_EXPANSION,seed,keyBlockLen,spec.getPRFHashAlg(),spec.getPRFHashLength(),spec.getPRFBlockSize());
    }
    ...
    if (isExportable == false) {
      ...
      if (protocolVersion >= 0x0302) {
        ...
        tmp=doTLS10PRF(clientKeyBytes,LABEL_CLIENT_WRITE_KEY,seed,expandedKeyLength,md5,sha);
        clientCipherKey=new SecretKeySpec(tmp,alg);
        tmp=doTLS10PRF(serverKeyBytes,LABEL_SERVER_WRITE_KEY,seed,expandedKeyLength,md5,sha);
