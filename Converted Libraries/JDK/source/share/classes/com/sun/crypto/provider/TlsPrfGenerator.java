...
abstract class TlsPrfGenerator extends KeyGeneratorSpi {
  ...
  final static byte[] LABEL_MASTER_SECRET={109,97,115,116,101,114,32,115,101,99,114,101,116};
  final static byte[] LABEL_KEY_EXPANSION={107,101,121,32,101,120,112,97,110,115,105,111,110};
  final static byte[] LABEL_CLIENT_WRITE_KEY={99,108,105,101,110,116,32,119,114,105,116,101,32,107,101,121};
  final static byte[] LABEL_SERVER_WRITE_KEY={115,101,114,118,101,114,32,119,114,105,116,101,32,107,101,121};
  final static byte[] LABEL_IV_BLOCK={73,86,32,98,108,111,99,107};
  ...
  SecretKey engineGenerateKey0(  boolean tls12){
    ...
    try {
      ...
      byte[] prfBytes=(tls12 ? doTLS12PRF(secret,labelBytes,spec.getSeed(),n,spec.getPRFHashAlg(),spec.getPRFHashLength(),spec.getPRFBlockSize()) : doTLS10PRF(secret,labelBytes,spec.getSeed(),n));
  ...
  static byte[] doTLS12PRF(  byte[] secret,  byte[] labelBytes,  byte[] seed,  int outputLength,  String prfHash,  int prfHashLength,  int prfBlockSize) throws NoSuchAlgorithmException, DigestException {
    if (prfHash == null) {
...
byte[] output=new byte[outputLength];
expand(md5,16,secret,0,seclen,labelBytes,seed,output,HMAC_ipad64.clone(),HMAC_opad64.clone());
expand(sha,20,secret,off,seclen,labelBytes,seed,output,HMAC_ipad64.clone(),HMAC_opad64.clone());
return output;
}
