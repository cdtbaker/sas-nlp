...
static {
  ...
  protected void engineInit(  byte[] encoded) throws IOException {
    ...
    for (int i=0; i < datum.length; i++) {
      ...
      if (data.isContextSpecific((byte)0x00)) {
        mdName=convertToStandardName(AlgorithmId.parse(data.data.getDerValue()).getName());
      }
  ...
  protected AlgorithmParameterSpec engineGetParameterSpec(  Class paramSpec) throws InvalidParameterSpecException {
    if (OAEPParameterSpec.class.isAssignableFrom(paramSpec)) {
      return new OAEPParameterSpec(mdName,"MGF1",mgfSpec,new PSource.PSpecified(p));
    }
 else {
      throw new InvalidParameterSpecException("Inappropriate parameter specification");
    }
  ...
  protected byte[] engineGetEncoded() throws IOException {
    ...
    try {
      ...
      throw new IOException("AlgorithmId " + mgfSpec.getDigestAlgorithm() + " impl not found");
    }
