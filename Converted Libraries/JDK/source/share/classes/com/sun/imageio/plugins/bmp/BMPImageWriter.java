 ...
 */
  ...
  public void setOutput(  Object output){
    ...
    if (output != null) {
      if (!(output instanceof ImageOutputStream))       throw new IllegalArgumentException(I18N.getString("BMPImageWriter0"));
 ...
 else       sourceRegion=sourceRegion.intersection(rect);
    }
...
break;
case 16:
if (spixels == null) spixels=new short[scanlineBytes / numBands];
for (int j=0, m=0; j < scanlineBytes; m++) {
spixels[m]=0;
if (compressionType == BMPConstants.BI_RGB) {
spixels[m]=(short)(((0x1f & pixels[j]) << 10) | ((0x1f & pixels[j + 1]) << 5) | ((0x1f & pixels[j + 2])));
j+=3;
}
 else {
...
break;
case 32:
if (ipixels == null) ipixels=new int[scanlineBytes / numBands];
if (numBands == 3) {
for (int j=0, m=0; j < scanlineBytes; m++) {
ipixels[m]=0;
if (compressionType == BMPConstants.BI_RGB) {
ipixels[m]=((0xff & pixels[j + 2]) << 16) | ((0xff & pixels[j + 1]) << 8) | ((0xff & pixels[j]));
j+=3;
}
 else {
for (int i=0; i < numBands; i++, j++) {
...
for (int i=0; i < BMPConstants.compressionTypeNames.length; i++) if (BMPConstants.compressionTypeNames[i].equals(typeString)) return i;
return 0;
}
