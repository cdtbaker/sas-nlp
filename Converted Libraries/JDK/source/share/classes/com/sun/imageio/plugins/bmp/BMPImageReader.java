...
if (size == 108) metadata.bmpVersion=VERSION_4;
 else if (size == 124) metadata.bmpVersion=VERSION_5;
redMask=(int)iis.readUnsignedInt();
greenMask=(int)iis.readUnsignedInt();
blueMask=(int)iis.readUnsignedInt();
...
if (ImageUtil.isIndicesForGrayscale(r,g,b)) colorModel=ImageUtil.createColorModel(null,sampleModel);
 else colorModel=new IndexColorModel(bitsPerPixel,(int)size,r,g,b);
}
 else if (bitsPerPixel == 16) {
numBands=3;
...
int idata[]=null;
if (sampleModel != null) {
if (sampleModel.getDataType() == DataBuffer.TYPE_BYTE) bdata=(byte[])((DataBufferByte)raster.getDataBuffer()).getData();
 else if (sampleModel.getDataType() == DataBuffer.TYPE_USHORT) sdata=(short[])((DataBufferUShort)raster.getDataBuffer()).getData();
 else if (sampleModel.getDataType() == DataBuffer.TYPE_INT) idata=(int[])((DataBufferInt)raster.getDataBuffer()).getData();
}
switch (imageType) {
...
int n=3 * i + k;
for (int b=0; b < destBands.length; b++) bdata[n + destBands[b]]=buf[m + sourceBands[b]];
}
k+=isBottomUp ? -lineStride : lineStride;
iis.skipBytes(skipLength);
processImageUpdate(bi,0,j,destinationRegion.width,1,1,1,new int[]{0});
...
iis.skipBytes(lineLength * (height - 1 - lastLine) << 1);
 ...
 else iis.skipBytes(lineLength * sourceRegion.y << 1);
...
iis.skipBytes(width * (height - 1 - lastLine) << 2);
 ...
 else iis.skipBytes(width * sourceRegion.y << 2);
...
if ((width & 1) == 1) bdata[pos]|=val[width - 1] << 4;
processImageUpdate(bi,0,lineNo,destinationRegion.width,1,1,1,new int[]{0});
finished++;
}
 else if ((lineNo - sourceRegion.y) % scaleY == 0) {
...
if ((p[0] == '\\') && (p[1] == '\\')) {
if ((p[2] == '?') && (p[3] == '\\')) {
return ((p[4] == 'U' || p[4] == 'u') && (p[5] == 'N' || p[5] == 'n') && (p[6] == 'C' || p[6] == 'c'));
}
 else {
return true;
}
