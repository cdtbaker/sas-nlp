 ...
 */
  ...
  JPEGMetadata(  boolean isStream,  boolean isThumb,  ImageInputStream iis,  JPEGImageReader reader) throws IOException {
    ...
    buffer.loadBuf(0);
    if (((buffer.buf[0] & 0xff) != 0xff) || ((buffer.buf[1] & 0xff) != JPEG.SOI) || ((buffer.buf[2] & 0xff) != 0xff)) {
...
buf=buffer.buf;
ptr=buffer.bufPtr;
if ((buf[ptr + 3] == 'J') && (buf[ptr + 4] == 'F') && (buf[ptr + 5] == 'I')&& (buf[ptr + 6] == 'F')&& (buf[ptr + 7] == 0)) {
if (inThumb) {
...
newGuy=new JFIFMarkerSegment(buffer);
 ...
 else if ((buf[ptr + 3] == 'J') && (buf[ptr + 4] == 'F') && (buf[ptr + 5] == 'X')&& (buf[ptr + 6] == 'X')&& (buf[ptr + 7] == 0)) {
if (isStream) {
...
case JPEG.APP2:
buffer.loadBuf(15);
if ((buffer.buf[buffer.bufPtr + 3] == 'I') && (buffer.buf[buffer.bufPtr + 4] == 'C') && (buffer.buf[buffer.bufPtr + 5] == 'C')&& (buffer.buf[buffer.bufPtr + 6] == '_')&& (buffer.buf[buffer.bufPtr + 7] == 'P')&& (buffer.buf[buffer.bufPtr + 8] == 'R')&& (buffer.buf[buffer.bufPtr + 9] == 'O')&& (buffer.buf[buffer.bufPtr + 10] == 'F')&& (buffer.buf[buffer.bufPtr + 11] == 'I')&& (buffer.buf[buffer.bufPtr + 12] == 'L')&& (buffer.buf[buffer.bufPtr + 13] == 'E')&& (buffer.buf[buffer.bufPtr + 14] == 0)) {
if (isStream) {
...
case JPEG.APP14:
buffer.loadBuf(8);
if ((buffer.buf[buffer.bufPtr + 3] == 'A') && (buffer.buf[buffer.bufPtr + 4] == 'd') && (buffer.buf[buffer.bufPtr + 5] == 'o')&& (buffer.buf[buffer.bufPtr + 6] == 'b')&& (buffer.buf[buffer.bufPtr + 7] == 'e')) {
if (isStream) {
...
markerSequence.add(new DHTMarkerSegment(jparam.getDCHuffmanTables(),jparam.getACHuffmanTables()));
}
 else {
...
markerSequence.add(new SOFMarkerSegment(wantProg,wantExtended,willSubsample,componentIDs,numComponents));
if (!wantProg) {
...
throw new IIOInvalidTreeException("Invalid " + (isStream ? "stream " : "image ") + "child: "+ childName,node);
}
}
}
/** 
