...
iis.reset();
WritableRaster raster=Raster.createInterleavedRaster(buffer,thumbWidth,thumbHeight,thumbWidth * 3,3,new int[]{0,1,2},null);
ColorModel cm=new ComponentColorModel(JPEG.JCS.sRGB,false,false,ColorModel.OPAQUE,DataBuffer.TYPE_BYTE);
...
int getWidth(){
int retval=0;
SOFMarkerSegment sof=(SOFMarkerSegment)thumbMetadata.findMarkerSegment(SOFMarkerSegment.class,true);
if (sof != null) {
...
dataLen-=2;
if (debug) {
System.out.println("chunkNum: " + chunkNum + ", numChunks: "+ numChunks+ ", dataLen: "+ dataLen);
}
