...
public class BMPMetadata extends IIOMetadata implements BMPConstants {
  ...
  private Node getNativeTree(){
    ...
    int version=0;
    for (int i=0; i < bmpVersion.length(); i++)     if (Character.isDigit(bmpVersion.charAt(i)))     version=bmpVersion.charAt(i) - '0';
    if (version >= 4) {
    ...
    if ((palette != null) && (paletteSize > 0)) {
      ...
      for (int i=0, j=0; i < paletteSize; i++) {
        ...
        if (numComps == 4)         addChildNode(entry,"Alpha",new Byte((byte)(palette[j++] & 0xff)));
      }
    }
    return root;
  }
