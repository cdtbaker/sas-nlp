 ...
 */
  static java.util.logging.Logger log=java.util.logging.Logger.getLogger(SignatureECDSA.class.getName());
  /** 
 * @inheritDoc 
 */
 ...
 */
  private static byte[] convertASN1toXMLDSIG(  byte asn1Bytes[]) throws IOException {
    ...
    if ((asn1Bytes[0] != 48) || (asn1Bytes[1] != asn1Bytes.length - 2) || (asn1Bytes[2] != 2)|| (i > 24)|| (asn1Bytes[4 + rLength] != 2)|| (j > 24)) {
      throw new IOException("Invalid ASN.1 format of ECDSA signature");
    }
    byte xmldsigBytes[]=new byte[48];
    System.arraycopy(asn1Bytes,(4 + rLength) - i,xmldsigBytes,24 - i,i);
    System.arraycopy(asn1Bytes,(6 + rLength + sLength) - j,xmldsigBytes,48 - j,j);
    return xmldsigBytes;
  }
  /** 
 ...
 */
  public SignatureECDSA() throws XMLSignatureException {
    ...
    if (log.isLoggable(java.util.logging.Level.FINE))     log.log(java.util.logging.Level.FINE,"Created SignatureECDSA using " + algorithmID);
 ...
 */
  protected boolean engineVerify(  byte[] signature) throws XMLSignatureException {
    try {
      ...
      if (log.isLoggable(java.util.logging.Level.FINE))       log.log(java.util.logging.Level.FINE,"Called ECDSA.verify() on " + Base64.encode(signature));
 ...
 */
  protected void engineInitVerify(  Key publicKey) throws XMLSignatureException {
    ...
    try {
      ...
      try {
        this._signatureAlgorithm=Signature.getInstance(_signatureAlgorithm.getAlgorithm());
      }
 catch (      Exception e) {
 ...
 */
  protected void engineInitSign(  Key privateKey,  SecureRandom secureRandom) throws XMLSignatureException {
    ...
    try {
      this._signatureAlgorithm.initSign((PrivateKey)privateKey,secureRandom);
    }
 catch (    InvalidKeyException ex) {
      throw new XMLSignatureException("empty",ex);
    }
