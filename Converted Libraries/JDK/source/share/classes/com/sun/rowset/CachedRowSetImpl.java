 ...
 */
  private void initProperties() throws SQLException {
    ...
    if ((rvh.size() > 0) && (isReadOnly() == false))     setReadOnly(false);
    ...
    while (data.next()) {
      ...
      for (i=1; i <= numCols; i++) {
        ...
        if (obj instanceof Struct) {
          ...
          if (map != null)           obj=new SerialArray((java.sql.Array)obj,map);
 else           obj=new SerialArray((java.sql.Array)obj);
        }
 ...
 */
  private void initMetaData(  RowSetMetaDataImpl md,  ResultSetMetaData rsmd) throws SQLException {
    ...
    for (int col=1; col <= numCols; col++) {
      ...
      if (rsmd.isAutoIncrement(col))       updateOnInsert=true;
 ...
 */
  private int getColIdxByName(  String name) throws SQLException {
    ...
    for (int i=1; i <= cols; ++i) {
      ...
      if (colName != null)       if (name.equalsIgnoreCase(colName))       return (i);
 else       continue;
    }
...
if (ret == true) absolutePos=numRows - numDeleted;
 else absolutePos=0;
return ret;
}
/** 
...
return i.equals(Integer.valueOf((int)0)) ? Boolean.valueOf(false) : Boolean.valueOf(true);
case java.sql.Types.TINYINT:
...
checkIndex(columnIndex);
checkCursor();
Object obj=convertNumeric(Integer.valueOf(x),java.sql.Types.INTEGER,RowSetMD.getColumnType(columnIndex));
getCurrentRow().setColumnObject(columnIndex,obj);
}
/** 
 * Sets the designated column in either the current row or the insert
...
if (insertRow == null) {
if (RowSetMD == null) throw new SQLException(resBundle.handleGetObject("cachedrowsetimpl.movetoins1").toString());
...
if (tabName == null) throw new SQLException(resBundle.handleGetObject("cachedrowsetimpl.tablename").toString());
 else tableName=tabName;
}
/** 
...
if (RowSetMD != null) {
numCols=RowSetMD.getColumnCount();
if (keys.length > numCols) throw new SQLException(resBundle.handleGetObject("cachedrowsetimpl.keycols").toString());
}
...
checkCursor();
if (onInsertRow == true) throw new SQLException(resBundle.handleGetObject("cachedrowsetimpl.invalidop").toString());
if (insertFlag) {
