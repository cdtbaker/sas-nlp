...
for (int j=0; j < i.length; j++) i[j]=Integer.parseInt((String)keyCols.elementAt(j));
rs.setKeyColumns(i);
}
break;
case PropClassTag:
...
break;
case ConcurrencyTag:
if (nullValue) throw new SQLException(resBundle.handleGetObject("xmlrch.badvalue").toString());
...
if (nullValue) throw new SQLException(resBundle.handleGetObject("xmlrch.badvalue1").toString());
