  ...
  public void addRowSet(  Joinable rowset) throws SQLException {
    ...
    if (rowset instanceof JdbcRowSetImpl) {
      ...
      try {
        ...
        for (int i=0; i < rowset.getMatchColumnIndexes().length; i++) {
          if (rowset.getMatchColumnIndexes()[i] != -1)           ++matchColumnCount;
 else           break;
        }
        int[] pCol=new int[matchColumnCount];
        for (int i=0; i < matchColumnCount; i++)         pCol[i]=rowset.getMatchColumnIndexes()[i];
    ...
    if (boolColId && boolColName) {
      ...
      if (boolColId) {
        ...
        for (int i=0; i < indices.size(); i++)         indexes[i]=((Integer)indices.get(i)).intValue();
    ...
    try {
      ...
      if (vecRowSetsInJOIN.isEmpty()) {
        ...
        for (int i=0; i < crsInternal.getMatchColumnIndexes().length; i++) {
          if (crsInternal.getMatchColumnIndexes()[i] != -1)           ++matchColumnCount;
 else           break;
        }
        rsmd.setColumnCount(crsInternal.getMetaData().getColumnCount() + cRowset.getMetaData().getColumnCount() - matchColumnCount);
            ...
            for (int j=1; j <= rowCount2; j++) {
              ...
              if (cRowset.next()) {
                ...
                for (int k=0; k < matchColumnCount; k++) {
                  if (!crsInternal.getObject(crsInternal.getMatchColumnIndexes()[k]).equals(cRowset.getObject(cRowset.getMatchColumnIndexes()[k]))) {
                    match=false;
                    break;
                  }
                }
        ...
        crsTemp.setOriginal();
        int[] pCol=new int[matchColumnCount];
        for (int i=0; i < matchColumnCount; i++)         pCol[i]=crsInternal.getMatchColumnIndexes()[i];
 ...
 */
  public String getWhereClause() throws SQLException {
    ...
    for (int i=0; i < sz; i++) {
      strWhereClause=strWhereClause.concat(((CachedRowSetImpl)vecRowSetsInJOIN.get(i)).getMatchColumnNames()[0]);
      if (i % 2 != 0) {
        strWhereClause=strWhereClause.concat("=");
      }
 else {
