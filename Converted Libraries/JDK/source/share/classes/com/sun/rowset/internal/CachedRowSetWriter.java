 ...
 */
  public boolean writeData(  RowSetInternal caller) throws SQLException {
    ...
    if (callerColumnCount < 1) {
      if (reader.getCloseConnection() == true)       con.close();
      return true;
    }
 ...
 */
 ...
 else           pstmt.setNull(i + 1,crs.getMetaData().getColumnType(i + 1));
        }
        idx=i;
        for (i=0; i < keyCols.length; i++) {
          if (params[i] != null) {
 ...
 */
  private boolean insertNewRow(  CachedRowSet crs,  PreparedStatement pstmt,  CachedRowSetImpl crsRes) throws SQLException {
    ...
    PreparedStatement pstmtSel=con.prepareStatement(selectCmd,ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
        ...
        if (primaryKeys[j] != null) {
          ...
          if (crsPK.equals(rsPK)) {
            ...
            for (i=1; i <= icolCount; i++) {
              ...
              if (colname.equals(primaryKeys[j]))               this.crsResolve.updateObject(i,rsPK);
 ...
 */
  private boolean deleteOriginalRow(  CachedRowSet crs,  CachedRowSetImpl crsRes) throws SQLException {
    ...
    pstmt=con.prepareStatement(selectCmd + deleteWhere,ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
    for (i=0; i < keyCols.length; i++) {
      if (params[i] != null) {
        pstmt.setObject(++idx,params[i]);
      }
 ...
 */
  private void initSQLStatements(  CachedRowSet caller) throws SQLException {
    ...
    for (i=1; i <= callerColumnCount; i++) {
      ...
      if (i < callerMd.getColumnCount())       selectCmd+=", ";
 else       selectCmd+=" ";
    }
    ...
    for (i=1; i <= callerColumnCount; i++) {
      ...
      if (i < callerMd.getColumnCount())       insertCmd+=", ";
 else       insertCmd+=") VALUES (";
    }
    for (i=1; i <= callerColumnCount; i++) {
      insertCmd+="?";
      if (i < callerColumnCount)       insertCmd+=", ";
 else       insertCmd+=")";
    }
  ...
  private void buildKeyDesc(  CachedRowSet crs) throws SQLException {
    ...
    if (keyCols == null || keyCols.length == 0) {
      ...
      for (int i=0; i < callerColumnCount; i++) {
        if (resultsetmd.getColumnType(i + 1) != java.sql.Types.CLOB && resultsetmd.getColumnType(i + 1) != java.sql.Types.STRUCT && resultsetmd.getColumnType(i + 1) != java.sql.Types.SQLXML && resultsetmd.getColumnType(i + 1) != java.sql.Types.BLOB && resultsetmd.getColumnType(i + 1) != java.sql.Types.ARRAY && resultsetmd.getColumnType(i + 1) != java.sql.Types.OTHER)         listKeys.add(i + 1);
      }
      keyCols=new int[listKeys.size()];
      for (int i=0; i < listKeys.size(); i++)       keyCols[i]=listKeys.get(i);
    }
 ...
 */
  ...
  void updateResolvedConflictToDB(  CachedRowSet crs,  Connection con) throws SQLException {
    ...
    strUpdate="UPDATE " + buildTableName(con.getMetaData(),crs.getMetaData().getCatalogName(1),crs.getMetaData().getSchemaName(1),crs.getTableName());
    strUpdate+="SET ";
    boolean first=true;
    for (int i=1; i <= icolCount; i++) {
      if (crs.columnUpdated(i)) {
 ...
 */
  ...
  public void commit(  CachedRowSetImpl crs,  boolean updateRowset) throws SQLException {
    con.commit();
    if (updateRowset) {
      if (crs.getCommand() != null)       crs.execute(con);
    }
    if (reader.getCloseConnection() == true) {
      con.close();
    }
