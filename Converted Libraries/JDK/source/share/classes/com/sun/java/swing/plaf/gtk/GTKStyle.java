 ...
 * @return SynthPainter
 */
  @Override public SynthPainter getPainter(  SynthContext state){
 ...
 * @return Insets.
 */
  @Override public Insets getInsets(  SynthContext state,  Insets insets){
 ...
 else     if (id == Region.MENU || id == Region.MENU_ITEM || id == Region.CHECK_BOX_MENU_ITEM || id == Region.RADIO_BUTTON_MENU_ITEM) {
 ...
 else     if (id == Region.PROGRESS_BAR || id == Region.SLIDER || id == Region.TABBED_PANE || id == Region.TABBED_PANE_CONTENT || id == Region.TOOL_BAR || id == Region.TOOL_BAR_DRAG_WINDOW || id == Region.TOOL_TIP) {
 ...
 else     if (id == Region.SEPARATOR || id == Region.POPUP_MENU_SEPARATOR || id == Region.TOOL_BAR_SEPARATOR) {
  ...
  private Insets getTextFieldInsets(  SynthContext context,  Insets insets){
    insets=getClassSpecificInsetsValue(context,"inner-border",getSimpleInsets(context,insets,2));
    int xThickness=getXThickness();
    int yThickness=getYThickness();
...
synchronized (ICONS_MAP) {
  ...
  private Icon getStockIcon(  SynthContext context,  String key,  int type){
    ...
    String propName=ICON_PROPERTY_PREFIX + key + '.'+ type+ '.'+ (direction == TextDirection.RTL ? "rtl" : "ltr");
