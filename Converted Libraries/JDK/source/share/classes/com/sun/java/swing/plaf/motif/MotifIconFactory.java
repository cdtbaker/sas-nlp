...
private static class CheckBoxIcon implements Icon, UIResource, Serializable {
    ...
    public void drawCheckBezel(    Graphics g,    int x,    int y,    int csize,    boolean shade,    boolean out,    boolean check,    boolean flat){
      Color oldColor=g.getColor();
      g.translate(x,y);
      if (!flat) {
        if (out) {
...
private static class RadioButtonIcon implements Icon, UIResource, Serializable {
    ...
    public void paintIcon(    Component c,    Graphics g,    int x,    int y){
      ...
      boolean isEnabled=model.isEnabled();
      boolean isSelected=model.isSelected();
      boolean checkIn=((isPressed && !isArmed && isSelected) || (isPressed && isArmed && !isSelected) || (!isPressed && isArmed && isSelected || (!isPressed && !isArmed && isSelected)));
      if (checkIn) {
        g.setColor(shadow);
