...
class PackageWriter extends BandStructure {
  ...
  void setArchiveOptions(){
    ...
    if (!testBit(archiveOptions,AO_DEFLATE_HINT) && minOptions != -1) {
      ...
      if (minOptions != maxOptions || minOptions != pkg.default_options) {
    ...
    if (verbose > 0)     Utils.log.info("Consensus version number in segment is " + bestVerStr);
    if (verbose > 0)     Utils.log.info("Highest version number in segment is " + Package.versionStringOf(pkg.getHighestClassVersion()));
    for (    Class cls : pkg.classes) {
      if (cls.getVersion() != bestVersion) {
        ...
        if (verbose > 1) {
          ...
          Utils.log.fine("Version " + clsVer + " of "+ cls+ " doesn't match package version "+ pkgVer);
        }
        cls.addAttribute(a);
      }
    }
    for (    File file : pkg.files) {
      ...
      if (len != (int)len) {
        ...
        if (verbose > 0)         Utils.log.info("Note: Huge resource file " + file.getFileName() + " forces 64-bit sizing");
        break;
      }
    }
    int cost0=0;
    ...
    if (verbose > 0)     Utils.log.info("archiveOptions = " + "0b" + Integer.toBinaryString(archiveOptions));
  }
  ...
  void writeArchiveHeader() throws IOException {
    ...
    if (!haveSpecial) {
      ...
      if (haveSpecial)       archiveOptions|=AO_HAVE_SPECIAL_FORMATS;
    }
    if (!haveSpecial)     headerDiscountForDebug+=AH_SPECIAL_FORMAT_LEN;
    ...
    if (!haveFiles) {
      ...
      if (haveFiles)       archiveOptions|=AO_HAVE_FILE_HEADERS;
    }
    if (!haveFiles)     headerDiscountForDebug+=AH_FILE_HEADER_LEN;
    boolean haveNumbers=testBit(archiveOptions,AO_HAVE_CP_NUMBERS);
    if (!haveNumbers) {
      haveNumbers|=pkg.cp.haveNumbers();
      if (haveNumbers)       archiveOptions|=AO_HAVE_CP_NUMBERS;
    }
    if (!haveNumbers)     headerDiscountForDebug+=AH_CP_NUMBER_LEN;
    assert (pkg.package_majver > 0);
    archive_header_0.putInt(pkg.package_minver);
    archive_header_0.putInt(pkg.package_majver);
    if (verbose > 0)     Utils.log.info("Package Version for this segment:" + Package.versionStringOf(pkg.getPackageVersion()));
    ...
    assert (archive_header_0.length() + archive_header_S.length() + archive_header_1.length() == AH_LENGTH - headerDiscountForDebug);
    archiveSize0=0;
    ...
    if (verbose > 1)     Utils.log.fine("archive sizes: " + archiveSize0 + "+"+ archiveSize1);
...
switch (tag) {
case CONSTANT_Utf8:
        if (count > 0)         assert (pkg.cp.getIndexByTag(tag).get(0) == ConstantPool.getUtf8Entry(""));
      break;
...
if (verbose > 0) Utils.log.info("Writing " + cpMap.length + " "+ ConstantPool.tagName(tag)+ " entries...");
if (optDumpBands) {
try {
...
while (prefix < limit && curChars[prefix] == prevChars[prefix]) prefix++;
prefixes[i]=prefix;
if (i >= PREFIX_SKIP_2) cp_Utf8_prefix.putInt(prefix);
...
if (verbose > 1) Utils.log.fine("big string[" + i + "] len="+ suffix+ " #wide="+ numWide+ " size="+ sizes[BYTE_SIZE]+ "/z="+ sizes[ZIP_SIZE]+ " coding "+ special);
...
if (verbose > 1) Utils.log.fine("big string[" + i + "] normalSize="+ normalSizes[BYTE_SIZE]+ "/z="+ normalSizes[ZIP_SIZE]+ " win="+ (specialZipSize < normalZipSize - minWin));
...
if (haveSizeHi) file_size_hi.putInt((int)(len >>> 32));
if (haveModtime) file_modtime.putInt(file.modtime - pkg.default_modtime);
if (haveOptions) file_options.putInt(file.options);
file.writeTo(file_bits.collectorStream());
if (verbose > 1) Utils.log.fine("Wrote " + len + " bytes of "+ file.name.stringValue());
}
if (verbose > 0) Utils.log.info("Wrote " + numFiles + " resource files");
}
@SuppressWarnings("unchecked") void collectAttributeLayouts(){
...
final int TOO_MANY_ATTRS=32 - 12 + 4;
if (nl >= TOO_MANY_ATTRS) {
int mask=1 << (LG_AO_HAVE_XXX_FLAGS_HI + i);
archiveOptions|=mask;
haveLongFlags=true;
if (verbose > 0) Utils.log.info("Note: Many " + Attribute.contextName(i) + " attributes forces 63-bit flags");
}
if (verbose > 1) {
...
if (verbose > 0) {
if (index < attrIndexLimit[i]) Utils.log.info("Using free flag bit 1<<" + index + " for "+ count+ " occurrences of "+ def);
 else Utils.log.info("Using overflow index " + index + " for "+ count+ " occurrences of "+ def);
}
}
}
}
...
@SuppressWarnings("unchecked") void writeAttrDefs() throws IOException {
...
int flags=ic.flags;
if (flags == 0) flags=ACC_IC_LONG_FORM;
...
if (verbose > 0) Utils.log.info("  ...scanning " + classes.length + " classes...");
int nwritten=0;
for (int i=0; i < classes.length; i++) {
Class cls=classes[i];
if (verbose > 1) Utils.log.fine("Scanning " + cls);
...
nwritten++;
if (verbose > 0 && (nwritten % 1000) == 0) Utils.log.info("Have scanned " + nwritten + " classes...");
}
}
...
if (!attrsOK && na > 0) sc=LONG_CODE_HEADER;
if (verbose > 2) {
...
writeCodeHandlers(c);
if (sc == LONG_CODE_HEADER || attrsOK) writeAttrs(ATTR_CONTEXT_CODE,c,c.thisClass());
}
...
void writeAttrs(int ctype,final Attribute.Holder h,Class cls) throws IOException {
MultiBand xxx_attr_bands=attrBands[ctype];
...
if (haveLongFlags) xxx_flags_hi.putInt(0);
return;
}
if (verbose > 3) Utils.log.fine("Transmitting attrs for " + h + " flags="+ Integer.toHexString(h.flags));
...
if (verbose > 3) Utils.log.fine("add attr @" + index + " "+ a+ " in "+ h);
if (index < attrIndexLimit[ctype] && testBit(flagMask,1L << index)) {
if (verbose > 3) Utils.log.fine("Adding flag bit 1<<" + index + " in "+ Long.toHexString(flagMask));
...
overflowCount+=1;
if (verbose > 3) Utils.log.fine("Adding overflow attr #" + overflowCount);
...
if (haveLongFlags) xxx_flags_hi.putInt((int)(flagsToAdd >>> 32));
 else assert ((flagsToAdd >>> 32) == 0);
assert ((h.flags & flagsToAdd) == 0) : (h + ".flags=" + Integer.toHexString(h.flags)+ "^"+ Long.toHexString(flagsToAdd));
}
...
if ("<init>".equals(ref.descRef.nameRef.stringValue()) == false) return -1;
ClassEntry refClass=ref.classRef;
if (refClass == curClass.thisClass) return _invokeinit_op + _invokeinit_self_option;
if (refClass == curClass.superClass) return _invokeinit_op + _invokeinit_super_option;
if (refClass == newClass) return _invokeinit_op + _invokeinit_new_option;
return -1;
}
...
if (refClass == curClass.thisClass) return self_bc;
if (refClass == curClass.superClass) return self_bc + _self_linker_super_flag;
return -1;
}
...
if (i.isNonstandard() && (!p200.getBoolean(Utils.COM_PREFIX + "invokedynamic") || i.getBC() != _xxxunusedxxx)) {
...
prevAload=false;
if (isAload) self_bc+=_self_linker_aload_flag;
