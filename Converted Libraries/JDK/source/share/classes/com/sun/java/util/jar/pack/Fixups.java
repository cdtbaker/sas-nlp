...
final class Fixups extends AbstractCollection {
  ...
  public void clear(){
    ...
    size=0;
    if (bigDescs != noBigDescs)     bigDescs[BIGSIZE]=MINBIGSIZE;
  }
  public byte[] getBytes(){
    return bytes;
  }
  @SuppressWarnings("unchecked") public void setBytes(  byte[] newBytes){
  ...
  static int fmtLen(  int fmt){
    return 1 + (fmt - U1_FORMAT) / (U2_FORMAT - U1_FORMAT);
  }
  static int descLoc(  int desc){
    return desc >>> LOC_SHIFT;
  }
  static int descFmt(  int desc){
    return desc & FMT_MASK;
  }
  static int descEnd(  int desc){
    return descLoc(desc) + fmtLen(descFmt(desc));
  }
 ...
 */
public static class Fixup implements Comparable {
int desc;
Entry entry;
...
if (this.size == 0 && entries.length < that.size) growEntries(that.size);
...
if (entries.length == size) growEntries(size * 2);
entries[size]=entry;
if (size == 0) {
head=tail=thisDesc;
}
...
if (thisLoc < prevLoc + prevLen) badOverlap(thisLoc);
tail=thisDesc;
if (!storeDesc(prevLoc,prevFmt,thisDesc)) {
int bigSize=bigDescs[BIGSIZE];
if (bigDescs.length == bigSize) growBigDescs();
...
public static Object add(Object prevFixups,byte[] bytes,int loc,int fmt,Entry e){
Fixups f;
if (prevFixups == null) {
...
Fixups f=(Fixups)fixups;
f.trimToSize();
if (f.size() == 0) fixups=null;
}
return fixups;
}
public static void visitRefs(Object fixups,Collection<Entry> refs){
if (fixups == null) {
}
