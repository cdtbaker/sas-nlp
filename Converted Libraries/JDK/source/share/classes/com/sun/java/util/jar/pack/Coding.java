 ...
 */
  public static int codeMax(  int B,  int H,  int S,  int nMax){
    ...
    if (range == 0)     return -1;
    if (S == 0 || range >= (long)1 << 32)     return saturate32(range - 1);
    long maxPos=range - 1;
    ...
    if (smax < 0)     return Integer.MAX_VALUE;
    return smax;
  }
  /** 
 ...
 */
  ...
  private static int decodeSign32(  long ux,  int S){
    assert (ux == toUnsigned32((int)ux)) : (Long.toHexString(ux));
    if (S == 0) {
      return (int)ux;
    }
    int sx;
  ...
  private static long encodeSign32(  int sx,  int S){
    ...
    assert (sx == decodeSign32(ux,S)) : (Long.toHexString(ux) + " -> " + Integer.toHexString(sx)+ " != "+ Integer.toHexString(decodeSign32(ux,S)));
    return ux;
  }
  public static void writeInt(  byte[] out,  int[] outpos,  int sx,  int B,  int H,  int S){
    ...
    assert (ux < codeRangeLong(B,H)) : Long.toHexString(ux);
    int L=256 - H;
    long sum=ux;
    int pos=outpos[0];
    for (int i=0; i < B - 1; i++) {
  ...
  public boolean canRepresentValue(  int x){
    if (isSubrange())     return canRepresentUnsigned(x);
 else     return canRepresentSigned(x);
  }
  /** 
 ...
 */
  ...
  public void readArrayFrom(  InputStream in,  int[] a,  int start,  int end) throws IOException {
    for (int i=start; i < end; i++)     a[i]=readFrom(in);
  ...
  public void writeArrayTo(  OutputStream out,  int[] a,  int start,  int end) throws IOException {
    ...
    for (int dstep=0; dstep < del; dstep++) {
      int[] deltas;
      if (!isSubrange())       deltas=makeDeltas(a,start,end,0,0);
 else       deltas=makeDeltas(a,start,end,min,max);
      a=deltas;
      start=0;
      end=deltas.length;
    }
 ...
 */
  boolean isSubrange(){
    return max < Integer.MAX_VALUE && ((long)max - (long)min + 1) <= Integer.MAX_VALUE;
  }
  /** 
 ...
 */
  Coding getValueCoding(){
    if (isDelta())     return Coding.of(B,H,0,del - 1);
 else     return this;
  }
  /** 
 ...
 */
  int reduceToUnsignedRange(  long value){
    if (value == (int)value && canRepresentUnsigned((int)value))     return (int)value;
    int range=getRange();
    assert (range > 0);
    value%=range;
  ...
  int reduceToSignedRange(  int value){
    if (canRepresentSigned(value))     return value;
 ...
 */
  ...
  public int compareTo(  Object x){
    ...
    if (dkey == 0)     dkey=this.B - that.B;
    if (dkey == 0)     dkey=this.H - that.H;
    if (dkey == 0)     dkey=this.S - that.S;
    return dkey;
  }
  /** 
 ...
 */
  public int distanceFrom(  Coding that){
    ...
    if (this.H == that.H) {
      ...
      thisHL*=thisHL;
      thatHL*=thatHL;
      if (thisHL > thatHL)       diffHL=ceil_lg2(1 + (thisHL - 1) / thatHL);
 else       diffHL=ceil_lg2(1 + (thatHL - 1) / thisHL);
    }
 ...
 */
  static int bitWidth(  int i){
    ...
    int w=0;
    int lo=i;
    if (lo < byteBitWidths.length)     return byteBitWidths[lo];
    int hi;
    hi=(lo >>> 16);
    if (hi != 0) {
      lo=hi;
 ...
 */
  ...
  boolean canRepresent(  int minValue,  int maxValue){
    ...
    if (del > 0) {
      if (isSubrange()) {
        return canRepresentUnsigned(maxValue) && canRepresentUnsigned(minValue);
      }
 else {
        return isFullRange();
      }
    }
 else     return canRepresentSigned(maxValue) && canRepresentSigned(minValue);
  }
 ...
 */
  public int getLength(  int value){
    if (isDelta() && isSubrange()) {
      if (!canRepresentUnsigned(value))       return Integer.MAX_VALUE;
  ...
  public int getLength(  int[] values,  int start,  int end){
    ...
    if (isDelta()) {
      int[] deltas;
      if (!isSubrange())       deltas=makeDeltas(values,start,end,0,0);
 else       deltas=makeDeltas(values,start,end,min,max);
      values=deltas;
      start=0;
    }
    int sum=len;
  ...
  public byte[] getMetaCoding(  Coding dflt){
    ...
    if (canonicalIndex > 0)     return new byte[]{(byte)canonicalIndex};
    return new byte[]{(byte)_meta_arb,(byte)(del + 2 * S + 8 * (B - 1)),(byte)(H - 1)};
  }
  public static int parseMetaCoding(  byte[] bytes,  int pos,  Coding dflt,  CodingMethod res[]){
    ...
    if (op == _meta_arb) {
      ...
      int B=(dsb / 8) + 1;
      int H=H_1 + 1;
      if (!((1 <= B && B <= B_MAX) && (0 <= S && S <= S_MAX) && (1 <= H && H <= H_MAX)&& (0 <= del && del <= 1)) || (B == 1 && H != 256) || (B == 5 && H == 256)) {
  ...
  String stringForDebug(){
    ...
    if (isSubrange())     str+=" subrange";
 else     if (!isFullRange())     str+=" MIDRANGE";
