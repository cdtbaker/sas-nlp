...
static {
    java.security.AccessController.doPrivileged(new sun.security.action.LoadLibraryAction("unpack"));
    initIDs();
  }
  ...
  void run(  InputStream inRaw,  JarOutputStream jstream,  ByteBuffer presetInput) throws IOException {
    ...
    final int modtime=Pack200.Packer.KEEP.equals(_props.getProperty(Utils.UNPACK_MODIFICATION_TIME,"0")) ? Constants.NO_MODTIME : _props.getTime(Utils.UNPACK_MODIFICATION_TIME);
    copyInOption(Utils.DEBUG_VERBOSE);
    copyInOption(Pack200.Unpacker.DEFLATE_HINT);
    if (modtime == Constants.NO_MODTIME)     copyInOption(Utils.UNPACK_MODIFICATION_TIME);
    updateProgress();
    for (; ; ) {
