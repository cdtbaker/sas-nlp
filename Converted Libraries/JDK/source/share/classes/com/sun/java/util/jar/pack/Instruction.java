...
class Instruction {
  ...
  public Instruction next(){
    int npc=pc + length;
    if (npc == bytes.length)     return null;
 else     return Instruction.at(bytes,npc,this);
  }
...
assert (w == 0);
if (length == 2) return getByte(bytes,pc + indexLoc);
 else return getShort(bytes,pc + indexLoc);
}
public void setCPIndex(int cpi){
int indexLoc=BC_INDEX[w][bc];
assert (indexLoc != 0);
if (length == 2) setByte(bytes,pc + indexLoc,cpi);
 else setShort(bytes,pc + indexLoc,cpi);
...
if (w == 0) return getByte(bytes,pc + slotLoc);
 else return getShort(bytes,pc + slotLoc);
}
...
int offset;
if (length == 3) offset=(short)getShort(bytes,pc + branchLoc);
 else offset=getInt(bytes,pc + branchLoc);
assert (offset + pc >= 0);
...
int branchLoc=BC_BRANCH[w][bc];
assert (branchLoc != 0);
if (length == 3) setShort(bytes,pc + branchLoc,targetPC - pc);
 else setInt(bytes,pc + branchLoc,targetPC - pc);
...
case 1:
return (byte)getByte(bytes,pc + conLoc);
case 2:
return (short)getShort(bytes,pc + conLoc);
}
assert (false);
return 0;
}
...
switch (length - conLoc) {
case 1:
setByte(bytes,pc + conLoc,con);
break;
case 2:
setShort(bytes,pc + conLoc,con);
break;
}
...
setIntAt(0,targetPC - pc);
}
protected int apc;
protected int intAt(int n){
return getInt(bytes,apc + n * 4);
}
protected void setIntAt(int n,int x){
setInt(bytes,apc + n * 4,x);
}
...
public int getCaseValue(int n){
return intAt(2 + n * 2 + 0);
}
public int getCaseLabel(int n){
return intAt(2 + n * 2 + 1) + pc;
}
public void setCaseCount(int caseCount){
...
public void setCaseValue(int n,int val){
setIntAt(2 + n * 2 + 0,val);
}
public void setCaseLabel(int n,int tpc){
setIntAt(2 + n * 2 + 1,tpc - pc);
}
...
return (o != null) && (o.getClass() == Instruction.class) && equals((Instruction)o);
}
public int hashCode(){
int hash=3;
...
if (this.bytes[this.pc + i] != that.bytes[that.pc + i]) return false;
}
return true;
}
static String labstr(int pc){
if (pc >= 0 && pc < 100000) return ((100000 + pc) + "").substring(1);
return pc + "";
}
public String toString(){
...
if (BC_LENGTH[1][i] == -1) BC_LENGTH[1][i]=(byte)(1 + BC_LENGTH[0][i]);
}
