...
public class WindowsInternalFrameTitlePane extends BasicInternalFrameTitlePane {
  ...
  protected void paintTitleBackground(  Graphics g){
    XPStyle xp=XPStyle.getXP();
    if (xp != null) {
      Part part=frame.isIcon() ? Part.WP_MINCAPTION : (frame.isMaximum() ? Part.WP_MAXCAPTION : Part.WP_CAPTION);
      ...
      Boolean gradientsOn=(Boolean)LookAndFeel.getDesktopPropertyValue("win.frame.captionGradientsOn",Boolean.valueOf(false));
      if (gradientsOn.booleanValue() && g instanceof Graphics2D) {
        ...
        int w=getWidth();
        if (isSelected) {
          GradientPaint titleGradient=new GradientPaint(0,0,selectedTitleColor,(int)(w * .75),0,selectedTitleGradientColor);
          g2.setPaint(titleGradient);
        }
 else {
          GradientPaint titleGradient=new GradientPaint(0,0,notSelectedTitleColor,(int)(w * .75),0,notSelectedTitleGradientColor);
          g2.setPaint(titleGradient);
        }
...
;
  ...
  private void showSystemPopupMenu(  Component invoker){
    ...
    if (!frame.isIcon()) {
      ...
      systemPopupMenu.show(invoker,getX() - dim.width,getY() - systemPopupMenu.getPreferredSize().height - dim.height);
    }
  }
...
public class WindowsTitlePaneLayout extends BasicInternalFrameTitlePane.TitlePaneLayout {
    ...
    private int layoutButton(    JComponent button,    Part part,    int x,    int y,    int w,    int h,    int gap,    boolean leftToRight){
      if (!leftToRight) {
        x-=w;
      }
    ...
    public void layoutContainer(    Container c){
      ...
      if (frame.isClosable()) {
        x=layoutButton(closeButton,Part.WP_CLOSEBUTTON,x,y,buttonWidth,buttonHeight,2,!leftToRight);
      }
      if (frame.isMaximizable()) {
        x=layoutButton(maxButton,Part.WP_MAXBUTTON,x,y,buttonWidth,buttonHeight,(xp != null) ? 2 : 0,!leftToRight);
      }
      if (frame.isIconifiable()) {
