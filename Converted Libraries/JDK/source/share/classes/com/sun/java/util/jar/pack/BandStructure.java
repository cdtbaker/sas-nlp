...
abstract class Band {
    ...
    protected Band(    String name,    Coding regularCoding){
      ...
      if (verbose > 2)       Utils.log.fine("Band " + seqForDebug + " is "+ name);
    }
    public Band init(){
      if (isReader)       readyToExpect();
 else       readyToCollect();
      return this;
    }
    boolean isReader(){
 ...
 */
    ...
    public String toString(){
      ...
      if (elementCountForDebug != 0)       str+="(" + elementCountForDebug + ")";
      return str;
    }
  }
 ...
 */
    protected void addValue(    int x){
      assert (assertCanChangeLength(this));
      if (length == values.length)       setCapacity(length < 1000 ? length * 10 : length * 2);
      values[length++]=x;
    }
    ...
    private boolean shouldVaryCoding(){
      ...
      if (effort < MAX_EFFORT && length < SHORT_BAND_HEURISTIC)       return false;
      return true;
    }
    protected void chooseBandCodings() throws IOException {
      ...
      if (!canVary || !shouldVaryCoding()) {
        if (regularCoding.canRepresent(values,0,length)) {
          ...
          assert (canVary);
          if (verbose > 1)           Utils.log.fine("regular coding fails in band " + name());
      ...
      if (metaCoding.length > 0) {
        if (outputSize >= 0)         outputSize+=computeEscapeSize();
    ...
    protected void readDataFrom(    InputStream in) throws IOException {
      ...
      if (length == 0)       return;
      if (verbose > 1)       Utils.log.fine("Reading band " + this);
      if (!canVaryCoding()) {
        bandCoding=regularCoding;
        metaCoding=noMetaCoding;
      }
        ...
        if (XB < 0) {
          ...
          if (verbose > 2)           Utils.log.fine("found X=" + X + " => XB="+ XB);
      ...
      if (bandCoding != regularCoding) {
        if (verbose > 1)         Utils.log.fine(name() + ": irregular coding " + bandCoding);
      }
...
class ByteBand extends Band {
    ...
    private void dumpBand() throws IOException {
      assert (optDumpBands);
      try {
        if (bytesForDump != null)         bytesForDump.writeTo(ds);
 else         bytes.writeTo(ds);
      }
     }
...
class CPRefBand extends ValueBand {
    ...
    public CPRefBand(    String name,    Coding regularCoding,    byte cpTag,    boolean nullOK){
      super(name,regularCoding);
      this.nullOK=nullOK;
      if (cpTag != CONSTANT_None)       setBandIndex(this,cpTag);
    }
 ...
 */
  ...
  int encodeRef(  Entry e,  Index ix){
    ...
    if (verbose > 2)     Utils.log.fine("putRef " + coding + " => "+ e);
    return coding;
  }
  Entry decodeRef(  int n,  Index ix){
    if (n < 0 || n >= ix.size())     Utils.log.warning("decoding bad ref " + n + " in "+ ix);
    Entry e=ix.getEntry(n);
    if (verbose > 2)     Utils.log.fine("getRef " + n + " => "+ e);
    return e;
  }
  ...
  protected static int decodeEscapeValue(  int X,  Coding regularCoding){
    if (regularCoding.B() == 1 || regularCoding.L() == 0)     return -1;
...
ix=getCPIndex(tag);
if (verbose > 2) Utils.log.fine("setConstantValueIndex " + f + " "+ ConstantPool.tagName(tag)+ " => "+ ix);
assert (ix != null);
}
...
protected List<List<Attribute.Layout>> attrDefs=new FixedList<>(ATTR_CONTEXT_LIMIT);
{
for (int i=0; i < ATTR_CONTEXT_LIMIT; i++) {
assert (attrIndexLimit[i] == 0);
attrIndexLimit[i]=32;
attrDefs.set(i,new ArrayList<Attribute.Layout>(Collections.nCopies(attrIndexLimit[i],(Attribute.Layout)null)));
}
...
if (ctype != ATTR_CONTEXT_METHOD) continue;
...
Band b=null;
if (e.bandIndex != Attribute.NO_BAND_INDEX) b=ab[e.bandIndex];
Coding rc=UNSIGNED5;
...
if (verbose > 1) Utils.log.fine("Making new bands for " + def);
...
{
int tem;
if ((tem=name.indexOf('[')) > 0) name=name.substring(0,tem);
if ((tem=name.indexOf('(')) > 0) name=name.substring(0,tem);
...
index=defList.size();
defList.add(def);
if (verbose > 0) Utils.log.info("Adding new attribute at " + def + ": "+ index);
...
if (verbose > (attrClassFileVersionMask == 0 ? 2 : 0)) Utils.log.fine("Fixing new attribute at " + index + ": "+ def+ (defList.get(index) == null ? "" : "; replacing " + defList.get(index)));
attrFlagMask[ctype]|=(1L << index);
...
assert (sc > 0 && sc <= 255);
for (int h=0; ; h++) {
if (sc < shortCodeHeader_h_base(h + 1)) return h;
}
}
...
if (!isSuper) return isField ? bc_thisfield : bc_thismethod;
 else return isField ? bc_superfield : bc_supermethod;
}
...
dumpDir.delete();
if (dumpDir.mkdir()) Utils.log.info("Dumping bands to " + dumpDir);
}
...
if (p0 == p1) Utils.log.warning("Already in phase " + p0);
 else Utils.log.warning("Unexpected phase " + p0 + " -> "+ p1);
return false;
}
...
String cstr;
if (rci >= 0) cstr=knowns[rci];
 else cstr="CODING" + rc.keyString();
...
if (prevForAssertMap == null) prevForAssertMap=new HashMap<>();
