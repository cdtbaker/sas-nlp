...
class PopulationCoding implements CodingMethod {
  ...
  public static Coding fitTokenCoding(  int fVlen,  int L){
    if (fVlen < 256)     return BandStructure.BYTE1;
    Coding longest=BandStructure.UNSIGNED5.setL(L);
    if (!longest.canRepresentUnsigned(fVlen))     return null;
    Coding tc=longest;
    for (Coding shorter=longest; ; ) {
      shorter=shorter.setB(shorter.B() - 1);
      if (shorter.umax() < fVlen)       break;
      tc=shorter;
    }
    return tc;
  }
  ...
  public void setTokenCoding(  CodingMethod tokenCoding){
    ...
    if (tokenCoding instanceof Coding && fValues != null) {
      ...
      if (tc == fitTokenCoding(fVlen,tc.L()))       this.L=tc.L();
    }
  }
  ...
  public int favoredValueMaxLength(){
    if (L == 0)     return Integer.MAX_VALUE;
 else     return BandStructure.UNSIGNED5.setL(L).umax();
  }
  ...
  public int getToken(  int value){
    if (symtab == null)     symtab=makeSymtab();
    int pos=Arrays.binarySearch(symtab,(long)value << 32);
    if (pos < 0)     pos=-pos - 1;
    if (pos < symtab.length && value == (int)(symtab[pos] >>> 32))     return (int)symtab[pos];
 else     return 0;
  }
  ...
  private Coding getTailCoding(  CodingMethod c){
    while (c instanceof AdaptiveCoding)     c=((AdaptiveCoding)c).tailCoding;
    return (Coding)c;
  }
  ...
  void writeSequencesTo(  OutputStream out,  int[] tokens,  int[] uValues) throws IOException {
    ...
    if (uValues.length > 0)     unfavoredCoding.writeArrayTo(out,uValues,0,uValues.length);
  }
  int computeSentinelValue(){
    ...
    if (fc.isDelta()) {
      ...
      int endVal;
      if (fc.getLength(min) <= fc.getLength(last))       return min;
 else       return last;
    }
  }
  public void readArrayFrom(  InputStream in,  int[] a,  int start,  int end) throws IOException {
    ...
    if (uVlen > 0)     unfavoredCoding.readArrayFrom(in,uValues,0,uVlen);
  ...
  int[] readFavoredValuesFrom(  InputStream in,  int maxForDebug) throws IOException {
    ...
    if (fc.isDelta()) {
      for (long state=0; ; ) {
        ...
        int val;
        if (fc.isSubrange())         val=fc.reduceToUnsignedRange(state);
 else         val=(int)state;
        state=val;
        if (fillp > 1 && (val == last || val == min))         break;
        if (fillp == lfValues.length)         lfValues=BandStructure.realloc(lfValues);
        lfValues[fillp++]=val;
        assert (uniqueValuesForDebug.add(val));
        assert (fillp <= maxForDebug);
        last=val;
  ...
  public static int parseMetaCoding(  byte[] bytes,  int pos,  Coding dflt,  CodingMethod res[]){
    ...
    if (FDef == 0)     pos=BandStructure.parseMetaCoding(bytes,pos,dflt,FCode);
    if (TDef == 0)     pos=BandStructure.parseMetaCoding(bytes,pos,dflt,TCode);
    if (UDef == 0)     pos=BandStructure.parseMetaCoding(bytes,pos,dflt,UCode);
  ...
  public String toString(){
    ...
    boolean verbose=(p200 != null && p200.getBoolean(Utils.COM_PREFIX + "verbose.pop"));
