...
public class WindowsIconFactory implements Serializable {
  ...
  public static Icon createFrameResizeIcon(){
    if (frame_resizeIcon == null)     frame_resizeIcon=new ResizeIcon();
...
static class VistaMenuItemCheckIconFactory implements MenuItemCheckIconFactory {
    ...
    public boolean isCompatible(    Object icon,    String prefix){
      return icon instanceof VistaMenuItemCheckIcon && ((VistaMenuItemCheckIcon)icon).type == getType(prefix);
    }
    public Icon getIcon(    String type){
      return new VistaMenuItemCheckIcon(type);
    }
    static int getIconWidth(){
      return XPStyle.getXP().getSkin(null,Part.MP_POPUPCHECK).getWidth() + 2 * OFFSET;
    }
...
private static class VistaMenuItemCheckIcon implements Icon, UIResource, Serializable {
        ...
        if (type == JCheckBoxMenuItem.class || type == JRadioButtonMenuItem.class) {
          ...
          if (b.isSelected()) {
            ...
            if (isEnabled(c,null)) {
              ...
              state=(type == JRadioButtonMenuItem.class) ? State.BULLETNORMAL : State.CHECKMARKNORMAL;
            }
 else {
              backgroundState=State.DISABLEDPUSHED;
              state=(type == JRadioButtonMenuItem.class) ? State.BULLETDISABLED : State.CHECKMARKDISABLED;
            }
            Skin skin;
            XPStyle xp=XPStyle.getXP();
      ...
      private static boolean isEnabled(      Component c,      State state){
        ...
        if (state == null) {
          ...
          return (state != State.DISABLED) && (state != State.DISABLEDHOT) && (state != State.DISABLEDPUSHED);
        }
      }
