  ...
  public Histogram(  int[][] matrix){
    ...
    for (int i=0; i < matrix.length; i++) {
      for (int j=1; j < matrix[i].length; j++) {
        hist2col[fillp++]=((long)matrix[i][j] << 32) | (LOW32 & matrix[i][0]);
      }
    }
 ...
 */
  ...
  public int getRowLength(  int rn){
    return matrix[rn].length - 1;
  }
  public int getRowValue(  int rn,  int vn){
    return matrix[rn][vn + 1];
  }
 ...
 */
  ...
  static private double round(  double x,  double scale){
 ...
 */
      ...
      if (row[0] != prevCount && fillp2 > fillp1) {
        ...
        if (!isSorted(row1,1,true)) {
          ...
          for (int j=2; j < row1.length; j++) {
            if (row1[j] != row1[j - 1])             row1[jfillp++]=row1[j];
          }
  ...
  public String[] getRowTitles(  String name){
    ...
    for (int i=0; i < matrix.length; i++) {
      ...
      histTitles[i]=name + "[" + i+ "]"+ " len="+ round(len,10)+ " ("+ count+ "*["+ unique+ "])"+ " ("+ cumWeight+ ":"+ wpct+ "%)"+ " ["+ cumUnique+ ":"+ upct+ "%]";
    }
    return histTitles;
  }
  /** 
 ...
 */
  public void print(  String name,  String[] histTitles,  PrintStream out){
    ...
    String title=(name + " len=" + round(tlen,10)+ " avgLen="+ round(avgLen,10)+ " weight("+ ltotalWeight+ ")"+ " unique["+ totalUnique+ "]"+ " avgWeight("+ round(avg,100)+ ")");
    if (histTitles == null) {
      out.println(title);
    }
 else {
...
for (boolean sizeOnly=true; ; sizeOnly=false) {
  ...
  for (int i=0; i <= sortedValues.length; i++) {
    int thisValue;
    if (i < sortedValues.length)     thisValue=sortedValues[i];
 else     thisValue=prevValue ^ -1;
    if (thisValue == prevValue) {
      prevCount+=1;
    }
 else {
      if (!sizeOnly && prevCount != 0) {
        hist[prevIndex]=((long)prevCount << 32) | (LOW32 & prevValue);
      }
      prevValue=thisValue;
      prevCount=1;
      prevIndex+=1;
...
for (int i=from + 1; i < values.length; i++) {
  if (strict ? !(values[i - 1] < values[i]) : !(values[i - 1] <= values[i])) {
    return false;
  }
}
return true;
