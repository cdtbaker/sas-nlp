 ...
 */
  @SuppressWarnings("unchecked") public SortedMap properties(){
    return props;
  }
  /** 
 ...
 */
  public void pack(  JarFile in,  OutputStream out) throws IOException {
    ...
    TimeZone tz=(props.getBoolean(Utils.PACK_DEFAULT_TIMEZONE)) ? null : TimeZone.getDefault();
    try {
...
@SuppressWarnings("unchecked") private class DoPack {
    ...
    final String unknownAttrCommand;
{
      ...
      if (!(Pack200.Packer.STRIP.equals(uaMode) || Pack200.Packer.PASS.equals(uaMode) || Pack200.Packer.ERROR.equals(uaMode))) {
    ...
    final Map<Attribute.Layout,String> attrCommands;
{
      ...
      String[] keys={Pack200.Packer.CLASS_ATTRIBUTE_PFX,Pack200.Packer.FIELD_ATTRIBUTE_PFX,Pack200.Packer.METHOD_ATTRIBUTE_PFX,Pack200.Packer.CODE_ATTRIBUTE_PFX};
      int[] ctypes={Constants.ATTR_CONTEXT_CLASS,Constants.ATTR_CONTEXT_FIELD,Constants.ATTR_CONTEXT_METHOD,Constants.ATTR_CONTEXT_CODE};
      for (int i=0; i < ctypes.length; i++) {
        ...
        for (        Object k : map.keySet()) {
          ...
          if (Pack200.Packer.STRIP.equals(layout) || Pack200.Packer.PASS.equals(layout) || Pack200.Packer.ERROR.equals(layout)) {
    ...
    final boolean keepFileOrder=props.getBoolean(Pack200.Packer.KEEP_FILE_ORDER);
    final boolean keepClassOrder=props.getBoolean(Utils.PACK_KEEP_CLASS_ORDER);
    final boolean keepModtime=Pack200.Packer.KEEP.equals(props.getProperty(Pack200.Packer.MODIFICATION_TIME));
    final boolean latestModtime=Pack200.Packer.LATEST.equals(props.getProperty(Pack200.Packer.MODIFICATION_TIME));
    final boolean keepDeflateHint=Pack200.Packer.KEEP.equals(props.getProperty(Pack200.Packer.DEFLATE_HINT));
{
...
{
      long limit;
      if (props.getProperty(Pack200.Packer.SEGMENT_LIMIT,"").equals(""))       limit=-1;
 else       limit=props.getLong(Pack200.Packer.SEGMENT_LIMIT);
...
{
      int ver;
      if ((ver=props.getInteger(Utils.COM_PREFIX + "min.class.majver")) != 0)       pkg.min_class_majver=(short)ver;
      if ((ver=props.getInteger(Utils.COM_PREFIX + "min.class.minver")) != 0)       pkg.min_class_minver=(short)ver;
      if ((ver=props.getInteger(Utils.COM_PREFIX + "max.class.majver")) != 0)       pkg.max_class_majver=(short)ver;
      if ((ver=props.getInteger(Utils.COM_PREFIX + "max.class.minver")) != 0)       pkg.max_class_minver=(short)ver;
      if ((ver=props.getInteger(Utils.COM_PREFIX + "package.minver")) != 0)       pkg.package_minver=(short)ver;
      if ((ver=props.getInteger(Utils.COM_PREFIX + "package.majver")) != 0)       pkg.package_majver=(short)ver;
    }
{
      int opt=props.getInteger(Utils.COM_PREFIX + "archive.options");
      if (opt != 0)       pkg.default_options|=opt;
    }
    ...
    boolean isMetaInfFile(    String name){
      return name.startsWith("/" + Utils.METAINF) || name.startsWith(Utils.METAINF);
    }
...
final class InFile {
      ...
      void copyTo(      Package.File file){
        if (modtime != Constants.NO_MODTIME)         file.modtime=modtime;
        file.options|=options;
      }
      InputStream getInputStream() throws IOException {
        if (jf != null)         return jf.getInputStream(je);
 else         return new FileInputStream(f);
      }
    ...
    private void noteRead(    InFile f){
      nread++;
      if (verbose > 2)       Utils.log.fine("...read " + f.name);
      if (verbose > 0 && (nread % 1000) == 0)       Utils.log.info("Have read " + nread + " files...");
    }
    void run(    JarInputStream in,    OutputStream out) throws IOException {
      ...
      for (JarEntry je; (je=in.getNextJarEntry()) != null; ) {
        ...
        long inflen=(isMetaInfFile(name)) ? 0L : inFile.getInputLength();
    ...
    void run(    JarFile in,    OutputStream out) throws IOException {
      ...
      if (verbose > 0)       Utils.log.info("Reading " + inFiles.size() + " files...");
      int numDone=0;
      for (      InFile inFile : inFiles) {
        ...
        if ((segmentSize+=inflen) > segmentLimit) {
          ...
          if (verbose > 1)           Utils.log.fine("Estimated segments to do: " + segsToDo);
          flushPartial(out,(int)Math.ceil(segsToDo));
        }
        InputStream strm=inFile.getInputStream();
        if (verbose > 1)         Utils.log.fine("Reading " + name);
        Package.File file=null;
        if (isClassFile(name)) {
    ...
    Package.File readFile(    String fname,    InputStream in) throws IOException {
      ...
      file.readFrom(in);
      if (file.isDirectory() && file.getFileLength() != 0)       throw new IllegalArgumentException("Non-empty directory: " + file.getFileName());
      return file;
    }
 ...
 */
    void flushPackage(    OutputStream out,    int nextCount) throws IOException {
      ...
      if (!keepFileOrder) {
        ...
        assert ((res=new ArrayList<>(pkg.files)).retainAll(pkg.getClassStubs()) || true);
