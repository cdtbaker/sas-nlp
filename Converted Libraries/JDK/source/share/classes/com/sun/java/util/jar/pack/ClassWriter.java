...
class ClassWriter {
  ...
  ClassWriter(  Class cls,  OutputStream out) throws IOException {
    ...
    if (verbose > 1)     Utils.log.fine("local CP=" + (verbose > 2 ? cpIndex.dumpString() : cpIndex.toString()));
  }
...
List mems;
if (!doMethods) mems=cls.getFields();
 else mems=cls.getMethods();
...
writeAttributes(!doMethod ? ATTR_CONTEXT_FIELD : ATTR_CONTEXT_METHOD,m);
}
...
out=savedOut;
if (verbose > 2) Utils.log.fine("Attribute " + a.name() + " ["+ buf.size()+ "]");
writeInt(buf.size());
buf.writeTo(out);
}
 else {
if (verbose > 2) Utils.log.fine("Attribute " + a.name() + " ["+ a.size()+ "]");
writeInt(a.size());
out.write(a.bytes());
}
}
