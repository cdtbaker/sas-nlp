 ...
 */
  ...
  static void validateId(  byte[] arr) throws IllegalArgumentException {
    ...
    if (((arr[0] & 0x80) == 0) && arr.length != 12)     throw new IllegalArgumentException("Very first bit = 0 and length != 12 octets");
 ...
 */
  public SnmpOid toOid(){
    ...
    for (int i=1; i <= engineId.length; i++)     oid[i]=(long)(engineId[i - 1] & 0xFF);
 ...
 */
  public static SnmpEngineId createEngineId(  String str,  String separator) throws IllegalArgumentException, UnknownHostException {
    ...
    try {
 ...
 else       objAddress=InetAddress.getLocalHost();
      try {
      ...
      if (!iana.equals(separator))       objIana=Integer.parseInt(iana);
      eng=SnmpEngineId.createEngineId(objAddress,objPort,objIana);
      eng.setStringValue(str);
      return eng;
    }
 ...
 */
  public static SnmpEngineId createEngineId(  InetAddress addr,  int port,  int iana){
    ...
    engineid[3]=(byte)(iana & 0x000000FF);
    engineid[4]=0x05;
    if (address.length == 4)     engineid[4]=0x01;
    if (address.length == 16)     engineid[4]=0x02;
