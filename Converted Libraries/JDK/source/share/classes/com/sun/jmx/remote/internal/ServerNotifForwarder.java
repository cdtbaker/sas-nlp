...
public class ServerNotifForwarder {
  public ServerNotifForwarder(  MBeanServer mbeanServer,  Map<String,?> env,  NotificationBuffer notifBuffer,  String connectionId){
    ...
    checkNotificationEmission=EnvHelp.computeBooleanFromString(env,"jmx.remote.x.check.notification.emission",false);
...
synchronized (listenerMap) {
      ...
      if (set == null)       set=Collections.singleton(idaf);
 else {
        if (set.size() == 1)         set=new HashSet<IdAndFilter>(set);
        set.add(idaf);
      }
...
synchronized (listenerMap) {
      ...
      if (set == null || !set.contains(idaf))       throw new ListenerNotFoundException("Listener not found");
      if (set.size() == 1)       listenerMap.remove(name);
 else       set.remove(idaf);
    }
  }
  private final NotificationBufferFilter bufferFilter=new NotificationBufferFilter(){
...
synchronized (listenerMap) {
      ...
      for (      IdAndFilter idaf : candidates) {
        ...
        if (nf == null || nf.isNotificationEnabled(notif)) {
          ...
          if (allowNotificationEmission(source,tn))           targetedNotifs.add(tn);
        }
      }
    }
  }
 ...
 */
  public void checkMBeanPermission(  final ObjectName name,  final String actions) throws InstanceNotFoundException, SecurityException {
