 ...
 * with the new MBeanServer. The new MBeanServer must register
 * this MBean in its MBean repository.
 * @param interceptors If <code>true</code>,{@link MBeanServerInterceptor} will be enabled (default is
 * <code>false</code>)
 ...
 * @param interceptors If <code>true</code>,{@link MBeanServerInterceptor} will be enabled (default is
 * <code>false</code>).
 * @param fairLock If {@code true}, the MBean repository will use a {@link java.util.concurrent.locks.ReentrantReadWriteLock#ReentrantReadWriteLock(boolean)fair locking} policy.
 */
 ...
 * @exception UnsupportedOperationException if{@link MBeanServerInterceptor}s
 * are not enabled on this object.
 * @see #interceptorsEnabled
 */
  public MBeanInstantiator getMBeanInstantiator(){
    if (interceptorsEnabled)     return instantiator;
 else     throw new UnsupportedOperationException("MBeanServerInterceptors are disabled.");
  }
  /** 
  ...
  private void initialize(){
    ...
    try {
      ...
      if (MBEANSERVER_LOGGER.isLoggable(Level.FINEST)) {
        MBEANSERVER_LOGGER.logp(Level.FINEST,JmxMBeanServer.class.getName(),"initialize","Unexpected security exception occurred",e);
      }
      throw e;
    }
catch (    Exception e) {
      if (MBEANSERVER_LOGGER.isLoggable(Level.FINEST)) {
        MBEANSERVER_LOGGER.logp(Level.FINEST,JmxMBeanServer.class.getName(),"initialize","Unexpected exception occurred",e);
      }
 ...
 * with the new MBeanServer. The new MBeanServer must register
 * this MBean in its MBean repository.
 * @param interceptors If <code>true</code>,{@link MBeanServerInterceptor}s will be enabled (default is
 * <code>false</code>).
 * Note: this parameter is not taken into account by this
 * implementation - the default value <code>false</code> is
 * always used.
  ...
  private AttributeList cloneAttributeList(  AttributeList list){
    if (list != null) {
      ...
      if (!list.getClass().equals(AttributeList.class)) {
        ...
        for (        Attribute attribute : alist)         newList.add(cloneAttribute(attribute));
        return newList;
      }
 else {
