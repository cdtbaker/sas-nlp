...
static abstract class NonNullMXBeanMapping extends MXBeanMapping {
    ...
    @Override public final Object fromOpenValue(    Object openValue) throws InvalidObjectException {
      if (openValue == null)       return null;
 else       return fromNonNullOpenValue(openValue);
    }
    @Override public final Object toOpenValue(    Object javaValue) throws OpenDataException {
      if (javaValue == null)       return null;
 else       return toNonNullOpenValue(javaValue);
    }
      ...
      if (c.getName().startsWith("java.lang.")) {
        try {
          ...
          if (primitiveType != void.class) {
            ...
            final MXBeanMapping primitiveArrayMapping=new IdentityMapping(primitiveArrayType,primitiveArrayOpenType);
 ...
 */
 ...
 else     throw new OpenDataException("Cannot map type: " + objType);
  }
  ...
  private MXBeanMapping makeArrayOrCollectionMapping(  Type collectionType,  Type elementType,  MXBeanMappingFactory factory) throws OpenDataException {
    ...
    if (elementOpenClass.isArray())     openArrayClassName="[" + elementOpenClass.getName();
 else     openArrayClassName="[L" + elementOpenClass.getName() + ";";
    try {
  ...
  private MXBeanMapping makeParameterizedTypeMapping(  ParameterizedType objType,  MXBeanMappingFactory factory) throws OpenDataException {
    ...
    if (rawType instanceof Class<?>) {
      ...
      if (c == List.class || c == Set.class || c == SortedSet.class) {
        ...
        if (c == SortedSet.class)         mustBeComparable(c,actuals[0]);
        ...
        if (c == Map.class || sortedMap) {
          ...
          if (sortedMap)           mustBeComparable(c,actuals[0]);
  ...
  private MXBeanMapping makeCompositeMapping(  Class<?> c,  MXBeanMappingFactory factory) throws OpenDataException {
    ...
    for (    Method method : methods) {
      ...
      if (gcInfoHack && propertyName.equals("CompositeType"))       continue;
...
private static final class IdentityMapping extends NonNullMXBeanMapping {
    ...
    @Override Object fromNonNullOpenValue(    Object openValue) throws InvalidObjectException {
      return openValue;
    }
    @Override Object toNonNullOpenValue(    Object javaValue) throws OpenDataException {
      return javaValue;
    }
  }
private static final class EnumMapping<T extends Enum<T>> extends NonNullMXBeanMapping {
    ...
    @Override final Object toNonNullOpenValue(    Object value){
      return ((Enum<?>)value).name();
    }
    @Override final T fromNonNullOpenValue(    Object value) throws InvalidObjectException {
      try {
...
private static final class ArrayMapping extends NonNullMXBeanMapping {
    ...
    @Override final Object toNonNullOpenValue(    Object value) throws OpenDataException {
      Object[] valueArray=(Object[])value;
      final int len=valueArray.length;
      final Object[] openArray=(Object[])Array.newInstance(getOpenClass().getComponentType(),len);
      for (int i=0; i < len; i++)       openArray[i]=elementMapping.toOpenValue(valueArray[i]);
      return openArray;
    }
    @Override final Object fromNonNullOpenValue(    Object openValue) throws InvalidObjectException {
      ...
      for (int i=0; i < openArray.length; i++)       valueArray[i]=elementMapping.fromOpenValue(openArray[i]);
      return valueArray;
    }
...
private static final class CollectionMapping extends NonNullMXBeanMapping {
    CollectionMapping(    Type targetType,    ArrayType<?> openArrayType,    Class<?> openArrayClass,    MXBeanMapping elementMapping){
      ...
      Class<?> c=(Class<?>)raw;
      final Class<?> collC;
      if (c == List.class)       collC=ArrayList.class;
 else       if (c == Set.class)       collC=HashSet.class;
 else       if (c == SortedSet.class)       collC=TreeSet.class;
 else {
        assert (false);
        collC=null;
      }
    ...
    @Override final Object toNonNullOpenValue(    Object value) throws OpenDataException {
      ...
      int i=0;
      for (      Object o : valueCollection)       openArray[i++]=elementMapping.toOpenValue(o);
      return openArray;
    }
    @Override final Object fromNonNullOpenValue(    Object openValue) throws InvalidObjectException {
      final Object[] openArray=(Object[])openValue;
...
private static final class MXBeanRefMapping extends NonNullMXBeanMapping {
    ...
    @Override final Object toNonNullOpenValue(    Object javaValue) throws OpenDataException {
      MXBeanLookup lookup=lookupNotNull(OpenDataException.class);
      ObjectName name=lookup.mxbeanToObjectName(javaValue);
      if (name == null)       throw new OpenDataException("No name for object: " + javaValue);
      return name;
    }
    @Override final Object fromNonNullOpenValue(    Object openValue) throws InvalidObjectException {
...
private static final class TabularMapping extends NonNullMXBeanMapping {
    ...
    @Override public void checkReconstructible() throws InvalidObjectException {
 ...
 */
    ...
    @Override final Object fromNonNullOpenValue(    Object value) throws InvalidObjectException {
      makeCompositeBuilder();
 ...
 */
    ...
    abstract Object fromCompositeData(    CompositeData cd,    String[] itemNames,    MXBeanMapping[] converters) throws InvalidObjectException ;
...
private static class CompositeBuilderCheckGetters extends CompositeBuilder {
    ...
    @Override Throwable possibleCause(){
    ...
    String applicable(    Method[] getters){
      ...
      for (int i=0; i < getters.length; i++) {
        ...
        try {
          ...
          if (setter.getReturnType() != void.class)           throw new Exception();
        }
 catch (        Exception e) {
...
private static final class CompositeBuilderViaConstructor extends CompositeBuilder {
    ...
    String applicable(    Method[] getters) throws InvalidObjectException {
      ...
      for (      Constructor<?> constr : constrs) {
        if (Modifier.isPublic(constr.getModifiers()) && constr.getAnnotation(propertyNamesClass) != null)         annotatedConstrList.add(constr);
      }
      if (annotatedConstrList.isEmpty())       return "no constructor has @ConstructorProperties annotation";
      annotatedConstructors=newList();
      Map<String,Integer> getterMap=newMap();
      String[] itemNames=getItemNames();
      for (int i=0; i < itemNames.length; i++)       getterMap.put(itemNames[i],i);
      ...
      for (      Constructor<?> constr : annotatedConstrList) {
        ...
        for (int i=0; i < getters.length; i++)         paramIndexes[i]=-1;
        ...
        for (        BitSet b : getterIndexSets) {
          if (a == b)           seen=true;
            ...
            if (!getterIndexSets.contains(u)) {
              ...
              for (int i=u.nextSetBit(0); i >= 0; i=u.nextSetBit(i + 1))               names.add(itemNames[i]);
    ...
    final Object fromCompositeData(    CompositeData cd,    String[] itemNames,    MXBeanMapping[] mappings) throws InvalidObjectException {
      ...
      BitSet present=new BitSet();
      for (int i=0; i < itemNames.length; i++) {
        if (ct.getType(itemNames[i]) != null)         present.set(i);
      }
      Constr max=null;
      for (      Constr constr : annotatedConstructors) {
        if (subset(constr.presentParams,present) && (max == null || subset(max.presentParams,constr.presentParams)))         max=constr;
      }
      if (max == null) {
      ...
      for (int i=0; i < itemNames.length; i++) {
        ...
        if (index >= 0)         params[index]=javaItem;
      }
      try {
...
private static final class CompositeBuilderViaProxy extends CompositeBuilder {
    ...
    String applicable(    Method[] getters){
      Class<?> targetClass=getTargetClass();
      if (!targetClass.isInterface())       return "not an interface";
      ...
      for (      Method m : methods) {
        ...
        try {
          ...
          if (!Modifier.isPublic(om.getModifiers()))           bad=mname;
        }
 catch (        NoSuchMethodException e) {
          bad=mname;
        }
      }
      if (bad != null)       return "contains methods other than getters (" + bad + ")";
      return null;
    }
    final Object fromCompositeData(    CompositeData cd,    String[] itemNames,    MXBeanMapping[] converters){
 ...
 */
  ...
  public static String propertyName(  Method m){
    ...
    if (name.startsWith("get"))     rest=name.substring(3);
 else     if (name.startsWith("is") && m.getReturnType() == boolean.class)     rest=name.substring(2);
    if (rest == null || rest.length() == 0 || m.getParameterTypes().length > 0 || m.getReturnType() == void.class || name.equals("getClass"))     return null;
    return rest;
  }
