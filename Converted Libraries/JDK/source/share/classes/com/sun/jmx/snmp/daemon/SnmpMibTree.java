...
final class SnmpMibTree {
  ...
  public SnmpMibAgent getAgentMib(  SnmpOid oid){
    ...
    if (node == null)     return defaultAgent;
 else     if (node.getAgentMib() == null)     return defaultAgent;
 else     return node.getAgentMib();
  }
...
final class TreeNode {
    ...
    public void printTree(    String ident){
      ...
      for (Enumeration e=agents.elements(); e.hasMoreElements(); ) {
        ...
        if (mib == null)         buff.append("empty ");
 else         buff.append(mib.getMibName() + " ");
      }
      ident+=" ";
      if (children == null) {
        return;
 ...
 */
    ...
    private void removeAgentFully(    SnmpMibAgent agent){
      ...
      for (Enumeration<TreeNode> e=children.elements(); e.hasMoreElements(); ) {
        ...
        if (node.agents.isEmpty())         v.add(node);
      }
    ...
    private void removeAgent(    SnmpMibAgent mib){
      if (!agents.contains(mib))       return;
      agents.removeElement(mib);
      if (!agents.isEmpty())       agent=agents.firstElement();
    }
 ...
 else       child.registerNode(oid,cursor + 1,agent);
    }
    private TreeNode retrieveChild(    long[] oid,    int current){
      long theValue=oid[current];
      for (Enumeration e=children.elements(); e.hasMoreElements(); ) {
        TreeNode node=(TreeNode)e.nextElement();
        if (node.match(theValue))         return node;
      }
      return null;
    }
