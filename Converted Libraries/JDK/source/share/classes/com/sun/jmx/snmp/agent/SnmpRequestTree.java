...
final class SnmpRequestTree {
  ...
  public static int mapSetException(  int errorStatus,  int version) throws SnmpStatusException {
    ...
    if (version == SnmpDefinitions.snmpVersionOne)     return errorCode;
    int mappedErrorCode=errorCode;
    if (errorCode == SnmpStatusException.noSuchObject)     mappedErrorCode=SnmpStatusException.snmpRspNotWritable;
 else     if (errorCode == SnmpStatusException.noSuchInstance)     mappedErrorCode=SnmpStatusException.snmpRspNotWritable;
  ...
  public static int mapGetException(  int errorStatus,  int version) throws SnmpStatusException {
    ...
    if (version == SnmpDefinitions.snmpVersionOne)     return errorCode;
    int mappedErrorCode=errorCode;
    if (errorCode == SnmpStatusException.noSuchObject)     mappedErrorCode=errorCode;
 else     if (errorCode == SnmpStatusException.noSuchInstance)     mappedErrorCode=errorCode;
 else     if (errorCode == SnmpStatusException.noAccess)     mappedErrorCode=SnmpStatusException.noSuchInstance;
 else     if (errorCode == SnmpStatusException.snmpRspInconsistentName)     mappedErrorCode=SnmpStatusException.noSuchInstance;
 else     if ((errorCode >= SnmpStatusException.snmpRspWrongType) && (errorCode <= SnmpStatusException.snmpRspInconsistentValue))     mappedErrorCode=SnmpStatusException.noSuchInstance;
 else     if (errorCode == SnmpStatusException.readOnly)     mappedErrorCode=SnmpStatusException.noSuchInstance;
 else     if (errorCode != SnmpStatusException.snmpRspAuthorizationError && errorCode != SnmpStatusException.snmpRspGenErr)     mappedErrorCode=SnmpStatusException.noSuchObject;
    return mappedErrorCode;
  }
...
static final class SnmpMibSubRequestImpl implements SnmpMibSubRequest {
    ...
    public void registerGetException(    SnmpVarBind var,    SnmpStatusException exception) throws SnmpStatusException {
      if (version == SnmpDefinitions.snmpVersionOne)       throw new SnmpStatusException(exception,getVarIndex(var) + 1);
      ...
      if (errorCode == SnmpStatusException.noSuchObject)       var.value=SnmpVarBind.noSuchObject;
 else       if (errorCode == SnmpStatusException.noSuchInstance)       var.value=SnmpVarBind.noSuchInstance;
 else       throw new SnmpStatusException(errorCode,getVarIndex(var) + 1);
    }
    ...
    public void registerCheckException(    SnmpVarBind var,    SnmpStatusException exception) throws SnmpStatusException {
      ...
      if (errorCode != mappedErrorCode)       throw new SnmpStatusException(mappedErrorCode,getVarIndex(var) + 1);
 else       throw new SnmpStatusException(exception,getVarIndex(var) + 1);
    }
 ...
 * the given sublist.
 */
    @SuppressWarnings("unchecked") void add(    int pos,    SnmpOid oid,    Vector<SnmpVarBind> v,    boolean isnew,    SnmpVarBind statusvb){
      if (entryoids == null) {
    ...
    public void addVarbind(    SnmpVarBind varbind,    SnmpOid entryoid,    boolean isnew,    SnmpVarBind statusvb) throws SnmpStatusException {
      ...
      if (entryoids == null) {
        ...
        if (statusvb != null) {
          if ((rs != null) && (rs != statusvb) && ((type == SnmpDefinitions.pduWalkRequest) || (type == SnmpDefinitions.pduSetRequestPdu))) {
            throw new SnmpStatusException(SnmpStatusException.snmpRspInconsistentValue);
          }
          rowstatus[pos]=statusvb;
        }
      }
      if (statusvb != varbind)       v.addElement(varbind);
    }
  ...
  void setPduType(  int pduType){
    type=pduType;
    setreqflag=((pduType == SnmpDefinitions.pduWalkRequest) || (pduType == SnmpDefinitions.pduSetRequestPdu));
  }
  void setGetNextFlag(){
    getnextflag=true;
  }
  ...
  SnmpMibSubRequest getSubRequest(  Handler handler,  int entry){
    if (handler == null)     return null;
    return new SnmpMibSubRequestImpl(request,handler.getEntrySubList(entry),handler.getEntryOid(entry),handler.isNewEntry(entry),getnextflag,handler.getRowStatusVarBind(entry));
  }
  ...
  private void registerNode(  SnmpMibNode meta,  int depth,  SnmpOid entryoid,  SnmpVarBind varbind,  boolean isnew,  SnmpVarBind statusvb) throws SnmpStatusException {
    if (meta == null) {
      SNMP_ADAPTOR_LOGGER.logp(Level.FINEST,SnmpRequestTree.class.getName(),"registerNode","meta-node is null!");
      return;
    }
    if (varbind == null) {
      SNMP_ADAPTOR_LOGGER.logp(Level.FINEST,SnmpRequestTree.class.getName(),"registerNode","varbind is null!");
      return;
    }
    ...
    if (entryoid == null)     handler.addVarbind(varbind);
 else     handler.addVarbind(varbind,entryoid,isnew,statusvb);
    return;
  }
