 ...
 * </ul>
 * @throws IllegalArgumentException if {@code env} is {@code null} or{@code env.get(prop)} is not {@code null} and{@code ((String)env.get(prop)).equalsIgnoreCase("false")} and{@code ((String)env.get(prop)).equalsIgnoreCase("true")} are{@code false}.
 * @throws ClassCastException if {@code env.get(prop)} cannot be cast
 * to {@code String}.
 */
  public static boolean computeBooleanFromString(  Map<String,?> env,  String prop,  boolean systemProperty){
    if (env == null)     throw new IllegalArgumentException("env map cannot be null");
 ...
 * to {@code String}.
 */
  public static boolean computeBooleanFromString(  Map<String,?> env,  String prop,  boolean systemProperty,  boolean defaultValue){
    if (env == null)     throw new IllegalArgumentException("env map cannot be null");
    String stringBoolean=(String)env.get(prop);
    if (stringBoolean == null && systemProperty) {
      stringBoolean=AccessController.doPrivileged(new GetPropertyAction(prop));
    }
 ...
 else     throw new IllegalArgumentException(prop + " must be \"true\" or \"false\" instead of \"" + stringBoolean+ "\"");
  }
  /** 
 ...
 */
  public static <K,V>Hashtable<K,V> mapToHashtable(  Map<K,V> map){
    ...
    for (Iterator<?> i=m.values().iterator(); i.hasNext(); )     if (i.next() == null)     i.remove();
