 ...
 */
 ...
 else     return mbeanToRegister.getClass().getName();
  }
...
synchronized (beingUnregistered) {
  ...
  private void exclusiveUnregisterMBean(  ObjectName name) throws InstanceNotFoundException, MBeanRegistrationException {
    ...
    if (instance instanceof MBeanRegistration)     preDeregisterInvoke((MBeanRegistration)instance);
    final Object resource=getResource(instance);
    final ResourceContext context=unregisterFromRepository(resource,instance,name);
    try {
      if (instance instanceof MBeanRegistration)       postDeregisterInvoke(name,(MBeanRegistration)instance);
    }
  finally {
      context.done();
    }
  ...
  public AttributeList getAttributes(  ObjectName name,  String[] attributes) throws InstanceNotFoundException, ReflectionException {
    ...
    if (sm == null)     allowedAttributes=attributes;
 else {
      final String classname=getClassName(instance);
  ...
  private static void rethrowMaybeMBeanException(  Throwable t) throws ReflectionException, MBeanException {
    if (t instanceof MBeanException)     throw (MBeanException)t;
    rethrow(t);
  }
  /** 
 ...
 */
  ...
  private static ObjectName preRegister(  DynamicMBean mbean,  MBeanServer mbs,  ObjectName name) throws InstanceAlreadyExistsException, MBeanRegistrationException {
    ObjectName newName=null;
    try {
      if (mbean instanceof MBeanRegistration)       newName=((MBeanRegistration)mbean).preRegister(mbs,name);
    }
 catch (    Throwable t) {
  ...
  private static void postRegister(  ObjectName logicalName,  DynamicMBean mbean,  boolean registrationDone,  boolean registerFailed){
    if (registerFailed && mbean instanceof DynamicMBean2)     ((DynamicMBean2)mbean).registerFailed();
    try {
      if (mbean instanceof MBeanRegistration)       ((MBeanRegistration)mbean).postRegister(registrationDone);
    }
 catch (    RuntimeException e) {
      MBEANSERVER_LOGGER.fine("While registering MBean [" + logicalName + "]: "+ "Exception thrown by postRegister: "+ "rethrowing <"+ e+ ">, but keeping the MBean registered");
      throw new RuntimeMBeanException(e,"RuntimeException thrown in postRegister method: " + "rethrowing <" + e + ">, but keeping the MBean registered");
    }
catch (    Error er) {
      MBEANSERVER_LOGGER.fine("While registering MBean [" + logicalName + "]: "+ "Error thrown by postRegister: "+ "rethrowing <"+ er+ ">, but keeping the MBean registered");
      throw new RuntimeErrorException(er,"Error thrown in postRegister method: " + "rethrowing <" + er + ">, but keeping the MBean registered");
    }
  }
  ...
  private static void postDeregisterInvoke(  ObjectName mbean,  MBeanRegistration moi){
    try {
      ...
      MBEANSERVER_LOGGER.fine("While unregistering MBean [" + mbean + "]: "+ "Exception thrown by postDeregister: "+ "rethrowing <"+ e+ ">, although the MBean is succesfully "+ "unregistered");
      throw new RuntimeMBeanException(e,"RuntimeException thrown in postDeregister method: " + "rethrowing <" + e + ">, although the MBean is sucessfully unregistered");
    }
catch (    Error er) {
      MBEANSERVER_LOGGER.fine("While unregistering MBean [" + mbean + "]: "+ "Error thrown by postDeregister: "+ "rethrowing <"+ er+ ">, although the MBean is succesfully "+ "unregistered");
 ...
 */
  private DynamicMBean getMBean(  ObjectName name) throws InstanceNotFoundException {
    ...
    if (obj == null) {
      if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
        MBEANSERVER_LOGGER.logp(Level.FINER,DefaultMBeanServerInterceptor.class.getName(),"getMBean",name + " : Found no object");
      }
  ...
  private static Object getResource(  DynamicMBean mbean){
    if (mbean instanceof DynamicMBean2)     return ((DynamicMBean2)mbean).getResource();
 else     return mbean;
  }
  ...
  private void removeNotificationListener(  ObjectName name,  NotificationListener listener,  NotificationFilter filter,  Object handback,  boolean removeAll) throws InstanceNotFoundException, ListenerNotFoundException {
    ...
    if (removeAll)     broadcaster.removeNotificationListener(listenerWrapper);
 else {
  ...
  private static <T extends NotificationBroadcaster>T getNotificationBroadcaster(  ObjectName name,  Object instance,  Class<T> reqClass){
    if (reqClass.isInstance(instance))     return reqClass.cast(instance);
    if (instance instanceof DynamicMBean2)     instance=((DynamicMBean2)instance).getResource();
    if (reqClass.isInstance(instance))     return reqClass.cast(instance);
 ...
 */
  public ClassLoader getClassLoader(  ObjectName loaderName) throws InstanceNotFoundException {
    ...
    Object resource=getResource(instance);
    if (!(resource instanceof ClassLoader))     throw new InstanceNotFoundException(loaderName.toString() + " is not a classloader");
 ...
 */
  ...
  private static String safeGetClassName(  DynamicMBean mbean){
    try {
      ...
      if (MBEANSERVER_LOGGER.isLoggable(Level.FINEST)) {
        MBEANSERVER_LOGGER.logp(Level.FINEST,DefaultMBeanServerInterceptor.class.getName(),"safeGetClassName","Exception getting MBean class name",e);
      }
      return null;
    }
  }
...
private static class ListenerWrapper implements NotificationListener {
    ...
    @Override public boolean equals(    Object o){
      if (!(o instanceof ListenerWrapper))       return false;
  ...
  private static String getClassName(  DynamicMBean mbean){
    if (mbean instanceof DynamicMBean2)     return ((DynamicMBean2)mbean).getClassName();
 else     return mbean.getMBeanInfo().getClassName();
  }
  private static void checkMBeanPermission(  DynamicMBean mbean,  String member,  ObjectName objectName,  String actions){
    SecurityManager sm=System.getSecurityManager();
    if (sm != null) {
 ...
 * or JMXNamespaces.
 */
  private ResourceContext registerWithRepository(  final Object resource,  final DynamicMBean object,  final ObjectName logicalName) throws InstanceAlreadyExistsException, MBeanRegistrationException {
    final ResourceContext context=makeResourceContextFor(resource,logicalName);
    repository.addMBean(object,logicalName,context);
    if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
      MBEANSERVER_LOGGER.logp(Level.FINER,DefaultMBeanServerInterceptor.class.getName(),"addObject","Send create notification of object " + logicalName.getCanonicalName());
    }
 ...
 * or JMXNamespaces.
 */
  private ResourceContext unregisterFromRepository(  final Object resource,  final DynamicMBean object,  final ObjectName logicalName) throws InstanceNotFoundException {
    final ResourceContext context=makeResourceContextFor(resource,logicalName);
    repository.remove(logicalName,context);
    if (MBEANSERVER_LOGGER.isLoggable(Level.FINER)) {
      MBEANSERVER_LOGGER.logp(Level.FINER,DefaultMBeanServerInterceptor.class.getName(),"unregisterMBean","Send delete notification of object " + logicalName.getCanonicalName());
    }
