 ...
 */
  static public String binary2ascii(  byte[] data,  int length){
    ...
    for (int i=0; i < length; i++) {
      ...
      int v=(data[i] & 0xf0);
      v=v >> 4;
      if (v < 10)       asciiData[j + 2]=(byte)('0' + v);
 else       asciiData[j + 2]=(byte)('A' + (v - 10));
      v=((data[i] & 0xf));
      if (v < 10)       asciiData[j + 1 + 2]=(byte)('0' + v);
 else       asciiData[j + 1 + 2]=(byte)('A' + (v - 10));
    }
 ...
 */
 ...
 else       throw new Error("BAD format :" + str);
      if (p[j + 1] >= '0' && p[j + 1] <= '9') {
        v+=(p[j + 1] - '0');
      }
