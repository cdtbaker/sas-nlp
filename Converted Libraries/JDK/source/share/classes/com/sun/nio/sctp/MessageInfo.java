 ...
 * @param addressFor a connected {@code SctpChannel} the address is the
 ...
 * @param streamNumberThe stream number that the message will be sent on
 * @return  The outgoing message info
 * @throws IllegalArgumentExceptionIf the streamNumber is negative or greater than {@code 65536}
 */
 ...
 * to {@code true}, and its {@link #payloadProtocolID() payloadProtocolID}value set to {@code 0}. These values, if required, can be set through
 * the appropriate setter method before sending the message.
 * @param associationThe association to send the message on
 * @param addressThe preferred peer address of the association to send the message
 * to, or {@code null} to use the peer primary address
 * @param streamNumberThe stream number that the message will be sent on.
 * @return  The outgoing message info
 * @throws IllegalArgumentExceptionIf {@code association} is {@code null}, or the streamNumber is
 ...
 */
  public static MessageInfo createOutgoing(  Association association,  SocketAddress address,  int streamNumber){
    if (association == null)     throw new IllegalArgumentException("association cannot be null");
    if (streamNumber < 0 || streamNumber > 65536)     throw new IllegalArgumentException("Invalid stream number");
    return new sun.nio.ch.SctpMessageInfoImpl(association,address,streamNumber);
  }
  /** 
 ...
 * complete. How the send channel interprets this value depends on the value
 * of its {@link SctpStandardSocketOptions#SCTP_EXPLICIT_COMPLETESCTP_EXPLICIT_COMPLETE} socket option.
 * @param complete {@code true} if, and only if, the message is complete
 ...
 * @return  {@code true} if the message is unordered, otherwise{@code false}
 */
  public abstract boolean isUnordered();
  /** 
 * Sets whether or not the message is unordered.
 * @param unordered {@code true} requests the un-ordered delivery of the message,{@code false} indicates that the message is ordered.
 * @return  This MessageInfo
 ...
 * @param ppidThe Payload Protocol Identifier, or {@code 0} indicate an
 ...
 * @param streamNumberThe stream number
 * @throws IllegalArgumentExceptionIf the streamNumber is negative or greater than {@code 65536}
 * @return  This MessageInfo
 */
 ...
 * has not been sent.
 * @param millisThe time period in milliseconds, or {@code 0} to indicate that no
