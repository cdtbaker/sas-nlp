...
private class SoftChannelMixerContainer {
  ...
  protected SoftControl co_master=new SoftControl(){
    ...
    public double[] get(    int instance,    String name){
      ...
      if (name.equals("balance"))       return balance;
      if (name.equals("volume"))       return volume;
      if (name.equals("coarse_tuning"))       return coarse_tuning;
      if (name.equals("fine_tuning"))       return fine_tuning;
      return null;
    }
  }
;
...
if (subid2 == 0x01) setVolume(val);
 else if (subid2 == 0x02) setBalance(val);
 else if (subid2 == 0x03) setFineTuning(val);
 else if (subid2 == 0x04) setCoarseTuning(val);
break;
case 0x05:
int ix=5;
...
if (entry.getKey() >= (timeStamp + msec_buffer_len)) return;
long msec_delay=entry.getKey() - timeStamp;
delay_midievent=(int)(msec_delay * (samplerate / 1000000.0) + 0.5);
if (delay_midievent > max_delay_midievent) delay_midievent=max_delay_midievent;
if (delay_midievent < 0) delay_midievent=0;
...
if (balance > 0.5) volume_left*=(1 - balance) * 2;
 else volume_right*=balance * 2;
...
msec_last_activity=(long)((sample_pos + silent_samples) * (1000000.0 / samplerate));
}
public void stopMixer(ModelChannelMixer mixer){
if (stoppedMixers == null) stoppedMixers=new HashSet<ModelChannelMixer>();
stoppedMixers.add(mixer);
}
public void registerMixer(ModelChannelMixer mixer){
if (registeredMixers == null) registeredMixers=new HashSet<SoftChannelMixerContainer>();
...
mixercontainer.buffers[i]=new SoftAudioBuffer(buffer_len,synth.getFormat());
}
...
if (i == 9) channels[i].programChange(0,0x78 * 128);
 else channels[i].programChange(0,0x79 * 128);
}
 else channels[i].programChange(0,0);
}
setVolume(0x7F * 128 + 0x7F);
setBalance(0x40 * 128 + 0x00);
setCoarseTuning(0x40 * 128 + 0x00);
setFineTuning(0x40 * 128 + 0x00);
globalParameterControlChange(new int[]{0x01 * 128 + 0x01},new long[]{0},new long[]{4});
globalParameterControlChange(new int[]{0x01 * 128 + 0x02},new long[]{0},new long[]{2});
}
...
if (slothpath.length == 0) return;
synchronized (control_mutex) {
if (slothpath[0] == 0x01 * 128 + 0x01) {
for (int i=0; i < paramsvalue.length; i++) {
reverb.globalParameterControlChange(slothpath,params[i],paramsvalue[i]);
}
}
if (slothpath[0] == 0x01 * 128 + 0x02) {
for (int i=0; i < paramsvalue.length; i++) {
chorus.globalParameterControlChange(slothpath,params[i],paramsvalue[i]);
}
}
}
}
public void processMessage(Object object){
if (object instanceof byte[]) processMessage((byte[])object);
if (object instanceof MidiMessage) processMessage((MidiMessage)object);
}
...
int status=0;
if (data.length > 0) status=data[0] & 0xFF;
if (status == 0xF0) {
...
int data1;
int data2;
if (data.length > 1) data1=data[1] & 0xFF;
 else data1=0;
if (data.length > 2) data2=data[2] & 0xFF;
 else data2=0;
...
if (pusher_silent) {
if (synth.weakstream != null) {
return (long)((sample_pos + synth.weakstream.silent_samples) * (1000000.0 / samplerate));
}
}
