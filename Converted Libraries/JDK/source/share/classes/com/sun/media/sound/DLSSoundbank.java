...
static private class DLSID {
    ...
    public boolean equals(    Object obj){
      ...
      DLSID t=(DLSID)obj;
      return i1 == t.i1 && s1 == t.s1 && s2 == t.s2 && x1 == t.x1 && x2 == t.x2 && x3 == t.x3 && x4 == t.x4 && x5 == t.x5 && x6 == t.x6 && x7 == t.x7 && x8 == t.x8;
    }
  }
  /** 
 * X = X & Y 
  ...
  private void readSoundbank(  InputStream inputstream) throws IOException {
    ...
    while (riff.hasNextChunk()) {
      ...
      if (chunk.getFormat().equals("LIST")) {
        if (chunk.getType().equals("INFO"))         readInfoChunk(chunk);
        if (chunk.getType().equals("lins"))         readLinsChunk(chunk);
        if (chunk.getType().equals("wvpl"))         readWvplChunk(chunk);
      }
 else {
...
if (format.equals("INAM")) info.name=chunk.readString(chunk.available());
 else if (format.equals("ICRD")) info.creationDate=chunk.readString(chunk.available());
 else if (format.equals("IENG")) info.engineers=chunk.readString(chunk.available());
 else if (format.equals("IPRD")) info.product=chunk.readString(chunk.available());
 else if (format.equals("ICOP")) info.copyright=chunk.readString(chunk.available());
 else if (format.equals("ICMT")) info.comments=chunk.readString(chunk.available());
 else if (format.equals("ISFT")) info.tools=chunk.readString(chunk.available());
 else if (format.equals("IARL")) info.archival_location=chunk.readString(chunk.available());
 else if (format.equals("IART")) info.artist=chunk.readString(chunk.available());
 else if (format.equals("ICMS")) info.commissioned=chunk.readString(chunk.available());
 else if (format.equals("IGNR")) info.genre=chunk.readString(chunk.available());
 else if (format.equals("IKEY")) info.keywords=chunk.readString(chunk.available());
 else if (format.equals("IMED")) info.medium=chunk.readString(chunk.available());
 else if (format.equals("ISBJ")) info.subject=chunk.readString(chunk.available());
 else if (format.equals("ISRC")) info.source=chunk.readString(chunk.available());
 else if (format.equals("ISRF")) info.source_form=chunk.readString(chunk.available());
 else if (format.equals("ITCH")) info.technician=chunk.readString(chunk.available());
}
}
...
RIFFReader chunk=riff.nextChunk();
if (chunk.getFormat().equals("LIST")) {
if (chunk.getType().equals("ins ")) readInsChunk(chunk);
}
}
}
...
if (readRgnChunk(split,subchunk)) instrument.getRegions().add(split);
}
...
if (subchunk.getFormat().equals("art1")) readArt1Chunk(modlist,subchunk);
}
...
if (subchunk.getFormat().equals("art2")) readArt2Chunk(modlist,subchunk);
}
instrument.getModulators().addAll(modlist);
}
}
...
if (!readCdlChunk(chunk)) return false;
}
...
if (size > 20) riff.skipBytes(size - 20);
...
if (size2 > 16) riff.skipBytes(size2 - 16);
}
}
...
RIFFReader chunk=riff.nextChunk();
if (chunk.getFormat().equals("LIST")) {
if (chunk.getType().equals("wave")) readWaveChunk(chunk);
}
}
}
...
for (int i=0; i < samples.size(); i++) ptbl.writeUnsignedInt(0);
...
for (Long offset : offsettable) writer.writeUnsignedInt(offset.longValue());
...
if (audioformat.getEncoding().equals(Encoding.PCM_UNSIGNED)) sampleformat=1;
 else if (audioformat.getEncoding().equals(Encoding.PCM_SIGNED)) sampleformat=1;
 else if (audioformat.getEncoding().equals(Encoding.PCM_FLOAT)) sampleformat=3;
...
insh_chunk.writeUnsignedInt(instrument.bank + (instrument.druminstrument ? 2147483648L : 0));
insh_chunk.writeUnsignedInt(instrument.preset);
RIFFWriter lrgn=writer.writeList("lrgn");
for (DLSRegion region : instrument.regions) writeRegion(lrgn,region,version);
...
if (version == 1) rgns=writer.writeList("rgn ");
if (version == 2) rgns=writer.writeList("rgn2");
if (rgns == null) return;
...
if (region.sampleoptions != null) writeSampleOptions(rgns.writeChunk("wsmp"),region.sampleoptions);
