...
public class SoftFilter {
  ...
  public void setFrequency(  double cent){
    if (cutoff == cent)     return;
    cutoff=cent;
    dirty=true;
  }
  public void setResonance(  double db){
    if (resonancedB == db)     return;
    resonancedB=db;
    dirty=true;
  }
  public void reset(){
  ...
  public void processAudio(  SoftAudioBuffer sbuffer){
    if (filtertype == FILTERTYPE_LP6)     filter1(sbuffer);
    if (filtertype == FILTERTYPE_LP12)     filter2(sbuffer);
    if (filtertype == FILTERTYPE_HP12)     filter2(sbuffer);
    if (filtertype == FILTERTYPE_BP12)     filter2(sbuffer);
    if (filtertype == FILTERTYPE_NP12)     filter2(sbuffer);
    if (filtertype == FILTERTYPE_LP24)     filter4(sbuffer);
    if (filtertype == FILTERTYPE_HP24)     filter4(sbuffer);
  }
  public void filter4(  SoftAudioBuffer sbuffer){
    ...
    if (wet > 0 || last_wet > 0) {
      ...
      if (Math.abs(x1) < 1.0E-8)       x1=0;
      if (Math.abs(x2) < 1.0E-8)       x2=0;
      if (Math.abs(y1) < 1.0E-8)       y1=0;
      if (Math.abs(y2) < 1.0E-8)       y2=0;
      this.x1=x1;
      this.x2=x2;
      this.y1=y1;
      this.y2=y2;
  ...
  public void filter2calc(){
    ...
    if (resonancedB > 30)     resonancedB=30;
    if (filtertype == FILTERTYPE_LP24 || filtertype == FILTERTYPE_HP24)     resonancedB*=0.6;
    ...
    if (filtertype == FILTERTYPE_LP12 || filtertype == FILTERTYPE_LP24) {
      ...
      if (r > 0.45) {
        if (wet == 0) {
          if (resonancedB < 0.00001)           wet=0.0f;
 else           wet=1.0f;
        }
        r=0.45;
      }
