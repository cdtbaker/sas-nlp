...
private class ModelAbstractResamplerStream implements SoftResamplerStreamer {
    ...
    public void setPitch(    float pitch){
      ...
      if (!started)       current_pitch[0]=this.target_pitch;
    }
    public void nextBuffer() throws IOException {
      ...
      while (ix[0] >= sector_size + pad) {
        ...
        for (int c=0; c < nrofchannels; c++) {
          ...
          for (int i=0; i < pad2; i++)           cbuffer[i]=cbuffer[i + sector_size];
        }
        int ret;
        if (nrofchannels == 1)         ret=stream.read(ibuffer[0],pad2,sector_size);
 else {
          int slen=sector_size * nrofchannels;
          if (sbuffer == null || sbuffer.length < slen)           sbuffer=new float[slen];
          ...
          if (sret == -1)           ret=-1;
            ...
            for (int i=0; i < nrofchannels; i++) {
              ...
              int ox=pad2;
              for (int j=0; j < ret; j++, ix+=ix_step, ox++)               buff[ox]=sbuffer[ix];
            }
          }
        }
        if (ret == -1) {
          ret=0;
          stream_eof=true;
          for (int i=0; i < nrofchannels; i++)           Arrays.fill(ibuffer[i],pad2,pad2 + sector_size,0f);
          return;
        }
        if (ret != sector_size) {
          for (int i=0; i < nrofchannels; i++)           Arrays.fill(ibuffer[i],pad2 + ret,pad2 + sector_size,0f);
        }
        ibuffer_order=true;
      }
    }
    ...
    public int read(    float[][] buffer,    int offset,    int len) throws IOException {
      if (eof)       return -1;
      if (noteOff_flag)       if ((loopmode & 2) != 0)       if (loopdirection)       loopmode=0;
