  ...
  private AudioFileFormat internal_getAudioFileFormat(  InputStream stream) throws UnsupportedAudioFileException, IOException {
    ...
    while (riffiterator.hasNextChunk()) {
      ...
      if (chunk.getFormat().equals("fmt ")) {
        ...
        if (format != 0xFFFE)         throw new UnsupportedAudioFileException();
        ...
        if (cbSize != 22)         throw new UnsupportedAudioFileException();
        validBitsPerSample=chunk.readUnsignedShort();
        if (validBitsPerSample > bits)         throw new UnsupportedAudioFileException();
    ...
    if (s_channelmask != null)     p.put("channelOrder",s_channelmask);
    if (channelMask != 0)     p.put("channelMask",channelMask);
