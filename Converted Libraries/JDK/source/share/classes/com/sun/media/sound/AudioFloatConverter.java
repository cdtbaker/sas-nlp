...
private static class AudioFloatConversion64L extends AudioFloatConverter {
    ...
    public float[] toFloatArray(    byte[] in_buff,    int in_offset,    float[] out_buff,    int out_offset,    int out_len){
      ...
      if (double_buff == null || double_buff.length < out_len + out_offset)       double_buff=new double[out_len + out_offset];
...
private static class AudioFloatConversion8U extends AudioFloatConverter {
    public float[] toFloatArray(    byte[] in_buff,    int in_offset,    float[] out_buff,    int out_offset,    int out_len){
      int ix=in_offset;
      int ox=out_offset;
      for (int i=0; i < out_len; i++)       out_buff[ox++]=((in_buff[ix++] & 0xFF) - 127) * (1.0f / 127.0f);
      return out_buff;
    }
    public byte[] toByteArray(    float[] in_buff,    int in_offset,    int in_len,    byte[] out_buff,    int out_offset){
      int ix=in_offset;
  ...
  public static AudioFloatConverter getConverter(  AudioFormat format){
    ...
    if (format.getEncoding().equals(Encoding.PCM_SIGNED)) {
      ...
      if (format.getSampleSizeInBits() == 32) {
        if (format.isBigEndian())         conv=new AudioFloatConversion32B();
 else         conv=new AudioFloatConversion32L();
      }
 else       if (format.getSampleSizeInBits() == 64) {
        if (format.isBigEndian())         conv=new AudioFloatConversion64B();
 else         conv=new AudioFloatConversion64L();
      }
    }
