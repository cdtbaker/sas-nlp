...
public class ModelStandardTransform implements ModelTransform {
  ...
  public double transform(  double value){
    double s;
    double a;
    if (direction == DIRECTION_MAX2MIN)     value=1.0 - value;
    if (polarity == POLARITY_BIPOLAR)     value=value * 2.0 - 1.0;
switch (transform) {
...
if (polarity == POLARITY_BIPOLAR) return (value > 0) ? 1 : -1;
 else return (value > 0.5) ? 1 : 0;
