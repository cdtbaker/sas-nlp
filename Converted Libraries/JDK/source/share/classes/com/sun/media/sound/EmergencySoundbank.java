    ...
    double datab[];
{
      ...
      for (int i=0; i < data.length; i+=2)       data[i]=(2.0 * (random.nextDouble() - 0.5));
      fft(data);
      for (int i=fftlen / 2; i < data.length; i++)       data[i]=0;
      for (int i=0; i < 2048 * m; i++) {
        data[i]*=Math.exp(-Math.abs((i - 23) / ((double)m)) * 1.2) + Math.exp(-Math.abs((i - 40) / ((double)m)) * 0.9);
      }
    ...
    double datab[];
{
      ...
      fft(data);
      for (int i=fftlen / 2; i < data.length; i++)       data[i]=0;
      for (int i=0; i < 2048 * m; i++)       data[i]*=Math.exp(-Math.abs((i - 37) / ((double)m)) * 0.05);
      randomPhase(data,new Random(3049912));
      ifft(data);
      normalize(data,0.6);
      data=realPart(data);
  ...
  public static SF2Layer new_horn(  SF2Soundbank sf2){
    ...
    double a_step=Math.pow(end_a / start_a,1.0 / 40.0);
    for (int i=0; i < 40; i++) {
      if (i == 0)       complexGaussianDist(data,base * (i + 1),0.1,a * 0.2);
 else       complexGaussianDist(data,base * (i + 1),0.1,a);
      a*=a_step;
    }
  ...
  public static SF2Layer new_sax(  SF2Soundbank sf2){
    ...
    double a_step=Math.pow(end_a / start_a,1.0 / 40.0);
    for (int i=0; i < 40; i++) {
      if (i == 0 || i == 2)       complexGaussianDist(data,base * (i + 1),0.1,a * 4);
 else       complexGaussianDist(data,base * (i + 1),0.1,a);
      a*=a_step;
    }
    ...
    }
{
      ...
      fft(data);
      for (int i=fftlen / 2; i < data.length; i++)       data[i]=0;
      for (int i=1024 * 4; i < 2048 * 4; i++)       data[i]=1.0 - (i - 4096) / 4096.0;
      for (int i=0; i < 300; i++) {
    ...
    for (int i=0; i < datah.length; i++)     datab[i]+=datah[i] * 0.02;
    normalize(datab,0.9);
    ...
    double datah[];
{
      ...
      data=realPart(data);
      double d_len=data.length;
      for (int i=0; i < data.length; i++)       data[i]*=(1.0 - (i / d_len));
      datab=data;
    }
{
      int fftlen=4096 * 4;
      double[] data=new double[2 * fftlen];
      Random random=new Random(3049912);
      for (int i=0; i < data.length; i+=2)       data[i]=(2.0 * (random.nextDouble() - 0.5)) * 0.1;
      fft(data);
      for (int i=fftlen / 2; i < data.length; i++)       data[i]=0;
      for (int i=1024 * 4; i < 2048 * 4; i++)       data[i]=1.0 - (i - 4096) / 4096.0;
      for (int i=0; i < 200; i++) {
    ...
    for (int i=0; i < datah.length; i++)     datab[i]+=datah[i] * 0.5;
    for (int i=0; i < 5; i++)     datab[i]*=i / 5.0;
    ...
    }
{
    ...
    for (int i=0; i < datah.length; i++)     datab[i]+=datah[i];
    for (int i=0; i < 5; i++)     datab[i]*=i / 5.0;
    ...
    }
{
      ...
      fft(data);
      for (int i=fftlen / 2; i < data.length; i++)       data[i]=0;
      for (int i=1024; i < 2048; i++)       data[i]=1.0 - (i - 1024) / 1024.0;
      for (int i=0; i < 512; i++)       data[i]=10 * i / 512.0;
    ...
    double datah[];
{
      ...
      for (int i=fftlen / 2; i < data.length; i++)       data[i]=0;
      for (int i=1024 * 4; i < 2048 * 4; i++)       data[i]=1.0 - (i - 4096) / 4096.0;
      for (int i=0; i < 2048; i++)       data[i]=0.2 + 0.8 * (i / 2048.0);
      randomPhase(data,new Random(3049912));
      ifft(data);
      normalize(data,0.9);
      data=realPart(data);
    ...
    for (int i=0; i < 5; i++)     datah[i]*=i / 5.0;
    ...
    double datab[];
{
    ...
    for (int i=0; i < 10; i++)     datab[i]*=i / 10.0;
  ...
  static public void normalize(  float[] data,  double target){
    ...
    double gain=target / maxvalue;
    for (int i=0; i < data.length; i++)     data[i * 2]*=gain;
  }
  ...
  static public void fadeUp(  double[] data,  int samples){
    ...
    for (int i=0; i < samples; i++)     data[i]*=i / dsamples;
  }
