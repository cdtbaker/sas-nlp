...
static {
    for (int i=0; i < dontResetControls.length; i++)     dontResetControls[i]=false;
  ...
  protected double portamento_time=1;
...
;
  ...
  private int findFreeVoice(  int x){
    ...
    for (int i=x; i < voices.length; i++)     if (!voices[i].active)     return i;
  ...
  protected void initVoice(  SoftVoice voice,  SoftPerformer p,  int voiceID,  int noteNumber,  int velocity,  int delay,  ModelConnectionBlock[] connectionBlocks,  ModelChannelMixer channelmixer,  boolean releaseTriggered){
    if (voice.active) {
      ...
      for (int i=0; i < voices.length; i++)       if (voices[i].active && voices[i].voiceID == voice.voiceID)       voices[i].soundOff();
      return;
    }
    ...
    if (controller[84] != 0) {
      voice.co_noteon_keynumber[0]=(tuning.getTuning(controller[84]) / 100.0) * (1f / 128f);
  ...
  public void play(  int performerIndex,  ModelConnectionBlock[] connectionBlocks){
    ...
    if (firstVoice) {
      ...
      if (p.exclusiveClass != 0) {
        ...
        for (int j=0; j < voices.length; j++) {
          if (voices[j].active && voices[j].channel == channel && voices[j].exclusiveClass == x) {
            if (!(p.selfNonExclusive && voices[j].note == noteNumber))             voices[j].shutdown();
          }
        }
      }
    }
    ...
    if (voiceNo == -1)     return;
    initVoice(voices[voiceNo],p,prevVoiceID,noteNumber,velocity,delay,connectionBlocks,current_mixer,releasetriggered);
  }
...
synchronized (control_mutex) {
      ...
      for (int i=0; i < voices.length; i++) {
        if (voices[i].active && voices[i].note == noteNumber)         voices[i].setPolyPressure(pressure);
      }
    }
  }
...
synchronized (control_mutex) {
      ...
      for (int i=0; i < voices.length; i++) {
        if (voices[i].active)         voices[i].setChannelPressure(pressure);
      }
    }
  }
...
synchronized (control_mutex) {
  ...
  protected void applyInstrumentCustomization(){
    ...
    for (int i=0; i < new_performers.length; i++) {
      ...
      if (cds_control_connections != null) {
        ...
        for (int j=0; j < cds_control_connections.length; j++)         connblocks.add(cds_control_connections[j]);
      }
      if (cds_polypressure_connections != null) {
        ...
        for (int j=0; j < cds_polypressure_connections.length; j++)         connblocks.add(cds_polypressure_connections[j]);
      }
      if (cds_channelpressure_connections != null) {
        ...
        for (int j=0; j < cds_channelpressure_connections.length; j++)         connblocks.add(cds_channelpressure_connections[j]);
      }
    }
  ...
  private ModelConnectionBlock[] createModelConnections(  ModelIdentifier sid,  int[] destination,  int[] range){
    ...
    for (int i=0; i < destination.length; i++) {
      ...
      if (d == 0) {
        ...
        ModelConnectionBlock conn=new ModelConnectionBlock(new ModelSource(sid,ModelStandardTransform.DIRECTION_MIN2MAX,ModelStandardTransform.POLARITY_UNIPOLAR,ModelStandardTransform.TRANSFORM_LINEAR),scale,new ModelDestination(new ModelIdentifier("osc","pitch")));
        conns.add(conn);
      }
      if (d == 1) {
        ...
        if (scale > 0) {
          conn=new ModelConnectionBlock(new ModelSource(sid,ModelStandardTransform.DIRECTION_MAX2MIN,ModelStandardTransform.POLARITY_UNIPOLAR,ModelStandardTransform.TRANSFORM_LINEAR),-scale,new ModelDestination(ModelDestination.DESTINATION_FILTER_FREQ));
        }
 else {
          conn=new ModelConnectionBlock(new ModelSource(sid,ModelStandardTransform.DIRECTION_MIN2MAX,ModelStandardTransform.POLARITY_UNIPOLAR,ModelStandardTransform.TRANSFORM_LINEAR),scale,new ModelDestination(ModelDestination.DESTINATION_FILTER_FREQ));
        }
        conns.add(conn);
      }
      if (d == 2) {
 ...
 else             if (s > 1)             value=1 + (value * (s - 1.0));
 else             return 0;
            return -((5.0 / 12.0) / Math.log(10)) * Math.log(value);
          }
        }
;
        ModelConnectionBlock conn=new ModelConnectionBlock(new ModelSource(sid,mt),-960,new ModelDestination(ModelDestination.DESTINATION_GAIN));
        conns.add(conn);
      }
      if (d == 3) {
        double scale=(r / 64.0 - 1.0) * 9600.0;
        ModelConnectionBlock conn=new ModelConnectionBlock(new ModelSource(ModelSource.SOURCE_LFO1,ModelStandardTransform.DIRECTION_MIN2MAX,ModelStandardTransform.POLARITY_BIPOLAR,ModelStandardTransform.TRANSFORM_LINEAR),new ModelSource(sid,ModelStandardTransform.DIRECTION_MIN2MAX,ModelStandardTransform.POLARITY_UNIPOLAR,ModelStandardTransform.TRANSFORM_LINEAR),scale,new ModelDestination(ModelDestination.DESTINATION_PITCH));
        conns.add(conn);
      }
      if (d == 4) {
...
;
        ModelConnectionBlock conn=new ModelConnectionBlock(new ModelSource(ModelSource.SOURCE_LFO1,ModelStandardTransform.DIRECTION_MIN2MAX,ModelStandardTransform.POLARITY_UNIPOLAR,ModelStandardTransform.TRANSFORM_LINEAR),new ModelSource(sid,mt),-960,new ModelDestination(ModelDestination.DESTINATION_GAIN));
        conns.add(conn);
      }
    }
  ...
  public void controlChangePerNote(  int noteNumber,  int controller,  int value){
    ...
    if (controller < 120) {
      for (int i=0; i < voices.length; i++)       if (voices[i].active)       voices[i].controlChange(controller,-1);
    }
 else     if (controller == 120) {
      for (int i=0; i < voices.length; i++)       if (voices[i].active)       voices[i].rpnChange(1,-1);
    }
 else     if (controller == 121) {
      for (int i=0; i < voices.length; i++)       if (voices[i].active)       voices[i].rpnChange(2,-1);
    }
  }
...
if (controller == 6) val=(val & 127) + (value << 7);
 else if (controller == 38) val=(val & (127 << 7)) + value;
 else if (controller == 96 || controller == 97) {
int step=1;
if (rpn_control == 2 || rpn_control == 3 || rpn_control == 4) step=128;
if (controller == 96) val+=step;
if (controller == 97) val-=step;
}
if (nrpn_control != RPN_NULL_VALUE) nrpnChange(nrpn_control,val);
if (rpn_control != RPN_NULL_VALUE) rpnChange(rpn_control,val);
break;
case 64:
boolean on=value >= 64;
if (sustain != on) {
...
for (int i=0; i < voices.length; i++) if (voices[i].active && voices[i].channel == channel) voices[i].redamp();
}
}
break;
case 65:
...
if (controller < 0x20) this.controller[controller + 0x20]=0;
for (int i=0; i < voices.length; i++) if (voices[i].active) voices[i].controlChange(controller,value);
}
}
...
pitchbend=bend;
for (int i=0; i < voices.length; i++) if (voices[i].active) voices[i].setPitchBend(bend);
}
}
...
if (controller == (0x01 << 7) + (0x08)) controlChange(76,value >> 7);
if (controller == (0x01 << 7) + (0x09)) controlChange(77,value >> 7);
if (controller == (0x01 << 7) + (0x0A)) controlChange(78,value >> 7);
if (controller == (0x01 << 7) + (0x20)) controlChange(74,value >> 7);
if (controller == (0x01 << 7) + (0x21)) controlChange(71,value >> 7);
if (controller == (0x01 << 7) + (0x63)) controlChange(73,value >> 7);
if (controller == (0x01 << 7) + (0x64)) controlChange(75,value >> 7);
if (controller == (0x01 << 7) + (0x66)) controlChange(72,value >> 7);
if (controller >> 7 == 0x18) controlChangePerNote(controller % 128,120,value >> 7);
if (controller >> 7 == 0x1A) controlChangePerNote(controller % 128,7,value >> 7);
if (controller >> 7 == 0x1C) controlChangePerNote(controller % 128,10,value >> 7);
if (controller >> 7 == 0x1D) controlChangePerNote(controller % 128,91,value >> 7);
if (controller >> 7 == 0x1E) controlChangePerNote(controller % 128,93,value >> 7);
}
...
for (int i=0; i < voices.length; i++) if (voices[i].active) voices[i].nrpnChange(controller,val_i[0]);
}
...
val_i[0]=value;
val_d[0]=val_i[0] * (1.0 / 16384.0);
for (int i=0; i < voices.length; i++) if (voices[i].active) voices[i].rpnChange(controller,val_i[0]);
}
...
setPitchBend(8192);
for (int i=0; i < 128; i++) {
if (!dontResetControls[i]) controlChange(i,0);
}
controlChange(71,64);
controlChange(72,64);
controlChange(73,64);
...
if (controller != 3 && controller != 4) rpnChange(controller,0);
}
for (int controller : co_midi_nrpn_nrpn.keySet()) nrpnChange(controller,0);
rpnChange(0,2 << 7);
rpnChange(1,64 << 7);
rpnChange(2,64 << 7);
rpnChange(5,64);
...
synchronized (control_mutex) {
for (int i=0; i < voices.length; i++) if (voices[i].on && voices[i].channel == channel && voices[i].releaseTriggered == false) {
voices[i].noteOff(0);
}
}
}
...
synchronized (control_mutex) {
for (int i=0; i < voices.length; i++) if (voices[i].on && voices[i].channel == channel) voices[i].soundOff();
}
}
...
synchronized (control_mutex) {
this.mute=mute;
for (int i=0; i < voices.length; i++) if (voices[i].active && voices[i].channel == channel) voices[i].setMute(mute);
}
}
...
if (!soloinuse) {
for (SoftChannel c : synthesizer.channels) c.setSoloMute(false);
return;
}
for (SoftChannel c : synthesizer.channels) c.setSoloMute(!c.solo);
}
}
...
this.solomute=mute;
for (int i=0; i < voices.length; i++) if (voices[i].active && voices[i].channel == channel) voices[i].setSoloMute(solomute);
}
}
