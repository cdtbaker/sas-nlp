  ...
  private InputStream datastream=new InputStream(){
    ...
    public int read(    byte[] b,    int off,    int len) throws IOException {
      if (_loopcount != 0) {
        ...
        if (pos + len >= bloopend)         if (pos < bloopend) {
          int offend=off + len;
          int o=off;
          while (off != offend) {
            if (pos == bloopend) {
              if (_loopcount == 0)               break;
              pos=bloopstart;
              if (_loopcount != LOOP_CONTINUOUSLY)               _loopcount--;
            }
            len=offend - off;
...
;
  ...
  protected void processControlLogic(){
    ...
    if (afis == null) {
      afis=AudioFloatInputStream.getInputStream(new AudioInputStream(datastream,format,AudioSystem.NOT_SPECIFIED));
  ...
  protected void processAudioLogic(  SoftAudioBuffer[] buffers){
    if (_active) {
      ...
      try {
        ...
        if (ret != in_nrofchannels)         Arrays.fill(readbuffer,ret,readlen,0);
      }
 catch (      IOException e) {
      }
      ...
      if (_eff1gain > 0.0002) {
        float[] eff1=buffers[SoftMixingMainMixer.CHANNEL_EFFECT1].array();
        for (int i=0, ix=0; i < bufferlen; i++, ix+=in_c) {
      ...
      if (_eff2gain > 0.0002) {
        float[] eff2=buffers[SoftMixingMainMixer.CHANNEL_EFFECT2].array();
        for (int i=0, ix=0; i < bufferlen; i++, ix+=in_c) {
  ...
  public long getMicrosecondLength(){
    return (long)(getFrameLength() * (1000000.0 / (double)getFormat().getSampleRate()));
  }
...
synchronized (control_mutex) {
  ...
  public void open(  AudioInputStream stream) throws LineUnavailableException, IOException {
    ...
    if (AudioFloatConverter.getConverter(stream.getFormat()) == null)     throw new IllegalArgumentException("Invalid format : " + stream.getFormat().toString());
    if (stream.getFrameLength() != AudioSystem.NOT_SPECIFIED) {
      ...
      while (len != data.length) {
        if (readsize > data.length - len)         readsize=data.length - len;
        int ret=stream.read(data,len,readsize);
        if (ret == -1)         break;
        if (ret == 0)         Thread.yield();
        len+=ret;
      }
...
synchronized (control_mutex) {
      if (end != -1) {
        if (end < start)         throw new IllegalArgumentException("Invalid loop points : " + start + " - "+ end);
        if (end * framesize > bufferSize)         throw new IllegalArgumentException("Invalid loop points : " + start + " - "+ end);
      }
      if (start * framesize > bufferSize)       throw new IllegalArgumentException("Invalid loop points : " + start + " - "+ end);
      if (0 < start)       throw new IllegalArgumentException("Invalid loop points : " + start + " - "+ end);
      loopstart=start;
      loopend=end;
      loop_sg=true;
    }
  ...
  public void setMicrosecondPosition(  long microseconds){
    setFramePosition((int)(microseconds * (((double)getFormat().getSampleRate()) / 1000000.0)));
  }
...
synchronized (control_mutex) {
  ...
  public long getMicrosecondPosition(){
    return (long)(getFramePosition() * (1000000.0 / (double)getFormat().getSampleRate()));
  }
