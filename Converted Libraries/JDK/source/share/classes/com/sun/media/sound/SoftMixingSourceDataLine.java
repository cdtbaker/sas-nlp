...
private static class NonBlockingFloatInputStream extends AudioFloatInputStream {
  ...
  public int write(  byte[] b,  int off,  int len){
    if (!isOpen())     return 0;
    if (len % framesize != 0)     throw new IllegalArgumentException("Number of bytes does not represent an integral number of sample frames.");
...
synchronized (cycling_buffer) {
  ...
  public void open(  AudioFormat format) throws LineUnavailableException {
    if (bufferSize == -1)     bufferSize=((int)(format.getFrameRate() / 2)) * format.getFrameSize();
...
synchronized (cycling_buffer) {
  ...
  public long getMicrosecondPosition(){
    return (long)(getLongFramePosition() * (1000000.0 / (double)getFormat().getSampleRate()));
  }
