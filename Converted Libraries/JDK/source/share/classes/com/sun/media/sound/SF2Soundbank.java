  ...
  private void readSoundbank(  InputStream inputstream) throws IOException {
    ...
    while (riff.hasNextChunk()) {
      ...
      if (chunk.getFormat().equals("LIST")) {
        ...
        if (chunk.getType().equals("sdta"))         readSdtaChunk(chunk);
        if (chunk.getType().equals("pdta"))         readPdtaChunk(chunk);
      }
    }
  }
        ...
        int count=chunk.available() / 4;
{
  ...
  private void writePdtaChunk(  RIFFWriter writer) throws IOException {
    ...
    for (    SF2Instrument preset : this.instruments) {
      ...
      for (      SF2InstrumentRegion region : preset.getRegions())       writeModulators(pmod_chunk,region.getModulators());
    }
    ...
    for (    SF2Layer instrument : this.layers) {
      ...
      for (      SF2LayerRegion region : instrument.getRegions())       writeModulators(imod_chunk,region.getModulators());
    }
