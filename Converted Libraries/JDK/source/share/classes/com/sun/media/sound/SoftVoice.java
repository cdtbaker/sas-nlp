...
public class SoftVoice extends VoiceStatus {
  ...
  private SoftControl co_noteon=new SoftControl(){
    ...
    public double[] get(    int instance,    String name){
      ...
      if (name.equals("keynumber"))       return keynumber;
      if (name.equals("velocity"))       return velocity;
      if (name.equals("on"))       return on;
      return null;
    }
  }
;
  ...
  private SoftControl co_mixer=new SoftControl(){
    ...
    public double[] get(    int instance,    String name){
      ...
      if (name.equals("pan"))       return pan;
      if (name.equals("balance"))       return balance;
      if (name.equals("reverb"))       return reverb;
      if (name.equals("chorus"))       return chorus;
      return null;
    }
  }
;
...
;
  ...
  private SoftControl co_filter=new SoftControl(){
    ...
    public double[] get(    int instance,    String name){
      if (name == null)       return null;
      if (name.equals("freq"))       return freq;
      if (name.equals("type"))       return ftype;
      if (name.equals("q"))       return q;
      return null;
    }
  }
;
  ...
  private int getValueKC(  ModelIdentifier id){
    if (id.getObject().equals("midi_cc")) {
      ...
      if (id.getVariable().equals("1"))       return 120;
      if (id.getVariable().equals("2"))       return 121;
    }
    return -1;
  }
  ...
  private double transformValue(  double value,  ModelDestination dst){
    if (dst.getTransform() != null)     return dst.getTransform().transform(value);
 else     return value;
  }
  private double processKeyBasedController(  double value,  int keycontrol){
    if (keycontrol == -1)     return value;
    if (softchannel.keybasedcontroller_active != null)     if (softchannel.keybasedcontroller_active[note] != null)     if (softchannel.keybasedcontroller_active[note][keycontrol]) {
      double key_controlvalue=softchannel.keybasedcontroller_value[note][keycontrol];
      if (keycontrol == 10 || keycontrol == 91 || keycontrol == 93)       return key_controlvalue;
  ...
  protected void noteOn(  int noteNumber,  int velocity,  int delay){
    ...
    for (int i=0; i < connections.length; i++) {
      ...
      if (conn.getDestination() != null)       connections_dst[i]=getValue(conn.getDestination().getIdentifier());
 ...
 else       filter_freq=440.0 * Math.exp(((co_filter_freq[0]) - 6900.0) * (Math.log(2.0) / 1200.0));
  ...
  protected void mixAudioStream(  SoftAudioBuffer in,  SoftAudioBuffer out,  SoftAudioBuffer dout,  float amp_from,  float amp_to){
    ...
    if (dout != null && delay != 0) {
      if (amp_from == amp_to) {
        ...
        float[] fin=in.array();
        int j=0;
        for (int i=delay; i < bufferlen; i++)         fout[i]+=fin[j++] * amp_to;
        fout=dout.array();
        for (int i=0; i < delay; i++)         fout[i]+=fin[j++] * amp_to;
      }
 else {
        float amp=amp_from;
  ...
  protected void processAudioLogic(  SoftAudioBuffer[] buffer){
    ...
    if (nrofchannels == 1) {
      ...
      if (rightdry == null && last_out_mixer_left == last_out_mixer_right && out_mixer_left == out_mixer_right) {
        ...
        mixAudioStream(leftdry,left,dleft,last_out_mixer_left,out_mixer_left);
        if (rightdry != null)         mixAudioStream(rightdry,right,dright,last_out_mixer_right,out_mixer_right);
 else         mixAudioStream(leftdry,right,dright,last_out_mixer_right,out_mixer_right);
      }
    }
    if (rightdry == null) {
