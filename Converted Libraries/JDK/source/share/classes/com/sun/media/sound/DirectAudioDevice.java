 ...
 */
  ...
  DirectAudioDevice(  DirectAudioDeviceProvider.DirectAudioDeviceInfo portMixerInfo){
    ...
    if (srcLineInfo != null) {
      ...
      sourceLineInfo[1]=new DirectDLI(Clip.class,srcLineInfo.getFormats(),srcLineInfo.getHardwareFormats(),32,AudioSystem.NOT_SPECIFIED);
    }
 else {
...
synchronized (formats) {
  ...
  private static void addFormat(  Vector v,  int bits,  int frameSizeInBytes,  int channels,  float sampleRate,  int encoding,  boolean signed,  boolean bigEndian){
...
while (!drained) {
synchronized (lockNative) {
if ((id == 0) || (!doIO) || !nIsStillDraining(id,isSource)) break;
}
if ((counter % 5) == 4) {
...
if (pos < 0) {
if (Printer.debug) Printer.debug("DirectLine.getLongFramePosition: Native reported pos=" + pos + "! is changed to 0. byteposition="+ bytePosition);
pos=0;
}
...
throw new IllegalArgumentException("illegal request to write " + "non-integral number of frames (" + len + " bytes, "+ "frameSize = "+ getFormat().getFrameSize()+ " bytes)");
}
if (off < 0) {
...
synchronized (lockNative) {
if (monitoring && doIO && id != 0 && !flushing && !noService) {
...
private Balance(){
super(FloatControl.Type.BALANCE,-1.0f,1.0f,(1.0f / 128.0f),-1,0.0f,"","Left","Center","Right");
}
public void setValue(float newValue){
...
private Pan(){
super(FloatControl.Type.PAN,-1.0f,1.0f,(1.0f / 128.0f),-1,0.0f,"","Left","Center","Right");
}
public void setValue(float newValue){
setValueImpl(newValue);
...
if (len % getFormat().getFrameSize() != 0) {
throw new IllegalArgumentException("illegal request to read " + "non-integral number of frames (" + len + " bytes, "+ "frameSize = "+ getFormat().getFrameSize()+ " bytes)");
}
if (off < 0) {
throw new ArrayIndexOutOfBoundsException(off);
}
...
int priority=Thread.NORM_PRIORITY + (Thread.MAX_PRIORITY - Thread.NORM_PRIORITY) / 3;
thread=JSSecurityManager.createThread(this,"Direct Clip",true,priority,false);
thread.start();
}
}
if (isAutoClosing()) {
...
if (Printer.debug) Printer.debug("  DirectClip.setFramePosition: " + " doIO=" + doIO + " newFramePosition="+ newFramePosition+ " clipBytePosition="+ clipBytePosition+ " bytePosition="+ bytePosition+ " getLongFramePosition()="+ getLongFramePosition());
...
private static native void nGetFormats(int mixerIndex,int deviceID,boolean isSource,Vector formats);
...
private static native int nWrite(long id,byte[] b,int off,int len,int conversionSize,float volLeft,float volRight);
