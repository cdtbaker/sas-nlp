...
private static class SimpleInstrumentPart {
  ...
  public ModelPerformer[] getPerformers(){
    int percount=0;
    for (    SimpleInstrumentPart part : parts)     if (part.performers != null)     percount+=part.performers.length;
    ...
    for (    SimpleInstrumentPart part : parts) {
      if (part.performers != null) {
        for (        ModelPerformer mperfm : part.performers) {
          ...
          if (part.exclusiveClass != -1)           performer.setExclusiveClass(part.exclusiveClass);
          if (part.keyFrom > performer.getKeyFrom())           performer.setKeyFrom(part.keyFrom);
          if (part.keyTo < performer.getKeyTo())           performer.setKeyTo(part.keyTo);
          if (part.velFrom > performer.getVelFrom())           performer.setVelFrom(part.velFrom);
          if (part.velTo < performer.getVelTo())           performer.setVelTo(part.velTo);
