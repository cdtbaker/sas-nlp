...
private static class AudioFloatFormatConverterInputStream extends InputStream {
    ...
    public int read(    byte[] b,    int off,    int len) throws IOException {
      int flen=len / fsize;
      if (readfloatbuffer == null || readfloatbuffer.length < flen)       readfloatbuffer=new float[flen];
...
private static class AudioFloatInputStreamResampler extends AudioFloatInputStream {
  ...
  private Encoding[] formats={Encoding.PCM_SIGNED,Encoding.PCM_UNSIGNED,Encoding.PCM_FLOAT};
  public AudioInputStream getAudioInputStream(  Encoding targetEncoding,  AudioInputStream sourceStream){
    ...
    if (targetEncoding.equals(Encoding.PCM_FLOAT))     bits=32;
  ...
  public Encoding[] getSourceEncodings(){
    return new Encoding[]{Encoding.PCM_SIGNED,Encoding.PCM_UNSIGNED,Encoding.PCM_FLOAT};
  }
