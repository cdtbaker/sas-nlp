...
private static class Info extends Mixer.Info {
  ...
  public SoftMixingMixer(){
    ...
    sourceLineInfo[0]=new DataLine.Info(SourceDataLine.class,formats_array,AudioSystem.NOT_SPECIFIED,AudioSystem.NOT_SPECIFIED);
    sourceLineInfo[1]=new DataLine.Info(Clip.class,formats_array,AudioSystem.NOT_SPECIFIED,AudioSystem.NOT_SPECIFIED);
  }
  public Line getLine(  Line.Info info) throws LineUnavailableException {
    if (!isLineSupported(info))     throw new IllegalArgumentException("Line unsupported: " + info);
  ...
  public int getMaxLines(  Line.Info info){
    if (info.getLineClass() == SourceDataLine.class)     return AudioSystem.NOT_SPECIFIED;
    if (info.getLineClass() == Clip.class)     return AudioSystem.NOT_SPECIFIED;
    return 0;
  }
...
synchronized (control_mutex) {
      if (mainmixer == null)       return new Line[0];
...
synchronized (SoftMixingMixerProvider.mutex) {
  ...
  public AudioInputStream openStream(  AudioFormat targetFormat) throws LineUnavailableException {
    if (isOpen())     throw new LineUnavailableException("Mixer is already open");
synchronized (control_mutex) {
      open=true;
      implicitOpen=false;
      if (targetFormat != null)       format=targetFormat;
