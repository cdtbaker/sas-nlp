...
private static class RandomAccessByteWriter implements RandomAccessWriter {
  ...
  private RIFFWriter(  RandomAccessWriter raf,  String format,  int chunktype) throws IOException {
    if (chunktype == 0)     if (raf.length() != 0)     raf.setLength(0);
    this.raf=raf;
    if (raf.getPointer() % 2 != 0)     raf.write(0);
    if (chunktype == 0)     raf.write("RIFF".getBytes("ascii"));
 else     if (chunktype == 1)     raf.write("LIST".getBytes("ascii"));
    ...
    if (chunktype != 2)     raf.write((format + "    ").substring(0,4).getBytes("ascii"));
  }
  ...
  public void close() throws IOException {
    ...
    if (bakchunktype == 0)     raf.close();
 else     raf.seek(fpointer);
    open=false;
    raf=null;
  }
  public void write(  int b) throws IOException {
  ...
  public void writeString(  String string,  int len) throws IOException {
    byte[] buff=string.getBytes();
    if (buff.length > len)     write(buff,0,len);
 else {
      write(buff);
      for (int i=buff.length; i < len; i++)       write(0);
    }
  }
