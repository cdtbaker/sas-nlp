...
public class SoftSincResampler extends SoftAbstractResampler {
  ...
  public SoftSincResampler(){
    ...
    for (int s=0; s < sinc_scale_size; s++) {
      ...
      for (int i=0; i < sinc_table_fsize; i++) {
        sinc_table[s][i]=sincTable(sinc_table_size,-i / ((float)sinc_table_fsize),scale);
      }
    }
  }
  ...
  public static float[] wHanning(  int size,  float offset){
    ...
    for (int k=0; k < size; k++) {
      window_table[k]=(float)(-0.5 * Math.cos(2.0 * Math.PI * (double)(k + offset) / (double)size) + 0.5);
    }
    return window_table;
  }
  public static float[] sincTable(  int size,  float offset,  float scale){
    ...
    for (int k=0; k < size; k++)     w[k]*=sinc((-center + k + offset) * scale) * scale;
    return w;
  }
  ...
  public void interpolate(  float[] in,  float[] in_offset,  float in_end,  float[] startpitch,  float pitchstep,  float[] out,  int[] out_offset,  int out_end){
    ...
    if (pitchstep == 0) {
      ...
      while (ix < ix_end && ox < ox_end) {
        ...
        float y=0;
        for (int i=0; i < sinc_table_size; i++, xx++)         y+=in[xx] * sinc_table[i];
        out[ox++]=y;
        ix+=pitch;
      }
    }
