  ...
  public SoftMixingMainMixer(  SoftMixingMixer mixer){
    ...
    InputStream in=new InputStream(){
      ...
      public int read(      byte[] b,      int off,      int len){
        ...
        while (off < offlen)         if (available() == 0)         fillBuffer();
 else {
...
synchronized (control_mutex) {
      ...
      openLines=openLinesList.toArray(new SoftMixingDataLine[openLinesList.size()]);
    }
  }
...
synchronized (control_mutex) {
      ...
      openLines=openLinesList.toArray(new SoftMixingDataLine[openLinesList.size()]);
      if (openLines.length == 0)       if (mixer.implicitOpen)       mixer.close();
    }
  }
