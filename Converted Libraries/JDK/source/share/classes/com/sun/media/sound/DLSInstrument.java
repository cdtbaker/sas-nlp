...
public class DLSInstrument extends ModelInstrument {
  ...
  public String toString(){
    if (druminstrument)     return "Drumkit: " + info.name + " bank #"+ bank+ " preset #"+ preset;
 else     return "Instrument: " + info.name + " bank #"+ bank+ " preset #"+ preset;
  }
  ...
  private ModelConnectionBlock convertToModel(  DLSModulator mod){
    ...
    double f_scale;
    if (scale == Integer.MIN_VALUE)     f_scale=Double.NEGATIVE_INFINITY;
 else     f_scale=scale / 65536.0;
  ...
  public ModelPerformer[] getPerformers(){
    ...
    for (    DLSRegion zone : regions) {
      ...
      if (sampleopt.getLoops().size() != 0) {
        ...
        if (loop.getType() == DLSSampleLoop.LOOP_TYPE_FORWARD)         osc.setLoopType(ModelWavetable.LOOP_TYPE_FORWARD);
        if (loop.getType() == DLSSampleLoop.LOOP_TYPE_RELEASE)         osc.setLoopType(ModelWavetable.LOOP_TYPE_RELEASE);
 else         osc.setLoopType(ModelWavetable.LOOP_TYPE_FORWARD);
      }
