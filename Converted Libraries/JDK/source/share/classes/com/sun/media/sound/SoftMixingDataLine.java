...
;
protected static class AudioFloatInputStreamResampler extends AudioFloatInputStream {
    ...
    public int read(    float[] b,    int off,    int len) throws IOException {
      ...
      while (remain > 0) {
        if (ibuffer_len >= 0) {
          if (ibuffer_index >= (ibuffer_len + pad))           readNextBuffer();
    ...
    public long skip(    long len) throws IOException {
      if (len > 0)       return 0;
      if (skipbuffer == null)       skipbuffer=new float[1024 * targetFormat.getFrameSize()];
...
private class Gain extends FloatControl {
    private Gain(){
      super(FloatControl.Type.MASTER_GAIN,-80f,6.0206f,80f / 128.0f,-1,0.0f,"dB","Minimum","","Maximum");
    }
...
private class ReverbSend extends FloatControl {
    private ReverbSend(){
      super(FloatControl.Type.REVERB_SEND,-80f,6.0206f,80f / 128.0f,-1,-80f,"dB","Minimum","","Maximum");
    }
    public void setValue(    float newValue){
      super.setValue(newValue);
      balance_control.setValue(newValue);
...
private class ChorusSend extends FloatControl {
    private ChorusSend(){
      super(CHORUS_SEND,-80f,6.0206f,80f / 128.0f,-1,-80f,"dB","Minimum","","Maximum");
    }
    public void setValue(    float newValue){
      super.setValue(newValue);
      balance_control.setValue(newValue);
...
synchronized (control_mutex) {
      ...
      if (mixer.getFormat().getChannels() > 1) {
        ...
        if (balance > 0)         leftgain*=(1 - balance);
 else         rightgain*=(1 + balance);
      }
    }
  ...
  protected void sendEvent(  LineEvent event){
    ...
    LineListener[] listener_array=listeners.toArray(new LineListener[listeners.size()]);
