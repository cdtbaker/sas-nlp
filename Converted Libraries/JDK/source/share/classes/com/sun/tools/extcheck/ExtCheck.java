...
public class ExtCheck {
  ...
  private void investigateTarget(  File targetFile){
    ...
    if (targetManifest == null)     error("No manifest available in " + targetFile);
    ...
    if (targetSpecTitle == null)     error("The target file does not have a specification title");
    if (targetSpecVersion == null)     error("The target file does not have a specification version");
    verboseMessage("Specification title:" + targetSpecTitle);
    verboseMessage("Specification version:" + targetSpecVersion);
    if (targetSpecVendor != null)     verboseMessage("Specification vendor:" + targetSpecVendor);
    if (targetImplVersion != null)     verboseMessage("Implementation version:" + targetImplVersion);
    if (targetImplVendor != null)     verboseMessage("Implementation vendor:" + targetImplVendor);
    verboseMessage("");
  }
  /** 
 ...
 */
  private boolean checkURLRecursively(  int indent,  URL url) throws IOException {
      ...
      if (attr != null) {
        ...
        if (title != null) {
          if (title.equals(targetSpecTitle)) {
            if (version != null) {
              if (version.equals(targetSpecVersion) || isNotOlderThan(version,targetSpecVersion)) {
                verboseMessage("");
                verboseMessage("CONFLICT DETECTED ");
                verboseMessage("Conflicting file:" + url);
                verboseMessage("Installed Version:" + version);
                if (implTitle != null)                 verboseMessage("Implementation Title:" + implTitle);
                if (implVersion != null)                 verboseMessage("Implementation Version:" + implVersion);
                if (implVendor != null)                 verboseMessage("Implementation Vendor:" + implVendor);
                return false;
              }
            }
          }
 ...
 */
  private boolean isNotOlderThan(  String already,  String target) throws NumberFormatException {
    ...
    while (dtok.hasMoreTokens() || stok.hasMoreTokens()) {
      ...
      if (sver < dver)       return false;
      if (sver > dver)       return true;
      if (dtok.hasMoreTokens())       dtok.nextToken();
