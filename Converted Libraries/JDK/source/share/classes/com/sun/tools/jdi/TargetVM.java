...
public class TargetVM implements Runnable {
  ...
  private void dumpPacket(  Packet packet,  boolean sending){
    ...
    if (sending) {
      vm.printTrace(direction + " Command. id=" + packet.id+ ", length="+ packet.data.length+ ", commandSet="+ packet.cmdSet+ ", command="+ packet.cmd+ ", flags="+ packet.flags);
    }
 else {
      String type=(packet.flags & Packet.Reply) != 0 ? "Reply" : "Event";
      vm.printTrace(direction + " " + type+ ". id="+ packet.id+ ", length="+ packet.data.length+ ", errorCode="+ packet.errorCode+ ", flags="+ packet.flags);
    }
...
synchronized (waitingQueue) {
          ...
          if (p2 != null)           waitingQueue.remove(idString);
        }
        if (p2 == null) {
