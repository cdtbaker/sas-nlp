...
class ChildSession extends Session {
  ...
  public ChildSession(  ExecutionManager runtime,  LaunchingConnector connector,  Map<String,Connector.Argument> arguments,  InputListener input,  OutputListener output,  OutputListener error,  OutputListener diagnostics){
    this(runtime,generalGetVM(diagnostics,connector,arguments),input,output,error,diagnostics);
  }
  ...
  @Override public boolean attach(){
    if (!connectToVMProcess()) {
      diagnostics.putString("Could not launch VM");
      return false;
    }
    OutputReader outputReader=new OutputReader("output reader","output",out,output,diagnostics);
 ...
 */
  ...
  static private void dumpFailedLaunchInfo(  OutputListener diagnostics,  Process process){
    try {
  ...
  static private VirtualMachine getVM(  OutputListener diagnostics,  String userVMArgs,  String cmdLine){
  ...
  static private VirtualMachine generalGetVM(  OutputListener diagnostics,  LaunchingConnector connector,  Map<String,Connector.Argument> arguments){
...
private static class OutputReader extends Thread {
    ...
    @Override public void run(){
      try {
        ...
        SwingUtilities.invokeLater(new Runnable(){
          @Override public void run(){
            diagnostics.putString("IO error reading " + streamName + " stream of child java interpreter");
          }
        }
);
      }
