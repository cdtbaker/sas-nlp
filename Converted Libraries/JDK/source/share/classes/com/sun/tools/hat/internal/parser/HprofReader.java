...
public class HprofReader extends Reader implements ArrayTypeCodes {
  ...
  public Snapshot read() throws IOException {
    ...
    for (; ; ) {
      ...
      if (length < 0) {
        throw new IOException("Bad record length of " + length + " at byte "+ toHex(currPos + 5)+ " of file.");
      }
      currPos+=9 + length;
switch (type) {
case HPROF_UTF8:
...
if (stackFrames == null) {
  ...
  stackFrames.put(new Long(id),new StackFrame(methodName,methodSig,className,sourceFile,lineNumber));
}
break;
}
case HPROF_TRACE:
...
if (debugLevel > 0) {
System.out.println("    Read heap sub-record type " + type + " at position "+ toHex(posAtEnd - bytesLeft));
}
bytesLeft--;
switch (type) {
...
return (identifierSize == 4) ? (Snapshot.SMALL_ID_MASK & (long)in.readInt()) : in.readLong();
}
...
JavaClass c=new JavaClass(id,name,superId,classLoaderId,signersId,protDomainId,fields,statics,instanceSize);
