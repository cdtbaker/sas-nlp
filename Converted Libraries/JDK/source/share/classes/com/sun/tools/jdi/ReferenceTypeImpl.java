...
public abstract class ReferenceTypeImpl extends TypeImpl implements ReferenceType {
  ...
  public ClassLoaderReference classLoader(){
    if (!isClassLoaderCached) {
      try {
        classLoader=(ClassLoaderReference)JDWP.ReferenceType.ClassLoader.process(vm,this).classLoader;
  ...
  public List<Field> fields(){
    ...
    if (fields == null) {
      if (vm.canGet1_5LanguageFeatures()) {
        ...
        try {
          jdwpFields=JDWP.ReferenceType.Fields.process(vm,this).declared;
        }
 catch (        JDWPException exc) {
          throw exc.toJDIException();
        }
  ...
  public List<Method> methods(){
    ...
    if (methods == null) {
      if (!vm.canGet1_5LanguageFeatures()) {
        ...
        try {
          declared=JDWP.ReferenceType.MethodsWithGeneric.process(vm,this).declared;
        }
 catch (        JDWPException exc) {
          throw exc.toJDIException();
        }
  ...
  private List<Method> methods1_4(){
    ...
    try {
      declared=JDWP.ReferenceType.Methods.process(vm,this).declared;
    }
 catch (    JDWPException exc) {
      throw exc.toJDIException();
    }
  ...
  void addToMethodMap(  Map<String,Method> methodMap,  List<Method> methodList){
    for (    Method method : methodList)     methodMap.put(method.name().concat(method.signature()),method);
  }
  ...
  List<InterfaceType> getInterfaces(){
    ...
    try {
      intfs=JDWP.ReferenceType.Interfaces.process(vm,this).interfaces;
    }
 catch (    JDWPException exc) {
      throw exc.toJDIException();
    }
...
synchronized (this) {
  ...
  String baseSourceName() throws AbsentInformationException {
    ...
    if (bsn == null) {
      try {
        bsn=JDWP.ReferenceType.SourceFile.process(vm,this).sourceFile;
      }
 catch (      JDWPException exc) {
 ...
 */
  ...
  public List<ObjectReference> instances(  long maxInstances){
    ...
    try {
      return Arrays.asList((ObjectReference[])JDWP.ReferenceType.Instances.process(vm,this,intMax).instances);
    }
 catch (    JDWPException exc) {
      throw exc.toJDIException();
    }
