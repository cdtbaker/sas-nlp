 ...
 */
 ...
 else     if (obj instanceof Number)     return format(new Date(((Number)obj).longValue()),toAppendTo,fieldPosition);
  ...
  private static DateFormat get(  int timeStyle,  int dateStyle,  int flags,  Locale loc){
    ...
    try {
      ...
      if (pool.hasProviders()) {
        DateFormat providersInstance=pool.getLocalizedObject(DateFormatGetter.INSTANCE,loc,timeStyle,dateStyle,flags);
