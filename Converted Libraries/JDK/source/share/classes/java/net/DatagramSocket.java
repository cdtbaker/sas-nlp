 ...
 * @throws IllegalArgumentExceptionif the address is null, or the port is out of range.
 * @throws SecurityExceptionif a security manager has been installed and it does
 ...
 * @param addr    The remote address.
 * @throws SocketExceptionif the connect fails
 * @throws IllegalArgumentExceptionif {@code addr} is {@code null}, or {@code addr} is a SocketAddress
 ...
 * guarantee that the exception will be thrown.
 * @exception java.nio.channels.IllegalBlockingModeExceptionif this socket has an associated channel,
 ...
 */
  public synchronized void setReuseAddress(  boolean on) throws SocketException {
    if (isClosed())     throw new SocketException("Socket is closed");
    if (oldImpl)     getImpl().setOption(SocketOptions.SO_REUSEADDR,new Integer(on ? -1 : 0));
 else     getImpl().setOption(SocketOptions.SO_REUSEADDR,Boolean.valueOf(on));
  }
  /** 
 ...
 * @param onwhether or not to have broadcast turned on.
 * @throws SocketExceptionif there is an error in the underlying protocol, such as an UDP
 * error.
 * @since 1.4
 * @see #getBroadcast()
 */
