 ...
 */
  static boolean canConvertArgument(  Class<?> src,  Class<?> dst,  int level){
    if (VerifyType.isNullConversion(src,dst)) {
      ...
      if (dst.isPrimitive())       return canPrimCast(src,dst);
 else       return canBoxArgument(src,dst);
    }
 else {
      if (dst.isPrimitive())       return canUnboxArgument(src,dst,level);
 else       return true;
    }
  }
  /** 
 ...
 */
  static MethodHandle makePermutation(  MethodType newType,  MethodHandle target,  int[] argumentMap){
    ...
    if (argumentMap.length != oldType.parameterCount())     throw newIllegalArgumentException("bad permutation: " + Arrays.toString(argumentMap));
    if (nullPermutation) {
    ...
    Class<?> exp=newType.returnType();
    Class<?> ret=oldType.returnType();
    if (!VerifyType.isNullConversion(ret,exp))     throw newIllegalArgumentException("bad return conversion for " + newType);
    for (int i=0; i < argumentMap.length; i++) {
      int j=argumentMap[i];
      Class<?> src=newType.parameterType(j);
      Class<?> dst=oldType.parameterType(i);
      if (!VerifyType.isNullConversion(src,dst))       throw newIllegalArgumentException("bad argument #" + j + " conversion for "+ newType);
    }
...
return ((long)argnum << 32 | (long)convOp << CONV_OP_SHIFT | (int)src << CONV_SRC_TYPE_SHIFT | (int)dest << CONV_DEST_TYPE_SHIFT | insertStackMove(stackMove));
}
...
return ((long)srcArg << 32 | (long)convOp << CONV_OP_SHIFT | (int)srcType << CONV_SRC_TYPE_SHIFT | (int)destType << CONV_DEST_TYPE_SHIFT | (int)destSlot << CONV_VMINFO_SHIFT);
}
 ...
 */
int conversionOp(){
return (conversion & CONV_OP_MASK) >> CONV_OP_SHIFT;
}
...
if ((!raw ? VerifyType.canPassUnchecked(src,dst) : VerifyType.canPassRaw(src,dst)) > 0) return 0;
if (raw && !src.isPrimitive() && !dst.isPrimitive()) return 0;
return -1;
}
...
if (type.parameterType(last) != arrayType) target=target.asType(type.changeParameterType(last,arrayType));
...
@Override public boolean isVarargsCollector(){
return true;
}
@Override public MethodHandle asFixedArity(){
return target;
}
@Override public MethodHandle asType(MethodType newType){
...
if (sflt) convMethod=((src == double.class) ? ValueConversions.convertFromDouble(dst) : ValueConversions.convertFromFloat(dst));
 else convMethod=((dst == double.class) ? ValueConversions.convertToDouble(src) : ValueConversions.convertToFloat(src));
...
if (!canCheckCast(src,boxType) || boxType == convType || !VerifyType.isNullConversion(convType,dst)) return false;
int diff=diffTypes(newType,targetType,false);
return (diff == arg + 1);
}
/** 
...
if (canPrimCast(midType,oldType,arg,dst)) fixPrim=makePrimCast(midType,target,arg,dst);
 else fixPrim=target;
...
Class<?> boxType=Wrapper.asWrapperType(convType);
convType=Wrapper.asPrimitiveType(convType);
if (!canCheckCast(boxType,dst) || boxType == convType || !VerifyType.isNullConversion(src,convType)) return false;
int diff=diffTypes(newType,targetType,false);
return (diff == arg + 1);
}
/** 
...
if (!VerifyType.isNullConversion(boxType,dst)) target=makeCheckCast(oldType.changeParameterType(arg,boxType),target,arg,dst);
...
if (dropArgPos < 0 || dropArgPos >= nptypes || dropArgCount < 1 || afterPos > nptypes || targetType.parameterCount() != nptypes - dropArgCount) return false;
if (afterCount != 0 && diffParamTypes(newType,afterPos,targetType,dropArgPos,afterCount,false) != 0) return false;
return true;
}
/** 
...
if (diffParamTypes(newType,swapArg1 + 1,targetType,swapArg1 + 1,swapArg2 - swapArg1 - 1,false) != 0) return false;
if (diffParamTypes(newType,swapArg2,targetType,swapArg1,1,false) != 0) return false;
if (diffParamTypes(newType,swapArg2 + 1,targetType,swapArg2 + 1,nptypes - swapArg2 - 1,false) != 0) return false;
return true;
}
/** 
...
if (rotateBy > MAX_ARG_ROTATION && rotateBy < argCount - MAX_ARG_ROTATION) return false;
if (diffReturnTypes(newType,targetType,false) != 0) return false;
int nptypes=newType.parameterCount();
if (targetType.parameterCount() != nptypes) return false;
...
if (diffParamTypes(newType,firstArg,targetType,argLimit - newChunk1,newChunk1,false) != 0) return false;
if (diffParamTypes(newType,firstArg + newChunk1,targetType,firstArg,newChunk2,false) != 0) return false;
return true;
}
/** 
...
if (spreadArgPos < 0 || spreadArgPos >= nptypes || spreadArgCount < 0 || targetType.parameterCount() != afterPos + afterCount) return false;
if (afterCount != 0 && diffParamTypes(newType,spreadArgPos + 1,targetType,afterPos,afterCount,false) != 0) return false;
...
assert (canSpreadArguments(newType,targetType,spreadArgType,spreadArgPos,spreadArgCount)) : "[newType, targetType, spreadArgType, spreadArgPos, spreadArgCount] = " + Arrays.asList(newType,targetType,spreadArgType,spreadArgPos,spreadArgCount);
int dest=T_VOID;
for (int i=0; i < spreadArgCount; i++) {
...
assert (rtype == void.class || targetType.parameterType(collectArgPos) == rtype) : Arrays.asList(targetType,collectorType,collectArgPos,collectArgCount);
return true;
}
/** 
...
assert (diffParamTypes(newType,collectArgPos,targetType,collectValCount,collectArgCount,false) == 0) : Arrays.asList(target,collector,collectArgPos,retainOriginalArgs);
}
...
long conv=makeSpreadConv(retainOriginalArgs ? OP_FOLD_ARGS : OP_COLLECT_ARGS,lastCollectArg,src,dest,stackMove);
