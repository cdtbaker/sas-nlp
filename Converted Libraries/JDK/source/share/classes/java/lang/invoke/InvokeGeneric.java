 ...
 */
  InvokeGeneric(  MethodType erasedCallerType) throws ReflectiveOperationException {
    ...
    assert initialInvoker.type().equals(erasedCallerType.insertParameterTypes(0,MethodType.class,MethodHandle.class)) : initialInvoker.type();
  }
 ...
 */
  ...
  private MethodHandle dispatcher(  String dispatchName) throws ReflectiveOperationException {
    return lookup().bind(this,dispatchName,MethodType.methodType(MethodHandle.class,MethodType.class,MethodHandle.class));
  }
 ...
 */
  ...
  private MethodHandle dispatchWithConversion(  MethodType callerType,  MethodHandle target){
    ...
    if (returnConversionNeeded(callerType,target))     finisher=addReturnConversion(finisher,callerType.returnType());
    return finisher;
  }
  ...
  private boolean returnConversionNeeded(  MethodType callerType,  MethodHandle target){
    ...
    if (needType == erasedCallerType.returnType())     return false;
