  ...
  static CallSite makeSite(  MethodHandle bootstrapMethod,  String name,  MethodType type,  Object info,  MemberName callerMethod,  int callerBCI){
    ...
    try {
      ...
      if (info == null) {
        ...
        if (bsmType.parameterCount() == 4 && bsmType.parameterType(3) == Object[].class)         binding=bootstrapMethod.invoke(caller,name,type,argv);
 else         binding=MethodHandles.spreadInvoker(bsmType,3).invoke(bootstrapMethod,caller,name,type,argv);
      }
      ...
      if (!site.getTarget().type().equals(type))       throw new WrongMethodTypeException("wrong type: " + site.getTarget());
    }
 catch (    Throwable ex) {
      BootstrapMethodError bex;
      if (ex instanceof BootstrapMethodError)       bex=(BootstrapMethodError)ex;
 else       bex=new BootstrapMethodError("call site initialization exception",ex);
      throw bex;
    }
    return site;
  }
