 ...
 * if implemented.
 * @return the platform MXBean that implements{@code mxbeanInterface}, or {@code null} if not exist.
 * @throws IllegalArgumentException if {@code mxbeanInterface}is not a platform management interface or
 ...
 */
  public static <T extends PlatformManagedObject>T getPlatformMXBean(  Class<T> mxbeanInterface){
    ...
    if (pc == null)     throw new IllegalArgumentException(mxbeanInterface.getName() + " is not a platform management interface");
    if (!pc.isSingleton())     throw new IllegalArgumentException(mxbeanInterface.getName() + " can have zero or more than one instances");
 ...
 * @return the set of {@code Class} objects, subinterface of{@link PlatformManagedObject} representing
