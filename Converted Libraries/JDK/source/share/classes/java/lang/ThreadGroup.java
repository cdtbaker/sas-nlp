 ...
 * @param listan array into which to put the list of threads
 * @return  the number of threads put into the array
 * @throws SecurityExceptionif {@linkplain #checkAccess checkAccess} determines that
 ...
 * @param listan array into which to put the list of threads
 * @param recurseif {@code true}, recursively enumerate all subgroups of this
 * thread group
 * @return  the number of threads put into the array
 * @throws SecurityExceptionif {@linkplain #checkAccess checkAccess} determines that
 * the current thread cannot access this thread group
 ...
 * @param listan array into which to put the list of thread groups
 ...
 * that the method only be used for debugging and monitoring purposes.
 * @param listan array into which to put the list of thread groups
 * @param recurseif {@code true}, recursively enumerate all subgroups
 * @return  the number of thread groups put into the array
 * @throws SecurityExceptionif {@linkplain #checkAccess checkAccess} determines that
 * the current thread cannot access this thread group
 * @since   JDK1.0
...
synchronized (this) {
      checkAccess();
      for (int i=0; i < nthreads; i++) {
        if (threads[i] == us)         suicide=true;
 else         if (suspend)         threads[i].suspend();
 else         threads[i].stop();
      }
      ngroupsSnapshot=ngroups;
      if (groups != null) {
 ...
 * @param tthe Thread to be added
 * @throws IllegalThreadStateExceptionif the Thread group has been destroyed
 */
  void add(  Thread t){
synchronized (this) {
      if (destroyed) {
 ...
 * @param tthe Thread whose start method was invoked
 * @param failedtrue if the thread could not be started successfully
 */
 ...
 * this thread group.
 * @param tthe Thread that has terminated
 */
 ...
 * @param tthe Thread to be removed
 */
