 ...
 * The transformer is called when classes are loaded, when they are{@linkplain #redefineClasses redefined}. and if <code>canRetransform</code> is true,
 * when they are {@linkplain #retransformClasses retransformed}.
 * See {@link java.lang.instrument.ClassFileTransformer#transformClassFileTransformer.transform} for the order
 * of transform calls.
 ...
 * <code>true</code> in the agent JAR file (as described in the{@linkplain java.lang.instrument package specification}) and the JVM supports
 * this capability.
 ...
 * the initial class file bytes can be transformed with the{@link java.lang.instrument.ClassFileTransformer ClassFileTransformer}.
 ...
 * false, the bytes returned by{@link java.lang.instrument.ClassFileTransformer#transform transform}during the last class load or redefine are
 ...
 * except that{@link java.lang.instrument.ClassFileTransformer#transform transform}is not called
 * </li>
 * <li>for each transformer that was added with <code>canRetransform</code>
 * true, the{@link java.lang.instrument.ClassFileTransformer#transform transform}method is called in these transformers
 * </li>
 ...
 * </ul>
 * <P>
 * The order of transformation is described in the{@link java.lang.instrument.ClassFileTransformer#transform transform} method.
 * This same order is used in the automatic reapplication of retransformation
 * incapable transforms.
 * <P>
 * The initial class file bytes represent the bytes passed to{@link java.lang.ClassLoader#defineClass ClassLoader.defineClass} or{@link #redefineClasses redefineClasses}(before any transformations
 ...
 * example in bytecode instrumentation){@link #retransformClasses retransformClasses}should be used.
 * <P>
 * This method operates on
 * a set in order to allow interdependent changes to more than one class at the same time
 ...
 * class loader", unsuccessfully searches for a class, the entries in the {@link java.util.jar.JarFile JAR file} will be searched as well.
 ...
 * @param jarfileThe JAR file to be searched when the bootstrap class loader
 * unsuccessfully searches for a class.
 * @throws NullPointerExceptionIf <code>jarfile</code> is <code>null</code>.
 ...
 * Specifies a JAR file with instrumentation classes to be defined by the
 * system class loader.
 * When the system class loader for delegation (see{@link java.lang.ClassLoader#getSystemClassLoader getSystemClassLoader()})
 * unsuccessfully searches for a class, the entries in the {@link java.util.jar.JarFile JarFile} will be searched as well.
 * <p> This method may be used multiple times to add multiple JAR files to be
 * searched in the order that this method was invoked.
 * <p> The agent should take care to ensure that the JAR does not contain any
 ...
 * Failure to observe this warning could result in unexpected
 * behavior that is difficult to diagnose (see{@link #appendToBootstrapClassLoaderSearchappendToBootstrapClassLoaderSearch}).
 ...
 * the JAR file is obtained by invoking the {@link java.util.zip.ZipFile#getNamegetName()} method on the <code>jarfile</code> and this is provided as the
 ...
 * unsuccessfully attempted to resolve a reference, then subsequent attempts to
 * resolve that reference will fail with the same error as the initial attempt.
 * <p> This method does not change the value of <code>java.class.path</code>{@link java.lang.System#getProperties system property}.
 * @param jarfileThe JAR file to be searched when the system class loader
 * unsuccessfully searches for a class.
 * @throws UnsupportedOperationExceptionIf the system class loader does not support appending a
 ...
 * When used with the{@link java.lang.instrument.ClassFileTransformer ClassFileTransformer},
 ...
 * @param transformerThe ClassFileTransformer which wraps using this prefix.
 * @param prefixThe prefix to apply to wrapped native methods when
