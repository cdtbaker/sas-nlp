 ...
 */
  @Deprecated protected abstract FontPeer getFontPeer(  String name,  int style);
  /** 
 ...
 * @exception HeadlessExceptionif <code>GraphicsEnvironment.isHeadless()</code>
 ...
 * @throws SecurityExceptionif a security manager exists and its <code>{@link java.lang.SecurityManager#checkAwtEventQueueAccess}</code>
 ...
 * @return all of this toolkit's {@link PropertyChangeListener}objects wrapped in {@code java.beans.PropertyChangeListenerProxy} objects
...
synchronized (this) {
      SelectiveAWTEventListener selectiveListener=(SelectiveAWTEventListener)listener2SelectiveListener.get(localL);
...
private static class DesktopPropertyChangeSupport extends PropertyChangeSupport {
    ...
    @Override public synchronized void addPropertyChangeListener(    String propertyName,    PropertyChangeListener listener){
    ...
    @Override public synchronized void removePropertyChangeListener(    String propertyName,    PropertyChangeListener listener){
      PropertyChangeSupport pcs=(PropertyChangeSupport)AppContext.getAppContext().get(PROP_CHANGE_SUPPORT_KEY);
      if (null != pcs) {
        pcs.removePropertyChangeListener(propertyName,listener);
      }
    }
    @Override public synchronized PropertyChangeListener[] getPropertyChangeListeners(){
      PropertyChangeSupport pcs=(PropertyChangeSupport)AppContext.getAppContext().get(PROP_CHANGE_SUPPORT_KEY);
      if (null != pcs) {
    ...
    @Override public synchronized PropertyChangeListener[] getPropertyChangeListeners(    String propertyName){
      PropertyChangeSupport pcs=(PropertyChangeSupport)AppContext.getAppContext().get(PROP_CHANGE_SUPPORT_KEY);
      if (null != pcs) {
    ...
    @Override public void firePropertyChange(    final PropertyChangeEvent evt){
 ...
 * @return {@code true} if events from extra mouse buttons are allowed to be processed and posted;{@code false} otherwise
