 ...
 * an ordered list of {@link AclEntry access-control-entries}, each specifying a{@link UserPrincipal} and the level of access for that user principal. This
 ...
 * <p> When a file system provides access to a set of {@link FileStorefile-systems} that are not homogeneous then only some of the file systems may
 * support ACLs. The {@link FileStore#supportsFileAttributeViewsupportsFileAttributeView} method can be used to test if a file system
 * supports ACLs.
 ...
 * the {@code AclFileAttributeView} and the {@link PosixFileAttributeView}are supported then these special user identities may be included in ACL {@link AclEntry entries} that are read or written. The file system's {@link UserPrincipalLookupService} may be used to obtain a {@link UserPrincipal}to represent these special identities by invoking the {@link UserPrincipalLookupService#lookupPrincipalByName lookupPrincipalByName}method. </p>
 * <p> <b>Usage Example:</b>
 ...
 * the ACL or owner attributes as if by invoking the {@link #getAcl getAcl} or{@link #getOwner getOwner} methods.
 * <p> The {@link Files#setAttribute setAttribute} method may be used to
 * update the ACL or owner attributes as if by invoking the {@link #setAcl setAcl}or {@link #setOwner setOwner} methods.
 * <h4> Setting the ACL when creating a file </h4>
 * <p> Implementations supporting this attribute view may also support setting
 * the initial ACL when creating a file or directory. The initial ACL
 * may be provided to methods such as {@link Files#createFile createFile} or {@link Files#createDirectory createDirectory} as an {@link FileAttribute} with {@link FileAttribute#name name} {@code "acl:acl"} and a {@link FileAttribute#valuevalue} that is the list of {@code AclEntry} objects.
 ...
 * does not support a mapping of {@link AclEntryType#AUDIT} or {@link AclEntryType#ALARM} entries, then this method ignores these entries when
 * writing the ACL.
 * <p> If an ACL entry contains a {@link AclEntry#principal user-principal}that is not associated with the same provider as this attribute view then{@link ProviderMismatchException} is thrown. Additional validation, if
 * any, is implementation dependent.
 * <p> If the file system supports other security related file attributes
 * (such as a file {@link PosixFileAttributes#permissionsaccess-permissions} for example), the updating the access control list
 * may also cause these security related attributes to be updated.
 * @param aclthe new access control list
 * @throws IOExceptionif an I/O error occurs or the ACL is invalid
 * @throws SecurityExceptionIn the case of the default provider, a security manager is
