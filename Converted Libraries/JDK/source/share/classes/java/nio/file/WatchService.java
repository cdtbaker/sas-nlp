 ...
 * its {@link Watchable#register register} method, returning a {@link WatchKey}to represent the registration. When an event for an object is detected the
 * key is <em>signalled</em>, and if not currently signalled, it is queued to
 * the watch service so that it can be retrieved by consumers that invoke the{@link #poll() poll} or {@link #take() take} methods to retrieve keys
 ...
 * <p> Registration with a watch service is cancelled by invoking the key's{@link WatchKey#cancel cancel} method. A key that is queued at the time that
 ...
 * any time then care should be taken to ensure that the key's {@code reset}method is only invoked after its events have been processed. The {@link #close close} method may be invoked at any time to close the service causing
 * any threads waiting to retrieve keys, to throw {@codeClosedWatchServiceException}.
 ...
 * discards events then it arranges for the key's {@link WatchKey#pollEventspollEvents} method to return an element with an event type of {@link StandardWatchEventKinds#OVERFLOW OVERFLOW}. This event can be used by the
 ...
 * directory is modified then it may result in a single {@link StandardWatchEventKinds#ENTRY_MODIFY ENTRY_MODIFY} event in some
 ...
 * <p> If a thread is currently blocked in the {@link #take take} or {@link #poll(long,TimeUnit) poll} methods waiting for a key to be queued then
 * it immediately receives a {@link ClosedWatchServiceException}. Any
 * valid keys associated with this watch service are {@link WatchKey#isValidinvalidated}.
 ...
 * @throws ClosedWatchServiceExceptionif this watch service is closed
 */
  WatchKey poll();
  /** 
 * Retrieves and removes the next watch key, waiting if necessary up to the
 * specified wait time if none are yet present.
 * @param timeouthow to wait before giving up, in units of unit
 * @param unita {@code TimeUnit} determining how to interpret the timeout
 * parameter
 * @return  the next watch key, or {@code null}
 * @throws ClosedWatchServiceExceptionif this watch service is closed, or it is closed while waiting
 * for the next key
 * @throws InterruptedExceptionif interrupted while waiting
 */
