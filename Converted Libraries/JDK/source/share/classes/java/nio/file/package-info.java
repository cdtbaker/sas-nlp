 ...
 * systems. The API to access file and file system attributes is defined in the{@link java.nio.file.attribute} package. The {@link java.nio.file.spi}package is used by service provider implementors wishing to extend the
 ...
 * The {@link java.io.File} class defines the {@link java.io.File#toPathtoPath} method to construct a {@link java.nio.file.Path} by converting
 * the abstract path represented by the {@code java.io.File} object. The resulting{@code Path} can be used to operate on the same file as the {@code File}object. The {@code Path} specification provides further information
 * on the <a href="Path.html#interop">interoperability</a> between {@code Path}and {@code java.io.File} objects. </p>
 * <h3>Visibility</h3>
 ...
 * The {@link java.nio.file.StandardOpenOption#SYNC SYNC} and {@link java.nio.file.StandardOpenOption#DSYNC DSYNC} options are used when opening a file
 * to require that updates to the file are written synchronously to the underlying
 * storage device. In the case of the default provider, and the file resides on
 * a local storage device, and the {@link java.nio.channels.SeekableByteChannelseekable} channel is connected to a file that was opened with one of these
 * options, then an invocation of the {@link java.nio.channels.WritableByteChannel#write(java.nio.ByteBuffer) write}method is only guaranteed to return when all changes made to the file
 ...
 * is made. Whether this guarantee is possible with other {@link java.nio.file.spi.FileSystemProvider provider} implementations is provider
 * specific. </p>
 * <h3>General Exceptions</h3>
 * Unless otherwise noted, passing a {@code null} argument to a constructor
 * or method of any class or interface in this package will cause a {@link java.lang.NullPointerException NullPointerException} to be thrown. Additionally,
 ...
 * objects associated with a {@link java.nio.file.FileSystem} that has been{@link java.nio.file.FileSystem#close closed}. Additionally, any methods
 * that attempt write access to a file system will throw {@link java.nio.file.ReadOnlyFileSystemException} when invoked on an object associated
 ...
 * this package created by one {@link java.nio.file.spi.FileSystemProviderprovider} with a parameter that is an object created by another provider,
 ...
 * Where the specific error cannot be detected then the more general {@codeIOException} is thrown.
 * @since 1.7
 */
