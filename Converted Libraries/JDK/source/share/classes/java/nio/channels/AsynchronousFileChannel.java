 ...
 * write method that initiates asynchronous operations. A {@link CompletionHandler}is specified as a parameter and is invoked to consume the result of the I/O
 ...
 * <p> Channels of this type are safe for use by multiple concurrent threads. The{@link Channel#close close} method may be invoked at any time, as specified
 * by the {@link Channel} interface. This causes all outstanding asynchronous
 * operations on the channel to complete with the exception {@link AsynchronousCloseException}. Multiple read and write operations may be
 ...
 * guaranteed to execute in the order that the operations were initiated. The{@link java.nio.ByteBuffer ByteBuffers} used when reading or writing are not
 ...
 * The {@link StandardOpenOption#READ READ} and {@link StandardOpenOption#WRITEWRITE} options determines if the file should be opened for reading and/or
 ...
 * @param fileThe path of the file to open or create
 ...
 * @throws UnsupportedOperationExceptionIf the {@code file} is associated with a provider that does not
 ...
 * @throws SecurityExceptionIf a security manager is installed and it denies an
 ...
 * @param sizeThe new size, a non-negative byte count
 * @return  This file channel
 * @throws NonWritableChannelExceptionIf this channel was not opened for writing
 * @throws ClosedChannelExceptionIf this channel is closed
 * @throws IllegalArgumentExceptionIf the new size is negative
 * @throws IOExceptionIf some other I/O error occurs
 */
 ...
 * I/O operations that this method is required to perform.  Passing{@code false} for this parameter indicates that only updates to the
 * file's content need be written to storage; passing {@code true}indicates that updates to both the file's content and metadata must be
 ...
 * @param metaDataIf {@code true} then this method is required to force changes
 ...
 * <p> The region specified by the {@code position} and {@code size}parameters need not be contained within, or even overlap, the actual
 * underlying file.  Lock regions are fixed in size; if a locked region
 * initially contains the end of the file and the file grows beyond the
 * region then the new portion of the file will not be covered by the lock.
 * If a file is expected to grow in size and a lock on the entire file is
 * required then a region starting at zero, and no smaller than the
 * expected maximum size of the file, should be locked.  The two-argument{@link #lock(Object,CompletionHandler)} method simply locks a region
 ...
 * @param positionThe position at which the locked region is to start; must be
 * non-negative
 * @param sizeThe size of the locked region; must be non-negative, and the sum{@code position}&nbsp;+&nbsp;{@code size} must be non-negative
 * @param shared {@code true} to request a shared lock, in which case this
 * channel must be open for reading (and possibly writing);{@code false} to request an exclusive lock, in which case this
 * channel must be open for writing (and possibly reading)
 * @param attachmentThe object to attach to the I/O operation; can be {@code null}
 * @param handlerThe handler for consuming the result
 * @throws OverlappingFileLockExceptionIf a lock that overlaps the requested region is already held by
 * this Java virtual machine, or there is already a pending attempt
 * to lock an overlapping region
 * @throws IllegalArgumentExceptionIf the preconditions on the parameters do not hold
 * @throws NonReadableChannelExceptionIf {@code shared} is true but this channel was not opened for reading
 * @throws NonWritableChannelExceptionIf {@code shared} is false but this channel was not opened for writing
 */
 ...
 * operation fails). The result passed to the completion handler is the
 * resulting {@code FileLock}.
 * <p> An invocation of this method of the form {@code ch.lock(att,handler)}behaves in exactly the same way as the invocation
 * <pre>
 * ch.{@link #lock(long,long,boolean,Object,CompletionHandler) lock}(0L, Long.MAX_VALUE, false, att, handler)
 * </pre>
 * @param attachmentThe object to attach to the I/O operation; can be {@code null}
 * @param handlerThe handler for consuming the result
 * @throws OverlappingFileLockExceptionIf a lock is already held by this Java virtual machine, or there
 ...
 * manner as the {@link #lock(long,long,boolean,Object,CompletionHandler)}method except that instead of specifying a completion handler, this
 * method returns a {@code Future} representing the pending result. The{@code Future}'s {@link Future#get() get} method returns the {@link FileLock} on successful completion.
 * @param positionThe position at which the locked region is to start; must be
 * non-negative
 * @param sizeThe size of the locked region; must be non-negative, and the sum{@code position}&nbsp;+&nbsp;{@code size} must be non-negative
 * @param shared {@code true} to request a shared lock, in which case this
 ...
 * pending result of the operation. The {@code Future}'s {@link Future#get()get} method returns the {@link FileLock} on successful completion.
 * <p> An invocation of this method behaves in exactly the same way as the
 * invocation
 * <pre>
 ...
 * <p> An invocation of this method of the form {@code ch.tryLock()}behaves in exactly the same way as the invocation
 * <pre>
 ...
 * read is attempted.
 * <p> This method works in the same manner as the {@link AsynchronousByteChannel#read(ByteBuffer,Object,CompletionHandler)}method, except that bytes are read starting at the given file position.
 * If the given file position is greater than the file's size at the time
 * that the read is attempted then no bytes are read.
 * @param dstThe buffer into which bytes are to be transferred
 * @param positionThe file position at which the transfer is to begin;
 * must be non-negative
 * @param attachmentThe object to attach to the I/O operation; can be {@code null}
 * @param handlerThe handler for consuming the result
 * @throws IllegalArgumentExceptionIf the position is negative or the buffer is read-only
 * @throws NonReadableChannelExceptionIf this channel was not opened for reading
 */
 ...
 * <p> This method works in the same manner as the {@link AsynchronousByteChannel#read(ByteBuffer)} method, except that bytes are
 ...
 * <p> This method works in the same manner as the {@link AsynchronousByteChannel#write(ByteBuffer,Object,CompletionHandler)}method, except that bytes are written starting at the given file position.
 ...
 * @param srcThe buffer from which bytes are to be transferred
 * @param positionThe file position at which the transfer is to begin;
 * must be non-negative
 * @param attachmentThe object to attach to the I/O operation; can be {@code null}
 * @param handlerThe handler for consuming the result
 * @throws IllegalArgumentExceptionIf the position is negative
 * @throws NonWritableChannelExceptionIf this channel was not opened for writing
 */
 ...
 * <p> This method works in the same manner as the {@link AsynchronousByteChannel#write(ByteBuffer)} method, except that bytes are
