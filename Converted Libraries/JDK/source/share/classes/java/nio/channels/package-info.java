 ...
 * other interfaces.
 * <p> The {@link java.nio.channels.ReadableByteChannel} interface specifies a{@link java.nio.channels.ReadableByteChannel#read read} method that reads bytes
 * from the channel into a buffer; similarly, the {@link java.nio.channels.WritableByteChannel} interface specifies a {@link java.nio.channels.WritableByteChannel#write write} method that writes bytes
 * from a buffer to the channel. The {@link java.nio.channels.ByteChannel}interface unifies these two interfaces for the common case of channels that can
 * both read and write bytes. The {@link java.nio.channels.SeekableByteChannel}interface extends the {@code ByteChannel} interface with methods to {@link java.nio.channels.SeekableByteChannel#position() query} and {@link java.nio.channels.SeekableByteChannel#position(long) modify} the channel's
 * current position, and its {@link java.nio.channels.SeekableByteChannel#sizesize}.
 * <p> The {@link java.nio.channels.ScatteringByteChannel} and {@link java.nio.channels.GatheringByteChannel} interfaces extend the {@link java.nio.channels.ReadableByteChannel} and {@link java.nio.channels.WritableByteChannel} interfaces, respectively, adding {@link java.nio.channels.ScatteringByteChannel#read read} and {@link java.nio.channels.GatheringByteChannel#write write} methods that take a
 ...
 * obtain the address to which the socket is bound, and methods to {@link java.nio.channels.NetworkChannel#getOption get} and {@link java.nio.channels.NetworkChannel#setOption set} socket options. The {@link java.nio.channels.MulticastChannel} interface specifies methods to join
 * Internet Protocol (IP) multicast groups.
 * <p> The {@link java.nio.channels.Channels} utility class defines static methods
 * that support the interoperation of the stream classes of the <tt>{@link java.io}</tt> package with the channel classes of this package.  An appropriate
 * channel can be constructed from an {@link java.io.InputStream} or an {@link java.io.OutputStream}, and conversely an {@link java.io.InputStream} or an{@link java.io.OutputStream} can be constructed from a channel.  A {@link java.io.Reader} can be constructed that uses a given charset to decode bytes
 ...
 * <p> A {@code FileChannel} is created by invoking one of its static {@link java.nio.channels.FileChannel#open open} methods, or by invoking the {@codegetChannel} method of a {@link java.io.FileInputStream}, {@link java.io.FileOutputStream}, or {@link java.io.RandomAccessFile} to return a
 ...
 * put into non-blocking mode, and {@link java.nio.channels.SelectableChannel#register <i>registered</i>}with a selector.  Registering a channel specifies the set of I/O operations
 ...
 * to be incorrect.
 * <p> This package defines selectable-channel classes corresponding to the {@link java.net.DatagramSocket}, {@link java.net.ServerSocket}, and {@link java.net.Socket} classes defined in the <tt>{@link java.net}</tt> package.
 ...
 * can be replaced by "plugging in" an alternative definition or instance of the{@link java.nio.channels.spi.SelectorProvider} class defined in the <tt>{@link java.nio.channels.spi}</tt> package.  It is not expected that many developers
 ...
 * multiplexed-I/O abstractions is performed by the {@link java.nio.channels.spi.AbstractInterruptibleChannel}, {@link java.nio.channels.spi.AbstractSelectableChannel}, {@link java.nio.channels.spi.AbstractSelectionKey}, and {@link java.nio.channels.spi.AbstractSelector} classes in the <tt>{@link java.nio.channels.spi}</tt> package.  When defining a custom selector provider,
 * only the {@link java.nio.channels.spi.AbstractSelector} and {@link java.nio.channels.spi.AbstractSelectionKey} classes should be subclassed
 * directly; custom channel classes should extend the appropriate {@link java.nio.channels.SelectableChannel} subclasses defined in this package.
 * <a name="async"></a>
 ...
 * for asynchronous reading, writing, and manipulating a file. As with the {@link java.nio.channels.FileChannel} it supports operations to truncate the file
 ...
 * purpose of resource sharing. A group has an associated {@link java.util.concurrent.ExecutorService} to which tasks are submitted to handle
 ...
 * own asynchronous channel groups or configure the {@code ExecutorService}that will be used for the default group.
 * <p> As with selectors, the implementatin of asynchronous channels can be
 * replaced by "plugging in" an alternative definition or instance of the {@link java.nio.channels.spi.AsynchronousChannelProvider} class defined in the
 ...
 * or method in any class or interface in this package will cause a {@link java.lang.NullPointerException NullPointerException} to be thrown.
 * @since 1.4
