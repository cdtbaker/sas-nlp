 ...
 */
  ...
  /** 
 * Write all remaining bytes in buffer to the given channel.
 * @throws IllegalBlockingExceptionIf the channel is selectable and configured non-blocking.
 */
 ...
 * @param chThe channel from which bytes will be read
 ...
 * @param chThe channel to which bytes will be written
 ...
 */
  public static InputStream newInputStream(  final AsynchronousByteChannel ch){
    ...
    return new InputStream(){
      ...
      @Override public synchronized int read(      byte[] bs,      int off,      int len) throws IOException {
        if ((off < 0) || (off > bs.length) || (len < 0)|| ((off + len) > bs.length)|| ((off + len) < 0)) {
          throw new IndexOutOfBoundsException();
        }
 ...
 */
  public static OutputStream newOutputStream(  final AsynchronousByteChannel ch){
    ...
    return new OutputStream(){
      ...
      @Override public synchronized void write(      byte[] bs,      int off,      int len) throws IOException {
        if ((off < 0) || (off > bs.length) || (len < 0)|| ((off + len) > bs.length)|| ((off + len) < 0)) {
          throw new IndexOutOfBoundsException();
        }
 else         if (len == 0) {
 ...
 * @param inThe stream from which bytes are to be read
 ...
 * its I/O operations to the given stream.  Closing the channel will in
 * turn cause the stream to be closed.  </p>
 * @param outThe stream to which bytes are to be written
 ...
 * @param chThe channel from which bytes will be read
 * @param decThe charset decoder to be used
 * @param minBufferCapThe minimum capacity of the internal byte buffer,
 ...
 * -1);</pre></blockquote>
 * @param chThe channel from which bytes will be read
 * @param csNameThe name of the charset to be used
 * @return  A new reader
 ...
 * @param chThe channel to which bytes will be written
 * @param encThe charset encoder to be used
 * @param minBufferCapThe minimum capacity of the internal byte buffer,
 * or <tt>-1</tt> if an implementation-dependent
 * default capacity is to be used
 * @return  A new writer
