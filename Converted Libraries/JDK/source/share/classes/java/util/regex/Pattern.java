 ...
 * @param regexThe expression to be compiled
 * @throws PatternSyntaxExceptionIf the expression's syntax is invalid
 */
 ...
 * flags.  </p>
 * @param regexThe expression to be compiled
 * @param flagsMatch flags, a bit mask that may include{@link #CASE_INSENSITIVE}, {@link #MULTILINE}, {@link #DOTALL},{@link #UNICODE_CASE}, {@link #CANON_EQ}, {@link #UNIX_LINES},{@link #LITERAL}, {@link #UNICODE_CHARACTER_CLASS}and {@link #COMMENTS}
 * @throws IllegalArgumentExceptionIf bit values other than those corresponding to the defined
 ...
 * </p>
 * @param inputThe character sequence to be matched
 ...
 * <td><tt>{ "b", "", ":and:f" }</tt></td></tr>
 * </table></blockquote>
 * @param inputThe character sequence to be split
 ...
 */
  private String composeOneStep(  String input){
    ...
    if (result.equals(firstTwoCharacters))     return null;
 else {
...
next();
if (has(MULTILINE)) {
if (has(UNIX_LINES)) node=new UnixCaret();
 else node=new Caret();
}
 else {
node=new Begin();
...
case '$':
next();
if (has(UNIX_LINES)) node=new UnixDollar(has(MULTILINE));
...
if (has(UNIX_LINES)) node=new UnixDot();
 else {
node=new Dot();
}
}
...
if (has(CASE_INSENSITIVE)) return new CIBackRef(refNum,has(UNICODE_CASE));
...
break;
case 'D':
if (create) root=has(UNICODE_CHARACTER_CLASS) ? new Utype(UnicodeProp.DIGIT).complement() : new Ctype(ASCII.DIGIT).complement();
return -1;
case 'E':
case 'F':
break;
...
break;
case 'S':
if (create) root=has(UNICODE_CHARACTER_CLASS) ? new Utype(UnicodeProp.WHITE_SPACE).complement() : new Ctype(ASCII.SPACE).complement();
return -1;
case 'T':
case 'U':
case 'V':
break;
case 'W':
if (create) root=has(UNICODE_CHARACTER_CLASS) ? new Utype(UnicodeProp.WORD).complement() : new Ctype(ASCII.WORD).complement();
return -1;
case 'X':
case 'Y':
break;
...
if (inclass) break;
if (create) {
if (has(UNIX_LINES)) root=new UnixDollar(false);
...
return c();
case 'd':
if (create) root=has(UNICODE_CHARACTER_CLASS) ? new Utype(UnicodeProp.DIGIT) : new Ctype(ASCII.DIGIT);
return -1;
case 'e':
return '\033';
case 'f':
...
if (!namedGroups().containsKey(name)) throw error("(named capturing group <" + name + "> does not exit");
if (create) {
if (has(CASE_INSENSITIVE)) root=new CIBackRef(namedGroups().get(name),has(UNICODE_CASE));
...
return '\r';
case 's':
if (create) root=has(UNICODE_CHARACTER_CLASS) ? new Utype(UnicodeProp.WHITE_SPACE) : new Ctype(ASCII.SPACE);
return -1;
case 't':
return '\t';
case 'u':
...
return '\013';
case 'w':
if (create) root=has(UNICODE_CHARACTER_CLASS) ? new Utype(UnicodeProp.WORD) : new Ctype(ASCII.WORD);
return -1;
case 'x':
return x();
case 'y':
...
int d;
if (ch < 256 && !(has(CASE_INSENSITIVE) && has(UNICODE_CASE) && (ch == 0xff || ch == 0xb5 || ch == 0x49 || ch == 0x69 || ch == 0x53 || ch == 0x73 || ch == 0x4b || ch == 0x6b || ch == 0xc5 || ch == 0xe5))) return bits.add(ch,flags());
return newSingle(ch);
}
/** 
...
if (has(CASE_INSENSITIVE)) return caseInsensitiveRangeFor(ch,m);
...
if (namedGroups().containsKey(name)) throw error("Named capturing group <" + name + "> is already defined");
capturingGroup=true;
...
Loop loop;
if (curly.type == GREEDY) loop=new Loop(this.localCount,temp);
 else loop=new LazyLoop(this.localCount,temp);
...
if (isSupplementary(ch)) return new SingleS(ch);
...
defCategory("C",((1 << Character.CONTROL) | (1 << Character.FORMAT) | (1 << Character.PRIVATE_USE)| (1 << Character.SURROGATE)));
