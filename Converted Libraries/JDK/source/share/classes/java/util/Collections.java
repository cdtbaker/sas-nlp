 ...
 * to sort a linked list in place.
 * @param list the list to be sorted.
 * @param c the comparator to determine the order of the list.  A{@code null} value indicates that the elements' <i>natural
 ...
 */
  public static <T>int binarySearch(  List<? extends T> list,  T key,  Comparator<? super T> c){
    ...
    if (list instanceof RandomAccess || list.size() < BINARYSEARCH_THRESHOLD)     return Collections.indexedBinarySearch(list,key,c);
 ...
 */
  ...
  @SuppressWarnings("unchecked") static <T>T[] zeroLengthArray(  Class<T> type){
...
;
    ...
    @SuppressWarnings("unchecked") Collection<E> checkedCopyOf(    Collection<? extends E> coll){
      Object[] a=null;
      try {
...
private static class CheckedMap<K,V> implements Map<K,V>, Serializable {
    ...
    @SuppressWarnings("unchecked") public void putAll(    Map<? extends K,? extends V> t){
...
private static class SingletonMap<K,V> extends AbstractMap<K,V> implements Serializable {
    ...
    public Set<Map.Entry<K,V>> entrySet(){
      if (entrySet == null)       entrySet=Collections.<Map.Entry<K,V>>singleton(new SimpleImmutableEntry<>(k,v));
      return entrySet;
    }
    public Collection<V> values(){
 ...
 * @throws NullPointerException if one collection contains a {@code null}element and {@code null} is not an eligible element for the other collection.
