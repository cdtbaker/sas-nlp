 ...
 */
  @SuppressWarnings("unchecked") public <T>T[] toArray(  T a[]){
    ...
    if (a.length < len)     return (T[])Arrays.copyOf(elements,len,a.getClass());
 else {
  ...
  @SuppressWarnings("unchecked") private E get(  Object[] a,  int index){
    return (E)a[index];
  }
  /** 
 * {@inheritDoc}
 ...
 */
  private void removeRange(  int fromIndex,  int toIndex){
    ...
    try {
      ...
      if (numMoved == 0)       setArray(Arrays.copyOf(elements,newlen));
 else {
        Object[] newElements=new Object[newlen];
 ...
 */
  public boolean addIfAbsent(  E e){
    ...
    try {
      ...
      Object[] newElements=new Object[len + 1];
      for (int i=0; i < len; ++i) {
        if (eq(e,elements[i]))         return false;
 else         newElements[i]=elements[i];
      }
      newElements[len]=e;
      setArray(newElements);
