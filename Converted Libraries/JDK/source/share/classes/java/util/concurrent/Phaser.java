 ...
 */
 ...
 else       if ((phase=(int)(s >>> PHASE_SHIFT)) < 0)       break;
 else       if (counts != EMPTY) {
        if (parent == null || reconcileState() == s) {
          if (unarrived == 0)           root.internalAwaitAdvance(phase,null);
 else           if (UNSAFE.compareAndSwapLong(this,stateOffset,s,s + adj))           break;
        }
      }
 else       if (parent == null) {
...
synchronized (this) {
 ...
 while (!UNSAFE.compareAndSwapLong(this,stateOffset,state,((long)phase << PHASE_SHIFT) | adj));
            break;
          }
        }
      }
 ...
 */
  private long reconcileState(){
    ...
    if (root != this) {
      int phase, u, p;
      while ((phase=(int)(root.state >>> PHASE_SHIFT)) != (int)(s >>> PHASE_SHIFT) && !UNSAFE.compareAndSwapLong(this,stateOffset,s,s=(((long)phase << PHASE_SHIFT) | (s & PARTIES_MASK) | ((p=(int)s >>> PARTIES_SHIFT) == 0 ? EMPTY : (u=(int)s & UNARRIVED_MASK) == 0 ? p : u))))       s=state;
    }
    return s;
  }
  /** 
 ...
 */
  public int awaitAdvanceInterruptibly(  int phase,  long timeout,  TimeUnit unit) throws InterruptedException, TimeoutException {
    ...
    if (p == phase) {
      ...
      p=root.internalAwaitAdvance(phase,node);
      if (node.wasInterrupted)       throw new InterruptedException();
 else       if (p == phase)       throw new TimeoutException();
    }
    return p;
  }
  /** 
