 ...
 * descriptions of the main components.  See also the{@link java.util.concurrent.locks} and{@link java.util.concurrent.atomic} packages.
 * <h2>Executors</h2>
 * <b>Interfaces.</b>{@link java.util.concurrent.Executor} is a simple standardized
 * interface for defining custom thread-like subsystems, including
 * thread pools, asynchronous IO, and lightweight task frameworks.
 * Depending on which concrete Executor class is being used, tasks may
 * execute in a newly created thread, an existing task-execution thread,
 * or the thread calling {@link java.util.concurrent.Executor#executeexecute}, and may execute sequentially or concurrently.{@link java.util.concurrent.ExecutorService} provides a more
 * complete asynchronous task execution framework.  An
 * ExecutorService manages queuing and scheduling of tasks,
 * and allows controlled shutdown.
 * The {@link java.util.concurrent.ScheduledExecutorService}subinterface and associated interfaces add support for
 ...
 * A {@link java.util.concurrent.RunnableFuture} is a {@code Future}that possesses a {@code run} method that upon execution,
 * sets its results.
 * <p>
 * <b>Implementations.</b>
 * Classes {@link java.util.concurrent.ThreadPoolExecutor} and{@link java.util.concurrent.ScheduledThreadPoolExecutor}provide tunable, flexible thread pools.
 ...
 * concrete class {@link java.util.concurrent.FutureTask}providing a common extensible implementation of Futures, and{@link java.util.concurrent.ExecutorCompletionService}, that
 * assists in coordinating the processing of groups of
 * asynchronous tasks.
 * <p>Class {@link java.util.concurrent.ForkJoinPool} provides an
 * Executor primarily designed for processing instances of {@link java.util.concurrent.ForkJoinTask} and its subclasses.  These
 ...
 * Class {@link java.util.concurrent.LinkedBlockingDeque}provides an implementation.
 * <h2>Timing</h2>
 ...
 * designed for use in multithreaded contexts:{@link java.util.concurrent.ConcurrentHashMap},{@link java.util.concurrent.ConcurrentSkipListMap},{@link java.util.concurrent.ConcurrentSkipListSet},{@link java.util.concurrent.CopyOnWriteArrayList}, and{@link java.util.concurrent.CopyOnWriteArraySet}.
 * When many threads are expected to access a given collection, a{@code ConcurrentHashMap} is normally preferable to a synchronized{@code HashMap}, and a {@code ConcurrentSkipListMap} is normally
 * preferable to a synchronized {@code TreeMap}.
 * A {@code CopyOnWriteArrayList} is preferable to a synchronized{@code ArrayList} when the expected number of reads and traversals
 * greatly outnumber the number of updates to a list.
 * <p>The "Concurrent" prefix used with some classes in this package
 * is a shorthand indicating several differences from similar
 * "synchronized" classes.  For example {@code java.util.Hashtable} and{@code Collections.synchronizedMap(new HashMap())} are
 * synchronized.  But {@link java.util.concurrent.ConcurrentHashMap} is "concurrent".  A
 ...
 * operation <i>happens-before</i> the read operation.  The{@code synchronized} and {@code volatile} constructs, as well as the{@code Thread.start()} and {@code Thread.join()} methods, can form
 ...
 * monitor <i>happens-before</i> every subsequent lock ({@code synchronized}block or method entry) of that same monitor.  And because
 ...
 * subsequent read of that same field.  Writes and reads of{@code volatile} fields have similar memory consistency effects
 ...
 * <li>Actions in a thread prior to the submission of a {@code Runnable}to an {@code Executor} <i>happen-before</i> its execution begins.
 * Similarly for {@code Callables} submitted to an {@code ExecutorService}.
 * <li>Actions taken by the asynchronous computation represented by a{@code Future} <i>happen-before</i> actions subsequent to the
 * retrieval of the result via {@code Future.get()} in another thread.
 * <li>Actions prior to "releasing" synchronizer methods such as{@code Lock.unlock}, {@code Semaphore.release}, and{@code CountDownLatch.countDown} <i>happen-before</i> actions
 * subsequent to a successful "acquiring" method such as{@code Lock.lock}, {@code Semaphore.acquire},{@code Condition.await}, and {@code CountDownLatch.await} on the
 * same synchronizer object in another thread.
 * <li>For each pair of threads that successfully exchange objects via
 * an {@code Exchanger}, actions prior to the {@code exchange()}in each thread <i>happen-before</i> those subsequent to the
 * corresponding {@code exchange()} in another thread.
 * <li>Actions prior to calling {@code CyclicBarrier.await} and{@code Phaser.awaitAdvance} (as well as its variants)
 * <i>happen-before</i> actions performed by the barrier action, and
 * actions performed by the barrier action <i>happen-before</i> actions
 * subsequent to a successful return from the corresponding {@code await}in other threads.
 * </ul>
 * @since 1.5
 */
