 ...
 * quite different properties, and may support multiple associated{@link Condition} objects.
 ...
 * C, then release B and acquire D and so on.  Implementations of the{@code Lock} interface enable the use of such techniques by
 * allowing a lock to be acquired and released in different scopes,
 * and allowing multiple locks to be acquired and released in any
 * order.
 * <p>With this increased flexibility comes additional
 * responsibility. The absence of block-structured locking removes the
 * automatic release of locks that occurs with {@code synchronized}methods and statements. In most cases, the following idiom
 * should be used:
 * <pre><tt>     Lock l = ...;
 * l.lock();
 * try {
 ...
 * It is recommended that to avoid confusion you never use {@code Lock}instances in this way, except within their own implementation.
 ...
 * acquisition of a lock may not be available in a given {@code Lock}class.  Consequently, an implementation is not required to define
 ...
 * circumstances and the exception type must be documented by that{@code Lock} implementation.
 */
  void lock();
  /** 
