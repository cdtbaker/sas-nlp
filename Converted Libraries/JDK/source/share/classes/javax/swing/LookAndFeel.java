 ...
 * font on the {@code JButton} if the return value from {@codebutton.getFont()} is {@code null} or implements {@codeUIResource}. On the other hand if {@code button.getFont()} returned
 ...
 * @param keys alternating pairs of {@code keystroke-action key}pairs as described in {@link #loadKeyBindings}
 ...
 * @throws NullPointerException if {@code keys} is{@code non-null}, not empty, and {@code retMap} is{@code null}
  ...
  public static void loadKeyBindings(  InputMap retMap,  Object[] keys){
    if (keys != null) {
      for (int counter=0, maxCounter=keys.length; counter < maxCounter; counter++) {
        ...
        KeyStroke ks=(keyStrokeO instanceof KeyStroke) ? (KeyStroke)keyStrokeO : KeyStroke.getKeyStroke((String)keyStrokeO);
 ...
 * the returned object, the image is loaded. If the image is {@codenon-null}, it is then wrapped in an {@code Icon} that implements {@codeUIResource}. The image is loaded using {@codeClass.getResourceAsStream(gifFile)}.
 * <p>
 ...
 * @return a {@code UIDefaults.LazyValue}; when resolved the{@code LazyValue} loads the specified image
 ...
 * @return disabled and selected icon, or {@code null} if a suitable{@code Icon} can not be generated.
 * @since 1.5
 */
