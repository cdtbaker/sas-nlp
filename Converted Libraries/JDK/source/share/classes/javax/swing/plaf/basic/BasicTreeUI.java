 ...
 */
  protected TreeCellEditor createDefaultCellEditor(){
    if (currentCellRenderer != null && (currentCellRenderer instanceof DefaultTreeCellRenderer)) {
      DefaultTreeCellEditor editor=new DefaultTreeCellEditor(tree,(DefaultTreeCellRenderer)currentCellRenderer);
      return editor;
    }
    ...
    if (initialPath != null && paintingEnumerator != null) {
      ...
      while (!done && paintingEnumerator.hasMoreElements()) {
        ...
        if (path != null) {
          ...
          if (isLeaf)           isExpanded=hasBeenExpanded=false;
 else {
 ...
 */
  protected void updateDepthOffset(){
    if (isRootVisible()) {
      if (getShowsRootHandles())       depthOffset=1;
 else       depthOffset=0;
    }
 else     if (!getShowsRootHandles())     depthOffset=-1;
 else     depthOffset=0;
  }
  /** 
  ...
  protected void updateCachedPreferredSize(){
    if (treeState != null) {
      ...
      if (isLargeModel()) {
        ...
        if (visRect.x == 0 && visRect.y == 0 && visRect.width == 0 && visRect.height == 0 && tree.getVisibleRowCount() > 0) {
          visRect.width=1;
      ...
      if (cellEditor.isCellEditable(event)) {
        ...
        if (event != null) {
          ...
          if (activeComponent != null) {
            MouseInputHandler handler=new MouseInputHandler(tree,activeComponent,event,focusedComponent);
 ...
 */
  protected void toggleExpandState(  TreePath path){
    if (!tree.isExpanded(path)) {
      ...
      updateSize();
      if (row != -1) {
        if (tree.getScrollsOnExpand())         ensureRowsAreVisible(row,row + treeState.getVisibleChildCount(path));
 else         ensureRowsAreVisible(row,row);
      }
    }
 else {
 ...
 */
  protected void selectPathForEvent(  TreePath path,  MouseEvent event){
    if (isMultiSelectEvent(event)) {
      ...
      if (tree.isPathSelected(path))       tree.removeSelectionPath(path);
 else       tree.addSelectionPath(path);
 ...
 */
    ...
    public void componentMoved(    ComponentEvent e){
      if (timer == null) {
        ...
        if (scrollPane == null)         updateSize();
 else {
          scrollBar=scrollPane.getVerticalScrollBar();
          if (scrollBar == null || !scrollBar.getValueIsAdjusting()) {
            if ((scrollBar=scrollPane.getHorizontalScrollBar()) != null && scrollBar.getValueIsAdjusting())             startTimer();
 else             updateSize();
          }
 else           startTimer();
        }
...
private class Handler implements CellEditorListener, FocusListener, KeyListener, MouseListener, MouseMotionListener, PropertyChangeListener, TreeExpansionListener, TreeModelListener, TreeSelectionListener, BeforeDrag {
 ...
 */
    ...
    public void focusGained(    FocusEvent e){
      if (tree != null) {
        ...
        if (pBounds != null)         tree.repaint(getRepaintPathBounds(pBounds));
      ...
      if (changedPaths != null) {
        ...
        if (maxCounter > 4) {
          ...
          for (counter=0; counter < maxCounter; counter++) {
            ...
            if (nodeBounds != null && visRect.intersects(nodeBounds))             tree.repaint(0,nodeBounds.y,nWidth,nodeBounds.height);
          }
        }
      }
      if (paintPaths) {
    ...
    public void treeNodesInserted(    TreeModelEvent e){
      if (treeState != null && e != null) {
        ...
        if (treeState.isExpanded(path)) {
          ...
          if (indices != null && (childCount - indices.length) == 0)           updateSize();
        }
      }
    }
    public void treeNodesRemoved(    TreeModelEvent e){
      if (treeState != null && e != null) {
        ...
        updateLeadSelectionRow();
        TreePath path=e.getTreePath();
        if (treeState.isExpanded(path) || treeModel.getChildCount(path.getLastPathComponent()) == 0)         updateSize();
      }
    }
    public void treeStructureChanged(    TreeModelEvent e){
      if (treeState != null && e != null) {
        ...
        if (pPath == null || treeState.isExpanded(pPath))         updateSize();
      }
    }
  }
