...
protected class NewFolderAction extends AbstractAction {
    ...
    public void actionPerformed(    ActionEvent e){
      ...
      if (!currentDirectory.exists()) {
        JOptionPane.showMessageDialog(fc,newFolderParentDoesntExistText,newFolderParentDoesntExistTitleText,JOptionPane.WARNING_MESSAGE);
        return;
      }
      File newFolder;
      try {
...
protected class ApproveSelectionAction extends AbstractAction {
    ...
    public void actionPerformed(    ActionEvent e){
      ...
      if (chooser.isMultiSelectionEnabled() && filename.length() > 1 && filename.charAt(0) == '"' && filename.charAt(filename.length() - 1) == '"') {
  ...
  private static boolean isGlobPattern(  String filename){
    return ((File.separatorChar == '\\' && (filename.indexOf('*') >= 0 || filename.indexOf('?') >= 0)) || (File.separatorChar == '/' && (filename.indexOf('*') >= 0 || filename.indexOf('?') >= 0 || filename.indexOf('[') >= 0)));
  }
