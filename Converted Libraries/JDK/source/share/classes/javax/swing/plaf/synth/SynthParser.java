 ...
 */
  private Insets parseInsets(  String insets,  String errorMsg) throws SAXException {
    ...
    return new Insets(nextInt(tokenizer,errorMsg),nextInt(tokenizer,errorMsg),nextInt(tokenizer,errorMsg),nextInt(tokenizer,errorMsg));
  }
  private void startStyle(  Attributes attributes) throws SAXException {
    ...
    for (int i=attributes.getLength() - 1; i >= 0; i--) {
      ...
      if (key.equals(ATTRIBUTE_CLONE)) {
        _style=(ParsedSynthStyle)((ParsedSynthStyle)lookup(attributes.getValue(i),ParsedSynthStyle.class)).clone();
      }
  ...
  private void startState(  Attributes attributes) throws SAXException {
    ...
    for (int i=attributes.getLength() - 1; i >= 0; i--) {
      ...
      if (key.equals(ATTRIBUTE_ID)) {
        ...
        _stateInfo=(ParsedSynthStyle.StateInfo)lookup(attributes.getValue(i),ParsedSynthStyle.StateInfo.class);
      }
 else       if (key.equals(ATTRIBUTE_CLONE)) {
        _stateInfo=(ParsedSynthStyle.StateInfo)((ParsedSynthStyle.StateInfo)lookup(attributes.getValue(i),ParsedSynthStyle.StateInfo.class)).clone();
      }
...
painter=new ImagePainter(!stretch,paintCenter,sourceInsets,destInsets,getResource(path),center);
}
register(id,painter);
if (_stateInfo != null) {
