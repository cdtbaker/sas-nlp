...
class LoweredBorder extends AbstractRegionPainter implements Border {
  ...
  private static final PaintContext PAINT_CONTEXT=new PaintContext(INSETS,new Dimension(IMG_SIZE,IMG_SIZE),false,PaintContext.CacheMode.NINE_SQUARE_SCALE,Integer.MAX_VALUE,Integer.MAX_VALUE);
  @Override protected Object[] getExtendedCacheKeys(  JComponent c){
 ...
 */
  ...
  private Color getLighter(  Color c,  float factor){
    return new Color(Math.min((int)(c.getRed() / factor),255),Math.min((int)(c.getGreen() / factor),255),Math.min((int)(c.getBlue() / factor),255));
  }
}
