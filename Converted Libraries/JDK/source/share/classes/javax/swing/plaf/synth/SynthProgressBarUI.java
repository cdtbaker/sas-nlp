 ...
 * @inheritDoc
 */
  @Override protected void setAnimationIndex(  int newValue){
    if (paintOutsideClip) {
 ...
 */
  @Override public void update(  Graphics g,  JComponent c){
    ...
    context.getPainter().paintProgressBarBackground(context,g,0,0,c.getWidth(),c.getHeight(),progressBar.getOrientation());
    paint(context,g);
    context.dispose();
  }
  /** 
    ...
    if (!pBar.isIndeterminate()) {
      ...
      if (percentComplete != 0.0) {
        if (pBar.getOrientation() == JProgressBar.HORIZONTAL) {
          ...
          width=(int)(percentComplete * (pBar.getWidth() - (pBarInsets.left + progressPadding + pBarInsets.right+ progressPadding)));
          height=pBar.getHeight() - (pBarInsets.top + progressPadding + pBarInsets.bottom+ progressPadding);
          ...
          x=pBarInsets.left + progressPadding;
          width=pBar.getWidth() - (pBarInsets.left + progressPadding + pBarInsets.right+ progressPadding);
          height=(int)(percentComplete * (pBar.getHeight() - (pBarInsets.top + progressPadding + pBarInsets.bottom+ progressPadding)));
    ...
    if (tileWhenIndeterminate && pBar.isIndeterminate()) {
      ...
      if (pBar.getOrientation() == JProgressBar.HORIZONTAL) {
        for (int i=x - tileWidth + offset; i <= width; i+=tileWidth) {
          context.getPainter().paintProgressBarForeground(context,g,i,y,tileWidth,height,pBar.getOrientation());
        }
      }
 else {
        for (int i=y - offset; i < height + tileWidth; i+=tileWidth) {
          context.getPainter().paintProgressBarForeground(context,g,x,i,width,tileWidth,pBar.getOrientation());
        }
      }
      g.setClip(clip);
    }
