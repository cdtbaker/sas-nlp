 ...
 */
  ...
  private static void checkResizeType(  int type,  boolean isComponentSpring){
    if (type < 0 && ((isComponentSpring && type != DEFAULT_SIZE && type != PREFERRED_SIZE) || (!isComponentSpring && type != PREFERRED_SIZE))) {
 ...
 * alignment.  This is a cover method for the more general {@codecreateParallelGroup(Alignment,boolean)} method with {@code true}supplied for the second argument.
 ...
 * @throws IllegalArgumentException if {@code components} is{@code null}, or contains {@code null}
 ...
 * @param components the {@code Component}s that are to have the same size
 * @param axis the axis to link the size along; one of{@code SwingConstants.HORIZONTAL} or{@code SwingConstans.VERTICAL}
 * @throws IllegalArgumentException if {@code components} is{@code null}, or contains {@code null}; or {@code axis}is not {@code SwingConstants.HORIZONTAL} or{@code SwingConstants.VERTICAL}
 */
 ...
 * @throws IllegalArgumentException if either of the components are{@code null} or {@code existingComponent} is not being managed
...
synchronized (parent.getTreeLock()) {
  ...
  private void prepare(  int sizeType){
    ...
    if (sizeType != SPECIFIC_SIZE && (getAutoCreateGaps() || getAutoCreateContainerGaps() || hasPreferredPaddingSprings)) {
...
horizontalGroup.insertAutopadding(HORIZONTAL,new ArrayList<AutoPreferredGapSpring>(1),new ArrayList<AutoPreferredGapSpring>(1),new ArrayList<ComponentSpring>(1),new ArrayList<ComponentSpring>(1),insert);
verticalGroup.insertAutopadding(VERTICAL,new ArrayList<AutoPreferredGapSpring>(1),new ArrayList<AutoPreferredGapSpring>(1),new ArrayList<ComponentSpring>(1),new ArrayList<ComponentSpring>(1),insert);
}
/** 
...
int size=constrain(operator(getSpringSize(getSpring(0),axis,type),getSpringSize(getSpring(1),axis,type)));
...
@Override boolean willHaveZeroSize(boolean treatAutopaddingAsZeroSized){
 ...
 * @throws IllegalArgumentException if {@code type}, {@code comp1} or{@code comp2} is {@code null}
 * @see LayoutStyle
 */
 ...
 * value &gt;= 0
 * @param max the maximum size; one of {@code DEFAULT_SIZE},{@code PREFERRED_SIZE} or a value &gt;= 0
 * @return this {@code SequentialGroup}
 */
public SequentialGroup addContainerGap(int pref,int max){
if ((pref < 0 && pref != DEFAULT_SIZE) || (max < 0 && max != DEFAULT_SIZE && max != PREFERRED_SIZE) || (pref >= 0 && max >= 0 && pref > max)) {
...
@Override void insertAutopadding(int axis,List<AutoPreferredGapSpring> leadingPadding,List<AutoPreferredGapSpring> trailingPadding,List<ComponentSpring> leading,List<ComponentSpring> trailing,boolean insert){
...
if ((pref < 0 && pref != DEFAULT_SIZE && pref != PREFERRED_SIZE) || (max < 0 && max != DEFAULT_SIZE && max != PREFERRED_SIZE) || (pref >= 0 && max >= 0 && pref > max)) {
...
padding=p.getPreferredGap((JComponent)source.getComponent(),(JComponent)target.getComponent(),type,position,host);
}
 else {
padding=10;
}
