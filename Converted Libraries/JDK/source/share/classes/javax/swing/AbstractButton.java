 ...
 */
  @Transient public Icon getDisabledIcon(){
 ...
 * @param exception the message to use in the{@code IllegalArgumentException} that is thrown for an invalid
 * value
 ...
 */
 ...
 else     if (propertyName == Action.SELECTED_KEY && AbstractAction.hasSelectedKey(action) && shouldUpdateSelectedStateFromAction()) {
  ...
  protected void fireItemStateChanged(  ItemEvent event){
    ...
    for (int i=listeners.length - 2; i >= 0; i-=2) {
      if (listeners[i] == ItemListener.class) {
        if (e == null) {
          e=new ItemEvent(AbstractButton.this,ItemEvent.ITEM_STATE_CHANGED,AbstractButton.this,event.getStateChange());
        }
    ...
    if (accessibleContext != null) {
      if (event.getStateChange() == ItemEvent.SELECTED) {
        ...
        accessibleContext.firePropertyChange(AccessibleContext.ACCESSIBLE_VALUE_PROPERTY,Integer.valueOf(0),Integer.valueOf(1));
      }
 else {
        accessibleContext.firePropertyChange(AccessibleContext.ACCESSIBLE_STATE_PROPERTY,AccessibleState.SELECTED,null);
        accessibleContext.firePropertyChange(AccessibleContext.ACCESSIBLE_VALUE_PROPERTY,Integer.valueOf(1),Integer.valueOf(0));
      }
    }
  }
