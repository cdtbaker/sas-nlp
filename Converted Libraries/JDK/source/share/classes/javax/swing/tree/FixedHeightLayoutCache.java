 ...
 */
  public Enumeration<TreePath> getVisiblePathsFrom(  TreePath path){
    ...
    node=getNodeForPath(parentPath,true,false);
    if (node != null && node.isExpanded()) {
      return new VisibleFHTreeStateNodeEnumeration(node,treeModel.getIndexOfChild(parentPath.getLastPathComponent(),path.getLastPathComponent()));
    }
    return null;
  }
  /** 
 ...
 */
    public FHTreeStateNode getChildAtModelIndex(    int index){
      for (int counter=getChildCount() - 1; counter >= 0; counter--)       if (((FHTreeStateNode)getChildAt(counter)).childIndex == index)       return (FHTreeStateNode)getChildAt(counter);
      return null;
    }
    /** 
 ...
 */
    protected void resetChildrenPaths(    TreePath parentPath){
      ...
      addMapping(this);
      for (int counter=getChildCount() - 1; counter >= 0; counter--)       ((FHTreeStateNode)getChildAt(counter)).resetChildrenPaths(path);
    }
    /** 
 ...
 */
    protected void removeFromMapping(){
      if (path != null) {
        removeMapping(this);
        for (int counter=getChildCount() - 1; counter >= 0; counter--)         ((FHTreeStateNode)getChildAt(counter)).removeFromMapping();
      }
    }
    /** 
 ...
 */
    protected void adjustRowBy(    int amount){
      row+=amount;
      if (isExpanded) {
        for (int counter=getChildCount() - 1; counter >= 0; counter--)         ((FHTreeStateNode)getChildAt(counter)).adjustRowBy(amount);
      }
    }
    /** 
 ...
 */
    protected void adjustRowBy(    int amount,    int startIndex){
      if (isExpanded) {
        for (int counter=getChildCount() - 1; counter >= startIndex; counter--)         ((FHTreeStateNode)getChildAt(counter)).adjustRowBy(amount);
      }
      FHTreeStateNode parent=(FHTreeStateNode)getParent();
      if (parent != null) {
 ...
 */
    protected void collapse(    boolean adjustRows){
      if (isExpanded) {
        ...
        if (adjustRows && isVisible() && treeSelectionModel != null)         treeSelectionModel.resetRowSelection();
      }
    }
    /** 
      ...
      if (childNode != null) {
        ...
        for (int counter=0; counter < maxCounter; counter++) {
          ...
          if (aChild.childIndex >= modelIndex) {
            ...
            for (; counter < maxCounter; counter++)             ((FHTreeStateNode)getChildAt(counter)).childIndex--;
            childCount--;
            return;
          }
        }
...
private class SearchInfo {
    ...
    protected TreePath getPath(){
      ...
      if (isNodeParentNode)       return node.getTreePath().pathByAddingChild(treeModel.getChild(node.getUserObject(),childIndex));
      return node.path;
    }
  }
  /** 
 ...
 */
    public TreePath nextElement(){
      ...
      if (nextIndex == -1)       retObject=parent.getTreePath();
        ...
        if (node == null)         retObject=parent.getTreePath().pathByAddingChild(treeModel.getChild(parent.getUserObject(),nextIndex));
