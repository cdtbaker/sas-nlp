 ...
 * @see #getDocument
 * @beaninfodescription: the text document model
 * bound: true
 * expert: true
 */
 ...
 * @beaninfodescription: desired space between the border and text area
 * bound: true
 */
 ...
 * @return the caret
 */
  @Transient public Caret getCaret(){
    return caret;
  }
  /** 
 ...
 * @param c the caret
 * @see #getCaret
 * @beaninfodescription: the caret used to select/navigate
 * bound: true
 * expert: true
 */
 ...
 * @see #getHighlighter
 * @beaninfodescription: object responsible for background highlights
 * bound: true
 * expert: true
 */
 ...
 * @see #getKeymap
 * @beaninfodescription: set of key event to action bindings to use
 * bound: true
 */
...
if (dropMode == DropMode.USE_SELECTION) {
  if (textLocation == null) {
    if (state != null) {
      ...
      if (!forDrop) {
        if (caret instanceof DefaultCaret) {
          ((DefaultCaret)caret).setDot(((Integer)vals[0]).intValue(),(Position.Bias)vals[3]);
          ((DefaultCaret)caret).moveDot(((Integer)vals[1]).intValue(),(Position.Bias)vals[4]);
        }
 else {
    ...
    if (dropLocation == null) {
      ...
      if (caret instanceof DefaultCaret) {
        ...
        visible=dc.isActive();
        retVal=new Object[]{Integer.valueOf(dc.getMark()),Integer.valueOf(dc.getDot()),Boolean.valueOf(visible),dc.getMarkBias(),dc.getDotBias()};
      }
 else {
        visible=caret.isVisible();
        retVal=new Object[]{Integer.valueOf(caret.getMark()),Integer.valueOf(caret.getDot()),Boolean.valueOf(visible)};
      }
  ...
  if (textLocation == null) {
    ...
    if (dropLocation == null) {
      boolean visible=caret instanceof DefaultCaret ? ((DefaultCaret)caret).isActive() : caret.isVisible();
 ...
 * @param c the color
 * @see #getCaretColor
 * @beaninfodescription: the color used to render the caret
 * bound: true
 * preferred: true
 */
 ...
 * @beaninfodescription: color used to render selection background
 * bound: true
 * preferred: true
 */
 ...
 * @beaninfodescription: color used to render selected text
 * bound: true
 * preferred: true
 */
 ...
 * @beaninfodescription: color used to render disabled text
 * bound: true
 * preferred: true
 */
 ...
 * @beaninfodescription: accelerator character used to grab focus
 * bound: true
 */
 ...
 * for <code>position</code> is less than zero or greater
 * than the component's text length
 * @beaninfodescription: the caret position
 */
 ...
 * @see #getText
 * @see DefaultCaret
 * @beaninfodescription: the text of this component
 */
 ...
 * @see #isEditable
 * @beaninfodescription: specifies if the text can be edited
 * bound: true
 */
 ...
 * @beaninfodescription: starting location of the selection.
 */
 ...
 * @beaninfodescription: ending location of the selection.
 */
...
try {
if (index + direction < model.getLength() && index + direction >= 0) {
charSequence=new AccessibleTextSequence(index + direction,index + direction + 1,model.getText(index + direction,1));
}
}
 catch (BadLocationException e) {
}
...
if (direction == 0) {
lineSequence=new AccessibleTextSequence(startIndex,endIndex,model.getText(startIndex,endIndex - startIndex + 1));
}
...
runText=model.getText(attributeRunStartIndex,attributeRunEndIndex - attributeRunStartIndex);
}
 catch (BadLocationException e) {
return null;
}
...
if (startIndex < 0 || startIndex > model.getLength() - 1 || endIndex < 0 || endIndex > model.getLength() - 1 || startIndex > endIndex) {
return null;
}
TextUI ui=getUI();
if (ui == null) {
...
ActionEvent ae=new ActionEvent(JTextComponent.this,ActionEvent.ACTION_PERFORMED,null,EventQueue.getMostRecentEventTime(),getCurrentEventModifiers());
...
public Boolean run(){
return isProcessInputMethodEventOverridden(JTextComponent.this.getClass());
}
}
);
