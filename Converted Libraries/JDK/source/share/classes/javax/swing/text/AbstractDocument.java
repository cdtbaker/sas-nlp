 ...
 */
  protected void insertUpdate(  DefaultDocumentEvent chng,  AttributeSet attr){
    ...
    if (chng.type == DocumentEvent.EventType.INSERT && chng.getLength() > 0 && !Boolean.TRUE.equals(getProperty(MultiByteProperty))) {
  ...
  void updateBidi(  DefaultDocumentEvent chng){
    ...
    if (firstSpanStart > 0) {
      ...
      if (prevLevel == levels[0]) {
        ...
        newElements.addElement(new BidiElement(bidiRoot,prevElem.getStartOffset(),firstPStart,prevLevel));
      }
 else {
        removeFromIndex++;
      }
    ...
    if ((firstSpanEnd == lastSpanStart) && (levels[0] == levels[levels.length - 1])) {
      ...
      newElements.addElement(new BidiElement(bidiRoot,firstSpanStart,firstSpanEnd + firstPStart,levels[0]));
      for (int i=firstSpanEnd; i < lastSpanStart; ) {
        int j;
        for (j=i; (j < levels.length) && (levels[j] == levels[i]); j++)         ;
        newElements.addElement(new BidiElement(bidiRoot,firstPStart + i,firstPStart + j,(int)levels[i]));
        i=j;
      }
      newElements.addElement(new BidiElement(bidiRoot,lastSpanStart + firstPStart,lastSpanEnd,levels[levels.length - 1]));
    }
