...
static {
  ...
  static public void writeDocument(  Document d,  OutputStream to) throws IOException {
    ...
    for (int idx=0; idx < max; idx++)     gen.writeParagraphElement(root.getElement(idx));
    gen.writeRTFTrailer();
  }
  ...
  void writeDocumentProperties(  Document doc) throws IOException {
    ...
    if (wroteSomething)     writeLineBreak();
  }
...
if (parm == MagicToken) colorNum=0;
 else colorNum=colorTable.get(parm).intValue();
...
int sub_count=el.getElementCount();
for (int idx=0; idx < sub_count; idx++) writeTextElement(el.getElement(idx));
}
}
...
array=s.array;
for (; pos < end; pos++) writeCharacter(array[pos]);
}
public void writeText(String s) throws IOException {
int pos, end;
pos=0;
end=s.length();
for (; pos < end; pos++) writeCharacter(s.charAt(pos));
}
public void writeRawString(String str) throws IOException {
int strlen=str.length();
for (int offset=0; offset < strlen; offset++) outputStream.write((int)str.charAt(offset));
}
