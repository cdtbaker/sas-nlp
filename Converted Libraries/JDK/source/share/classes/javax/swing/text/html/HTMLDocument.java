 ...
 */
  private void updateFrame(  Element element,  String url){
    try {
      writeLock();
      DefaultDocumentEvent changes=new DefaultDocumentEvent(element.getStartOffset(),1,DocumentEvent.EventType.CHANGE);
 ...
 */
  private void removeElementsAtEnd(  Element e,  int index,  int count,  int start,  int end) throws BadLocationException {
    ...
    DefaultDocumentEvent dde=new DefaultDocumentEvent(start - 1,end - start + 1,DocumentEvent.EventType.REMOVE);
    if (isLeaf) {
...
class HeadAction extends BlockAction {
      public void start(      HTML.Tag t,      MutableAttributeSet a){
        inHead=true;
        if ((insertTag == null && !insertAfterImplied) || (insertTag == HTML.Tag.HEAD) || (insertAfterImplied && (foundInsertTag || !a.isDefined(IMPLIED)))) {
          super.start(t,a);
        }
      }
      public void end(      HTML.Tag t){
      ...
      private void handleLink(      AttributeSet attr){
        ...
        if (type.equals("text/css")) {
          ...
          if (rel != null) {
            rel=rel.toLowerCase();
            if ((media.indexOf("all") != -1 || media.indexOf("screen") != -1) && (rel.equals("stylesheet") || (rel.equals("alternate stylesheet") && title.equals(defaultStyle)))) {
 ...
 */
    private void adjustEndSpecsForPartialInsert(){
      ...
      if (insertTagDepthDelta < 0) {
        ...
        while (removeCounter < 0 && size >= 0 && parseBuffer.elementAt(size - 1).getType() == ElementSpec.EndTagType) {
        ...
        for (int counter=parseBuffer.size() - 1; counter >= 0; counter--) {
          ...
          if (spec.getType() == ElementSpec.ContentType) {
            if (spec.getArray()[spec.getLength() - 1] != '\n') {
              ...
              parseBuffer.insertElementAt(new ElementSpec(attrs,ElementSpec.ContentType,NEWLINE,0,1),counter + 1);
            }
            break;
          }
        }
 ...
 */
 ...
 else         if ((insertTag != null && !isInsertTag(t)) || (insertAfterImplied && (attr == null || attr.isDefined(IMPLIED) || t == HTML.Tag.IMPLIED))) {
          return false;
        }
      ...
      if (!insertAfterImplied && (popDepth > 0 || pushDepth > 0)) {
        try {
          if (offset == 0 || !getText(offset - 1,1).equals("\n")) {
            ...
            ElementSpec es=new ElementSpec(newAttrs,ElementSpec.ContentType,NEWLINE,0,NEWLINE.length);
            if (joinP) {
