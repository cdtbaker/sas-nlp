...
static class BooleanAttribute extends GenericAttribute implements RTFAttribute {
    ...
    public boolean setDefault(    MutableAttributeSet target){
      if (swingDefault != rtfDefault || (target.getAttribute(swingName) != null))       target.addAttribute(swingName,Boolean.valueOf(rtfDefault));
      return true;
    }
...
static class AssertiveAttribute extends GenericAttribute implements RTFAttribute {
    ...
    public boolean set(    MutableAttributeSet target){
      if (swingValue == null)       target.removeAttribute(swingName);
 else       target.addAttribute(swingName,swingValue);
      return true;
    }
    public boolean set(    MutableAttributeSet target,    int parameter){
      return false;
...
static class NumericAttribute extends GenericAttribute implements RTFAttribute {
    ...
    public boolean set(    MutableAttributeSet target,    int parameter){
      Number swingValue;
      if (scale == 1f)       swingValue=Integer.valueOf(parameter);
 else       swingValue=new Float(parameter / scale);
    ...
    public boolean setDefault(    MutableAttributeSet target){
      ...
      if (old != null && ((scale == 1f && old.intValue() == rtfDefault) || (Math.round(old.floatValue() * scale) == rtfDefault)))       return true;
