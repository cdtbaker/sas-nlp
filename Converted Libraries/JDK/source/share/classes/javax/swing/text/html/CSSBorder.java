 ...
 */
  final static Attribute[][] ATTRIBUTES={{Attribute.BORDER_TOP_COLOR,Attribute.BORDER_RIGHT_COLOR,Attribute.BORDER_BOTTOM_COLOR,Attribute.BORDER_LEFT_COLOR},{Attribute.BORDER_TOP_STYLE,Attribute.BORDER_RIGHT_STYLE,Attribute.BORDER_BOTTOM_STYLE,Attribute.BORDER_LEFT_STYLE},{Attribute.BORDER_TOP_WIDTH,Attribute.BORDER_RIGHT_WIDTH,Attribute.BORDER_BOTTOM_WIDTH,Attribute.BORDER_LEFT_WIDTH}};
  /** 
 ...
 */
  final static Object[] DEFAULTS={Attribute.BORDER_COLOR,PARSERS[1].parseCssValue(Attribute.BORDER_STYLE.getDefaultValue()),PARSERS[2].parseCssValue(Attribute.BORDER_WIDTH.getDefaultValue())};
  /** 
...
static class GrooveRidgePainter extends ShadowLightPainter {
    ...
    public void paint(    Polygon shape,    Graphics g,    Color color,    int side){
      ...
      int[] lengthPattern={length,length};
      Color[] colorPattern=((side + 1) % 4 < 2) == (type == Value.GROOVE) ? new Color[]{getShadowColor(color),getLightColor(color)} : new Color[]{getLightColor(color),getShadowColor(color)};
      paintStrokes(r,g,View.Y_AXIS,lengthPattern,colorPattern);
    }
  }
  /** 
...
static class InsetOutsetPainter extends ShadowLightPainter {
    ...
    public void paint(    Polygon shape,    Graphics g,    Color color,    int side){
      g.setColor(((side + 1) % 4 < 2) == (type == Value.INSET) ? getShadowColor(color) : getLightColor(color));
      g.fillPolygon(shape);
    }
  }
  /** 
