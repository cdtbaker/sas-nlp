 ...
 * the usual rules mean that it will have an attribute called{@code CacheSize} of type {@code long}.  The {@code @Units}annotation, given the above definition, will ensure that the{@link MBeanAttributeInfo} for this attribute will have a{@code Descriptor} that has a field called {@code units} with
 ...
 * the <a href="Descriptor.html#immutableInfo">{@codeimmutableInfo}</a> field for an MBean.  The fields from the annotations
 ...
 * <td>Wrapped value ({@code Integer.valueOf(5)},{@code Boolean.FALSE}, etc)</td></tr>
 * <tr><td>Class constant (e.g. {@code Thread.class})</td>
 * <td>Class name from {@link Class#getName()}(e.g. {@code "java.lang.Thread"})</td></tr>
 * <tr><td>Enum constant (e.g. {@link ElementType#FIELD})</td>
 * <td>Constant name from {@link Enum#name()}(e.g. {@code "FIELD"})</td></tr>
 ...
 * @since 1.6
 */
@Documented @Retention(RetentionPolicy.RUNTIME) @Target(ElementType.METHOD) public @interface DescriptorKey {String value();
}
