 ...
 * interface is an MXBean interface if its name ends with {@codeMXBean}, as in {@code SomethingMXBean}.  The following interfaces
 ...
 * memory pool.  If these were separate items, obtained with separate{@link MBeanServer#getAttribute getAttribute} calls, then we could
 ...
 * standard classes from the Java platform.  Using arrays and the{@link javax.management.openmbean.CompositeData CompositeData} and{@link javax.management.openmbean.TabularData TabularData} classes
 ...
 * <td><pre>
 * String name = (String)
 * mbeanServer.{@link MBeanServer#getAttributegetAttribute}(objectName, "Name");
 ...
 * JMX.<b>{@link JMX#newMBeanProxy(MBeanServerConnection,ObjectName,Class) newMBeanProxy}</b>(
 * mbeanServer,
 * objectName,
 ...
 * JMX.<b>{@link JMX#newMXBeanProxy(MBeanServerConnection,ObjectName,Class) newMXBeanProxy}</b>(
 * mbeanServer,
 * objectName,
 ...
 * mbeanServer.{@link MBeanServer#registerMBeanregisterMBean}(pool, objectName);
 * }
 * </pre></td>
 * <td><pre>
 * {
 ...
 * <p>When an object is registered in the MBean Server using the{@code registerMBean} or {@code createMBean} methods of the{@link MBeanServer} interface, the object's class is examined
 * to determine what type of MBean it is:</p>
 * <ul>
 * <li>If the class implements the interface {@link DynamicMBean}then the MBean is a Dynamic MBean.  Note that the class{@code StandardMBean} implements this interface, so this
 ...
 * the attempt to register it will generate {@link NotCompliantMBeanException}.
 * </ul>
 ...
 * The method {@code final Class getClass()} inherited from {@codeObject} is ignored when looking for getters.</li>
 ...
 * <p>An MXBean is a kind of Open MBean, as defined by the {@link javax.management.openmbean} package.  This means that the types of
 ...
 * an instance of a subclass of {@link javax.management.openmbean.OpenType}.</li>
 ...
 * <ul>
 * <li>The Open Type, <em>opentype(</em>{@codeList<String>}<em>)</em>, is {@link ArrayType}<code>(1, </code>{@link SimpleType#STRING}<code>)</code>, representing a 1-dimensional
 * array of <code>String</code>s.</li>
 * <li>The mapped Java type, <em>opendata(</em>{@codeList<String>}<em>)</em>, is {@code String[]}.</li>
 * <li>A {@code List<String>} can be converted to a {@code String[]}using {@link List#toArray(Object[]) List.toArray(new
 * String[0])}.</li>
 * <li>A {@code String[]} can be converted to a {@code List<String>}using {@link Arrays#asList Arrays.asList}.</li>
 * </ul>
 ...
 * generated by {@link JMX#newMXBeanProxy(MBeanServerConnection,ObjectName,Class) JMX.newMXBeanProxy}, it is the return values
 ...
 * <td>{@code SimpleType.INTEGER},<br>{@code SimpleType.BOOLEAN}, etc</td>
 ...
 * </tr>
 * <tr>
 * <td>{@code List<}<em>E</em>{@code >}<br>{@code Set<}<em>E</em>{@code >}<br>{@code SortedSet<}<em>E</em>{@code >} (see below)</td>
 * <td>same as for <em>E</em>{@code []}</td>
 * <td>same as for <em>E</em>{@code []}</td>
 * </tr>
 * <tr>
 * <td>An enumeration <em>E</em><br>
 * (declared in Java as {@code enum }<em>E</em>{@code} 
 * ...}})</td>
 ...
 * ({@code boolean}, {@code byte}, {@code short}, {@code int}, {@codelong}, {@code float}, {@code double}, {@code char}) are mapped to the
 * corresponding boxed types from {@code java.lang}, namely {@codeBoolean}, {@code Byte}, etc.  The Open Type is the corresponding{@code SimpleType}.  Thus, <em>opentype(</em>{@codelong}<em>)</em> is {@code SimpleType.LONG}, and
 * <em>opendata(</em>{@code long}<em>)</em> is {@codejava.lang.Long}.</p>
 * <p>An array of primitive type such as {@code long[]} can be represented
 * directly as an Open Type.  Thus, <em>openType(</em>{@codelong[]}<em>)</em> is {@codeArrayType.getPrimitiveArrayType(long[].class)}, and
 * <em>opendata(</em>{@code long[]}<em>)</em> is {@codelong[]}.</p>
 * <p>In practice, the difference between a plain {@code int} and {@codeInteger}, etc, does not show up because operations in the JMX API
 ...
 * <p>A {@code List<}<em>E</em>{@code >} or {@codeSet<}<em>E</em>{@code >}, such as {@code List<String>} or {@codeSet<ObjectName>}, is mapped in the same way as an array of the
 * same element type, such as {@code String[]} or {@codeObjectName[]}.</p>
 * <p>A {@code SortedSet<}<em>E</em>{@code >} is also mapped in the
 * same way as an <em>E</em>{@code []}, but it is only convertible if
 * <em>E</em> is a class or interface that implements {@link java.lang.Comparable}.  Thus, a {@code SortedSet<String>} or{@code SortedSet<Integer>} is convertible, but a {@codeSortedSet<int[]>} or {@code SortedSet<List<String>>} is not.  The
 * conversion of a {@code SortedSet} instance will fail with an{@code IllegalArgumentException} if it has a
 * non-null {@link java.util.SortedSet#comparator()comparator()}.</p>
 * <p>A {@code List<}<em>E</em>{@code >} is reconstructed as a{@code java.util.ArrayList<}<em>E</em>{@code >};
 * a {@code Set<}<em>E</em>{@code >} as a{@code java.util.HashSet<}<em>E</em>{@code >};
 * a {@code SortedSet<}<em>E</em>{@code >} as a{@code java.util.TreeSet<}<em>E</em>{@code >}.</p>
 * <h3>Mappings for maps ({@code Map<}<em>K</em>,<em>V</em>{@code >} etc)</h3>
 * <p>A {@code Map<}<em>K</em>,<em>V</em>{@code >} or {@codeSortedMap<}<em>K</em>,<em>V</em>{@code >}, for example {@codeMap<String,ObjectName>}, has Open Type {@link TabularType} and is mapped
 * to a {@link TabularData}.
 * The {@code TabularType} has two items called {@code key} and{@code value}.  The Open Type of {@code key} is
 ...
 * <p>For example, the {@code TabularType} for a {@codeMap<String,ObjectName>} might be constructed with code like
 * this:</p>
 * <pre>
 * String typeName =
 ...
 * <em>K</em> is a class or interface that implements {@link java.lang.Comparable}.  Thus, a {@code SortedMap<String,int[]>}is convertible, but a{@code SortedMap<int[],String>} is not.  The conversion of a{@code SortedMap} instance will fail with an {@codeIllegalArgumentException} if it has a non-null {@link java.util.SortedMap#comparator() comparator()}.</p>
 ...
 * or another class implementing {@codeTabularData} that serializes as {@code TabularDataSupport}.</p>
 ...
 * implementing the {@code ProductMXBean} interface.  The{@code ModuleMXBean} object and the returned {@codeProductMXBean} objects must both be registered as MXBeans in the
 * same MBean Server.</p>
 * <p>The method {@code ModuleMXBean.getProduct()} defines an
 * attribute called {@code Product}.  The Open Type for this
 * attribute is {@code SimpleType.OBJECTNAME}, and the corresponding{@code ObjectName} value will be the name under which the
 * referenced {@code ProductMXBean} is registered in the MBean
 * Server.</p>
 * <p>If you make an MXBean proxy for a {@code ModuleMXBean} and
 * call its {@code getProduct()} method, the proxy will map the{@code ObjectName} back into a {@code ProductMXBean} by making
 * another MXBean proxy.  More formally, when a proxy made with{@link JMX#newMXBeanProxy(MBeanServerConnection,ObjectName,Class)JMX.newMXBeanProxy(mbeanServerConnection, objectNameX,
 * interfaceX)} needs to map {@code objectNameY} back into {@codeinterfaceY}, another MXBean interface, it does so with {@codeJMX.newMXBeanProxy(mbeanServerConnection, objectNameY,
 * interfaceY)}.  The implementation may return a proxy that was
 * previously created by a call to {@code JMX.newMXBeanProxy}with the same parameters, or it may create a new proxy.</p>
 * <p>The reverse mapping is illustrated by the following change to the{@code ModuleMXBean} interface:</p>
 * <pre>
 * public interface ModuleMXBean {
 * public ProductMXBean getProduct();
 ...
 * }
 * </pre>
 * <p>The presence of the {@code setProduct} method now means that the{@code Product} attribute is read/write.  As before, the value
 * of this attribute is an {@code ObjectName}.  When the attribute is
 * set, the {@code ObjectName} must be converted into the{@code ProductMXBean} object that the {@code setProduct} method
 * expects.  This object will be an MXBean proxy for the given{@code ObjectName} in the same MBean Server.</p>
 * <p>If you make an MXBean proxy for a {@code ModuleMXBean} and
 * call its {@code setProduct} method, the proxy will map its{@code ProductMXBean} argument back into an {@code ObjectName}.
 * This will only work if the argument is in fact another proxy,
 * for a {@code ProductMXBean} in the same {@codeMBeanServerConnection}.  The proxy can have been returned from
 * another proxy (like {@code ModuleMXBean.getProduct()} which
 * returns a proxy for a {@code ProductMXBean}); or it can have
 * been created by {@link JMX#newMXBeanProxy(MBeanServerConnection,ObjectName,Class)JMX.newMXBeanProxy}; or it can have been created using {@link java.lang.reflect.Proxy Proxy} with an invocation handler that
 * is {@link MBeanServerInvocationHandler} or a subclass.</p>
 * <p>If the same MXBean were registered under two different{@code ObjectName}s, a reference to that MXBean from another
 ...
 * general, notably because it does not work well for MBeans that are{@link NotificationBroadcaster}s.</p>
 ...
 * it to a {@link CompositeType} as follows.  The type name of this{@code CompositeType} is determined by the <a href="#type-names">
 ...
 * convertible type, then <em>opentype(J)</em> is a {@codeCompositeType} with one item for every getter.  If the getter is
 * <blockquote>
 * <code><em>T</em> get<em>Name</em>()</code>
 * </blockquote>
 * then the item in the {@code CompositeType} is called {@code name}and has type <em>opentype(T)</em>.  For example, if the item is
 * <blockquote>
 * <code>String getOwner()</code>
 * </blockquote>
 * then the item is called {@code owner} and has Open Type {@codeSimpleType.STRING}.  If the getter is
 * <blockquote>
 * <code>boolean is<em>Name</em>()</code>
 * </blockquote>
 * then the item in the {@code CompositeType} is called {@code name}and has type {@code SimpleType.BOOLEAN}.</p>
 ...
 * convention.  In a Standard MBean or MXBean interface, a method{@code getOwner} defines an attribute called {@code Owner}, while
 * in a Java Bean or mapped {@code CompositeType}, a method {@codegetOwner} defines a property or item called {@code owner}.</p>
 * <p>If two methods produce the same item name (for example, {@codegetOwner} and {@code isOwner}, or {@code getOwner} and {@codegetowner}) then the type is not convertible.</p>
 * <p>When the Open Type is {@code CompositeType}, the corresponding
 * mapped Java type (<em>opendata(J)</em>) is {@link CompositeData}.  The mapping from an instance of <em>J</em> to a{@code CompositeData} corresponding to the {@code CompositeType}just described is done as follows.  First, if <em>J</em>
 * implements the interface {@link CompositeDataView}, then that
 * interface's {@link CompositeDataView#toCompositeDatatoCompositeData} method is called to do the conversion.
 ...
 * used to represent a type as Open Data is {@link CompositeDataSupport}, or another class implementing {@codeCompositeData} that serializes as {@codeCompositeDataSupport}.</p>
 ...
 * determine how to reconstruct instances of <em>J</em> from{@code CompositeData}.  The first applicable rule in the list is
 * the one that will be used.</p>
 * <ol>
 * <li><p>If <em>J</em> has a method<br>{@code public static }<em>J </em>{@code from(CompositeData cd)}<br>
 * then that method is called to reconstruct an instance of
 * <em>J</em>.</p></li>
 * <li><p>Otherwise, if <em>J</em> has at least one public
 * constructor with a {@link java.beans.ConstructorPropertiesConstructorProperties} annotation, then one
 ...
 * constructor with the appropriate reconstructed items from the{@code CompositeData}.  The constructor to be called will be
 * determined at runtime based on the items actually present in
 * the {@code CompositeData}, given that this {@codeCompositeData} might come from an earlier version of
 ...
 * called with the reconstructed items from the {@code CompositeData}to restore the values.  For example, if there is a method<br>{@code public List<String> getNames()}<br>
 * then there must also be a method<br>{@code public void setNames(List<String> names)}<br>
 ...
 * using a {@link java.lang.reflect.Proxy} with a {@link CompositeDataInvocationHandler} backed by the {@codeCompositeData} being converted.</p></li>
 * <li><p>Otherwise, <em>J</em> is not reconstructible.</p></li>
 * </ol>
 * <p>Here are examples showing different ways to code a type {@codeNamedNumber} that consists of an {@code int} and a {@codeString}.  In each case, the {@code CompositeType} looks like this:</p>
 * <blockquote>
 * <pre>{@link CompositeType}(
 ...
 * interfaces.  This is a consequence of the immutability of {@link CompositeType}.  For example, the following type could not be the
 ...
 * parameter, or operation will be represented respectively by an{@link MBeanAttributeInfo}, {@link MBeanParameterInfo}, or{@link MBeanOperationInfo} whose {@code getType()} or {@codegetReturnType()} returns the primitive name ("{@code int}" etc).
 * This is so even though the mapping rules above specify that the
 * <em>opendata</em> mapping is the wrapped type ({@code Integer}etc).</p>
 * <p>The array of public constructors returned by {@link MBeanInfo#getConstructors()} for an MXBean that is directly
 ...
 * The list returned for an MXBean that is constructed using the{@link StandardMBean} class is derived in the same way as for
 ...
 * <p>The array of notification types returned by {@link MBeanInfo#getNotifications()} for an MXBean that is directly
 * registered in the MBean Server will be empty if the MXBean does
 * not implement the {@link NotificationBroadcaster} interface.
 * Otherwise, it will be the result of calling {@link NotificationBroadcaster#getNotificationInfo()} at the time the MXBean
 * was registered.  Even if the result of this method changes
 * subsequently, the result of {@code MBeanInfo.getNotifications()}will not.  The list returned for an MXBean that is constructed
 * using the {@link StandardMBean} or {@link StandardEmitterMBean}class is derived in the same way as for Standard MBeans.</p>
 * <p>The {@link Descriptor} for all of the{@code MBeanAttributeInfo}, {@code MBeanParameterInfo}, and{@code MBeanOperationInfo} objects contained in the {@code MBeanInfo}will have a field {@code openType} whose value is the {@link OpenType}specified by the mapping rules above.  So even when {@code getType()}is "{@code int}", {@code getDescriptor().getField("openType")} will
 ...
 * below.</p>
 * <p>The {@code Descriptor} for the {@code MBeanInfo} will have a field{@code mxbean} whose value is the string "{@code true}".</p>
 ...
 * <em>genericstring(T)</em> is the value returned by {@link Class#getName()}, for example {@code "int"} or {@code"java.lang.String"}.
 ...
 * is {@code "int[]"}, and <em>genericstring({@codeList<String>[][]})</em> is {@code"java.util.List<java.lang.String>[][]"}.
 * <li>Otherwise, <em>T</em> is a parameterized type such as {@codeList<String>} and <em>genericstring(T)</em> consists of the
 * following: the fully-qualified name of the parameterized type as
 * returned by {@code Class.getName()}; a left angle bracket ({@code"<"}); <em>genericstring(A)</em> where <em>A</em> is the first
 ...
 * represented by the string {@code "[I"} returned by {@codeClass.getName()}, but if a method returns {@code List<int[]>},
 * this will be represented by the string {@code"java.util.List<int[]>"}.
 * <h3>Exceptions</h3>
 ...
 * example if it references a type like {@link java.util.Randomjava.util.Random} that has no getters.  Or it can happen when an
 * instance is being converted (a return value from a method in an
 * MXBean or a parameter to a method in an MXBean proxy), for
 * example when converting from {@code SortedSet<String>} to {@codeString[]} if the {@code SortedSet} has a non-null {@codeComparator}.</p>
 ...
 * <p>Depending on the context, the {@code OpenDataException} or{@code InvalidObjectException} may be wrapped in another
 * exception such as {@link RuntimeMBeanException} or {@link UndeclaredThrowableException}.  For every thrown exception,
 * the condition <em>C</em> will be true: "<em>e</em> is {@codeOpenDataException} or {@code InvalidObjectException} (as
 * appropriate), or <em>C</em> is true of <em>e</em>.{@link Throwable#getCause() getCause()}".</p>
 * @since 1.6
 */
@Documented @Retention(RetentionPolicy.RUNTIME) @Target(ElementType.TYPE) public @interface MXBean {/** 
