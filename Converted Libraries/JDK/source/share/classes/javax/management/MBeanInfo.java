 ...
 */
  public MBeanInfo(  String className,  String description,  MBeanAttributeInfo[] attributes,  MBeanConstructorInfo[] constructors,  MBeanOperationInfo[] operations,  MBeanNotificationInfo[] notifications,  Descriptor descriptor) throws IllegalArgumentException {
    ...
    if (descriptor == null)     descriptor=ImmutableDescriptor.EMPTY_DESCRIPTOR;
 ...
 */
 ...
 else     return as.clone();
  }
  private MBeanAttributeInfo[] fastGetAttributes(){
    if (arrayGettersSafe)     return nonNullAttributes();
 ...
 */
 ...
 else     return os.clone();
  }
  private MBeanOperationInfo[] fastGetOperations(){
    if (arrayGettersSafe)     return nonNullOperations();
 else     return getOperations();
  }
 ...
 */
 ...
 else     return cs.clone();
  }
  private MBeanConstructorInfo[] fastGetConstructors(){
    if (arrayGettersSafe)     return nonNullConstructors();
 else     return getConstructors();
  }
 ...
 */
 ...
 else     return ns.clone();
  }
  private MBeanNotificationInfo[] fastGetNotifications(){
    if (arrayGettersSafe)     return nonNullNotifications();
 else     return getNotifications();
  }
 ...
 */
  @Override public boolean equals(  Object o){
    ...
    if (!isEqual(getClassName(),p.getClassName()) || !isEqual(getDescription(),p.getDescription()) || !getDescriptor().equals(p.getDescriptor())) {
      return false;
    }
  ...
  @Override public int hashCode(){
    ...
    hashCode=getClassName().hashCode() ^ getDescriptor().hashCode() ^ arrayHashCode(fastGetAttributes())^ arrayHashCode(fastGetOperations())^ arrayHashCode(fastGetConstructors())^ arrayHashCode(fastGetNotifications());
    return hashCode;
  }
      ...
      for (int i=0; i < methods.length; i++) {
        ...
        if (methodName.startsWith("get") && method.getParameterTypes().length == 0 && method.getReturnType().isArray()) {
          try {
            ...
            if (!submethod.equals(method))             return false;
          }
 ...
 * @serialDataFor compatibility reasons, an object of this class is serialized as follows.
 * <ul>
 ...
 * @serialDataFor compatibility reasons, an object of this class is deserialized as follows.
 * <ul>
