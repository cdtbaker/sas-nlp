 ...
 */
  public RoleUnresolvedList(  List<RoleUnresolved> list) throws IllegalArgumentException {
    if (list == null)     throw new IllegalArgumentException("Null parameter");
    checkTypeSafe(list);
    super.addAll(list);
  }
  /** 
 ...
 * be added.</p>
 * @throws IllegalArgumentException if this {@code RoleUnresolvedList}contains an element that is not a {@code RoleUnresolved}.
 * @since 1.6
 */
  @SuppressWarnings("unchecked") public List<RoleUnresolved> asList(){
    if (!typeSafe) {
      if (tainted)       checkTypeSafe(this);
      typeSafe=true;
    }
 ...
 */
  ...
  @Override public boolean add(  Object o){
    if (!tainted)     tainted=isTainted(o);
    if (typeSafe)     checkTypeSafe(o);
    return super.add(o);
  }
  @Override public void add(  int index,  Object element){
    if (!tainted)     tainted=isTainted(element);
    if (typeSafe)     checkTypeSafe(element);
    super.add(index,element);
  }
  @Override public boolean addAll(  Collection<?> c){
    if (!tainted)     tainted=isTainted(c);
    if (typeSafe)     checkTypeSafe(c);
 ...
 */
  private static void checkTypeSafe(  Collection<?> c){
    try {
      RoleUnresolved r;
      for (      Object o : c)       r=(RoleUnresolved)o;
    }
 catch (    ClassCastException e) {
      throw new IllegalArgumentException(e);
    }
