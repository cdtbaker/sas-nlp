 ...
 * <p>Each connector server is created by an instance of {@link JMXConnectorServerProvider}.  This instance is found as follows.  Suppose
 * the given {@link JMXServiceURL} looks like
 * <code>"service:jmx:<em>protocol</em>:<em>remainder</em>"</code>.
 * Then the factory will attempt to find the appropriate {@link JMXConnectorServerProvider} for <code><em>protocol</em></code>.  Each
 ...
 * <ol>
 * <li>If the <code>environment</code> parameter to {@link #newJMXConnectorServer(JMXServiceURL,Map,MBeanServer)newJMXConnectorServer} contains the key
 ...
 * </blockquote>
 * <p>If the <code>environment</code> parameter to {@link #newJMXConnectorServer(JMXServiceURL,Map,MBeanServer)newJMXConnectorServer} contains the key
 ...
 * <p>Otherwise, a problem with the provider found is signalled by a{@link JMXProviderException} whose {@link JMXProviderException#getCause() <em>cause</em>} indicates the
 ...
 * <code>newJMXConnectorServer</code> method is the result of calling{@link JMXConnectorServerProvider#newJMXConnectorServer(JMXServiceURL,Map,MBeanServer) newJMXConnectorServer} on the provider.</p>
 ...
 * <code>environment</code> parameter to {@link #newJMXConnectorServer(JMXServiceURL,Map,MBeanServer)JMXConnectorServerFactory.newJMXConnectorServer}, if there was one.
 * Additionally, if the
 ...
 * requests received from the client, possibly after consulting an
 * MBean-specific class loader.  The value associated with this
 * attribute is an instance of {@link javax.management.ObjectNameObjectName}.</p>
 */
  ...
  private static JMXConnectorServer getConnectorServerAsService(  ClassLoader loader,  JMXServiceURL url,  Map<String,?> map,  MBeanServer mbs) throws IOException {
    ...
    while (providers.hasNext()) {
      try {
        ...
        if (logger.traceOn())         logger.trace("getConnectorAsService","URL[" + url + "] Service provider exception: "+ e);
 ...
 * resultant server is not started until its {@link JMXConnectorServer#start() start} method is called.</p>
 * @param serviceURL the address of the new connector server.  The
 * actual address of the new connector server, as returned by its{@link JMXConnectorServer#getAddress() getAddress} method, will
 ...
 */
  public static JMXConnectorServer newJMXConnectorServer(  JMXServiceURL serviceURL,  Map<String,?> environment,  MBeanServer mbeanServer) throws IOException {
    ...
    if (environment == null)     envcopy=new HashMap<String,Object>();
 else {
    ...
    JMXConnectorServerProvider provider=JMXConnectorFactory.getProvider(serviceURL,envcopy,providerClassName,targetInterface,loader);
    IOException exception=null;
    if (provider == null) {
      if (loader != null) {
        try {
          JMXConnectorServer connection=getConnectorServerAsService(loader,serviceURL,envcopy,mbeanServer);
      ...
      provider=JMXConnectorFactory.getProvider(protocol,PROTOCOL_PROVIDER_DEFAULT_PACKAGE,JMXConnectorFactory.class.getClassLoader(),providerClassName,targetInterface);
    }
    if (provider == null) {
