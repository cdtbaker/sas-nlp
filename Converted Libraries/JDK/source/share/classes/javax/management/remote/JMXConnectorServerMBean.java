 ...
 * not yet listen for connections.  Only when its {@link #start start}method has been called does it start listening for connections.</p>
 * @since 1.5
 */
 ...
 * when the connector server has been stopped will generate an{@link IOException}.</p>
 ...
 * exception is not thrown from this method.  A {@link JMXConnectionNotification} with type {@link JMXConnectionNotification#FAILED} is emitted from this MBean
 ...
 * returns successfully and remains active until either its{@link #stop stop} method is called or the connector server
 * fails.</p>
 ...
 * unaffected.</p>
 * <p>This method can be called more than once with different{@link MBeanServerForwarder} objects.  The result is a chain
 ...
 * <code>MBeanServer</code> object, then that object is given to{@link MBeanServerForwarder#setMBeanServermbsf.setMBeanServer}.  If doing so produces an exception, this
 ...
 * @exception IllegalArgumentException if the call to {@link MBeanServerForwarder#setMBeanServer mbsf.setMBeanServer} fails
 ...
 * <p>
 * The address returned may not be the exact original {@link JMXServiceURL}that was supplied when creating the connector server, since the original
 ...
 * address returned is the actual {@link JMXServiceURL} of the{@link JMXConnectorServer}. This is the address that clients supply
 ...
 * can be provided to {@link JMXConnector#connect(Map)JMXConnector.connect(Map)}.  Can be null, which is equivalent
 * to an empty map.
