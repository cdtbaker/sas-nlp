 ...
 */
  RMIConnection doNewClient(  Object credentials) throws IOException {
    ...
    if (tracing) {
      if (subject != null)       logger.trace("newClient","subject is not null");
 ...
 * from this method.</p>
 * @exception IOException if {@link #closeServer()} or one of the{@link RMIConnection#close()} calls threw
        ...
        for (Iterator<WeakReference<RMIConnection>> it=clientList.iterator(); it.hasNext(); ) {
          ...
          if (client != null) {
            try {
              ...
              if (tracing)               logger.trace("close","Failed to close client: " + e);
              if (debug)               logger.debug("close",e);
 ...
 */
  ...
  private static synchronized String makeConnectionId(  String protocol,  Subject subject){
    ...
    if (clientHost.length() > 0)     buf.append("//").append(clientHost);
    buf.append(" ");
    if (subject != null) {
    ...
    if (logger.traceOn())     logger.trace("newConnectionId","connectionId=" + buf);
    return buf.toString();
  }
...
synchronized (clientList) {
      for (Iterator<WeakReference<RMIConnection>> it=clientList.iterator(); it.hasNext(); ) {
        WeakReference<RMIConnection> wr=it.next();
        if (wr.get() == null)         it.remove();
      }
    }
  }
  synchronized NotificationBuffer getNotifBuffer(){
    if (notifBuffer == null)     notifBuffer=ArrayNotificationBuffer.getNotificationBuffer(mbeanServer,env);
    return notifBuffer;
  }
