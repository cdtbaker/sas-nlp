 ...
 * <li>{@code d:type=F*o,name=Bar} will match e.g.{@code d:type=Fo,name=Bar} and {@code d:type=Frodo,name=Bar}.</li>
 ...
 * @since 1.5
 */
@SuppressWarnings("serial") public class ObjectName implements Comparable<ObjectName>, QueryExp {
  /** 
...
c=name_chars[index];
if (c == '*') {
if (_property_list_pattern) throw new MalformedObjectNameException("Cannot have several '*' characters in pattern " + "property list");
 else {
_property_list_pattern=true;
if ((++index < len) && (name_chars[index] != ',')) throw new MalformedObjectNameException("Invalid character found after '*': end of " + "name or ',' expected");
 else if (index == len) {
if (property_index == 0) {
_kp_array=_Empty_property_array;
_ca_array=_Empty_property_array;
...
in_index=index;
key_index=in_index;
if (name_chars[in_index] == '=') throw new MalformedObjectNameException("Invalid key (empty)");
...
if (name_chars[in_index - 1] != '=') throw new MalformedObjectNameException("Unterminated key property part");
...
if (in_index == len - 1) {
if (quoted_value) throw new MalformedObjectNameException("Invalid ending character `" + name_chars[in_index] + "'");
 else throw new MalformedObjectNameException("Invalid ending comma");
}
 else in_index++;
if (!value_pattern) {
...
for (int i=0; i < nb_props; i++) _ca_array[i]=keys_map.get(keys[i]);
...
if (property == null) throw new NullPointerException("key property can't be null");
for (int i=0; i < _ca_array.length; i++) {
Property prop=_ca_array[i];
String key=prop.getKeyString(_canonicalName);
if (key.equals(property)) return (prop instanceof PatternProperty);
}
...
final int total_size=_canonicalName.length() - _domain_length - 1- (_property_list_pattern ? 2 : 0);
...
if (name._domain_pattern || name._property_list_pattern || name._property_value_pattern) return false;
if (!_domain_pattern && !_property_list_pattern && !_property_value_pattern) return _canonicalName.equals(name._canonicalName);
...
if (_property_value_pattern && !_property_list_pattern && (name._ca_array.length != _ca_array.length)) return false;
