...
/** 
 * <p>Additional metadata for a JMX element.  A {@code Descriptor}is associated with a {@link MBeanInfo}, {@link MBeanAttributeInfo}, etc.
 * It consists of a collection of fields.  A field is a name and an
 * associated value.</p>
 * <p>Field names are not case-sensitive.  The names {@code descriptorType},{@code descriptortype}, and {@code DESCRIPTORTYPE} are all equivalent.
 * However, the case that was used when the field was first set is preserved
 * in the result of the {@link #getFields} and {@link #getFieldNames}methods.</p>
 ...
 * descriptors are usually instances of{@link javax.management.modelmbean.DescriptorSupport} or a subclass.
 ...
 * below, and each one has a corresponding constant in the {@link JMX}class.  For example, the field {@code defaultValue} is represented
 ...
 * <p>Note that the values in the {@code defaultValue}, {@codelegalValues}, {@code maxValue}, and {@code minValue} fields should
 * be consistent with the type returned by the {@code getType()}method for the associated {@code MBeanAttributeInfo} or {@codeMBeanParameterInfo}.  For MXBeans, this means that they should be
 ...
 * <td>Default value for an attribute or parameter.  See{@link javax.management.openmbean}.</td>
 ...
 * the {@code descriptionResourceKey} field can be found, for example{@code "com.example.myapp.MBeanResources"}.  The meaning of this
 ...
 * MBean that does not define {@code immutableInfo} as {@code true}that the MBeanInfo is not likely to change within this period and
 ...
 * registered directly in the MBean Server or created using the {@link StandardMBean} class will have this field in its MBeanInfo
 * Descriptor.</td>
 * <tr id="legalValues"><td><i>legalValues</i></td>
 * <td>{@literal Set<?>}</td><td>MBeanAttributeInfo<br>MBeanParameterInfo</td>
 * <td>Legal values for an attribute or parameter.  See{@link javax.management.openmbean}.</td>
 * <tr id="locale"><td>locale</td>
 * <td>String</td><td>Any</td>
 * <td>The {@linkplain Locale locale} of the description in this{@code MBeanInfo}, {@code MBeanAttributeInfo}, etc, as returned
 * by {@link Locale#toString()}.</td>
 * <tr id="maxValue"><td><i>maxValue</i><td>Object</td>
 * <td>MBeanAttributeInfo<br>MBeanParameterInfo</td>
 * <td>Maximum legal value for an attribute or parameter.  See{@link javax.management.openmbean}.</td>
 ...
 * <td>MBeanAttributeInfo<br>MBeanParameterInfo</td>
 * <td>Minimum legal value for an attribute or parameter.  See{@link javax.management.openmbean}.</td>
 * <tr id="mxbean"><td><i>mxbean</i><td>String</td>
 * <td>MBeanInfo</td>
 * <td>The string {@code "true"} or {@code "false"} according as this
 * MBean is an {@link MXBean}.  A Standard MBean or MXBean registered
 * directly with the MBean Server or created using the {@link StandardMBean} class will have this field in its MBeanInfo
 * Descriptor.</td>
 * <tr id="openType"><td><i>openType</i><td>{@link OpenType}</td>
 * <td>MBeanAttributeInfo<br>MBeanOperationInfo<br>MBeanParameterInfo</td>
 * <td><p>The Open Type of this element.  In the case of {@codeMBeanAttributeInfo} and {@code MBeanParameterInfo}, this is the
 * Open Type of the attribute or parameter.  In the case of {@codeMBeanOperationInfo}, it is the Open Type of the return value.  This
 * field is set in the Descriptor for all instances of {@link OpenMBeanAttributeInfoSupport}, {@link OpenMBeanOperationInfoSupport}, and {@link OpenMBeanParameterInfoSupport}.  It is also set for attributes,
 * operations, and parameters of MXBeans.</p>
 * <p>This field can be set for an {@code MBeanNotificationInfo}, in
 * which case it indicates the Open Type that the {@link Notification#getUserData() user data} will have.</td>
 * <tr id="originalType"><td><i>originalType</i><td>String</td>
 * <td>MBeanAttributeInfo<br>MBeanOperationInfo<br>MBeanParameterInfo</td>
 * <td><p>The original Java type of this element as it appeared in the{@link MXBean} interface method that produced this {@codeMBeanAttributeInfo} (etc).  For example, a method<br> <code>public
 * </code> {@link MemoryUsage}<code> getHeapMemoryUsage();</code><br>
 * in an MXBean interface defines an attribute called {@codeHeapMemoryUsage} of type {@link CompositeData}.  The {@codeoriginalType} field in the Descriptor for this attribute will have
 ...
 * application may also define versions of this model, and use the{@code "since"} field to record the version in which an element
 * first appeared.</td>
 ...
 * value is measured, for example {@code "bytes"} or {@code"seconds"}.</td>
 * </table>
 ...
 * if the descriptor is immutable (wrapped exception is{@link UnsupportedOperationException}).
 */
 ...
 * @return Object array of field values. If the list of {@code fieldNames}is empty, you will get an empty array.
 */
 ...
 * Wrapped exception is {@link IllegalArgumentException} if{@code fieldNames} or {@code fieldValues} is null, or if
 * the arrays are of different lengths, or if there is an
 * illegal value in one of them.
 * Wrapped exception is {@link UnsupportedOperationException}if the descriptor is immutable, and the call would change
 * its contents.
 * @see #getFields
 */
