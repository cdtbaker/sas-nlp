...
class ContinuationContext implements Context, Resolver {
  ...
  protected Context getTargetContext() throws NamingException {
    if (contCtx == null) {
      ...
      if (contCtx == null)       throw (NamingException)cpe.fillInStackTrace();
    }
    return contCtx;
  }
  ...
  public ResolveResult resolveToClass(  Name name,  Class<? extends Context> contextType) throws NamingException {
    if (cpe.getResolvedObj() == null)     throw (NamingException)cpe.fillInStackTrace();
    Resolver res=NamingManager.getResolver(cpe.getResolvedObj(),cpe.getAltName(),cpe.getAltNameCtx(),env);
    if (res == null)     throw (NamingException)cpe.fillInStackTrace();
