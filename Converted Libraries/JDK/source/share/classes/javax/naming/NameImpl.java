...
class NameImpl {
  ...
  private final boolean isMeta(  String n,  int i){
    return (isA(n,i,syntaxEscape) || isA(n,i,syntaxBeginQuote1) || isA(n,i,syntaxBeginQuote2)|| isSeparator(n,i));
  }
  ...
  private final int extractComp(  String name,  int i,  int len,  Vector comps) throws InvalidNameException {
    String beginQuote;
    String endQuote;
    boolean start=true;
    boolean one=false;
    ...
    while (i < len) {
      if (start && ((one=isA(name,i,syntaxBeginQuote1)) || isA(name,i,syntaxBeginQuote2))) {
        ...
        if (i >= len)         throw new InvalidNameException(name + ": no close quote");
        i+=endQuote.length();
 ...
 else       if (isA(name,i,syntaxTypevalSeparator) && ((one=isA(name,i + syntaxTypevalSeparator.length(),syntaxBeginQuote1)) || isA(name,i + syntaxTypevalSeparator.length(),syntaxBeginQuote2))) {
        beginQuote=one ? syntaxBeginQuote1 : syntaxBeginQuote2;
        endQuote=one ? syntaxEndQuote1 : syntaxEndQuote2;
        i+=syntaxTypevalSeparator.length();
        answer.append(syntaxTypevalSeparator + beginQuote);
        for (i+=beginQuote.length(); ((i < len) && !name.startsWith(endQuote,i)); i++) {
          if (isA(name,i,syntaxEscape) && isA(name,i + syntaxEscape.length(),endQuote)) {
            i+=syntaxEscape.length();
          }
        ...
        if (i >= len)         throw new InvalidNameException(name + ": typeval no close quote");
        i+=endQuote.length();
    ...
    if (syntaxDirection == RIGHT_TO_LEFT)     comps.insertElementAt(answer.toString(),0);
 else     comps.addElement(answer.toString());
    return i;
  }
  ...
  private final void recordNamingConvention(  Properties p){
    ...
    if (syntaxEndQuote1 == null && syntaxBeginQuote1 != null)     syntaxEndQuote1=syntaxBeginQuote1;
 else     if (syntaxBeginQuote1 == null && syntaxEndQuote1 != null)     syntaxBeginQuote1=syntaxEndQuote1;
    syntaxBeginQuote2=p.getProperty("jndi.syntax.beginquote2");
    syntaxEndQuote2=p.getProperty("jndi.syntax.endquote2");
    if (syntaxEndQuote2 == null && syntaxBeginQuote2 != null)     syntaxEndQuote2=syntaxBeginQuote2;
 else     if (syntaxBeginQuote2 == null && syntaxEndQuote2 != null)     syntaxBeginQuote2=syntaxEndQuote2;
  ...
  NameImpl(  Properties syntax,  String n) throws InvalidNameException {
    ...
    for (int i=0; i < len; ) {
      ...
      String comp=rToL ? (String)components.firstElement() : (String)components.lastElement();
      if (comp.length() >= 1) {
        compsAllEmpty=false;
      }
      if (i < len) {
  ...
  NameImpl(  Properties syntax,  Enumeration comps){
    this(syntax);
    while (comps.hasMoreElements())     components.addElement(comps.nextElement());
  }
 ...
 else       if (syntaxEscape != null)       escapeSeparator=true;
    }
 ...
 else       if (syntaxEscape != null)       escapeSeparator2=true;
    }
    if (beginQuote != null) {
  ...
  public String toString(){
    ...
    for (int i=0; i < size; i++) {
      if (syntaxDirection == RIGHT_TO_LEFT) {
        comp=stringifyComp((String)components.elementAt(size - 1 - i));
      }
 else {
        comp=stringifyComp((String)components.elementAt(i));
      }
      if ((i != 0) && (syntaxSeparator != null))       answer.append(syntaxSeparator);
      if (comp.length() >= 1)       compsAllEmpty=false;
      answer=answer.append(comp);
    }
    if (compsAllEmpty && (size >= 1) && (syntaxSeparator != null))     answer=answer.append(syntaxSeparator);
      ...
      if (target.size() == this.size()) {
        ...
        while (mycomps.hasMoreElements()) {
          ...
          if (syntaxCaseInsensitive) {
            if (!(my.equalsIgnoreCase(his)))             return false;
          }
 else {
 ...
 */
  ...
  public boolean addAll(  int posn,  Enumeration comps) throws InvalidNameException {
    boolean added=false;
