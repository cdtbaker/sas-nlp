        ...
        try {
          if (isOrdered()) {
            ...
            while (theirs.hasMoreElements()) {
              if (find(theirs.nextElement()) < 0)               return false;
            }
          }
        }
 ...
 */
  ...
  private int find(  Object target){
    ...
    if (target == null) {
      int ct=values.size();
      for (int i=0; i < ct; i++) {
        if (values.elementAt(i) == null)         return i;
      }
    }
      ...
      for (int i=0; i < ct; i++) {
        ...
        if (it != null && cl == it.getClass() && arrayEquals(target,it))         return i;
      }
    }
 else {
 ...
 */
  private static boolean arrayEquals(  Object a1,  Object a2){
    int len;
    if ((len=Array.getLength(a1)) != Array.getLength(a2))     return false;
    for (int j=0; j < len; j++) {
