    ...
    while (enum_.hasMoreElements()) {
      ...
      try {
        ...
        if (cp.getAlgorithm().equals(CryptoPermission.ALG_NAME_WILDCARD)) {
          ...
          if (cp.getCheckParam()) {
            newCp=new CryptoPermission(alg,cp.getMaxKeySize(),cp.getAlgorithmParameterSpec(),cp.getExemptionMechanism());
          }
 else {
 ...
 */
  private CryptoPermission getDefaultPermission(  String alg){
    Enumeration enum_=defaultPolicy.getPermissionCollection(alg).elements();
