...
public class JulesAATileGenerator implements AATileGenerator {
  final static ExecutorService rasterThreadPool=Executors.newCachedThreadPool();
  ...
  public JulesAATileGenerator(  Shape s,  AffineTransform at,  Region clip,  BasicStroke bs,  boolean thin,  boolean normalize,  int[] bbox){
    ...
    threaded=ENABLE_THREADING && rasterTileCnt >= THREAD_MIN && CPU_CNT >= 2;
    if (threaded) {
  ...
  private static native long rasterizeTrapezoidsNative(  long pixmanImagePtr,  int[] traps,  int[] trapPos,  int trapCnt,  byte[] buffer,  int xOff,  int yOff);
  ...
  protected boolean doTrapsCoverTile(  GrowableIntArray trapList,  int tileIndex){
    ...
    for (int i=0; i < trapList.getSize(); i++) {
      ...
      if (traps.getP1XLeft(trapPos) > tileStartX || traps.getP2XLeft(trapPos) > tileStartX || traps.getP1XRight(trapPos) < tileEndX || traps.getP2XRight(trapPos) < tileEndX || traps.getTop(trapPos) != lastBottom) {
        return false;
      }
  ...
  protected JulesTile rasterizeTile(  int tileIndex,  JulesTile tile){
    ...
    if (trapCont.getTileAlpha() == 127) {
      long pixmanImgPtr=rasterizeTrapezoidsNative(tile.getPixmanImgPtr(),traps.getTrapArray(),trapList.getArray(),trapList.getSize(),tile.getImgBuffer(),tileOffsetX,tileOffsetY);
