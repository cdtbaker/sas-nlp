...
public class TileWorker implements Runnable {
  ...
  final ArrayList<JulesTile> rasterizedTileConsumerCache=new ArrayList<JulesTile>();
  ...
  public void run(){
    ...
    for (int i=workerStartIndex; i < tileGenerator.getTileCount(); i++) {
      ...
      if (tile != null && tile.getTileAlpha() == 127) {
        JulesTile rasterizedTile=tileGenerator.rasterizeTile(i,tileCache.getIdleTileWorker(tileGenerator.getTileCount() - i - 1));
 ...
 */
  public JulesTile getPreRasterizedTile(  int tilePos){
    JulesTile tile=null;
    if (rasterizedTileConsumerCache.size() == 0 && tilePos >= workerStartIndex) {
