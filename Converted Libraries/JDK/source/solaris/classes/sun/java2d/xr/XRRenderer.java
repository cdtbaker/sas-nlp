 ...
 */
  ...
  public void drawLine(  SunGraphics2D sg2d,  int x1,  int y1,  int x2,  int y2){
    try {
      ...
      XRSurfaceData xrsd=(XRSurfaceData)sg2d.surfaceData;
      tileManager.addLine(x1 + transx,y1 + transy,x2 + transx,y2 + transy);
  ...
  protected void fillSpans(  SunGraphics2D sg2d,  SpanIterator si,  int transx,  int transy){
    ...
    try {
      ...
      while (si.nextSpan(spanBox)) {
        tileManager.addRect(spanBox[0] + transx,spanBox[1] + transy,spanBox[2] - spanBox[0],spanBox[3] - spanBox[1]);
      }
      tileManager.fillMask(((XRSurfaceData)sg2d.surfaceData));
    }
  finally {
