 ...
 */
  ...
  public void validatePipe(  SunGraphics2D sg2d){
    ...
    if (sg2d.compositeState < SunGraphics2D.COMP_XOR && (sg2d.paintState < SunGraphics2D.PAINT_TEXTURE || sg2d.composite == null || !(sg2d.composite instanceof AlphaComposite) || ((AlphaComposite)sg2d.composite).getAlpha() == 1.0f)) {
      textpipe=xrtextpipe;
    }
 else {
    ...
    if (sg2d.antialiasHint == SunHints.INTVAL_ANTIALIAS_ON && JulesPathBuf.isCairoAvailable()) {
  ...
  protected MaskFill getMaskFill(  SunGraphics2D sg2d){
    if (sg2d.paintState > SunGraphics2D.PAINT_ALPHACOLOR && !XRPaints.isValid(sg2d)) {
      return null;
    }
 ...
 */
  ...
  protected XRSurfaceData(  XRBackend renderQueue){
    super(XRSurfaceData.IntRgbX11,new DirectColorModel(24,0x00FF0000,0x0000FF00,0x000000FF));
 ...
 */
  public void validateCopyAreaGC(  Region gcClip,  boolean needExposures){
    if (validatedGCClip != gcClip) {
      if (gcClip != null)       renderQueue.setGCClipRectangles(xgc,gcClip);
...
transformInUse=false;
 ...
 else if (!transformInUse || (transformInUse && !sxForm.equals(validatedSourceTransform))) {
...
if (JulesPathBuf.isCairoAvailable()) {
  aaShapePipe=new JulesShapePipe(XRCompositeManager.getInstance(this));
...
public XRInternalSurfaceData(XRBackend renderQueue,int pictXid,AffineTransform transform){
super(renderQueue);
