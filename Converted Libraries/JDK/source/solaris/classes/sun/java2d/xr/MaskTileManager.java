...
public class MaskTileManager {
  ...
  public MaskTileManager(  XRCompositeManager xrMgr,  int parentXid){
    ...
    con.renderRectangle(maskPicture,XRUtils.PictOpClear,new XRColor(Color.black),0,0,MASK_SIZE,MASK_SIZE);
    ...
    lineMaskPicture=con.createPicture(lineMaskPixmap,XRUtils.PictStandardA8);
    con.renderRectangle(lineMaskPicture,XRUtils.PictOpClear,new XRColor(Color.black),0,0,MASK_SIZE,MASK_SIZE);
 ...
 */
  public void fillMask(  XRSurfaceData dst){
    ...
    if (maskRequired) {
      ...
      if (dirtyArea.getWidth() <= MASK_SIZE && dirtyArea.getHeight() <= MASK_SIZE) {
        compositeSingleTile(dst,mainTile,dirtyArea,maskRequired,0,0,maskColor);
      }
 else {
 ...
 */
  public int uploadMask(  int w,  int h,  int maskscan,  int maskoff,  byte[] mask){
    ...
    if (mask != null) {
      float maskAlpha=xrMgr.isTexturePaintActive() ? xrMgr.getExtraAlpha() : 1.0f;
      con.putMaskImage(maskPixmap,maskGC,mask,0,0,0,0,w,h,maskoff,maskscan,maskAlpha);
      maskPic=maskPicture;
    }
 ...
 */
  public void clearUploadMask(  int mask,  int w,  int h){
    if (mask == maskPicture) {
      con.renderRectangle(maskPicture,XRUtils.PictOpClear,XRColor.NO_ALPHA,0,0,w,h);
    }
  }
  /** 
  ...
  protected void compositeSingleTile(  XRSurfaceData dst,  MaskTile tile,  DirtyRegion dirtyArea,  boolean maskRequired,  int tileStartX,  int tileStartY,  XRColor maskColor){
    if (tile.rects.getSize() > 0) {
      ...
      if (maskRequired) {
        ...
        if (rectCnt > 1) {
          con.renderRectangles(maskPicture,XRUtils.PictOpSrc,maskColor,tile.rects);
          mask=maskPicture;
        }
 else {
        ...
        xrMgr.XRComposite(XRUtils.None,mask,dst.getPicture(),x,y,tileDirtyArea.x,tileDirtyArea.y,x,y,width,height);
        if (rectCnt > 1) {
          con.renderRectangle(maskPicture,XRUtils.PictOpClear,XRColor.NO_ALPHA,tileDirtyArea.x,tileDirtyArea.y,width,height);
        }
        tile.reset();
      }
 else       if (rectCnt > 0) {
 ...
 */
  protected void tileRects(){
    ...
    for (int i=0; i < rects.getSize(); i++) {
      ...
      int tileXLength=((rects.getX(i) + rects.getWidth(i)) / MASK_SIZE + 1) - tileXStartIndex;
      int tileYLength=((rects.getY(i) + rects.getHeight(i)) / MASK_SIZE + 1) - tileYStartIndex;
      for (int n=0; n < tileYLength; n++) {
        for (int m=0; m < tileXLength; m++) {
          int tileIndex=xTiles * (tileYStartIndex + n) + tileXStartIndex + m;
