...
public abstract class X11SurfaceDataProxy extends SurfaceDataProxy implements Transparency {
  ...
  @Override public SurfaceData validateSurfaceData(  SurfaceData srcData,  SurfaceData cachedData,  int w,  int h){
    if (cachedData == null) {
      cachedData=X11SurfaceData.createData(x11gc,w,h,x11gc.getColorModel(),null,0,getTransparency());
    }
    return cachedData;
  }
  /** 
...
public static class Opaque extends X11SurfaceDataProxy {
    ...
    @Override public boolean isSupportedOperation(    SurfaceData srcData,    int txtype,    CompositeType comp,    Color bgColor){
      return (txtype < SunGraphics2D.TRANSFORM_TRANSLATESCALE && (CompositeType.SrcOverNoEa.equals(comp) || CompositeType.SrcNoEa.equals(comp)));
    }
  }
  /** 
...
public static class Bitmask extends X11SurfaceDataProxy {
    ...
    @Override public boolean isSupportedOperation(    SurfaceData srcData,    int txtype,    CompositeType comp,    Color bgColor){
      ...
      if (CompositeType.SrcOverNoEa.equals(comp) || (CompositeType.SrcNoEa.equals(comp) && bgColor != null)) {
        return true;
      }
      return false;
    }
