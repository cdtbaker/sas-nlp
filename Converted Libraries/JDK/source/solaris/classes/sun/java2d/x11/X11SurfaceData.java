...
static {
    if (!isX11SurfaceDataInitialized() && !GraphicsEnvironment.isHeadless()) {
      String magPresent=(String)java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction("javax.accessibility.screen_magnifier_present"));
      boolean tryDGA=magPresent == null || !"true".equals(magPresent);
      initIDs(XORComposite.class,tryDGA);
      String xtextpipe=(String)java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction("sun.java2d.xtextpipe"));
 ...
 */
  ...
  public void validatePipe(  SunGraphics2D sg2d){
    if (sg2d.antialiasHint != SunHints.INTVAL_ANTIALIAS_ON && sg2d.paintState <= sg2d.PAINT_ALPHACOLOR && (sg2d.compositeState <= sg2d.COMP_ISCOPY || sg2d.compositeState == sg2d.COMP_XOR)) {
      if (x11txpipe == null) {
...
return new X11PixmapSurfaceData(gc,width,height,image,getSurfaceType(gc,transparency,true),cm,drawable,transparency);
}
...
return getGC(clip,SunGraphics2D.COMP_ISCOPY,null,validatedPixel,needExposures);
}
...
validatedClip=clip;
if (clip != null) {
XSetClip(xgc,clip.getLoX(),clip.getLoY(),clip.getHiX(),clip.getHiY(),(clip.isRectangular() ? null : clip));
}
 else {
XResetClip(xgc);
}
...
@Override public boolean canSourceSendExposures(int x,int y,int w,int h){
return true;
}
/** 
