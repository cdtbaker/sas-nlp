 ...
 */
  ...
  public PhysicalFont registerFromFcInfo(  FcCompFont fcInfo){
    ...
    if (physFont != null) {
      if (isTTC) {
        Font2D f2d=fm.findFont2D(fcInfo.firstFont.familyName,fcInfo.style,FontManager.NO_FALLBACK);
    ...
    if (physFont == null && fm.isDeferredFont(fontFile) == true) {
  ...
  public CompositeFont getFontConfigFont(  String name,  int style){
    ...
    CompositeFont jdkFont=(CompositeFont)fm.findFont2D(fcInfo.jdkName,style,FontManager.LOGICAL_FALLBACK);
    ...
    for (int i=0; i < fontConfigFonts.length; i++) {
      ...
      if (fc != fcInfo && physFont.getFamilyName(null).equals(fc.firstFont.familyName) && !fc.firstFont.fontFile.equals(physFont.platName) && family.getFontWithExactStyleMatch(fc.style) == null) {
 ...
 */
  ...
  private static native void getFontConfig(  String locale,  FontConfigInfo fcInfo,  FcCompFont[] fonts,  boolean includeFallbacks);
