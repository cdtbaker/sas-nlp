...
class NativeStrike extends PhysicalStrike {
  ...
  NativeStrike(  NativeFont nativeFont,  FontStrikeDesc desc){
    ...
    if (nativeFont.isBitmapDelegate) {
      ...
      if ((ttype & ~AffineTransform.TYPE_UNIFORM_SCALE) != 0 || desc.glyphTx.getScaleX() <= 0) {
        numGlyphs=0;
        return;
      }
    }
    ...
    this.disposer=new NativeStrikeDisposer(nativeFont,desc,pScalerContext);
  }
  ...
  private native long createScalerContext(  byte[] nameBytes,  int ptSize,  double scale);
