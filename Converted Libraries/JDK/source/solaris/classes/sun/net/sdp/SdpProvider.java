...
public class SdpProvider extends NetHooks.Provider {
  ...
  private static enum Action {  BIND,   CONNECT}
...
private static class PortRangeRule implements Rule {
    ...
    @Override public boolean match(    Action action,    InetAddress address,    int port){
...
private static class AddressPortRangeRule extends PortRangeRule {
    ...
    @Override public boolean match(    Action action,    InetAddress address,    int port){
      ...
      if (candidate.length != addressAsBytes.length)       return false;
      for (int i=0; i < prefixByteCount; i++) {
        if (candidate[i] != addressAsBytes[i])         return false;
      }
      if ((prefixByteCount < addressAsBytes.length) && ((candidate[prefixByteCount] & mask) != (addressAsBytes[prefixByteCount] & mask)))       return false;
  ...
  private void convertTcpToSdpIfMatch(  FileDescriptor fdObj,  Action action,  InetAddress address,  int port) throws IOException {
    ...
    if (log != null) {
      String addr=(address instanceof Inet4Address) ? address.getHostAddress() : "[" + address.getHostAddress() + "]";
      if (matched) {
  ...
  @Override public void implBeforeTcpBind(  FileDescriptor fdObj,  InetAddress address,  int port) throws IOException {
    if (enabled)     convertTcpToSdpIfMatch(fdObj,Action.BIND,address,port);
  }
  @Override public void implBeforeTcpConnect(  FileDescriptor fdObj,  InetAddress address,  int port) throws IOException {
    if (enabled)     convertTcpToSdpIfMatch(fdObj,Action.CONNECT,address,port);
  }
}
