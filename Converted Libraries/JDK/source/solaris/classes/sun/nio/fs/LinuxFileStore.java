 ...
 * resides.
 */
  @Override UnixMountEntry findMountEntry() throws IOException {
    ...
    while (parent != null) {
      ...
      if (attrs.dev() != dev())       break;
      path=parent;
      parent=parent.getParent();
    }
    byte[] dir=path.asByteArray();
    for (    UnixMountEntry entry : fs.getMountEntries("/proc/mounts")) {
      if (Arrays.equals(dir,entry.dir()))       return entry;
    }
  ...
  private boolean isExtendedAttributesEnabled(  UnixPath path){
    try {
      ...
      try {
        ...
        if (e.errno() == UnixConstants.ENODATA)         return true;
      }
 finally {
  ...
  @Override public boolean supportsFileAttributeView(  Class<? extends FileAttributeView> type){
    if (type == DosFileAttributeView.class || type == UserDefinedFileAttributeView.class) {
      ...
      if (status == FeatureStatus.PRESENT)       return true;
      if (status == FeatureStatus.NOT_PRESENT)       return false;
      if ((entry().hasOption("user_xattr")))       return true;
      if (entry().fstype().equals("ext3") || entry().fstype().equals("ext4"))       return false;
      if (!xattrChecked) {
  ...
  @Override public boolean supportsFileAttributeView(  String name){
    if (name.equals("dos"))     return supportsFileAttributeView(DosFileAttributeView.class);
