...
class UnixPath extends AbstractPath {
  ...
  static String normalizeAndCheck(  String input){
    ...
    for (int i=0; i < n; i++) {
      ...
      if ((c == '/') && (prevChar == '/'))       return normalize(input,n,i - 1);
      checkNotNul(input,c);
      prevChar=c;
    }
    if (prevChar == '/')     return normalize(input,n,n - 1);
    return input;
  }
  private static void checkNotNul(  String input,  char c){
    if (c == '\u0000')     throw new InvalidPathException(input,"Nul character not allowed");
  }
  ...
  private static byte[] encode(  String input){
    ...
    if (ce == null) {
      ce=Charset.defaultCharset().newEncoder().onMalformedInput(CodingErrorAction.REPORT).onUnmappableCharacter(CodingErrorAction.REPORT);
    ...
    int len=bb.position();
    if (len != ba.length)     ba=Arrays.copyOf(ba,len);
    return ba;
  }
  byte[] asByteArray(){
    return path;
...
synchronized (this) {
  ...
  @Override public UnixFileSystem getFileSystem(){
    return fs;
  }
  @Override public UnixPath getRoot(){
  ...
  @Override public UnixPath getFileName(){
    ...
    if (count == 1 && path.length > 0 && path[0] != '/')     return this;
  ...
  @Override public UnixPath getName(  int index){
    ...
    if (index >= offsets.length)     throw new IllegalArgumentException();
  ...
  private static byte[] resolve(  byte[] base,  byte[] child){
    ...
    if (baseLength == 0 || child[0] == '/')     return child;
    byte[] result;
  ...
  @Override public UnixPath resolve(  Path obj){
    byte[] other=toUnixPath(obj).path;
    if (other.length > 0 && other[0] == '/')     return ((UnixPath)obj);
  ...
  @Override public UnixPath relativize(  Path obj){
    ...
    if (other.equals(this))     return emptyPath();
    if (this.isAbsolute() != other.isAbsolute())     throw new IllegalArgumentException("'other' is different type of Path");
    ...
    for (int i=0; i < count; i++) {
      ...
      if (path[begin] == '.') {
        if (len == 1) {
          ...
          if (path[begin + 1] == '.')           hasDotDot=true;
        }
      }
    }
    if (hasDotDot) {
 ...
 while (prevRemaining > remaining);
  ...
  @Override public boolean startsWith(  Path other){
    if (!(Objects.requireNonNull(other) instanceof UnixPath))     return false;
    ...
    if (i < path.length && this.path[i] != '/')     return false;
    return true;
  }
  ...
  @Override public String toString(){
    if (stringValue == null)     stringValue=new String(path);
    return stringValue;
  }
  int openForAttributeAccess(  boolean followLinks) throws IOException {
    ...
    try {
      ...
      if (getFileSystem().isSolaris() && x.errno() == EINVAL)       x.setError(ELOOP);
      if (x.errno() == ELOOP)       throw new FileSystemException(getPathForExecptionMessage(),null,x.getMessage() + " or unable to access attributes of symbolic link");
  ...
  void checkRead(){
    ...
    if (sm != null)     sm.checkRead(getPathForPermissionCheck());
  }
  void checkWrite(){
    SecurityManager sm=System.getSecurityManager();
    if (sm != null)     sm.checkWrite(getPathForPermissionCheck());
  }
  void checkDelete(){
    SecurityManager sm=System.getSecurityManager();
    if (sm != null)     sm.checkDelete(getPathForPermissionCheck());
  }
  @Override public UnixPath toAbsolutePath(){
    if (isAbsolute()) {
      return this;
    }
    SecurityManager sm=System.getSecurityManager();
  ...
  @Override public Path toRealPath(  LinkOption... options) throws IOException {
    ...
    for (int i=0; i < absolute.getNameCount(); i++) {
      ...
      if ((element.asByteArray().length == 1) && (element.asByteArray()[0] == '.'))       continue;
  ...
  @Override public WatchKey register(  WatchService watcher,  WatchEvent.Kind<?>[] events,  WatchEvent.Modifier... modifiers) throws IOException {
    ...
    if (!(watcher instanceof AbstractWatchService))     throw new ProviderMismatchException();
    checkRead();
