 ...
 */
  @Override public SecureDirectoryStream<Path> newDirectoryStream(  Path obj,  LinkOption... options) throws IOException {
    ...
    ds.readLock().lock();
    try {
      if (!ds.isOpen())       throw new ClosedDirectoryStreamException();
      int newdfd1=-1;
      int newdfd2=-1;
      long ptr=0L;
      try {
        ...
        if (newdfd1 != -1)         UnixNativeDispatcher.close(newdfd1);
        if (newdfd2 != -1)         UnixNativeDispatcher.close(newdfd2);
        if (x.errno() == UnixConstants.ENOTDIR)         throw new NotDirectoryException(file.toString());
 ...
 */
  ...
  @Override public void deleteFile(  Path file) throws IOException {
    implDelete(file,true,0);
  }
  @Override public void deleteDirectory(  Path dir) throws IOException {
    implDelete(dir,true,AT_REMOVEDIR);
  }
  /** 
 * Rename/move file in this directory to another (open) directory
 */
  @Override public void move(  Path fromObj,  SecureDirectoryStream<Path> dir,  Path toObj) throws IOException {
    UnixPath from=getName(fromObj);
    UnixPath to=getName(toObj);
    if (dir == null)     throw new NullPointerException();
    if (!(dir instanceof UnixSecureDirectoryStream))     throw new ProviderMismatchException();
    ...
    try {
      ...
      try {
        if (!this.ds.isOpen() || !that.ds.isOpen())         throw new ClosedDirectoryStreamException();
        try {
  ...
  @SuppressWarnings("unchecked") private <V extends FileAttributeView>V getFileAttributeViewImpl(  UnixPath file,  Class<V> type,  boolean followLinks){
 ...
 */
  @Override public <V extends FileAttributeView>V getFileAttributeView(  Class<V> type){
    return getFileAttributeViewImpl(null,type,false);
  }
  /** 
 * Returns file attribute view bound to dfd/filename.
 */
  @Override public <V extends FileAttributeView>V getFileAttributeView(  Path obj,  Class<V> type,  LinkOption... options){
    UnixPath file=getName(obj);
    boolean followLinks=Util.followLinks(options);
...
private class BasicFileAttributeViewImpl implements BasicFileAttributeView {
    ...
    int open() throws IOException {
      int oflags=O_RDONLY;
      if (!followLinks)       oflags|=O_NOFOLLOW;
      try {
    ...
    @Override public BasicFileAttributes readAttributes() throws IOException {
      ...
      try {
        ...
        try {
          UnixFileAttributes attrs=(file == null) ? UnixFileAttributes.get(dfd) : UnixFileAttributes.get(dfd,file,followLinks);
    ...
    @Override public void setTimes(    FileTime lastModifiedTime,    FileTime lastAccessTime,    FileTime createTime) throws IOException {
      checkWriteAccess();
      ds.readLock().lock();
      try {
        if (!ds.isOpen())         throw new ClosedDirectoryStreamException();
        ...
        try {
          ...
          try {
            futimes(fd,lastAccessTime.to(TimeUnit.MICROSECONDS),lastModifiedTime.to(TimeUnit.MICROSECONDS));
          }
 catch (          UnixException x) {
            x.rethrowAsIOException(file);
          }
          ...
          if (file != null)           UnixNativeDispatcher.close(fd);
        }
      }
  finally {
        ds.readLock().unlock();
...
private class PosixFileAttributeViewImpl extends BasicFileAttributeViewImpl implements PosixFileAttributeView {
    ...
    @Override public PosixFileAttributes readAttributes() throws IOException {
      SecurityManager sm=System.getSecurityManager();
      if (sm != null) {
        if (file == null)         ds.directory().checkRead();
 else         ds.directory().resolve(file).checkRead();
        sm.checkPermission(new RuntimePermission("accessUserInformation"));
      }
      ds.readLock().lock();
      try {
    ...
    @Override public void setPermissions(    Set<PosixFilePermission> perms) throws IOException {
      ...
      try {
        ...
        try {
          ...
          if (file != null && fd >= 0)           UnixNativeDispatcher.close(fd);
        }
      }
  finally {
        ds.readLock().unlock();
    ...
    @Override public void setOwner(    UserPrincipal owner) throws IOException {
      ...
      if (owner instanceof UnixUserPrincipals.Group)       throw new IOException("'owner' parameter can't be a group");
      int uid=((UnixUserPrincipals.User)owner).uid();
      setOwners(uid,-1);
    }
    @Override public void setGroup(    GroupPrincipal group) throws IOException {
