...
private static class Flags {
  ...
  private static void copyDirectory(  UnixPath source,  UnixFileAttributes attrs,  UnixPath target,  Flags flags) throws IOException {
    ...
    if (!flags.copyBasicAttributes && !flags.copyPosixAttributes && !flags.copyNonPosixAttributes)     return;
    int dfd=-1;
    try {
    ...
    try {
      if (flags.copyPosixAttributes) {
        try {
          ...
          if (flags.failIfUnableToCopyPosix)           x.rethrowAsIOException(target);
        }
      }
      if (flags.copyNonPosixAttributes && (dfd >= 0)) {
        ...
        try {
          ...
          if (flags.failIfUnableToCopyNonPosix)           x.rethrowAsIOException(source);
        }
        if (sfd >= 0) {
      ...
      if (flags.copyBasicAttributes) {
        try {
          ...
          if (flags.failIfUnableToCopyBasic)           x.rethrowAsIOException(target);
        }
      }
      done=true;
    }
  ...
  static void move(  UnixPath source,  UnixPath target,  CopyOption... options) throws IOException {
    ...
    if (flags.atomicMove) {
      try {
        ...
        if (x.errno() == EXDEV) {
          throw new AtomicMoveNotSupportedException(source.getPathForExecptionMessage(),target.getPathForExecptionMessage(),x.errorString());
        }
        x.rethrowAsIOException(source,target);
      }
      return;
    ...
    if (targetExists) {
      if (sourceAttrs.isSameFile(targetAttrs))       return;
      if (!flags.replaceExisting) {
        throw new FileAlreadyExistsException(target.getPathForExecptionMessage());
      }
      try {
        ...
        if (targetAttrs.isDirectory() && (x.errno() == EEXIST || x.errno() == ENOTEMPTY)) {
          throw new DirectoryNotEmptyException(target.getPathForExecptionMessage());
        }
        x.rethrowAsIOException(target);
      }
    }
    ...
    try {
      ...
      if (sourceAttrs.isDirectory() && (x.errno() == EEXIST || x.errno() == ENOTEMPTY)) {
        throw new DirectoryNotEmptyException(source.getPathForExecptionMessage());
      }
      x.rethrowAsIOException(source);
    }
  }
  static void copy(  final UnixPath source,  final UnixPath target,  CopyOption... options) throws IOException {
    ...
    if (targetExists) {
      if (sourceAttrs.isSameFile(targetAttrs))       return;
      if (!flags.replaceExisting)       throw new FileAlreadyExistsException(target.getPathForExecptionMessage());
      try {
        if (targetAttrs.isDirectory()) {
          rmdir(target);
        }
