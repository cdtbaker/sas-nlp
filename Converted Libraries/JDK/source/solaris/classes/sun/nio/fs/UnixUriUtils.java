 ...
 */
  static Path fromUri(  UnixFileSystem fs,  URI uri){
    ...
    if (!uri.toString().startsWith("file:///"))     return new File(uri).toPath();
    String p=uri.getRawPath();
    int len=p.length();
    if (len == 0)     throw new IllegalArgumentException("URI path component is empty");
    if (p.endsWith("/") && len > 1)     len--;
    ...
    while (pos < len) {
      ...
      if (c == '%') {
        ...
        if (b == 0)         throw new IllegalArgumentException("Nul character not allowed");
      }
 else {
        assert c < 0x80;
        b=(byte)c;
    ...
    if (rlen != result.length)     result=Arrays.copyOf(result,rlen);
 ...
 */
  static URI toUri(  UnixPath up){
    ...
    if (sb.charAt(sb.length() - 1) != '/') {
      try {
        if (UnixFileAttributes.get(up,true).isDirectory())         sb.append('/');
      }
