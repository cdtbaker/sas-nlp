 ...
 */
class SolarisUserDefinedFileAttributeView extends AbstractUserDefinedFileAttributeView {
  private byte[] nameAsBytes(  UnixPath file,  String name) throws IOException {
    byte[] bytes=name.getBytes();
    if (bytes.length == 0 || bytes[0] == '.') {
      if (bytes.length <= 1 || (bytes.length == 2 && bytes[1] == '.')) {
        throw new FileSystemException(file.getPathForExecptionMessage(),null,"'" + name + "' is not a valid name");
      }
    }
    return bytes;
  }
    ...
    try {
      try {
        ...
        try {
          ...
          if (size > Integer.MAX_VALUE)           throw new ArithmeticException("Extended attribute value too large");
          return (int)size;
        }
  finally {
          close(afd);
  ...
  @Override public int read(  String name,  ByteBuffer dst) throws IOException {
    ...
    try {
      try {
        ...
        try {
          if (fc.size() > dst.remaining())           throw new IOException("Extended attribute file too large");
          int total=0;
        ...
        throw new FileSystemException(file.getPathForExecptionMessage(),null,"Unable to read extended attribute '" + name + "': "+ x.getMessage());
      }
    }
  finally {
      close(fd);
  ...
  @Override public int write(  String name,  ByteBuffer src) throws IOException {
    ...
    try {
      try {
        int afd=openat(fd,nameAsBytes(file,name),(O_CREAT | O_WRONLY | O_TRUNC| O_XATTR),UnixFileModeAttribute.ALL_PERMISSIONS);
        ...
        throw new FileSystemException(file.getPathForExecptionMessage(),null,"Unable to write extended attribute '" + name + "': "+ x.getMessage());
      }
    }
  finally {
      close(fd);
 ...
 */
  ...
  private static void copyExtendedAttribute(  int ofd,  byte[] name,  int nfd) throws UnixException {
    ...
    try {
      int dst=openat(nfd,name,(O_CREAT | O_WRONLY | O_TRUNC| O_XATTR),UnixFileModeAttribute.ALL_PERMISSIONS);
      try {
