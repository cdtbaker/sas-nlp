...
class LinuxDosFileAttributeView extends UnixFileAttributeViews.Basic implements DosFileAttributeView {
  ...
  @Override public DosFileAttributes readAttributes() throws IOException {
    file.checkRead();
 ...
 */
  private int getDosAttribute(  int fd) throws UnixException {
    ...
    try {
      int len=LinuxNativeDispatcher.fgetxattr(fd,DOS_XATTR_NAME_AS_BYTES,buffer.address(),size);
      if (len > 0) {
        if (unsafe.getByte(buffer.address() + len - 1) == 0)         len--;
        byte[] buf=new byte[len];
        unsafe.copyMemory(null,buffer.address(),buf,Unsafe.ARRAY_BYTE_BASE_OFFSET,len);
