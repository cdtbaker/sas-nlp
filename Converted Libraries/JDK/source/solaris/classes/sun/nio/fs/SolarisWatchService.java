...
class SolarisWatchService extends AbstractWatchService {
  ...
  @Override WatchKey register(  Path dir,  WatchEvent.Kind<?>[] events,  WatchEvent.Modifier... modifiers) throws IOException {
...
private class SolarisWatchKey extends AbstractWatchKey implements DirectoryNode {
    ...
    @Override public void addChild(    Path name,    EntryNode node){
      if (children == null)       children=new HashMap<Path,EntryNode>();
      children.put(name,node);
    }
    @Override public void removeChild(    Path name){
      children.remove(name);
    }
    @Override public EntryNode getChild(    Path name){
      if (children != null)       return children.get(name);
      return null;
    }
  }
  /** 
 ...
 */
    ...
    @Override void wakeup() throws IOException {
      try {
        portSend(port,0);
      }
 catch (      UnixException x) {
    ...
    @Override Object implRegister(    Path obj,    Set<? extends WatchEvent.Kind<?>> events,    WatchEvent.Modifier... modifiers){
      if (modifiers.length > 0) {
        for (        WatchEvent.Modifier modifier : modifiers) {
          if (modifier == null)           return new NullPointerException();
          if (modifier instanceof com.sun.nio.file.SensitivityWatchEventModifier)           continue;
    ...
    @Override void implCancelKey(    WatchKey obj){
 ...
 */
    void processDirectoryEvents(    SolarisWatchKey key,    int mask){
      if ((mask & (FILE_MODIFIED | FILE_ATTRIB)) != 0) {
        registerChildren(key.getDirectory(),key,key.events().contains(StandardWatchEventKinds.ENTRY_CREATE));
      }
    }
    /** 
 ...
 */
    boolean processEntryEvents(    EntryNode node,    int mask){
      ...
      if (((mask & (FILE_REMOVED)) != 0) && events.contains(StandardWatchEventKinds.ENTRY_DELETE)) {
        ...
        if (removed)         key.signalEvent(StandardWatchEventKinds.ENTRY_DELETE,node.name());
      }
      return false;
    }
    /** 
 ...
 */
    void registerChildren(    UnixPath dir,    SolarisWatchKey parent,    boolean sendEvents){
      ...
      if (parent.events().contains(StandardWatchEventKinds.ENTRY_MODIFY))       events|=(FILE_MODIFIED | FILE_ATTRIB);
 ...
 */
    void updateEvents(    SolarisWatchKey key,    Set<? extends WatchEvent.Kind<?>> events){
      ...
      if (key.children != null) {
        ...
        if (isModifyEnabled)         ev|=(FILE_MODIFIED | FILE_ATTRIB);
        for (        Map.Entry<Path,EntryNode> entry : key.children.entrySet()) {
          long object=entry.getValue().object();
          try {
            portAssociate(port,PORT_SOURCE_FILE,object,ev);
          }
 catch (          UnixException x) {
          }
        }
