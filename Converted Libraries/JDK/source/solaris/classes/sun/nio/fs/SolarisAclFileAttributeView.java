 ...
 * NFSv4 ACLs on ZFS.
 */
class SolarisAclFileAttributeView extends AbstractAclFileAttributeView {
...
if (who == null) throw new AssertionError("ACE who not handled");
}
 else {
if ((flags & ACE_IDENTIFIER_GROUP) > 0) who=UnixUserPrincipals.fromGid(uid);
 else who=UnixUserPrincipals.fromUid(uid);
}
...
if ((flags & ACE_NO_PROPAGATE_INHERIT_ACE) > 0) aceFlags.add(AclEntryFlag.NO_PROPAGATE_INHERIT);
if ((flags & ACE_INHERIT_ONLY_ACE) > 0) aceFlags.add(AclEntryFlag.INHERIT_ONLY);
AclEntry ace=AclEntry.newBuilder().setType(aceType).setPrincipal(who).setPermissions(aceMask).setFlags(aceFlags).build();
acl.add(ace);
}
return acl;
}
...
if (enabled == _ACL_ACE_ENABLED) return true;
}
...
throw new FileSystemException(file.getPathForExecptionMessage(),null,x.getMessage() + " (file system does not support NFSv4 ACLs)");
}
...
if (x.errno() == EINVAL && (n < 3)) throw new IOException("ACL must contain at least 3 entries");
x.rethrowAsIOException(file);
}
 finally {
unsafe.freeMemory(address);
...
if (owner instanceof UnixUserPrincipals.Group) throw new IOException("'owner' parameter is a group");
