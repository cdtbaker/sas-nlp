...
static class User implements UserPrincipal {
    ...
    int uid(){
      if (isGroup)       throw new AssertionError();
      return id;
    }
    int gid(){
    ...
    @Override public boolean equals(    Object obj){
      ...
      if (this.id == -1 && other.id == -1)       return this.name.equals(other.name);
      return true;
    }
...
static class Group extends User implements GroupPrincipal {
  ...
  static UserPrincipal lookupUser(  String name) throws IOException {
    if (name.equals(SPECIAL_OWNER.getName()))     return SPECIAL_OWNER;
    if (name.equals(SPECIAL_GROUP.getName()))     return SPECIAL_GROUP;
    if (name.equals(SPECIAL_EVERYONE.getName()))     return SPECIAL_EVERYONE;
