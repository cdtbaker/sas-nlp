...
public class SctpServerChannelImpl extends SctpServerChannel implements SelChImpl {
  ...
  private enum ChannelState {  UNINITIALIZED,   INUSE,   KILLPENDING,   KILLED}
 ...
 */
  ...
  @Override public SctpServerChannel bind(  SocketAddress local,  int backlog) throws IOException {
synchronized (lock) {
synchronized (stateLock) {
  ...
  @Override public SctpServerChannel bindAddress(  InetAddress address) throws IOException {
    return bindUnbindAddress(address,true);
  }
  @Override public SctpServerChannel unbindAddress(  InetAddress address) throws IOException {
...
synchronized (stateLock) {
  ...
  @Override public SctpChannel accept() throws IOException {
synchronized (lock) {
      if (!isOpen())       throw new ClosedChannelException();
      if (!isBound())       throw new NotYetBoundException();
      SctpChannel sc=null;
...
synchronized (stateLock) {
      ...
      if (thread != 0)       NativeThread.signal(thread);
 ...
 */
  private boolean translateReadyOps(  int ops,  int initialOps,  SelectionKeyImpl sk){
    ...
    if (((ops & PollArrayWrapper.POLLIN) != 0) && ((intOps & SelectionKey.OP_ACCEPT) != 0))     newOps|=SelectionKey.OP_ACCEPT;
    sk.nioReadyOps(newOps);
    return (newOps & ~oldOps) != 0;
  }
  ...
  @Override public <T>SctpServerChannel setOption(  SctpSocketOption<T> name,  T value) throws IOException {
...
synchronized (stateLock) {
  ...
  @Override public <T>T getOption(  SctpSocketOption<T> name) throws IOException {
    if (name == null)     throw new NullPointerException();
    if (!supportedOptions().contains(name))     throw new UnsupportedOperationException("'" + name + "' not supported");
synchronized (stateLock) {
      if (!isOpen())       throw new ClosedChannelException();
...
synchronized (stateLock) {
  ...
  private static native int accept0(  FileDescriptor ssfd,  FileDescriptor newfd,  InetSocketAddress[] isaa) throws IOException ;
static {
    Util.load();
