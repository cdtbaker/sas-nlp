...
synchronized (this) {
  ...
  @Override void startPoll(  int fd,  int events){
    try {
...
private class EventHandlerTask implements Runnable {
    public void run(){
      ...
      try {
        ...
        if (remaining == 0 && isShutdown())         implClose();
      }
    }
  }
  private static native void init();
  private static native int portCreate() throws IOException ;
  private static native void portAssociate(  int port,  int source,  long object,  int events) throws IOException ;
  private static native void portGet(  int port,  long pe) throws IOException ;
  private static native int portGetn(  int port,  long address,  int max) throws IOException ;
