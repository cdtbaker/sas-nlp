...
static {
  ...
  private void updateEvents(){
    ...
    int events=0;
    if (readPending)     events|=Port.POLLIN;
    if (connectPending || writePending)     events|=Port.POLLOUT;
    if (events != 0)     port.startPoll(fdVal,events);
  }
...
synchronized (updateLock) {
    ...
    if (finishRead) {
      if (finishWrite)       finishWrite(false);
 ...
 */
  @Override public void onEvent(  int events,  boolean mayInvokeDirect){
  ...
  @Override public void onCancel(  PendingFuture<?,?> task){
    if (task.getContext() == OpType.CONNECT)     killConnect();
    if (task.getContext() == OpType.READ)     killReading();
    if (task.getContext() == OpType.WRITE)     killWriting();
  }
...
synchronized (stateLock) {
  ...
  @Override @SuppressWarnings("unchecked") <A>Future<Void> implConnect(  SocketAddress remote,  A attachment,  CompletionHandler<Void,? super A> handler){
    if (!isOpen()) {
...
synchronized (stateLock) {
    ...
    try {
      begin();
      if (notifyBeforeTcpConnect)       NetHooks.beforeTcpConnect(fd,isa.getAddress(),isa.getPort());
...
synchronized (updateLock) {
      ...
      if (!(exc instanceof AsynchronousCloseException))       lockAndUpdateEvents();
      end();
    }
 ...
 */
  @Override @SuppressWarnings("unchecked") <V extends Number,A>Future<V> implRead(  boolean isScatteringRead,  ByteBuffer dst,  ByteBuffer[] dsts,  long timeout,  TimeUnit unit,  A attachment,  CompletionHandler<V,? super A> handler){
 ...
 * Initiates a read or scattering read operation
 */
  @Override @SuppressWarnings("unchecked") <V extends Number,A>Future<V> implWrite(  boolean isGatheringWrite,  ByteBuffer src,  ByteBuffer[] srcs,  long timeout,  TimeUnit unit,  A attachment,  CompletionHandler<V,? super A> handler){
