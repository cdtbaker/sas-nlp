...
public class SctpResultContainer {
  ...
  SctpMessageInfoImpl getMessageInfo(){
    assert type() == MESSAGE;
    if (value instanceof SctpMessageInfoImpl)     return (SctpMessageInfoImpl)value;
    return null;
  }
  SctpSendFailed getSendFailed(){
    assert type() == SEND_FAILED;
    if (value instanceof SctpSendFailed)     return (SctpSendFailed)value;
    return null;
  }
  SctpAssocChange getAssociationChanged(){
    assert type() == ASSOCIATION_CHANGED;
    if (value instanceof SctpAssocChange)     return (SctpAssocChange)value;
    return null;
  }
  SctpPeerAddrChange getPeerAddressChanged(){
    assert type() == PEER_ADDRESS_CHANGED;
    if (value instanceof SctpPeerAddrChange)     return (SctpPeerAddrChange)value;
    return null;
  }
  SctpShutdown getShutdown(){
    assert type() == SHUTDOWN;
    if (value instanceof SctpShutdown)     return (SctpShutdown)value;
    return null;
  }
