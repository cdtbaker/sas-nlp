...
public class SctpMultiChannelImpl extends SctpMultiChannel implements SelChImpl {
  ...
  private enum ChannelState {  UNINITIALIZED,   KILLPENDING,   KILLED}
...
;
  ...
  @Override public SctpMultiChannel bind(  SocketAddress local,  int backlog) throws IOException {
...
synchronized (stateLock) {
  ...
  @Override public SctpMultiChannel bindAddress(  InetAddress address) throws IOException {
    return bindUnbindAddress(address,true);
  }
  @Override public SctpMultiChannel unbindAddress(  InetAddress address) throws IOException {
...
synchronized (stateLock) {
  ...
  @Override public Set<Association> associations() throws ClosedChannelException, NotYetBoundException {
synchronized (stateLock) {
      if (!isOpen())       throw new ClosedChannelException();
      if (!isBound())       throw new NotYetBoundException();
 ...
 */
  ...
  @Override public boolean translateAndSetReadyOps(  int ops,  SelectionKeyImpl sk){
...
synchronized (stateLock) {
  ...
  @Override public <T>SctpMultiChannel setOption(  SctpSocketOption<T> name,  T value,  Association association) throws IOException {
...
synchronized (stateLock) {
  ...
  @Override @SuppressWarnings("unchecked") public <T>T getOption(  SctpSocketOption<T> name,  Association association) throws IOException {
    if (name == null)     throw new NullPointerException();
    if (!supportedOptions().contains(name))     throw new UnsupportedOperationException("'" + name + "' not supported");
synchronized (stateLock) {
      if (association != null && (name.equals(SCTP_PRIMARY_ADDR) || name.equals(SCTP_SET_PEER_PRIMARY_ADDR))) {
...
synchronized (stateLock) {
 ...
 while (handler == null ? true : (invokeNotificationHandler(resultContainer,handler,attachment) == HandlerResult.CONTINUE));
  ...
  private int receive(  int fd,  ByteBuffer dst,  SctpResultContainer resultContainer) throws IOException {
    int pos=dst.position();
    int lim=dst.limit();
    assert (pos <= lim);
    int rem=(pos <= lim ? lim - pos : 0);
    if (dst instanceof DirectBuffer && rem > 0)     return receiveIntoNativeBuffer(fd,resultContainer,dst,rem,pos);
  ...
  private int receiveIntoNativeBuffer(  int fd,  SctpResultContainer resultContainer,  ByteBuffer bb,  int rem,  int pos) throws IOException {
...
switch (not.event()) {
case COMM_UP:
        Association newAssociation=new SctpAssociationImpl(sac.assocId(),sac.maxInStreams(),sac.maxOutStreams());
...
synchronized (stateLock) {
if (streamNumber < 0 || streamNumber >= assoc.maxOutboundStreams()) throw new InvalidStreamException();
}
}
...
if (addr != null) {
if (!assoc.equals(addressMap.get(addr))) throw new IllegalArgumentException("given preferred address is not part of this association");
address=addr;
}
}
 else if (addr != null) {
...
SecurityManager sm=System.getSecurityManager();
if (sm != null) sm.checkConnect(addr.getAddress().getHostAddress(),addr.getPort());
}
}
 else {
...
if (src instanceof DirectBuffer) return sendFromNativeBuffer(fd,src,target,assocId,streamNumber,unordered,ppid);
...
assert (pos <= lim);
int rem=(pos <= lim ? lim - pos : 0);
int written=send0(fd,((DirectBuffer)bb).address() + pos,rem,target,assocId,streamNumber,unordered,ppid);
if (written > 0) bb.position(pos + written);
return written;
}
@Override public SctpMultiChannel shutdown(Association association) throws IOException {
synchronized (stateLock) {
checkAssociation(association);
if (!isOpen()) throw new ClosedChannelException();
...
@Override public Set<SocketAddress> getRemoteAddresses(Association association) throws IOException {
synchronized (stateLock) {
checkAssociation(association);
if (!isOpen()) throw new ClosedChannelException();
try {
...
@Override public SctpChannel branch(Association association) throws IOException {
synchronized (stateLock) {
checkAssociation(association);
if (!isOpen()) throw new ClosedChannelException();
...
private static int receive0(int fd,SctpResultContainer resultContainer,long address,int length) throws IOException {
return SctpChannelImpl.receive0(fd,resultContainer,address,length,false);
}
private static int send0(int fd,long address,int length,SocketAddress target,int assocId,int streamNumber,boolean unordered,int ppid) throws IOException {
return SctpChannelImpl.send0(fd,address,length,target,assocId,streamNumber,unordered,ppid);
}
static {
Util.load();
