...
public class SctpNet {
  ...
  /** 
 * @param oneToOneif {@code true} returns a one-to-one sctp socket, otherwise
 ...
 */
  ...
  static void bindx(  int fd,  InetAddress[] addrs,  int port,  boolean add) throws IOException {
    bindx(fd,addrs,port,addrs.length,add,IPv4MappedAddresses());
  }
  static Set<SocketAddress> getLocalAddresses(  int fd) throws IOException {
    ...
    if (saa != null) {
      ...
      for (      SocketAddress sa : saa)       set.add(sa);
    }
    return set;
  }
  ...
  static void setSocketOption(  int fd,  SctpSocketOption name,  Object value,  int assocId) throws IOException {
    ...
    if (!type.isInstance(value))     throw new IllegalArgumentException("Invalid option value");
 ...
 else     if (name.equals(SCTP_PRIMARY_ADDR) || name.equals(SCTP_SET_PEER_PRIMARY_ADDR)) {
      SocketAddress addr=(SocketAddress)value;
      if (addr == null)       throw new IllegalArgumentException("Invalid option value");
      Net.checkAddress(addr);
      InetSocketAddress netAddr=(InetSocketAddress)addr;
      if (name.equals(SCTP_PRIMARY_ADDR)) {
        setPrimAddrOption0(fd,assocId,netAddr.getAddress(),netAddr.getPort());
      }
 else {
        setPeerPrimAddrOption0(fd,assocId,netAddr.getAddress(),netAddr.getPort(),IPv4MappedAddresses());
      }
    }
 else     if (name.equals(SCTP_DISABLE_FRAGMENTS) || name.equals(SCTP_EXPLICIT_COMPLETE) || name.equals(SCTP_FRAGMENT_INTERLEAVE)|| name.equals(SCTP_NODELAY)|| name.equals(SO_SNDBUF)|| name.equals(SO_RCVBUF)|| name.equals(SO_LINGER)) {
  ...
  static void setIntOption(  int fd,  SctpSocketOption name,  Object value) throws IOException {
    ...
    if (name == SO_RCVBUF || name == SO_SNDBUF) {
      ...
      int i=((Integer)value).intValue();
      if (i < 0 || i > 2)       throw new IllegalArgumentException("Invalid value for SCTP_FRAGMENT_INTERLEAVE");
    }
    int arg;
  ...
  static Object getIntOption(  int fd,  SctpSocketOption name) throws IOException {
    Class<?> type=name.type();
    if (type != Integer.class && type != Boolean.class)     throw new AssertionError("Should not reach here");
    if (!(name instanceof SctpStdSocketOption))     throw new AssertionError("Should not reach here");
    int value=getIntOption0(fd,((SctpStdSocketOption)name).constValue());
    if (type == Integer.class) {
  ...
  static native void bindx(  int fd,  InetAddress[] addrs,  int port,  int length,  boolean add,  boolean preferIPv6) throws IOException ;
  static native int getIntOption0(  int fd,  int opt) throws IOException ;
  static native void setIntOption0(  int fd,  int opt,  int arg) throws IOException ;
  static native SocketAddress[] getLocalAddresses0(  int fd) throws IOException ;
  static native SocketAddress[] getRemoteAddresses0(  int fd,  int assocId) throws IOException ;
  static native int branch0(  int fd,  int assocId) throws IOException ;
  static native void setPrimAddrOption0(  int fd,  int assocId,  InetAddress ia,  int port) throws IOException ;
  static native void setPeerPrimAddrOption0(  int fd,  int assocId,  InetAddress ia,  int port,  boolean preferIPv6) throws IOException ;
  static native SocketAddress getPrimAddrOption0(  int fd,  int assocId) throws IOException ;
  static native void getInitMsgOption0(  int fd,  int[] retVals) throws IOException ;
  static native void setInitMsgOption0(  int fd,  int arg1,  int arg2) throws IOException ;
