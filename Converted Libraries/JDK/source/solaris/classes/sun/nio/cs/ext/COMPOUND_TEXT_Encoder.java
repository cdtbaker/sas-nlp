 ...
 */
  private static final Map<String,CharsetEncoder> encodingToEncoderMap=Collections.synchronizedMap(new HashMap<String,CharsetEncoder>(21,1.0f));
...
static {
  ...
  public COMPOUND_TEXT_Encoder(  Charset cs){
    super(cs,(float)(CompoundTextSupport.MAX_CONTROL_SEQUENCE_LEN + 2),(float)(CompoundTextSupport.MAX_CONTROL_SEQUENCE_LEN + 2));
    try {
  ...
  protected CoderResult encodeLoop(  CharBuffer src,  ByteBuffer des){
    ...
    try {
      while (inOff < inEnd && cr.isUnderflow()) {
        ...
        if (charBuf[0] <= '\u0008' || (charBuf[0] >= '\u000B' && charBuf[0] <= '\u001F') || (charBuf[0] >= '\u0080' && charBuf[0] <= '\u009F')) {
          charBuf[0]='?';
        }
 ...
 else           if (escSequence[1] == (byte)0x25 && escSequence[2] == (byte)0x2F) {
        ...
        if (nonStandardCharsetBuffer == null) {
          ...
          bytebuf.flip();
          nonStandardCharsetBuffer.write(byteBuf,0,bytebuf.limit());
          numNonStandardChars++;
        }
        inOff++;
      }
  ...
  protected CoderResult implFlush(  ByteBuffer out){
    CoderResult cr=(nonStandardCharsetBuffer != null) ? flushNonStandardCharsetBuffer(out) : flushEncoder(encoder,out);
    reset();
    return cr;
  }
  ...
  private CoderResult flushNonStandardCharsetBuffer(  ByteBuffer out){
    if (numNonStandardChars > 0) {
      byte[] flushBuf=new byte[(int)encoder.maxBytesPerChar() * numNonStandardChars];
    ...
    if (out.remaining() < (numBytes - nonStandardBytesOff) + nonStandardBytesOff * (((numBytes - nonStandardBytesOff) / ((1 << 14) - 1)) + 1)) {
      return CoderResult.OVERFLOW;
    }
    byte[] nonStandardBytes=nonStandardCharsetBuffer.toByteArray();
    do {
      out.put((byte)0x1B);
      out.put((byte)0x25);
      out.put((byte)0x2F);
      out.put(nonStandardBytes[3]);
      int toWrite=Math.min(numBytes - nonStandardBytesOff,(1 << 14) - 1 - nonStandardEncodingLen);
      out.put((byte)(((toWrite + nonStandardEncodingLen) / 0x80) | 0x80));
      out.put((byte)(((toWrite + nonStandardEncodingLen) % 0x80) | 0x80));
 ...
 */
  ...
  protected void implReplaceWith(  byte[] newReplacement){
    if (encoder != null)     encoder.replaceWith(newReplacement);
  }
  /** 
 ...
 */
  ...
  private void initEncoder(  CharsetEncoder enc){
    try {
      enc.onUnmappableCharacter(CodingErrorAction.REPLACE).replaceWith(replacement());
    }
