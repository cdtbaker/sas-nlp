...
public class COMPOUND_TEXT_Decoder extends CharsetDecoder {
  ...
  private ByteArrayOutputStream queue=new ByteArrayOutputStream(), encodingQueue=new ByteArrayOutputStream();
  private CharsetDecoder glDecoder, grDecoder, nonStandardDecoder, lastDecoder;
...
CoderResult cr=CoderResult.UNDERFLOW;
if ((newByte >= 0x00 && newByte <= 0x1F) || (newByte >= 0x80 && newByte <= 0x9F)) {
char newChar;
switch (newByte) {
case 0x1B:
...
if (!cb.hasRemaining()) return CoderResult.OVERFLOW;
 else cb.put(newChar);
}
 else {
...
break;
case 0x31:
return malformedInput((versionSequenceAllowed) ? ERR_VERMANDATORY : ERR_VERSTART);
default :
return escapeSequenceOther(newByte);
}
}
...
if (newByte >= 0x21 && newByte <= (state == CHARSET_NLIIF ? 0x23 : 0x2F)) {
state=CHARSET_NLIF;
queue.write(newByte);
}
 else if (newByte >= 0x40 && newByte <= 0x7E) {
...
if (newByte >= 0x21 && newByte <= (state == CHARSET_NRIIF ? 0x23 : 0x2F)) {
state=CHARSET_NRIF;
queue.write(newByte);
}
 else if (newByte >= 0x40 && newByte <= 0x7E) {
...
if (newByte >= 0x21 && newByte <= (state == CHARSET_LIIF ? 0x23 : 0x2F)) {
state=CHARSET_LIF;
queue.write(newByte);
return CoderResult.UNDERFLOW;
}
...
if (newByte >= 0x21 && newByte <= (state == CHARSET_RIIF ? 0x23 : 0x2F)) {
state=CHARSET_RIF;
queue.write(newByte);
return CoderResult.UNDERFLOW;
}
...
if (state == CHARSET_NLIIF || state == CHARSET_NLIF || state == CHARSET_LIIF || state == CHARSET_LIF) {
...
state=NORMAL_BYTES;
 ...
 else if (state == CHARSET_NRIIF || state == CHARSET_NRIF || state == CHARSET_RIIF || state == CHARSET_RIF) {
...
int badInputLength=queue.size() + 1;
queue.reset();
...
CoderResult cr=CoderResult.UNDERFLOW;
if (lastDecoder != null) cr=flushDecoder(lastDecoder,out);
if (state != NORMAL_BYTES) cr=CoderResult.malformedForLength(0);
reset();
return cr;
}
/** 
...
if (glDecoder != null) glDecoder.onMalformedInput(newAction);
if (grDecoder != null) grDecoder.onMalformedInput(newAction);
if (nonStandardDecoder != null) nonStandardDecoder.onMalformedInput(newAction);
}
protected void implOnUnmappableCharacter(CodingErrorAction newAction){
if (glDecoder != null) glDecoder.onUnmappableCharacter(newAction);
if (grDecoder != null) grDecoder.onUnmappableCharacter(newAction);
if (nonStandardDecoder != null) nonStandardDecoder.onUnmappableCharacter(newAction);
}
protected void implReplaceWith(String newReplacement){
if (glDecoder != null) glDecoder.replaceWith(newReplacement);
if (grDecoder != null) grDecoder.replaceWith(newReplacement);
if (nonStandardDecoder != null) nonStandardDecoder.replaceWith(newReplacement);
}
private void initDecoder(CharsetDecoder dec){
dec.onUnmappableCharacter(CodingErrorAction.REPLACE).replaceWith(replacement());
}
}
