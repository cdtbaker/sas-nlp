...
static {
    ...
    osname=java.security.AccessController.doPrivileged(new sun.security.action.GetPropertyAction("os.name"));
  }
  ...
  String[] lpcFirstCom={"/usr/sbin/lpc status | grep : | sed -ne '1,1 s/://p'","/usr/sbin/lpc status | grep -E '^[ 0-9a-zA-Z_-]*@' | awk -F'@' '{print $1}'"};
  String[] lpcAllCom={"/usr/sbin/lpc status | grep : | sed -e 's/://'","/usr/sbin/lpc -a status | grep -E '^[ 0-9a-zA-Z_-]*@' | awk -F'@' '{print $1}' | sort"};
  ...
  public synchronized void refreshServices(){
    ...
    for (int p=0; p < printers.length; p++) {
      ...
      if ((defaultPrintService != null) && printers[p].equals(defaultPrintService.getName())) {
        ...
        if (printServices == null) {
          ...
          if (CUPSPrinter.isCupsRunning()) {
            try {
              printerList.add(new IPPPrintService(printers[p],printerURIs[p],true));
            }
 catch (            Exception e) {
  ...
  public synchronized PrintService getDefaultPrintService(){
    ...
    if (defaultPrintService == null) {
      if (CUPSPrinter.isCupsRunning()) {
        try {
          PrintService defaultPS=new IPPPrintService(defaultPrinter,new URL("http://" + CUPSPrinter.getServer() + ":"+ CUPSPrinter.getPort()+ "/"+ defaultPrinter));
...
synchronized (this) {
      ...
      for (int i=0; i < lookupListeners.size(); i++) {
        listener=(BackgroundLookupListener)lookupListeners.elementAt(i);
  ...
  private String getDefaultPrinterNameBSD(){
    ...
    if ((cmdIndex == BSD_LPD_NG) && (names[0].startsWith("missingprinter"))) {
      return null;
    }
    return names[0];
  }
  ...
  private String[] getAllPrinterNamesSysV(){
    ...
    for (int i=0; i < names.length; i++) {
      if (!names[i].equals("_default") && !names[i].equals(defaultPrinter) && !names[i].equals("")) {
...
private class PrinterChangeListener extends Thread {
    public void run(){
      ...
      while (true) {
        ...
        if ((printServices != null) && (printServices.length > minRefreshTime)) {
