 ...
 */
  ...
  public static int getDatatransferTimeout(){
    Integer dt=(Integer)AccessController.doPrivileged(new GetIntegerAction("sun.awt.datatransfer.timeout"));
    if (dt == null || dt <= 0) {
 ...
 */
  @Override public boolean isNativeGTKAvailable(){
 ...
 */
  protected Object lazilyLoadGTKIcon(  String longname){
    ...
    TextDirection dir=("ltr".equals(str[4]) ? TextDirection.LTR : TextDirection.RTL);
 ...
 */
  public BufferedImage getStockIcon(  final int widgetType,  final String stockId,  final int iconSize,  final int direction,  final String detail){
    if (!loadGTK()) {
      return null;
    }
 else {
 ...
 */
  public void loadIconCallback(  byte[] data,  int width,  int height,  int rowStride,  int bps,  int channels,  boolean alpha){
    ...
    ColorModel colorModel=new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB),alpha,false,ColorModel.TRANSLUCENT,DataBuffer.TYPE_BYTE);
  ...
  private native boolean load_stock_icon(  int widget_type,  String stock_id,  int iconSize,  int textDirection,  String detail);
  ...
  protected RenderingHints getDesktopAAHints(){
    ...
    boolean aa=Boolean.valueOf(((aaValue instanceof Number) && ((Number)aaValue).intValue() != 0));
    Object aaHint;
    if (aa) {
      String subpixOrder=(String)getDesktopProperty("gnome.Xft/RGBA");
  ...
  private native boolean gtkCheckVersionImpl(  int major,  int minor,  int micro);
  /** 
 * Returns {@code true} if the GTK+ library is compatible with the given
 * version.
 * @param majorThe required major version.
 * @param minorThe required minor version.
 * @param microThe required micro version.
