 ...
 */
    ...
    private void needBytes(    int n) throws IndexOutOfBoundsException {
      ...
      throw new IndexOutOfBoundsException("at " + idx + " need "+ n+ " length "+ dlen);
    }
    ...
    private int getCARD16() throws IndexOutOfBoundsException {
      ...
      int val;
      if (isLittle) {
        val=((data[idx + 0] & 0xff)) | ((data[idx + 1] & 0xff) << 8);
      }
 else {
        val=((data[idx + 0] & 0xff) << 8) | ((data[idx + 1] & 0xff));
      }
      idx+=2;
      return val;
    }
    private int getINT32() throws IndexOutOfBoundsException {
      ...
      int val;
      if (isLittle) {
        val=((data[idx + 0] & 0xff)) | ((data[idx + 1] & 0xff) << 8) | ((data[idx + 2] & 0xff) << 16)| ((data[idx + 3] & 0xff) << 24);
      }
 else {
        val=((data[idx + 0] & 0xff) << 24) | ((data[idx + 1] & 0xff) << 16) | ((data[idx + 2] & 0xff) << 8)| ((data[idx + 3] & 0xff) << 0);
      }
      idx+=4;
      return val;
    }
 ...
 */
    private void updateOne(    long currentSerial) throws IndexOutOfBoundsException, IllegalArgumentException {
      ...
      if (type == TYPE_INTEGER) {
        ...
        int b=getCARD16();
        int a=getCARD16();
        value=new Color(r / 65535.0f,g / 65535.0f,b / 65535.0f,a / 65535.0f);
      }
 else {
        throw new IllegalArgumentException("Unknown type: " + type);
      }
