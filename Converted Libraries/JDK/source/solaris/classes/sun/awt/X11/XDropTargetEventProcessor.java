...
final class XDropTargetEventProcessor {
  ...
  private boolean doProcessEvent(  XEvent ev){
    if (ev.get_type() == (int)XConstants.DestroyNotify && protocol != null && ev.get_xany().get_window() == protocol.getSourceWindow()) {
      protocol.cleanup();
      protocol=null;
      return false;
    }
    if (ev.get_type() == (int)XConstants.PropertyNotify) {
      ...
      if (xproperty.get_atom() == MotifDnDConstants.XA_MOTIF_DRAG_RECEIVER_INFO.getAtom()) {
    ...
    if (protocol != null) {
      if (protocol.getMessageType(xclient) != XDropTargetProtocol.UNKNOWN_MESSAGE) {
    ...
    if (protocol == null) {
      ...
      while (dropTargetProtocols.hasNext()) {
        ...
        if (dropTargetProtocol.getMessageType(xclient) == XDropTargetProtocol.UNKNOWN_MESSAGE) {
          continue;
        }
