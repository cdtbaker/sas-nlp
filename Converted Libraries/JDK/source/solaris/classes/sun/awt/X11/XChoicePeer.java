...
public class XChoicePeer extends XComponentPeer implements ChoicePeer, ToplevelStateListener {
  ...
  void preInit(  XCreateWindowParams params){
    ...
    helper=new ListHelper(unfurledChoice,getGUIcolors(),numItems,false,true,false,target.getFont(),MAX_UNFURLED_ITEMS,TEXT_SPACE,ITEM_MARGIN,BORDER_WIDTH,SCROLLBAR_WIDTH);
  }
...
case KeyEvent.VK_KP_DOWN:
{
        if (helper.getItemCount() > 1) {
          ...
          postEvent(new ItemEvent((Choice)target,ItemEvent.ITEM_STATE_CHANGED,((Choice)target).getItem(newIdx),ItemEvent.SELECTED));
          repaint();
        }
        break;
      }
...
if (unfurled) {
if (!(isMouseEventInChoice(e) || unfurledChoice.isMouseEventInside(e))) {
hidePopdownMenu();
}
...
if (!firstPress && !isMouseInListArea && isMouseEventInside&& dragging) {
hidePopdownMenu();
}
if (!helper.isEmpty()) {
...
if (e.getButton() == MouseEvent.BUTTON1 && (!firstPress || wasDragged)) {
postEvent(new ItemEvent((Choice)target,ItemEvent.ITEM_STATE_CHANGED,((Choice)target).getItem(newIdx),ItemEvent.SELECTED));
}
if (choiceListener != null) {
choiceListener.unfurledChoiceClosing();
...
return new Dimension(w + TEXT_XPAD + WIDGET_OFFSET,fm.getMaxAscent() + fm.getMaxDescent() + TEXT_YPAD);
}
public void layout(){
}
/** 
...
height=2 * BORDER_WIDTH + numItemsDisplayed * (helper.getItemHeight() + 2 * ITEM_MARGIN);
}
 else {
x=global.x;
y=global.y + XChoicePeer.this.height;
width=Math.max(XChoicePeer.this.width,helper.getMaxItemWidth() + 2 * (BORDER_WIDTH + ITEM_MARGIN + TEXT_SPACE) + (helper.isVSBVisible() ? SCROLLBAR_WIDTH : 0));
height=2 * BORDER_WIDTH + numItemsDisplayed * (helper.getItemHeight() + 2 * ITEM_MARGIN);
}
if (x < 0) {
x=0;
...
public void toFront(){
if (choiceListener != null) choiceListener.unfurledChoiceOpening(helper);
...
log.fine("transX=" + transX + ", transY="+ transY+ ",width="+ width+ ", height="+ height+ ", newIdx="+ newIdx+ " on "+ target);
}
if ((newIdx >= 0) && (newIdx < helper.getItemCount()) && (newIdx != helper.getSelectedIndex())) {
...
helper.paintAllItems(g,colors,getBounds());
}
...
public void run(){
if (target.isFocusable() && getParentTopLevel().isFocusableWindow()) {
...
int eventId=e.getID();
if ((unfurledChoice.isMouseEventInside(me) || (!firstPress && eventId == MouseEvent.MOUSE_DRAGGED))) {
return handleMouseEventByChoice(me);
}
if (eventId == MouseEvent.MOUSE_MOVED) {
return handleMouseEventByChoice(me);
}
if (!firstPress && !(isMouseEventInChoice(me) || unfurledChoice.isMouseEventInside(me)) && (eventId == MouseEvent.MOUSE_PRESSED || eventId == MouseEvent.MOUSE_RELEASED || eventId == MouseEvent.MOUSE_CLICKED)) {
return handleMouseEventByChoice(me);
}
}
}
...
if (x < 0 || x > choiceRect.width || y < 0 || y > choiceRect.height) {
return false;
}
return true;
}
