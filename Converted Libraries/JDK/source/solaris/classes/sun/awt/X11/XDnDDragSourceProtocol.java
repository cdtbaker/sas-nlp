...
class XDnDDragSourceProtocol extends XDragSourceProtocol {
  private static final PlatformLogger logger=PlatformLogger.getLogger("sun.awt.X11.xembed.xdnd.XDnDDragSourceProtocol");
 ...
 */
  protected void initializeDragImpl(  int actions,  Transferable contents,  Map formatMap,  long[] formats) throws InvalidDnDOperationException, IllegalArgumentException, XException {
    ...
    try {
      ...
      XDnDConstants.XA_XdndActionList.setAtomData(window,XAtom.XA_ATOM,data,action_count);
    ...
    try {
      ...
      XToolkit.WITH_XERROR_HANDLER(XErrorHandler.VerifyChangePropertyHandler.getInstance());
      XDnDConstants.XA_XdndTypeList.setAtomData(window,XAtom.XA_ATOM,data,formats.length);
      XToolkit.RESTORE_XERROR_HANDLER();
      if (XToolkit.saved_error != null && XToolkit.saved_error.get_error_code() != XConstants.Success) {
        cleanup();
        throw new XException("Cannot write XdndActionList property");
    ...
    if (!XDnDConstants.XDnDSelection.setOwner(contents,formatMap,formats,XConstants.CurrentTime)) {
      cleanup();
  ...
  public TargetWindowInfo getTargetWindowInfo(  long window){
    ...
    if (status == XConstants.Success && wpg1.getData() != 0 && wpg1.getActualType() == XAtom.XA_ATOM) {
      ...
      if (targetVersion >= XDnDConstants.XDND_MIN_PROTOCOL_VERSION) {
        long proxy=0;
        int protocolVersion=targetVersion < XDnDConstants.XDND_PROTOCOL_VERSION ? targetVersion : XDnDConstants.XDND_PROTOCOL_VERSION;
        WindowPropertyGetter wpg2=new WindowPropertyGetter(window,XDnDConstants.XA_XdndProxy,0,1,false,XAtom.XA_WINDOW);
        try {
  ...
  public void sendEnterMessage(  long[] formats,  int sourceAction,  int sourceActions,  long time){
    ...
    try {
      ...
      long data1=getTargetProtocolVersion() << XDnDConstants.XDND_PROTOCOL_SHIFT;
