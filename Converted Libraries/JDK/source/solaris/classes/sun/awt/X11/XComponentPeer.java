 ...
 */
  ...
  final public boolean requestFocus(  Component lightweightChild,  boolean temporary,  boolean focusedWindowChangeAllowed,  long time,  CausedFocusEvent.Cause cause){
    if (XKeyboardFocusManagerPeer.processSynchronousLightweightTransfer(target,lightweightChild,temporary,focusedWindowChangeAllowed,time)) {
      return true;
    }
    int result=XKeyboardFocusManagerPeer.shouldNativelyFocusHeavyweight(target,lightweightChild,temporary,focusedWindowChangeAllowed,time,cause);
switch (result) {
case XKeyboardFocusManagerPeer.SNFH_FAILURE:
      return false;
case XKeyboardFocusManagerPeer.SNFH_SUCCESS_PROCEED:
    if (focusLog.isLoggable(PlatformLogger.FINER))     focusLog.finer("Proceeding with request to " + lightweightChild + " in "+ target);
...
return XKeyboardFocusManagerPeer.deliverFocus(lightweightChild,(Component)target,temporary,focusedWindowChangeAllowed,time,cause);
...
if (target == e.getSource() && !target.isFocusOwner() && XKeyboardFocusManagerPeer.shouldFocusOnClick(target)) {
...
handleJavaMouseWheelEvent((MouseWheelEvent)e);
 ...
 else handleJavaMouseEvent((MouseEvent)e);
}
...
Window parentWindow=(Window)parentXWindow.getTarget();
if (parentXWindow.isFocusableWindow() && parentXWindow.isSimpleWindow() && XKeyboardFocusManagerPeer.getCurrentNativeFocusedWindow() != parentWindow) {
...
if (!paintPending && !paintArea.isEmpty() && !AWTAccessor.getComponentAccessor().getIgnoreRepaint(target)) {
postEvent(new PaintEvent(target,PaintEvent.PAINT,new Rectangle()));
}
isLayouting=false;
}
...
Color newForeground=new Color((int)(tc.getRed() * ave),(int)(tc.getGreen() * ave),(int)(tc.getBlue() * ave));
...
int sbmax=ctr + w2 / 2;
{
Color d=new Color((int)(bg.getRed() * 0.85),(int)(bg.getGreen() * 0.85),(int)(bg.getBlue() * 0.85));
g.setColor(d);
if (horizontal) {
...
xBackBuffer=graphicsConfig.createBackBufferImage(target,backBuffer);
}
@Override public BufferCapabilities getBackBufferCaps(){
...
graphicsConfig.flip(this,target,xBackBuffer,x1,y1,x2,y2,flipAction);
}
...
shapeLog.finer("*** INFO: Setting shape: PEER: " + this + "; WINDOW: "+ getWindow()+ "; TARGET: "+ target+ "; SHAPE: "+ shape);
}
XToolkit.awtLock();
try {
if (shape != null) {
XlibWrapper.SetRectangularShape(XToolkit.getDisplay(),getWindow(),shape.getLoX(),shape.getLoY(),shape.getHiX(),shape.getHiY(),(shape.isRectangular() ? null : shape));
}
 else {
