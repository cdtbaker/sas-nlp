...
public class XEmbedCanvasPeer extends XCanvasPeer implements WindowFocusListener, KeyEventPostProcessor, ModalityListener, WindowIDProvider {
  ...
  void initDispatching(){
    ...
    try {
      ...
      XlibWrapper.XSelectInput(XToolkit.getDisplay(),xembed.handle,XConstants.StructureNotifyMask | XConstants.PropertyChangeMask);
...
int status=XlibWrapper.XGetWindowAttributes(XToolkit.getDisplay(),xembed.handle,wattr.pData);
...
if (xembedLog.isLoggable(PlatformLogger.FINER)) xembedLog.fine("Mapping state of the client has changed, old state: " + currently_mapped + ", new state: "+ new_mapped);
if (new_mapped) {
XToolkit.awtLock();
try {
