...
class XFramePeer extends XDecoratedPeer implements FramePeer {
  ...
  void preInit(  XCreateWindowParams params){
    ...
    if (log.isLoggable(PlatformLogger.FINE)) {
      log.fine("Frame''s initial attributes: decor {0}, resizable {1}, undecorated {2}, initial state {3}",Integer.valueOf(winAttr.decorations),Boolean.valueOf(winAttr.initialResizability),Boolean.valueOf(!winAttr.nativeDecor),Integer.valueOf(winAttr.initialState));
    }
  }
...
synchronized (getStateLock()) {
  ...
  void changeState(  int newState){
    ...
    stateLog.finer("Changing state, old state {0}, new state {1}(iconic {2})",Integer.valueOf(state),Integer.valueOf(newState),Boolean.valueOf(iconic));
