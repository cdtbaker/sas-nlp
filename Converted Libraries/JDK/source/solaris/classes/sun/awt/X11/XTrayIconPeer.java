    ...
    if (XWM.getWMID() != XWM.METACITY_WM) {
      ...
      parentXED=new XEventDispatcher(){
        public void dispatchEvent(        XEvent ev){
          ...
          ctrLog.fine("ConfigureNotify on parent of {0}: {1}x{2}+{3}+{4} (old: {5}+{6})",XTrayIconPeer.this,ce.get_width(),ce.get_height(),ce.get_x(),ce.get_y(),old_x,old_y);
          if (ce.get_height() != TRAY_ICON_HEIGHT && ce.get_width() != TRAY_ICON_WIDTH) {
            ...
            XlibWrapper.XMoveResizeWindow(XToolkit.getDisplay(),eframeParentID,ce.get_x(),ce.get_y() + ce.get_height() / 2 - TRAY_ICON_HEIGHT / 2,TRAY_ICON_WIDTH,TRAY_ICON_HEIGHT);
            ...
            XlibWrapper.XMoveResizeWindow(XToolkit.getDisplay(),eframeParentID,ce.get_x() + ce.get_width() / 2 - TRAY_ICON_WIDTH / 2,ce.get_y(),TRAY_ICON_WIDTH,TRAY_ICON_HEIGHT);
            ...
            if (ex_height != 0) {
              ...
              XlibWrapper.XMoveWindow(XToolkit.getDisplay(),eframeParentID,ce.get_x(),ce.get_y() + ex_height / 2 - TRAY_ICON_HEIGHT / 2);
            }
 else             if (ex_width != 0) {
              ctrLog.fine("ConfigureNotify on parent of {0}. Move detected. Centering by \"X\".",XTrayIconPeer.this);
              XlibWrapper.XMoveWindow(XToolkit.getDisplay(),eframeParentID,ce.get_x() + ex_width / 2 - TRAY_ICON_WIDTH / 2,ce.get_y());
            }
 else {
...
;
    ...
    if (!isTrayIconDisplayed || eframeParentID == 0 || eframeParentID == XToolkit.getDefaultRootWindow()) {
...
static class TrayIconEventProxy implements MouseListener, MouseMotionListener {
    ...
    public void handleEvent(    MouseEvent e){
      ...
      Point coord=XBaseWindow.toOtherWindow(xtiPeer.getWindow(),XToolkit.getDefaultRootWindow(),e.getX(),e.getY());
      if (e.isPopupTrigger()) {
    ...
    public void mouseClicked(    MouseEvent e){
      if ((e.getClickCount() > 1 || xtiPeer.balloon.isVisible()) && e.getButton() == MouseEvent.BUTTON1) {
        ActionEvent aev=new ActionEvent(xtiPeer.target,ActionEvent.ACTION_PERFORMED,xtiPeer.target.getActionCommand(),e.getWhen(),e.getModifiers());
