...
synchronized (winToDispatcher) {
    ...
    while (true) {
      ...
      try {
        ...
        if (windowToXWindow(ev.get_xany().get_window()) != null) {
          Component owner=XKeyboardFocusManagerPeer.getCurrentNativeFocusOwner();
          if (owner != null) {
  ...
  protected static final Object targetToPeer(  Object target){
    ...
    if (p != null)     return p;
 else     return SunToolkit.targetToPeer(target);
  }
  ...
  public <T extends DragGestureRecognizer>T createDragGestureRecognizer(  Class<T> recognizerClass,  DragSource ds,  Component c,  int srcActions,  DragGestureListener dgl){
    if (MouseDragGestureRecognizer.class.equals(recognizerClass))     return (T)new XMouseDragGestureRecognizer(ds,c,srcActions,dgl);
 else     return null;
  }
 ...
 */
  public synchronized static boolean getSunAwtDisableGtkFileDialogs(){
    if (sunAwtDisableGtkFileDialogs == null) {
      sunAwtDisableGtkFileDialogs=getBooleanSystemProperty("sun.awt.disableGtkFileDialogs");
    }
 ...
 */
  ...
  @Override public boolean getLockingKeyState(  int key){
...
String prop=(String)AccessController.doPrivileged(new sun.security.action.GetPropertyAction("sun.awt.backingStore"));
if (prop == null) {
