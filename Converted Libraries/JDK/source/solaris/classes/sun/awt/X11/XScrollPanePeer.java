...
static class XScrollPaneContentWindow extends XWindow {
  ...
  boolean setScrollbarSpace(){
    ...
    if (must_scroll)     scroll(x,y,VERTICAL | HORIZONTAL);
    return changed;
  }
  void setViewportSize(){
    clip.xSetBounds(MARGIN,MARGIN,width - (2 * MARGIN) - vsbSpace,height - (2 * MARGIN) - hsbSpace);
  }
...
drawScrollbar(ng,bg,SCROLLBAR - 2,sbw,hmin,hmax,hval,hvis,true);
}
  finally {
ng.dispose();
}
...
drawScrollbar(ng,bg,SCROLLBAR - 2,sbh,vmin,vmax,vval,vvis,false);
}
  finally {
ng.dispose();
}
