...
public class XBaseWindow {
  ...
  public static final String PARENT_WINDOW="parent window", BOUNDS="bounds", OVERRIDE_REDIRECT="overrideRedirect", EVENT_MASK="event mask", VALUE_MASK="value mask", BORDER_PIXEL="border pixel", COLORMAP="color map", DEPTH="visual depth", VISUAL_CLASS="visual class", VISUAL="visual", EMBEDDED="embedded", DELAYED="delayed", PARENT="parent", BACKGROUND_PIXMAP="pixmap", VISIBLE="visible", SAVE_UNDER="save under", BACKING_STORE="backing store", BIT_GRAVITY="bit gravity";
  ...
  static enum InitialiseState {  INITIALISING,   NOT_INITIALISED,   INITIALISED,   FAILED_INITIALISATION}
...
this(new XCreateWindowParams(new Object[]{BOUNDS,bounds,PARENT_WINDOW,Long.valueOf(parentWindow)}));
}
/** 
...
this(new XCreateWindowParams(new Object[]{PARENT_WINDOW,Long.valueOf(parentWindow),EMBEDDED,Boolean.TRUE}));
}
/** 
...
window=XlibWrapper.XCreateWindow(XToolkit.getDisplay(),parentWindow.longValue(),bounds.x,bounds.y,bounds.width,bounds.height,0,depth.intValue(),visual_class.intValue(),visual.longValue(),value_mask,xattr.pData);
if (window == 0) {
...
if (insLog.isLoggable(PlatformLogger.FINER)) insLog.finer("Setting hints, resulted flags " + XlibWrapper.hintsToString(flags) + ", values "+ hints);
