...
public final class XDragSourceContextPeer extends SunDragSourceContextPeer implements XDragSourceProtocolListener {
  ...
  private static final int ROOT_EVENT_MASK=(int)XConstants.ButtonMotionMask | (int)XConstants.KeyPressMask | (int)XConstants.KeyReleaseMask;
  private static final int GRAB_EVENT_MASK=(int)XConstants.ButtonPressMask | (int)XConstants.ButtonMotionMask | (int)XConstants.ButtonReleaseMask;
 ...
 */
  ...
  protected void setNativeCursor(  long nativeCtxt,  Cursor c,  int cType){
    ...
    XlibWrapper.XChangeActivePointerGrab(XToolkit.getDisplay(),GRAB_EVENT_MASK,xcursor,XConstants.CurrentTime);
  }
...
int action=SunDragSourceContextPeer.convertModifiersToDropAction(XWindow.getModifiers(state,0,0),sourceActions);
...
postDragSourceDragEvent(targetAction,XWindow.getModifiers(xmotion.get_state(),0,0),xRoot,yRoot,DISPATCH_MOUSE_MOVED);
}
if (eventState != xmotion.get_state()) {
if (updateSourceAction(xmotion.get_state()) && dragProtocol != null) {
postDragSourceDragEvent(targetAction,XWindow.getModifiers(xmotion.get_state(),0,0),xRoot,yRoot,DISPATCH_CHANGED);
}
eventState=xmotion.get_state();
}
updateTargetWindow(xmotion);
if (dragProtocol != null) {
dragProtocol.sendMoveMessage(xmotion.get_x_root(),xmotion.get_y_root(),sourceAction,sourceActions,xmotion.get_time());
}
}
private void processDrop(XButtonEvent xbutton){
try {
dragProtocol.initiateDrop(xbutton.get_x_root(),xbutton.get_y_root(),sourceAction,sourceActions,xbutton.get_time());
}
 catch (XException e) {
...
if (!dragInProgress && dragProtocol != null && xde.get_window() == dragProtocol.getTargetWindow()) {
