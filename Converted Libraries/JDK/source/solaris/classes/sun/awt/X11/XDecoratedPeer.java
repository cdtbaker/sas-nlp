...
abstract class XDecoratedPeer extends XWindowPeer {
  ...
  private void updateMinSizeHints(){
    if (isResizable()) {
      ...
      if (minimumSize != null) {
        ...
        setSizeHints(XUtilConstants.PMinSize | (isLocationByPlatform() ? 0 : (XUtilConstants.PPosition | XUtilConstants.USPosition)),getX(),getY(),minWidth,minHeight);
        if (isVisible()) {
  ...
  public Graphics getGraphics(){
    ...
    return getGraphics(content.surfaceData,compAccessor.getForeground(target),compAccessor.getBackground(target),compAccessor.getFont(target));
  }
  ...
  public void reshape(  WindowDimensions newDimensions,  int op,  boolean userReshape){
    ...
    try {
      if (!isReparented() || !isVisible()) {
        insLog.fine("- not reparented({0}) or not visible({1}), default reshape",Boolean.valueOf(isReparented()),Boolean.valueOf(visible));
        Point oldLocation=getLocation();
        Point newLocation=new Point(AWTAccessor.getComponentAccessor().getX((Component)target),AWTAccessor.getComponentAccessor().getY((Component)target));
...
if (insLog.isLoggable(PlatformLogger.FINE)) insLog.fine("For the operation {0} new dimensions are {1}",operationToString(operation),dims);
...
if (insLog.isLoggable(PlatformLogger.FINE)) {
insLog.fine("reparented={0}, visible={1}, WM={2}, decorations={3}",isReparented(),isVisible(),runningWM,getDecorations());
}
if (!isReparented() && isVisible() && runningWM != XWM.NO_WM && !XWM.isNonReparentingWM() && getDecorations() != XWindowAttributesData.AWT_DECOR_NONE) {
...
WindowDimensions newDimensions=new WindowDimensions(newLocation,new Dimension(xe.get_width(),xe.get_height()),copy(currentInsets),true);
insLog.finer("Insets are {0}, new dimensions {1}",currentInsets,newDimensions);
...
if (insLog.isLoggable(PlatformLogger.FINE)) insLog.fine("Setting shell bounds on " + this + " to "+ rec);
XToolkit.awtLock();
try {
updateSizeHints(rec.x,rec.y,rec.width,rec.height);
...
if (insLog.isLoggable(PlatformLogger.FINE)) insLog.fine("Setting shell size on " + this + " to "+ rec);
XToolkit.awtLock();
try {
updateSizeHints(rec.x,rec.y,rec.width,rec.height);
XlibWrapper.XResizeWindow(XToolkit.getDisplay(),getShell(),rec.width,rec.height);
...
if (insLog.isLoggable(PlatformLogger.FINE)) insLog.fine("Setting shell position on " + this + " to "+ rec);
XToolkit.awtLock();
try {
updateSizeHints(rec.x,rec.y,rec.width,rec.height);
XlibWrapper.XMoveWindow(XToolkit.getDisplay(),getShell(),rec.x,rec.y);
...
if (insLog.isLoggable(PlatformLogger.FINE)) insLog.fine("getLocationOnScreen {0} not reparented: {1} ",this,location);
return location;
}
}
  finally {
...
@Override boolean isOverrideRedirect(){
...
focusLog.finer("Current window is: active={0}, focused={1}",Boolean.valueOf(target == activeWindow),Boolean.valueOf(target == focusedWindow));
