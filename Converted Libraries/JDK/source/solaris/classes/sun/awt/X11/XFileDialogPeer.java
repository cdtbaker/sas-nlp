 ...
 */
  public void itemStateChanged(  ItemEvent itemEvent){
    if (itemEvent.getID() != ItemEvent.ITEM_STATE_CHANGED || itemEvent.getStateChange() == ItemEvent.DESELECTED) {
      return;
    }
  ...
  String getParentDirectory(){
    ...
    if (!this.dir.equals("/")) {
      if (dir.endsWith("/"))       parent=parent.substring(0,parent.lastIndexOf("/"));
 ...
 */
  ...
  public void unfurledChoiceOpening(  ListHelper choiceHelper){
    ...
    if (pathChoice.getItem(0).equals(pathField.getText()))     return;
...
class Separator extends Canvas {
  ...
  public void paint(  Graphics g){
    ...
    g.setColor(brighter);
    if (orientation == HORIZONTAL)     g.drawLine(x1,y2 + 1,x2,y2 + 1);
 else     g.drawLine(x1 + 1,y2,x2 + 1,y2);
  }
}
