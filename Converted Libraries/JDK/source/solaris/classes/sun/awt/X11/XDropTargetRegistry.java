...
final class XDropTargetRegistry {
  ...
  private final HashMap<Long,Runnable> delayedRegistrationMap=new HashMap<Long,Runnable>();
...
private static final class EmbeddedDropSiteEntry {
      ...
      while (iter.hasNext()) {
        ...
        if (dest_x >= 0 && dest_y >= 0) {
          ...
          try {
            ...
            if (wattr.get_map_state() != XConstants.IsUnmapped && dest_x < wattr.get_width() && dest_y < wattr.get_height()) {
              return window;
            }
          }
  finally {
  ...
  private final HashMap<Long,EmbeddedDropSiteEntry> embeddedDropSiteRegistry=new HashMap<Long,EmbeddedDropSiteEntry>();
  ...
  private void registerProtocols(  long embedder,  boolean protocols,  List<XDropTargetProtocol> supportedProtocols){
    ...
    if (!supportedProtocols.isEmpty()) {
      ...
      dropTargetProtocols=XDragAndDropProtocols.getDropTargetProtocols();
    }
    XlibWrapper.XGrabServer(XToolkit.getDisplay());
    try {
      while (dropTargetProtocols.hasNext()) {
        XDropTargetProtocol dropTargetProtocol=(XDropTargetProtocol)dropTargetProtocols.next();
        if ((protocols == XEMBED_PROTOCOLS) == dropTargetProtocol.isXEmbedSupported()) {
...
synchronized (this) {
      EmbeddedDropSiteEntry entry=(EmbeddedDropSiteEntry)embeddedDropSiteRegistry.get(lToplevel);
      if (entry == null) {
        return;
      }
...
synchronized (this) {
      entry=(EmbeddedDropSiteEntry)embeddedDropSiteRegistry.get(lToplevel);
      if (entry == null) {
        if (peer != null) {
          peer.setXEmbedDropTarget();
          entry=new EmbeddedDropSiteEntry(0,0,Collections.<XDropTargetProtocol>emptyList());
        }
 else {
          entry=registerEmbedderDropSite(toplevel);
          registerProtocols(toplevel,NON_XEMBED_PROTOCOLS,entry.getSupportedProtocols());
        }
...
synchronized (entry) {
      if (peer == null) {
        if (!isXEmbedClient) {
          registerProtocols(toplevel,XEMBED_PROTOCOLS,entry.getSupportedProtocols());
        }
 else {
          Iterator dropTargetProtocols=XDragAndDropProtocols.getDropTargetProtocols();
          while (dropTargetProtocols.hasNext()) {
...
synchronized (this) {
  ...
  public void registerXEmbedClient(  long canvasWindow,  long clientWindow){
    XDragSourceProtocol xdndDragProtocol=XDragAndDropProtocols.getDragSourceProtocol(XDragAndDropProtocols.XDnD);
    XDragSourceProtocol.TargetWindowInfo info=xdndDragProtocol.getTargetWindowInfo(clientWindow);
    if (info != null && info.getProtocolVersion() >= XDnDConstants.XDND_MIN_PROTOCOL_VERSION) {
