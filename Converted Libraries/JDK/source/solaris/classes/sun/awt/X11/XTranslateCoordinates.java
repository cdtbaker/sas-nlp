...
public class XTranslateCoordinates {
  ...
  public XTranslateCoordinates(  long scr_w,  long dest_w,  int src_x,  int src_y){
    ...
    set_src_x(src_x);
    set_src_y(src_y);
    sun.java2d.Disposer.addRecord(this,disposer=new UnsafeXDisposerRecord("XTranslateCoordinates",dest_x_ptr,dest_y_ptr,child_ptr));
  }
  ...
  public int execute(  XErrorHandler errorHandler){
    ...
    try {
      ...
      int status=XlibWrapper.XTranslateCoordinates(XToolkit.getDisplay(),get_scr_w(),get_dest_w(),get_src_x(),get_src_y(),dest_x_ptr,dest_y_ptr,child_ptr);
      if (errorHandler != null) {
