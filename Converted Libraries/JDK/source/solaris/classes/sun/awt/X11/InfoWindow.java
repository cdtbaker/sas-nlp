...
,BALLOON_SHOW_TIME);
      ...
      if (text != null) {
        ...
        do {
          ...
          if (end == BreakIterator.DONE || text.substring(start,end).length() >= 50) {
            lineLabels[nLines].setText(text.substring(start,end == BreakIterator.DONE ? iter.last() : end));
            textPanel.add(lineLabels[nLines++]);
            start=end;
          }
          if (nLines == BALLOON_WORD_LINE_MAX_COUNT) {
            if (end != BreakIterator.DONE) {
              lineLabels[nLines - 1].setText(new String(lineLabels[nLines - 1].getText() + " ..."));
            }
            break;
          }
        }
 while (end != BreakIterator.DONE);
      ...
      if (iconImage != null) {
        ...
        iconCanvas.setSize(BALLOON_ICON_WIDTH,(BALLOON_ICON_HEIGHT > tpSize.height ? BALLOON_ICON_HEIGHT : tpSize.height));
        iconCanvas.validate();
      }
      SunToolkit.executeOnEventHandlerThread(target,new Runnable(){
        public void run(){
          ...
          show(new Point(parLoc.x + parSize.width / 2,parLoc.y + parSize.height / 2),BALLOON_TRAY_ICON_INDENT);
          if (iconImage != null) {
    ...
    private void loadGtkImages(){
      if (!gtkImagesLoaded) {
        errorImage=(Image)Toolkit.getDefaultToolkit().getDesktopProperty("gtk.icon.gtk-dialog-error.6.rtl");
        warnImage=(Image)Toolkit.getDefaultToolkit().getDesktopProperty("gtk.icon.gtk-dialog-warning.6.rtl");
        infoImage=(Image)Toolkit.getDefaultToolkit().getDesktopProperty("gtk.icon.gtk-dialog-info.6.rtl");
...
private class ActionPerformer extends MouseAdapter {
      public void mouseClicked(      MouseEvent e){
        ...
        if (e.getButton() == MouseEvent.BUTTON1) {
          ActionEvent aev=new ActionEvent(target,ActionEvent.ACTION_PERFORMED,liveArguments.getActionCommand(),e.getWhen(),e.getModifiers());
