 ...
 */
  ...
  protected synchronized void setContentsNative(  Transferable contents){
    ...
    if (!selection.setOwner(contents,formatMap,formats,XToolkit.getCurrentServerTime())) {
      this.owner=null;
      this.contents=null;
    }
  }
  ...
  private void checkChangeHere(  Transferable contents){
    if (areFlavorListenersRegistered()) {
      checkChange(DataTransferer.getInstance().getFormatsForTransferableAsArray(contents,flavorMap));
    }
  }
  private static int getPollInterval(){
synchronized (XClipboard.classLock) {
      if (pollInterval <= 0) {
        pollInterval=AccessController.doPrivileged(new GetIntegerAction("awt.datatransfer.clipboard.poll.interval",defaultPollInterval));
        if (pollInterval <= 0) {
  ...
  private XAtom getTargetsPropertyAtom(){
    if (null == targetsPropertyAtom) {
      targetsPropertyAtom=XAtom.get("XAWT_TARGETS_OF_SELECTION:" + selection.getSelectionAtom().getName());
    }
...
synchronized (XClipboard.classLock) {
  ...
  private void getTargetsDelayed(){
    ...
    try {
      ...
      if (isSelectionNotifyProcessed || curTime >= (convertSelectionTime + UNIXToolkit.getDatatransferTimeout())) {
        ...
        XlibWrapper.XConvertSelection(XToolkit.getDisplay(),selection.getSelectionAtom().getAtom(),XDataTransferer.TARGETS_ATOM.getAtom(),getTargetsPropertyAtom().getAtom(),XWindow.getXAWTRootWindow().getWindow(),XConstants.CurrentTime);
        isSelectionNotifyProcessed=false;
      }
    }
  finally {
  ...
  private void checkChange(  XSelectionEvent xse){
    ...
    if (propertyAtom == XConstants.None) {
      ...
      WindowPropertyGetter targetsGetter=new WindowPropertyGetter(XWindow.getXAWTRootWindow().getWindow(),XAtom.get(propertyAtom),0,XSelection.MAX_LENGTH,true,XConstants.AnyPropertyType);
      try {
