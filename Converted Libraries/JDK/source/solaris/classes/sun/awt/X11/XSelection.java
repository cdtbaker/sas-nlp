 ...
 */
 ...
 while (propertyGetter == dataGetter && System.currentTimeMillis() < startTime + UNIXToolkit.getDatatransferTimeout());
  ...
  public long[] getTargets(  long time){
    ...
    long[] targets=null;
synchronized (lock) {
      WindowPropertyGetter targetsGetter=new WindowPropertyGetter(XWindow.getXAWTRootWindow().getWindow(),selectionPropertyAtom,0,MAX_LENGTH,true,XConstants.AnyPropertyType);
      try {
        XToolkit.awtLock();
        try {
  ...
  public byte[] getData(  long format,  long time) throws IOException {
    ...
    byte[] data=null;
synchronized (lock) {
      WindowPropertyGetter dataGetter=new WindowPropertyGetter(XWindow.getXAWTRootWindow().getWindow(),selectionPropertyAtom,0,MAX_LENGTH,false,XConstants.AnyPropertyType);
      try {
        XToolkit.awtLock();
        try {
          ...
          int len=0;
{
          ...
          while (true) {
            WindowPropertyGetter incrDataGetter=new WindowPropertyGetter(XWindow.getXAWTRootWindow().getWindow(),selectionPropertyAtom,0,MAX_LENGTH,false,XConstants.AnyPropertyType);
            try {
              XToolkit.awtLock();
  ...
  void validateDataGetter(  WindowPropertyGetter propertyGetter) throws IOException {
  ...
  private void handleSelectionRequest(  XSelectionRequestEvent xsre){
    ...
    if (ownershipTime != 0 && (requestTime == XConstants.CurrentTime || requestTime >= ownershipTime)) {
  ...
  private boolean handleMultipleRequest(  final long requestor,  long property){
    ...
    boolean conversionSucceeded=false;
    WindowPropertyGetter wpg=new WindowPropertyGetter(requestor,XAtom.get(property),0,MAX_LENGTH,false,XConstants.AnyPropertyType);
    try {
      wpg.execute();
      if (wpg.getActualFormat() == 32 && (wpg.getNumberOfItems() % 2) == 0) {
        ...
        if (writeBack) {
          XToolkit.awtLock();
          try {
            XlibWrapper.XChangeProperty(XToolkit.getDisplay(),requestor,property,wpg.getActualType(),wpg.getActualFormat(),XConstants.PropModeReplace,wpg.getData(),wpg.getNumberOfItems());
          }
  finally {
            XToolkit.awtUnlock();
          }
...
try {
  ...
  try {
    ...
    XlibWrapper.XSelectInput(XToolkit.getDisplay(),requestor,wattr.get_your_event_mask() | XConstants.PropertyChangeMask);
  }
  finally {
    XToolkit.awtUnlock();
  }
...
if (xpe.get_window() == requestor && xpe.get_state() == XConstants.PropertyDelete && xpe.get_atom() == property) {
  ...
  XToolkit.awtLock();
  try {
    XlibWrapper.XChangeProperty(XToolkit.getDisplay(),requestor,property,target,format,XConstants.PropModeReplace,nativeDataPtr,count);
  }
  finally {
    XToolkit.awtUnlock();
  }
...
case XConstants.PropertyNotify:
XPropertyEvent xpe=ev.get_xproperty();
if (xpe.get_state() == XConstants.PropertyNewValue && xpe.get_atom() == selectionPropertyAtom.getAtom()) {
XToolkit.awtLock();
try {
