...
class XWINProtocol extends XProtocol implements XStateProtocol, XLayerProtocol {
  ...
  public void setLayer(  XWindowPeer window,  int layer){
    if (window.isShowing()) {
      ...
      XToolkit.awtLock();
      try {
        XlibWrapper.XSendEvent(XToolkit.getDisplay(),XlibWrapper.RootWindow(XToolkit.getDisplay(),window.getScreenNumber()),false,XConstants.SubstructureNotifyMask,req.pData);
      }
  finally {
        XToolkit.awtUnlock();
      }
