 ...
 */
  private XIconSize[] getIconSizes(){
    ...
    try {
      ...
      long status=XlibWrapper.XGetIconSizes(display,XToolkit.getDefaultRootWindow(),XlibWrapper.larg1,XlibWrapper.iarg1);
      if (status == 0) {
        return null;
      }
  ...
  private Dimension calcIconSize(  int widthHint,  int heightHint){
    ...
    for (int i=0; i < sizeList.length; i++) {
      if (widthHint >= sizeList[i].get_min_width() && widthHint <= sizeList[i].get_max_width() && heightHint >= sizeList[i].get_min_height() && heightHint <= sizeList[i].get_max_height()) {
        found=true;
    ...
    if (!found) {
      ...
      if (widthHint > sizeList[0].get_max_width() || heightHint > sizeList[0].get_max_height()) {
        ...
        if (wdiff >= hdiff) {
          ...
          saveHeight=(int)(((double)sizeList[0].get_max_width() / widthHint) * heightHint);
        }
 else {
          saveWidth=(int)(((double)sizeList[0].get_max_height() / heightHint) * widthHint);
 ...
 */
  void replaceImage(  Image img){
    ...
    try {
      ...
      iconPixmap=XlibWrapper.XCreatePixmap(XToolkit.getDisplay(),XlibWrapper.RootWindow(XToolkit.getDisplay(),visInfo.get_screen()),iconWidth,iconHeight,awtImage.get_Depth());
      if (iconPixmap == 0) {
      ...
      if (srcBuf instanceof DataBufferByte) {
        ...
        for (int i=0; i < buf.length; i++) {
          buf[i]=(buf[i] >= num_colors) ? 0 : cdata.get_awt_icmLUT2Colors(buf[i]);
        }
      ...
      long dst=XlibWrapper.XCreateImage(XToolkit.getDisplay(),visInfo.get_visual(),(int)awtImage.get_Depth(),(int)XConstants.ZPixmap,0,bytes,iconWidth,iconHeight,32,bpsl);
      if (dst == 0) {
      ...
      try {
        XlibWrapper.XPutImage(XToolkit.getDisplay(),iconPixmap,gc,dst,0,0,0,0,iconWidth,iconHeight);
      }
  finally {
 ...
 */
  void replaceMask(  Image img){
    ...
    try {
      ...
      iconMask=XlibWrapper.XCreateBitmapFromData(XToolkit.getDisplay(),XlibWrapper.RootWindow(XToolkit.getDisplay(),visInfo.get_screen()),Native.toData(destBuf),iconWidth,iconHeight);
    }
  finally {
      XToolkit.awtUnlock();
    }
