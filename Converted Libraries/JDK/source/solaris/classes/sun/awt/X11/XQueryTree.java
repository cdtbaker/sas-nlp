...
public class XQueryTree {
  ...
  public XQueryTree(  long w){
    set_w(w);
    sun.java2d.Disposer.addRecord(this,disposer=new UnsafeXDisposerRecord("XQueryTree",new long[]{root_ptr,parent_ptr,nchildren_ptr},new long[]{children_ptr}));
    set_children(0);
  }
  public int execute(){
    return execute(null);
  ...
  public int execute(  XErrorHandler errorHandler){
    ...
    try {
      ...
      int status=XlibWrapper.XQueryTree(XToolkit.getDisplay(),get_w(),root_ptr,parent_ptr,children_ptr,nchildren_ptr);
      if (errorHandler != null) {
