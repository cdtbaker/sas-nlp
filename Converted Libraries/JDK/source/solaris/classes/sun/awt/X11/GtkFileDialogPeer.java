...
static {
  ...
  private native void run(  String title,  int mode,  String dir,  String file,  FilenameFilter filter,  boolean isMultipleMode,  int x,  int y);
  private native void quit();
  @Override public native void toFront();
  @Override public native void setBounds(  int x,  int y,  int width,  int height,  int op);
  /** 
 ...
 */
  private void setFileInternal(  String directory,  String[] filenames){
    ...
    if (filenames == null) {
      ...
      accessor.setDirectory(fd,directory + (directory.endsWith(File.separator) ? "" : File.separator));
 ...
 */
  private boolean filenameFilterCallback(  String fullname){
    ...
    return fd.getFilenameFilter().accept(new File(filen.getParent()),filen.getName());
  }
...
;
  ...
  @Override public void setDirectory(  String dir){
  }
  @Override public void setFile(  String file){
  }
  @Override public void setFilenameFilter(  FilenameFilter filter){
  }
