...
synchronized (_globalLock) {
    ...
    try {
      if (ctxt != 0 && !e.isConsumed()) {
        ...
        while (dropTargetProtocols.hasNext()) {
          ...
          if (dropTargetProtocol.sendResponse(ctxt,e.getID(),returnValue)) {
            break;
          }
        }
      }
  ...
  protected void doDropDone(  boolean success,  int dropAction,  boolean isLocal){
    ...
    if (ctxt != 0) {
      try {
        ...
        while (dropTargetProtocols.hasNext()) {
          XDropTargetProtocol dropTargetProtocol=(XDropTargetProtocol)dropTargetProtocols.next();
          if (dropTargetProtocol.sendDropDone(ctxt,success,dropAction)) {
            break;
          }
        }
      }
  ...
  public void forwardEventToEmbedded(  long embedded,  long ctxt,  int eventID){
    ...
    while (dropTargetProtocols.hasNext()) {
      XDropTargetProtocol dropTargetProtocol=(XDropTargetProtocol)dropTargetProtocols.next();
      if (dropTargetProtocol.forwardEventToEmbedded(embedded,ctxt,eventID)) {
        break;
      }
    }
  }
static final class XDropTargetProtocolListenerImpl implements XDropTargetProtocolListener {
    private final static XDropTargetProtocolListener theInstance=new XDropTargetProtocolListenerImpl();
    private XDropTargetProtocolListenerImpl(){
    }
    static XDropTargetProtocolListener getInstance(){
      return theInstance;
    }
    public void handleDropTargetNotification(    XWindow xwindow,    int x,    int y,    int dropAction,    int actions,    long[] formats,    long nativeCtxt,    int eventID){
      ...
      peer.postDropTargetEvent(component,x,y,dropAction,actions,formats,nativeCtxt,eventID,!SunDropTargetContextPeer.DISPATCH_SYNC);
    }
  }
}
