...
public class X11GraphicsConfig extends GraphicsConfiguration implements SurfaceManager.ProxiedGraphicsConfig {
  ...
  protected X11GraphicsConfig(  X11GraphicsDevice device,  int visualnum,  int depth,  int colormap,  boolean doubleBuffer){
    ...
    Disposer.addRecord(disposerReferent,new X11GCDisposerRecord(x11CfgData));
  }
  /** 
 ...
 */
  public synchronized RenderLoops getSolidLoops(  SurfaceType stype){
    if (solidloops == null) {
      solidloops=SurfaceData.makeRenderLoops(SurfaceType.OpaqueColor,CompositeType.SrcNoEa,stype);
    }
    return solidloops;
  }
  /** 
 ...
 */
  public synchronized ColorModel getColorModel(){
    if (colorModel == null) {
      ...
      if (colorModel == null)       colorModel=Toolkit.getDefaultToolkit().getColorModel();
    }
    return colorModel;
  }
  /** 
...
return new SunVolatileImage(target,target.getWidth(),target.getHeight(),Long.valueOf(backBuffer));
}
/** 
...
@Override public boolean isTranslucencyCapable(){
