...
static {
    ...
    for (int i=environ.length - 1; i > 0; i-=2)     theEnvironment.put(Variable.valueOf(environ[i - 1]),Value.valueOf(environ[i]));
  ...
  private static void validateVariable(  String name){
    if (name.indexOf('=') != -1 || name.indexOf('\u0000') != -1)     throw new IllegalArgumentException("Invalid environment variable name: \"" + name + "\"");
  }
  private static void validateValue(  String value){
    if (value.indexOf('\u0000') != -1)     throw new IllegalArgumentException("Invalid environment variable value: \"" + value + "\"");
  }
...
;
    ...
    private static Map.Entry<Variable,Value> vvEntry(    final Object o){
      if (o instanceof StringEntry)       return ((StringEntry)o).e;
