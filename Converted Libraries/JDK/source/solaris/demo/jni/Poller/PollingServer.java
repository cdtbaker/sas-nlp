 ...
 */
  ...
  public PollingServer(  int concurrency){
    ...
    try {
      ...
      BufferedReader ctrlReader=new BufferedReader(new InputStreamReader(ctrlSock.getInputStream()));
      ...
      System.out.println("Receiving " + bytesToRead + " bytes from "+ totalConn+ " client connections");
      timestart=System.currentTimeMillis();
      for (int consumerThread=0; consumerThread < concurrency; consumerThread++) {
...
synchronized (eventSync) {
        ...
        if (loopWaits > 1)         System.out.println("Done waiting...loops = " + loopWaits + " events "+ numEvents+ " bytes read : "+ bytesRead);
...
synchronized (eventSync) {
      ...
      System.out.println("Time for all reads (" + totalConn + " sockets) : "+ (timestop - timestart));
  ...
  public static void main(  String args[]){
    int concurrency;
    if (args.length == 1)     concurrency=java.lang.Integer.valueOf(args[0]).intValue();
 else     concurrency=Poller.getNumCPUs() + 1;
