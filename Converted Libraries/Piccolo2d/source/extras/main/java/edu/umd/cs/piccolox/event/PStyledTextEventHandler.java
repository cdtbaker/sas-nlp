  ...
  public void dispatchEventToEditor(  final PInputEvent event){
    SwingUtilities.invokeLater(new Runnable(){
      public void run(){
        SwingUtilities.invokeLater(new Runnable(){
          public void run(){
            final MouseEvent me=new MouseEvent(editor,MouseEvent.MOUSE_PRESSED,event.getWhen(),event.getModifiers() | InputEvent.BUTTON1_MASK,(int)(event.getCanvasPosition().getX() - editor.getX()),(int)(event.getCanvasPosition().getY() - editor.getY()),1,false);
  ...
  public void reshapeEditor(){
    if (editedText != null) {
      ...
      if (editedText.getConstrainWidthToTextWidth()) {
        ...
        width=(int)(editedText.getWidth() - textInsets.left - textInsets.right + editorInsets.left + editorInsets.right + TEXT_EDIT_PADDING);
      }
      ...
      if (editedText.getConstrainHeightToTextHeight()) {
        ...
        height=(int)(editedText.getHeight() - textInsets.top - textInsets.bottom + editorInsets.top + editorInsets.bottom + TEXT_EDIT_PADDING);
      }
...
private static final class DefaultTextEditor extends JTextPane {
    ...
    public DefaultTextEditor(){
      EmptyBorder padding=new EmptyBorder(TEXT_EDIT_PADDING,TEXT_EDIT_PADDING,TEXT_EDIT_PADDING,TEXT_EDIT_PADDING);
