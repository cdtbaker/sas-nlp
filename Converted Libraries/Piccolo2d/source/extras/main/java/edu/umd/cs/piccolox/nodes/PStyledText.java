 ...
 */
  ...
  private void applyFontAttribute(  final RunInfo paragraphRange,  final AttributedString attributedString,  final Element curElement,  final Font font){
    if (font != null) {
  ...
  private void applyStrikeThroughAttribute(  final RunInfo paragraphRange,  final AttributedString attributedString,  final Element curElement,  final AttributeSet attributes){
  ...
  private void applyUnderlineAttribute(  final RunInfo paragraphRange,  final AttributedString attributedString,  final Element curElement,  final AttributeSet attributes){
  ...
  private void applyBackgroundAttribute(  final StyleContext style,  final RunInfo paragraphRange,  final AttributedString attributedString,  final Element curElement,  final AttributeSet attributes){
  ...
  private Font extractFont(  final StyleContext style,  final int pos,  final Element rootElement,  final AttributeSet attributes){
    Font font=null;
  ...
  private Font extractFontFromDefaultStyledDocument(  final DefaultStyledDocument styledDocument,  final StyleContext style,  final int pos,  final Element rootElement){
  ...
  public void recomputeLayout(){
    ...
    while (contentIterator.hasNext()) {
      ...
      while (measurer.getPosition() < itr.getEndIndex()) {
        ...
        final FontMetrics metrics=StyleContext.getDefaultStyleContext().getFontMetrics((Font)itr.getAttribute(TextAttribute.FONT));
    ...
    for (int i=0; i < lines.length; i++) {
      ...
      for (int j=0; j < lineInfo.segments.size(); j++) {
        ...
        if (sInfo.background != null) {
          ...
          g2.fill(new Rectangle2D.Double(curX,y - lineInfo.maxAscent,width,lineInfo.maxAscent + lineInfo.maxDescent + lineInfo.leading));
        }
        sInfo.applyFont(g2);
