 ...
 */
  public static boolean contains(  final double x,  final double y,  final double x1,  final double y1,  final double x2,  final double y2,  final boolean min,  final boolean max,  final double distance){
    double dx=x2 - x1;
    double dy=y2 - y1;
    if (dx == 0 && dy == 0) {
      return false;
 ...
 */
  public static boolean intersects(  final double x1,  final double y1,  final double x2,  final double y2,  final double x3,  final double y3,  final double x4,  final double y4,  final boolean min1,  final boolean max1,  final boolean min2,  final boolean max2){
 ...
 */
  public boolean intersects(  final double x,  final double y,  final double w,  final double h){
    ...
    for (int i=0; i < points.getPointCount(); i++) {
      ...
      x2=points.getX(i);
      y2=points.getY(i);
      if (intersects(x,y,x + w,y,x1,y1,x2,y2,true,true,true,true) || intersects(x + w,y,x + w,y + h,x1,y1,x2,y2,true,true,true,true) || intersects(x + w,y + h,x,y + h,x1,y1,x2,y2,true,true,true,true)|| intersects(x,y + h,x,y,x1,y1,x2,y2,true,true,true,true)) {
        return true;
      }
    }
    return false;
