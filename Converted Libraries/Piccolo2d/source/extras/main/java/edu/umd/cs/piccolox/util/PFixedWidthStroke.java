...
public class PFixedWidthStroke extends PSemanticStroke implements Serializable {
  ...
  private static final BasicStroke DEFAULT_STROKE=new BasicStroke(1.0f,BasicStroke.CAP_SQUARE,BasicStroke.JOIN_MITER,DEFAULT_MITER_LIMIT,null,0.0f);
 ...
 */
  public PFixedWidthStroke(  final float width,  final int cap,  final int join,  final float miterlimit,  final float[] dash,  final float dashPhase){
 ...
 */
  protected Stroke newStroke(  final float activeScale){
    ...
    return new BasicStroke(getLineWidth() / activeScale,getEndCap(),getLineJoin(),sanitizedMiterLimit,tmpDash,getDashPhase() / activeScale);
  }
  /** 
