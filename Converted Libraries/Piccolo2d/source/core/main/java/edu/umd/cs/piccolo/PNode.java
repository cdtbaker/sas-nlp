 ...
 */
  public PInterpolatingActivity animateToBounds(  final double x,  final double y,  final double width,  final double height,  final long duration){
    if (duration == 0) {
      setBounds(x,y,width,height);
      return null;
    }
    final PBounds dst=new PBounds(x,y,width,height);
    final PInterpolatingActivity interpolatingActivity=new PInterpolatingActivity(duration,PUtil.DEFAULT_ACTIVITY_STEP_RATE){
      private PBounds src;
      ...
      public void setRelativeTargetValue(      final float zeroToOne){
        PNode.this.setBounds(src.x + zeroToOne * (dst.x - src.x),src.y + zeroToOne * (dst.y - src.y),src.width + zeroToOne * (dst.width - src.width),src.height + zeroToOne * (dst.height - src.height));
      }
 ...
 * @return the newly scheduled activity
 */
  public PTransformActivity animateTransformToBounds(  final double x,  final double y,  final double width,  final double height,  final long duration){
 ...
 * @return the newly scheduled activity
 */
  public PTransformActivity animateToPositionScaleRotation(  final double x,  final double y,  final double scale,  final double theta,  final long duration){
...
;
      final PTransformActivity ta=new PTransformActivity(duration,PUtil.DEFAULT_ACTIVITY_STEP_RATE,t,destTransform);
      addActivity(ta);
      return ta;
    }
  }
 ...
 */
  protected void firePropertyChange(  final int propertyCode,  final String propertyName,  final Object oldValue,  final Object newValue){
  ...
  protected boolean validateFullBounds(){
    ...
    if (fullBoundsInvalid || childBoundsInvalid || boundsVolatile|| childBoundsVolatile) {
      ...
      if (fullBoundsInvalid) {
        ...
        final boolean fullBoundsChanged=fullBoundsCache.x != oldX || fullBoundsCache.y != oldY || fullBoundsCache.width != oldWidth || fullBoundsCache.height != oldHeight || fullBoundsCache.isEmpty() != oldEmpty;
 ...
 * scheduled
 */
  public PActivity animateToRelativePosition(  final Point2D srcPt,  final Point2D destPt,  final Rectangle2D destBounds,  final int millis){
    double srcx, srcy;
    double destx, desty;
    double dx, dy;
    Point2D pt1, pt2;
 ...
 */
  public void setTransparency(  final float newTransparency){
    if (Math.abs(transparency - newTransparency) > TRANSPARENCY_RESOLUTION) {
      ...
      invalidatePaint();
      firePropertyChange(PROPERTY_CODE_TRANSPARENCY,PROPERTY_TRANSPARENCY,new Float(oldTransparency),new Float(newTransparency));
    }
  }
  /** 
