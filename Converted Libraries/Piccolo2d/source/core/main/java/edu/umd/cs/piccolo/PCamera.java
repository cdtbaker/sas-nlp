 ...
 */
  public PTransformActivity animateViewToCenterBounds(  final Rectangle2D centerBounds,  final boolean shouldScaleToFit,  final long duration){
    ...
    if (shouldScaleToFit) {
      final double s=Math.min(viewBounds.getWidth() / centerBounds.getWidth(),viewBounds.getHeight() / centerBounds.getHeight());
...
;
    final PTransformActivity transformActivity=new PTransformActivity(duration,PUtil.DEFAULT_ACTIVITY_STEP_RATE,t,destination);
 ...
 * @throws IllegalArgumentException if <code>viewConstraint</code> is not one of {@link #VIEW_CONSTRAINT_NONE},{@link #VIEW_CONSTRAINT_CENTER}, or {@link #VIEW_CONSTRAINT_CENTER}
 */
  public void setViewConstraint(  final int viewConstraint){
    if (viewConstraint != VIEW_CONSTRAINT_NONE && viewConstraint != VIEW_CONSTRAINT_CENTER && viewConstraint != VIEW_CONSTRAINT_ALL) {
