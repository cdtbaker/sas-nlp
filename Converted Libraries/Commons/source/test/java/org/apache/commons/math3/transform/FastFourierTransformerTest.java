 ...
 */
  ...
  @Test public void testTransformComplexSizeNotAPowerOfTwo(){
    final int n=127;
    ...
    for (int i=0; i < norm.length; i++) {
      for (int j=0; j < type.length; j++) {
        ...
        try {
          ...
          Assert.fail(norm[i] + ", " + type[j]+ ": MathIllegalArgumentException was expected");
        }
  ...
  @Test public void testTransformRealSizeNotAPowerOfTwo(){
    final int n=127;
    ...
    for (int i=0; i < norm.length; i++) {
      for (int j=0; j < type.length; j++) {
        ...
        try {
          fft.transform(f,0.0,Math.PI,n,type[j]);
          fft.transform(f,0.0,Math.PI,n,type[j]);
          Assert.fail(norm[i] + ", " + type[j]+ ": NotStrictlyPositiveException was expected");
        }
 ...
 */
  ...
  private static void doTestTransformComplex(  final int n,  final double tol,  final DftNormalization normalization,  final TransformType type){
    ...
    for (int i=0; i < n; i++) {
      ...
      Assert.assertEquals(msg,re,actual[i].getReal(),tol * FastMath.abs(re));
      final double im=s * expected[i].getImaginary();
      Assert.assertEquals(msg,im,actual[i].getImaginary(),tol * FastMath.abs(re));
    }
  }
  ...
  @Test public void testTransformComplex(){
    final DftNormalization[] norm;
    norm=DftNormalization.values();
    final TransformType[] type;
    type=TransformType.values();
  ...
  @Test public void testStandardTransformReal(){
    final DftNormalization[] norm;
    norm=DftNormalization.values();
    final TransformType[] type;
    type=TransformType.values();
  ...
  @Test public void testStandardTransformFunction(){
 ...
 */
  ...
  @Test public void test2DData(){
    ...
    for (int i=0; i < goodOutput.length; i++) {
      TransformUtils.scaleArray(goodOutput[i],FastMath.sqrt(goodOutput[i].length) * FastMath.sqrt(goodOutput.length));
    }
  ...
  @Test public void test2DDataUnitary(){
    FastFourierTransformer transformer;
    transformer=new FastFourierTransformer(DftNormalization.UNITARY);
    double tolerance=1E-12;
    Complex[][] input=new Complex[][]{new Complex[]{new Complex(1,0),new Complex(2,0)},new Complex[]{new Complex(3,1),new Complex(4,2)}};
