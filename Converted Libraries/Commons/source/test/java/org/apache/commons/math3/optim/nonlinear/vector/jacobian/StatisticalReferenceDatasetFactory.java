  ...
  public static StatisticalReferenceDataset createKirby2() throws IOException {
    ...
    try {
      dataset=new StatisticalReferenceDataset(in){
        @Override public double getModelValue(        final double x,        final double[] a){
          final double p=a[0] + x * (a[1] + x * a[2]);
          final double q=1.0 + x * (a[3] + x * a[4]);
          return p / q;
        }
        @Override public double[] getModelDerivatives(        final double x,        final double[] a){
...
;
  ...
  public static StatisticalReferenceDataset createMGH17() throws IOException {
    ...
    try {
      dataset=new StatisticalReferenceDataset(in){
        @Override public double getModelValue(        final double x,        final double[] a){
          return a[0] + a[1] * FastMath.exp(-a[3] * x) + a[2] * FastMath.exp(-a[4] * x);
        }
        @Override public double[] getModelDerivatives(        final double x,        final double[] a){
          final double[] dy=new double[5];
          dy[0]=1.0;
    ...
    try {
      dataset=new StatisticalReferenceDataset(in){
        @Override public double getModelValue(        final double x,        final double[] a){
          ...
          return a[0] * FastMath.exp(-a[3] * x) + a[1] * FastMath.exp(-a[4] * x) + a[2] * FastMath.exp(-a[5] * x);
        }
        @Override public double[] getModelDerivatives(        final double x,        final double[] a){
