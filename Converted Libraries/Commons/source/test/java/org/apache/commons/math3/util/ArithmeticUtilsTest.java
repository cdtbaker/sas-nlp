 ...
 * @version $Id: ArithmeticUtilsTest.java 1371680 2012-08-10 12:15:23Z luc $
 */
 ...
 */
  ...
  @Test public void testAddAndCheck(){
  ...
  @Test public void testAddAndCheckLong(){
  ...
  @Test public void testBinomialCoefficient(){
    ...
    for (int i=0; i < n.length; i++) {
      ...
      Assert.assertEquals(n[i] + " choose " + k[i],expected,ArithmeticUtils.binomialCoefficient(n[i],k[i]));
      Assert.assertEquals(n[i] + " choose " + k[i],expected,ArithmeticUtils.binomialCoefficientDouble(n[i],k[i]),0.0);
      Assert.assertEquals("log(" + n[i] + " choose "+ k[i]+ ")",FastMath.log(expected),ArithmeticUtils.binomialCoefficientLog(n[i],k[i]),0.0);
    }
  }
 ...
 * JIRA: MATH-241
 */
  @Test public void testBinomialCoefficientLarge() throws Exception {
    for (int n=0; n <= 200; n++) {
      for (int k=0; k <= n; k++) {
        long ourResult=-1;
        long exactResult=-1;
        ...
        if (!shouldThrow && exactResult > 1) {
          Assert.assertEquals(n + " choose " + k,1.,ArithmeticUtils.binomialCoefficientDouble(n,k) / exactResult,1e-10);
          Assert.assertEquals(n + " choose " + k,1,ArithmeticUtils.binomialCoefficientLog(n,k) / FastMath.log(exactResult),1e-10);
        }
      }
    }
  ...
  @Test public void testFactorialFail(){
    try {
  ...
  @Test public void testMulAndCheck(){
    int big=Integer.MAX_VALUE;
    int bigNeg=Integer.MIN_VALUE;
  ...
  @Test public void testSubAndCheck(){
    int big=Integer.MAX_VALUE;
    int bigNeg=Integer.MIN_VALUE;
  ...
  @Test public void testSubAndCheckErrorMessage(){
    int big=Integer.MAX_VALUE;
    try {
      ArithmeticUtils.subAndCheck(big,-1);
      Assert.fail("Expecting MathArithmeticException");
  ...
  @Test public void testSubAndCheckLong(){
    long max=Long.MAX_VALUE;
    long min=Long.MIN_VALUE;
  ...
  @Test public void testIsPowerOfTwo(){
    final int n=1025;
  ...
  @Test public void testStirlingS2(){
    ...
    for (int n=1; n < 30; ++n) {
      ...
      if (n > 2) {
        ...
        Assert.assertEquals(ArithmeticUtils.binomialCoefficient(n,2),ArithmeticUtils.stirlingS2(n,n - 1));
      }
  ...
  @Test(expected=NotPositiveException.class) public void testStirlingS2NegativeN(){
    ArithmeticUtils.stirlingS2(3,-1);
  }
  @Test(expected=NumberIsTooLargeException.class) public void testStirlingS2LargeK(){
    ArithmeticUtils.stirlingS2(3,4);
  }
  @Test(expected=MathArithmeticException.class) public void testStirlingS2Overflow(){
