...
public static class MissingRequiredPropertyException extends IllegalArgumentException {
 ...
 */
    public static Method findStaticMethod(    final String className,    final String methodName,    final List<Class<?>> signature) throws SecurityException, ClassNotFoundException {
    ...
    private static String getPropertyAsString(    final Properties properties,    final String key){
    ...
    private static int getPropertyAsInteger(    final Properties properties,    final String key){
      final String value=properties.getProperty(key);
      if (value == null) {
        throw new MissingRequiredPropertyException(key);
      }
    ...
    private ApplicationProperties(    final String fullyQualifiedName,    final String signature,    final String inputFileMask,    final String outputFileMask,    final int from,    final int to,    final int by){
      ...
      try {
        ...
        this.method=findStaticMethod(className,methodName,parameterTypes);
      }
    ...
    public static final ApplicationProperties create(    final Properties properties){
      ...
      methodFullyQualifiedName=getPropertyAsString(properties,METHOD_KEY);
  ...
  public static Object readAndWritePrimitiveValue(  final DataInputStream in,  final DataOutputStream out,  final Class<?> type) throws IOException {
    if (!type.isPrimitive()) {
  ...
  public static SummaryStatistics assessAccuracy(  final Method method,  final DataInputStream in,  final DataOutputStream out) throws IOException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
  ...
  public static void main(  final String[] args) throws IOException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    if (args.length == 0) {
