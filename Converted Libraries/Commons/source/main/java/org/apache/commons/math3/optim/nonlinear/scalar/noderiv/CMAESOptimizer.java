 ...
 */
  private void initializeCMA(  double[] guess){
    ...
    damps=(1 + 2 * Math.max(0,Math.sqrt((mueff - 1) / (dimension + 1)) - 1)) * Math.max(0.3,1 - dimension / (1e-6 + maxIterations)) + cs;
 ...
 */
  private boolean updateEvolutionPaths(  RealMatrix zmean,  RealMatrix xold){
    ...
    normps=ps.getFrobeniusNorm();
    final boolean hsig=normps / Math.sqrt(1 - Math.pow(1 - cs,2 * iterations)) / chiN < 1.4 + 2 / ((double)dimension + 1);
