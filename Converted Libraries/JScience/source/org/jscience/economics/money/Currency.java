 ...
 */
  private static final LocalContext.Reference<Currency> REFERENCE=new LocalContext.Reference<Currency>();
  /** 
 * Holds the exchanges rate to the reference currency.
 */
  private static final LocalMap<String,Double> TO_REFERENCE=new LocalMap<String,Double>();
  /** 
 ...
 * </pre>
 * @param refAmount the amount stated in the {@link #getReferenceCurrency}equals to one unit of this {@link Currency}.
 ...
 */
  public double getExchangeRate(){
    ...
    if (refAmount == null)     throw new ConversionException("Exchange rate not set for " + this.getCode());
  ...
  @Override public Unit<? super Money> getStandardUnit(){
    return Money.BASE_UNIT;
  }
  @Override public UnitConverter toStandardUnit(){
    return _toBaseUnit;
  }
  /** 
...
private static class Converter extends UnitConverter {
    ...
    @Override public double convert(    double x) throws ConversionException {
      Double refAmount=TO_REFERENCE.get(_code);
      if (refAmount == null)       throw new ConversionException("Exchange rate not set for " + _code);
      return _invert ? x / refAmount.doubleValue() : x * refAmount.doubleValue();
    }
    @Override public boolean isLinear(){
      return true;
    }
    @Override public boolean equals(    Object obj){
      if (this == obj)       return true;
      if (!(obj instanceof Converter))       return false;
