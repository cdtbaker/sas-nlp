...
public class CompoundCRS<C1 extends Coordinates<?>,C2 extends Coordinates<?>> extends CoordinateReferenceSystem<CompoundCoordinates<C1,C2>> {
  ...
  final CoordinateSystem _coordinateSystem=new CoordinateSystem(){
    public int getDimension(){
      return _first.getCoordinateSystem().getDimension() + _next.getCoordinateSystem().getDimension();
    }
    public CoordinateSystemAxis getAxis(    int dimension) throws IndexOutOfBoundsException {
      ...
      return (dimension < firstDimension) ? _first.getCoordinateSystem().getAxis(dimension) : _next.getCoordinateSystem().getAxis(dimension - firstDimension);
    }
...
;
  ...
  @Override protected CompoundCoordinates<C1,C2> coordinatesOf(  AbsolutePosition position){
    C1 c1=_first.coordinatesOf(position);
    C2 c2=_next.coordinatesOf(position);
    return CompoundCoordinates.valueOf(c1,c2);
  }
  @Override protected AbsolutePosition positionOf(  CompoundCoordinates<C1,C2> coordinates,  AbsolutePosition position){
