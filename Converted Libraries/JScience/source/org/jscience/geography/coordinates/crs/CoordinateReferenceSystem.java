...
/** 
 * This class represents an arbitrary system of reference for which {@link Coordinates coordinates} of same significance can be stated.
...
private class GeneralConverter<T extends Coordinates<?>> implements CoordinatesConverter<C,T> {
    ...
    public T convert(    C source){
      AbsolutePosition position=positionOf(source,new AbsolutePosition());
 ...
 * @throws ConversionException if a conversion error occurs.
 */
  protected abstract AbsolutePosition positionOf(  C coordinates,  AbsolutePosition position);
  /** 
...
static class Axis implements CoordinateSystemAxis {
    ...
    public Axis(    final String name,    String abbreviation,    Unit<?> unit,    AxisDirection direction){
      _name=new Name(name);
