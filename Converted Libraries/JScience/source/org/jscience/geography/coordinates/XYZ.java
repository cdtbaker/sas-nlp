 ...
 */
  public static final GeocentricCRS<XYZ> CRS=new GeocentricCRS<XYZ>(){
    @Override protected XYZ coordinatesOf(    AbsolutePosition position){
      double latitude=position.latitudeWGS84.doubleValue(RADIAN);
      double longitude=position.longitudeWGS84.doubleValue(RADIAN);
      double height=(position.heightWGS84 != null) ? position.heightWGS84.doubleValue(METRE) : 0.0;
      ...
      double z=((1.0 - WGS84.getEccentricitySquared()) * roc + height) * sinLat;
      return XYZ.valueOf(x,y,z,METRE);
    }
    @Override protected AbsolutePosition positionOf(    XYZ coordinates,    AbsolutePosition position){
      ...
      final double height=xy / Math.cos(latitude) - WGS84.verticalRadiusOfCurvature(latitude);
 ...
 */
  public static XYZ valueOf(  Float64Vector vector,  Unit<Length> unit){
    if (vector.getDimension() != 3)     throw new DimensionException("3-dimensional vector expected");
 ...
 */
  public Float64Vector toVector(  Unit<Length> unit){
    if (unit == METRE)     return Float64Vector.valueOf(_x,_y,_z);
    UnitConverter cvtr=METRE.getConverterTo(unit);
    return Float64Vector.valueOf(cvtr.convert(_x),cvtr.convert(_y),cvtr.convert(_z));
  }
  @Override public GeocentricCRS<XYZ> getCoordinateReferenceSystem(){
    return CRS;
  }
  ...
  @Override public XYZ copy(){
    return XYZ.valueOf(_x,_y,_z,METRE);
  }
  static final XMLFormat<XYZ> XML=new XMLFormat<XYZ>(XYZ.class){
    @Override public XYZ newInstance(    Class<XYZ> cls,    InputElement xml) throws XMLStreamException {
