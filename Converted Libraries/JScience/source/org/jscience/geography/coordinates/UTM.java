 ...
 */
  public static Measure<Integer,Length> UTM_FALSE_EASTING=Measure.valueOf(500000,METRE);
  /** 
 ...
 */
  public static Measure<Integer,Length> UTM_FALSE_NORTHING=Measure.valueOf(10000000,METRE);
  /** 
 ...
 */
  public static final Measure<Integer,Angle> UTM_NORTHERN_LIMIT=Measure.valueOf(84,DEGREE_ANGLE);
  /** 
 * The southern limit of the UTM grid. Beyond this limit the distortion
 * introduced by the transverse Mercator projection is impractically
 * large, and the UPS grid is used instead.
 */
  public static final Measure<Integer,Angle> UTM_SOUTHERN_LIMIT=Measure.valueOf(-80,DEGREE_ANGLE);
  /** 
 ...
 * easting to avoid using negative numbers in the coordinates.
 */
  public static Measure<Integer,Length> UPS_FALSE_EASTING=Measure.valueOf(2000000,METRE);
  /** 
 ...
 */
  public static Measure<Integer,Length> UPS_FALSE_NORTHING=Measure.valueOf(2000000,METRE);
 ...
 */
  public static final ProjectedCRS<UTM> CRS=new ProjectedCRS<UTM>(){
    ...
    @Override protected UTM coordinatesOf(    AbsolutePosition position){
      ...
      final double latitude=position.latitudeWGS84.doubleValue(NonSI.DEGREE_ANGLE);
      if (latitude > SOUTHERN_LIMIT_IN_DEGREES && latitude < NORTHERN_LIMIT_IN_DEGREES) {
    ...
    @Override protected AbsolutePosition positionOf(    UTM coordinates,    AbsolutePosition position){
      final LatLong latLong;
      if (coordinates.latitudeZone() < 'C' || coordinates.latitudeZone() > 'X') {
 ...
 */
  public final double eastingValue(  Unit<Length> unit){
    return unit.equals(METRE) ? _easting : METRE.getConverterTo(unit).convert(_easting);
  }
  /** 
 ...
 */
  public final double northingValue(  Unit<Length> unit){
    return unit.equals(METRE) ? _northing : METRE.getConverterTo(unit).convert(_northing);
  }
  @Override public ProjectedCRS<UTM> getCoordinateReferenceSystem(){
    return CRS;
  }
 ...
 */
  public static UTM latLongToUtm(  LatLong latLong,  ReferenceEllipsoid ellipsoid){
    ...
    final double t4=(kn1 * cos5Phi / 720.0) * (61.0 - 58.0 * tan2Phi + tan4Phi + 270.0 * e2c2 - 330.0 * t2e2c2 + 445.0 * e4c4 - 680.0 * t2e4c4 + 324.0 * e6c6 - 600.0 * t2e6c6 + 88.0 * e8c8 - 192.0 * t2e8c8);
    ...
    final double t8=(kn2 * cos5Phi / 120.0) * (5.0 - 18.0 * tan2Phi + tan4Phi + 14.0 * e2c2 - 58.0 * t2e2c2 + 13.0 * e4c4 - 64.0 * t2e4c4 + 4.0 * e6c6 - 24.0 * t2e6c6);
    ...
    return UTM.valueOf(longitudeZone,latitudeZone,easting,northing,METRE);
  }
  /** 
 ...
 */
  public static LatLong utmToLatLong(  UTM utm,  ReferenceEllipsoid ellipsoid){
    ...
    final double t12=tanPhi / (720.0 * rho * nu5* K06) * (61.0 + 90.0 * tan2Phi + 45.0 * tan4Phi + 46.0 * e2c2 - 252.0 * t2e2c2 - 90.0 * t4e2c2 - 3.0 * e4c4 - 66.0 * t2e4c4 + 225.0 * t4e4c4 + 100.0 * e6c6 + 84.0 * t2e6c6 + 88.0 * e8c8 - 192.0 * t2e8c8);
    final double t13=tanPhi / (40320.0 * rho * nu7* K08) * (1385.0 + 3633.0 * tan2Phi + 4095.0 * tan4Phi + 1575.0 * tan6Phi);
    final double t14=1.0 / (cosPhi * nu * K0);
    final double t15=1.0 / (6.0 * cosPhi * nu3* K03) * (1.0 + 2.0 * tan2Phi + e2c2);
    final double t16=1.0 / (120.0 * cosPhi * nu5* K05) * (5.0 + 28.0 * tan2Phi + 24.0 * tan4Phi + 6.0 * e2c2 + 8.0 * t2e2c2 - 3.0 * e4c4 + 4.0 * t2e4c4 - 4.0 * e6c6 + 24.0 * t2e6c6);
    final double t17=1.0 / (5040.0 * cosPhi * nu7* K07) * (61.0 + 662.0 * tan2Phi + 1320.0 * tan4Phi + 720.0 * tan6Phi);
 ...
 */
  public static LatLong upsToLatLong(  UTM ups,  ReferenceEllipsoid ellipsoid){
    ...
    final double dN=ups.northingValue(METRE) - UPS_FALSE_NORTHING.doubleValue(METRE);
    final double dE=ups.eastingValue(METRE) - UPS_FALSE_EASTING.doubleValue(METRE);
    if (dE == 0.0 && dN == 0.0) {
    ...
    final double phi=chi + aBar * Math.sin(2.0 * chi) + bBar * Math.sin(4.0 * chi) + cBar * Math.sin(6.0 * chi) + dBar * Math.sin(8.0 * chi);
  ...
  @Override public UTM copy(){
    return UTM.valueOf(_longitudeZone,_latitudeZone,_easting,_northing,METRE);
  }
  static final XMLFormat<UTM> XML=new XMLFormat<UTM>(UTM.class){
    @Override public UTM newInstance(    Class<UTM> cls,    InputElement xml) throws XMLStreamException {
