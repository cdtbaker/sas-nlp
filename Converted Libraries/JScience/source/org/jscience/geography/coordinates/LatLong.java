 ...
 */
  public static final GeographicCRS<LatLong> CRS=new GeographicCRS<LatLong>(){
    @Override protected LatLong coordinatesOf(    AbsolutePosition position){
      return LatLong.valueOf(position.latitudeWGS84.doubleValue(DEGREE_ANGLE),position.longitudeWGS84.doubleValue(DEGREE_ANGLE),DEGREE_ANGLE);
    }
    @Override protected AbsolutePosition positionOf(    LatLong coordinates,    AbsolutePosition position){
      position.latitudeWGS84=Measure.valueOf(coordinates._latitude,DEGREE_ANGLE);
      position.longitudeWGS84=Measure.valueOf(coordinates._longitude,DEGREE_ANGLE);
      return position;
    }
 ...
 */
  private static final UnitConverter DEGREE_TO_RADIAN=DEGREE_ANGLE.getConverterTo(RADIAN);
  /** 
 ...
 */
  ...
  private static final ObjectFactory<LatLong> FACTORY=new ObjectFactory<LatLong>(){
    @Override protected LatLong create(){
      return new LatLong();
    }
  }
;
 ...
 */
  public final double latitudeValue(  Unit<Angle> unit){
    return (unit == DEGREE_ANGLE) ? _latitude : (unit == RADIAN) ? DEGREE_TO_RADIAN.convert(_latitude) : DEGREE_ANGLE.getConverterTo(unit).convert(_latitude);
  }
  /** 
 ...
 */
  public final double longitudeValue(  Unit<Angle> unit){
    return (unit == DEGREE_ANGLE) ? _longitude : (unit == RADIAN) ? DEGREE_TO_RADIAN.convert(_longitude) : DEGREE_ANGLE.getConverterTo(unit).convert(_longitude);
  }
  @Override public GeographicCRS<LatLong> getCoordinateReferenceSystem(){
    return CRS;
  }
  ...
  static final XMLFormat<LatLong> XML=new XMLFormat<LatLong>(LatLong.class){
    @Override public LatLong newInstance(    Class<LatLong> cls,    InputElement xml) throws XMLStreamException {
      return FACTORY.object();
    }
    public void write(    LatLong latLong,    OutputElement xml) throws XMLStreamException {
      xml.setAttribute("latitude",latLong._latitude);
      xml.setAttribute("longitude",latLong._longitude);
    }
    public void read(    InputElement xml,    LatLong latLong) throws XMLStreamException {
