 ...
 */
  public static final VerticalCRS<Altitude> CRS=new VerticalCRS<Altitude>(){
    @Override protected Altitude coordinatesOf(    AbsolutePosition position){
      return Altitude.valueOf(position.heightWGS84.doubleValue(METRE),METRE);
    }
    @Override protected AbsolutePosition positionOf(    Altitude coordinates,    AbsolutePosition position){
 ...
 */
  public static Altitude valueOf(  double value,  Unit<Length> unit){
    ...
    altitude._meters=(unit == METRE) ? value : unit.getConverterTo(METRE).convert(value);
    return altitude;
  }
  private static final ObjectFactory<Altitude> FACTORY=new ObjectFactory<Altitude>(){
    @Override protected Altitude create(){
      return new Altitude();
    }
  }
;
  ...
  public final double doubleValue(  Unit<Length> unit){
    return (unit == METRE) ? _meters : METRE.getConverterTo(unit).convert(_meters);
  }
  ...
  public int compareTo(  Measurable<Length> measure){
    ...
    return (_meters > meters) ? 1 : (_meters < meters) ? -1 : 0;
  }
  @Override public Altitude copy(){
    return Altitude.valueOf(_meters,METRE);
  }
  static final XMLFormat<Altitude> XML=new XMLFormat<Altitude>(Altitude.class){
    @Override public Altitude newInstance(    Class<Altitude> cls,    InputElement xml) throws XMLStreamException {
