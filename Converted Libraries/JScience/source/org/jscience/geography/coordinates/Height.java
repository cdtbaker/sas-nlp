 ...
 */
  public static final VerticalCRS<Height> CRS=new VerticalCRS<Height>(){
    @Override protected Height coordinatesOf(    AbsolutePosition position){
      if (position.heightWGS84 instanceof Height)       return (Height)position.heightWGS84;
      return Height.valueOf(position.heightWGS84.doubleValue(METRE),METRE);
    }
    @Override protected AbsolutePosition positionOf(    Height coordinates,    AbsolutePosition position){
 ...
 */
  public static Height valueOf(  double value,  Unit<Length> unit){
    ...
    height._meters=(unit == METRE) ? value : unit.getConverterTo(METRE).convert(value);
    return height;
  }
  private static final ObjectFactory<Height> FACTORY=new ObjectFactory<Height>(){
    @Override protected Height create(){
      return new Height();
    }
  }
;
  ...
  public final double doubleValue(  Unit<Length> unit){
    return unit.equals(METRE) ? _meters : METRE.getConverterTo(unit).convert(_meters);
  }
  ...
  public int compareTo(  Measurable<Length> arg0){
    ...
    return (_meters > arg0InMeter) ? 1 : (_meters < arg0InMeter) ? -1 : 0;
  }
  @Override public Height copy(){
    return Height.valueOf(_meters,METRE);
  }
  static final XMLFormat<Height> XML=new XMLFormat<Height>(Height.class){
    @Override public Height newInstance(    Class<Height> cls,    InputElement xml) throws XMLStreamException {
