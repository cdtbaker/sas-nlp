 ...
 */
  public static final TemporalCRS<Time> CRS=new TemporalCRS<Time>(){
    @Override protected Time coordinatesOf(    AbsolutePosition position){
      if (position.timeUTC instanceof Time)       return (Time)position.timeUTC;
      return Time.valueOf(position.timeUTC.doubleValue(SECOND),SECOND);
    }
    @Override protected AbsolutePosition positionOf(    Time coordinates,    AbsolutePosition position){
 ...
 */
  ...
  private static final ObjectFactory<Time> FACTORY=new ObjectFactory<Time>(){
    @Override protected Time create(){
      return new Time();
    }
  }
;
 ...
 */
  ...
  public final double doubleValue(  Unit<Duration> unit){
    return unit.equals(SECOND) ? _seconds : SECOND.getConverterTo(unit).convert(_seconds);
  }
  ...
  public int compareTo(  Measurable<Duration> arg0){
    ...
    return (_seconds > arg0InSecond) ? 1 : (_seconds < arg0InSecond) ? -1 : 0;
  }
  public Time copy(){
    return Time.valueOf(_seconds,SECOND);
  }
  static final XMLFormat<Time> XML=new XMLFormat<Time>(Time.class){
    @Override public Time newInstance(    Class<Time> cls,    InputElement xml) throws XMLStreamException {
