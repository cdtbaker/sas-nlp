 ...
 */
  static final XMLFormat<Rational> XML=new XMLFormat<Rational>(Rational.class){
    @Override public Rational newInstance(    Class<Rational> cls,    InputElement xml) throws XMLStreamException {
 ...
 */
  public static Rational valueOf(  CharSequence chars){
    ...
    if (sep >= 0) {
      ...
      LargeInteger divisor=LargeInteger.valueOf(txt.subtext(sep + 1,chars.length()));
      return valueOf(dividend,divisor);
    }
 else {
      return valueOf(LargeInteger.valueOf(txt),LargeInteger.ONE);
    }
  }
  /** 
 ...
 */
  public LargeInteger round(){
    ...
    return isNegative() ? _dividend.minus(halfDivisor).divide(_divisor) : _dividend.plus(halfDivisor).divide(_divisor);
  }
  /** 
 ...
 */
  public Rational inverse(){
    if (_dividend.isZero())     throw new ArithmeticException("Dividend is zero");
    return _dividend.isNegative() ? Rational.valueOf(_divisor.opposite(),_dividend.opposite()) : Rational.valueOf(_divisor,_dividend);
  }
  /** 
 ...
 */
  public double doubleValue(){
    if (_dividend.isNegative())     return -this.abs().doubleValue();
