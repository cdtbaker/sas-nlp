 ...
 */
  private static final TextFormat<Complex> CARTESIAN_FORMAT=new TextFormat<Complex>(){
    ...
    public Complex parse(    CharSequence csq,    Cursor cursor){
      ...
      if ((op != '+') && (op != '-'))       throw new NumberFormatException("'+' or '-' expected");
      cursor.skip(' ',csq);
      double imaginary=TypeFormat.parseDouble(csq,cursor);
      char i=cursor.next(csq);
      if (i != 'i')       throw new NumberFormatException("'i' expected");
 ...
 */
  static final XMLFormat<Complex> XML=new XMLFormat<Complex>(Complex.class){
    @Override public Complex newInstance(    Class<Complex> cls,    InputElement xml) throws XMLStreamException {
 ...
 */
  public int hashCode(){
    int h=Float.floatToIntBits((float)_real) ^ Float.floatToIntBits((float)(_imaginary * MathLib.PI));
    h+=~(h << 9);
    h^=(h >>> 14);
    h+=(h << 4);
    return h ^ (h >>> 10);
