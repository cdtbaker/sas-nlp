 ...
 */
  static int shiftLeft(  int wordShift,  int bitShift,  long[] x,  int xSize,  long[] z){
    ...
    while (i > 0) {
      z[--j]=((tmp << bitShift) & MASK_63) | ((tmp=x[--i]) >>> shiftRight);
    }
 ...
 */
  static int shiftRight(  int wordShift,  int bitShift,  long[] x,  int xSize,  long[] z){
    ...
    long tmp=x[i];
    while (i < xSize - 1) {
      z[j++]=(tmp >>> bitShift) | ((tmp=x[++i]) << shiftLeft) & MASK_63;
    }
    tmp>>>=bitShift;
    z[j]=tmp;
    return (tmp != 0) ? j + 1 : j;
...
static final class MultiplyLogic implements Runnable {
    private static final ObjectFactory<MultiplyLogic> FACTORY=new ObjectFactory<MultiplyLogic>(){
      @Override protected MultiplyLogic create(){
