 ...
 */
  static final XMLFormat<FloatingPoint> XML=new XMLFormat<FloatingPoint>(FloatingPoint.class){
    @Override public FloatingPoint newInstance(    Class<FloatingPoint> cls,    InputElement xml) throws XMLStreamException {
      return FloatingPoint.valueOf(xml.getAttribute("value"));
    }
    public void write(    FloatingPoint FloatingPoint,    OutputElement xml) throws XMLStreamException {
 ...
 */
  private static final LocalContext.Reference<Integer> DIGITS=new LocalContext.Reference<Integer>(20);
  /** 
 ...
 */
  public static FloatingPoint valueOf(  double doubleValue){
    if (doubleValue == 0.0)     return FloatingPoint.ZERO;
    if (doubleValue == 1.0)     return FloatingPoint.ONE;
    if (Double.isNaN(doubleValue) || Double.isInfinite(doubleValue))     return FloatingPoint.NaN;
 ...
 */
  public static FloatingPoint valueOf(  CharSequence chars){
    ...
    if (real.getError() != 0)     throw new IllegalArgumentException("No error allowed");
 ...
 */
  public static void setDigits(  int digits){
    if (digits <= 0)     throw new IllegalArgumentException("digits: " + digits + " has to be greater than 0");
    DIGITS.set(digits);
  }
  /** 
 ...
 */
  public FloatingPoint plus(  FloatingPoint that){
    ...
    return FloatingPoint.valueOf(_significand.plus(thatScaled),_exponent).normalize();
  }
  /** 
 ...
 */
  public FloatingPoint minus(  FloatingPoint that){
    if (this._exponent > that._exponent)     return that.opposite().plus(this);
    int pow10Scaling=that._exponent - this._exponent;
    LargeInteger thatScaled=that._significand.times10pow(pow10Scaling);
    return FloatingPoint.valueOf(_significand.minus(thatScaled),_exponent).normalize();
  }
  /** 
 ...
 */
  public FloatingPoint inverse(){
    if (_significand.isZero())     return NaN;
    int pow10=DIGITS.get() + _significand.digitLength();
    LargeInteger dividend=LargeInteger.ONE.times10pow(pow10);
    return FloatingPoint.valueOf(dividend.divide(_significand),-pow10 - _exponent).normalize();
  }
  /** 
 ...
 */
  public FloatingPoint divide(  FloatingPoint that){
    if (that._significand.isZero())     return NaN;
 ...
 */
  public Text toText(){
    ...
    if (this._significand.isZero())     return Text.valueOf("0.0");
 ...
 */
  private FloatingPoint normalize(){
    ...
    if (thisDigits > digits) {
      ...
      if (exponent > Integer.MAX_VALUE)       return NaN;
      if (exponent < Integer.MIN_VALUE)       return ZERO;
      _exponent=(int)exponent;
    }
    return this;
  }
  @Override public FloatingPoint copy(){
    if (this == NaN)     return NaN;
