 ...
 */
  public static final Configurable<Integer> PRIME_CERTAINTY=new Configurable<Integer>(100);
  /** 
 ...
 */
  static final TextFormat<LargeInteger> DECIMAL_FORMAT=new TextFormat<LargeInteger>(){
    @Override public Appendable format(    LargeInteger li,    Appendable out) throws IOException {
      return LargeInteger.format(li,10,out);
    }
    @Override public LargeInteger parse(    CharSequence csq,    Cursor cursor){
 ...
 */
  private static final ArrayFactory<LargeInteger> ARRAY_FACTORY=new ArrayFactory<LargeInteger>(){
    @Override protected LargeInteger create(    int capacity){
 ...
 */
  private static final ObjectFactory<LargeInteger> NO_ARRAY_FACTORY=new ObjectFactory<LargeInteger>(){
    @Override protected LargeInteger create(){
 ...
 */
  static final XMLFormat<LargeInteger> XML=new XMLFormat<LargeInteger>(LargeInteger.class){
    @Override public LargeInteger newInstance(    Class<LargeInteger> cls,    InputElement xml) throws XMLStreamException {
      return LargeInteger.valueOf(xml.getAttribute("value"));
    }
    public void write(    LargeInteger li,    OutputElement xml) throws XMLStreamException {
 ...
 */
  public static LargeInteger valueOf(  long value){
    ...
    if (value == Long.MIN_VALUE)     return LONG_MIN_VALUE;
 ...
 */
  public static LargeInteger valueOf(  byte[] bytes,  int offset,  int length){
    ...
    if (isNegative && wordIndex < 0)     wordIndex=0;
    li._size=wordIndex + 1;
    li._isNegative=isNegative;
    if (isNegative) {
      li._size=Calculus.add(li._words,li._size,ONE._words,1,li._words);
    }
    return li;
  }
  /** 
    ...
    if (_isNegative) {
      ...
      for (int i=bytesLength + offset; i > offset; bitIndex+=8) {
        if (bitIndex < 63 - 8) {
          ...
          byte bits=(byte)word;
          word=(++wordIndex < _size) ? _words[wordIndex] + borrow : borrow;
          borrow=word >> 63;
          word=~word & MASK_63;
          bitIndex-=63;
 ...
 */
  public static LargeInteger valueOf(  CharSequence csq,  int radix){
    ...
    try {
      ...
      if (cursor.hasNext())       throw new NumberFormatException("Cannot parse " + csq + " at "+ cursor);
 ...
 */
  public int digitLength(){
    ...
    return (LargeInteger.ONE.times10pow(min).isLargerThan(this)) ? min : min + 1;
  }
 ...
 */
  public LargeInteger abs(){
    if (_isNegative)     return this.opposite();
    return this;
  }
  /** 
 ...
 */
  public LargeInteger plus(  LargeInteger that){
    if (this._size < that._size)     return that.plus(this);
    if ((this._isNegative != that._isNegative) && (that._size != 0))     return this.minus(that.opposite());
    LargeInteger li=ARRAY_FACTORY.array(_size + 1);
    li._size=Calculus.add(_words,_size,that._words,that._size,li._words);
 ...
 */
  public LargeInteger minus(  LargeInteger that){
    if ((this._isNegative != that._isNegative) && (that._size != 0))     return this.plus(that.opposite());
    if (that.isLargerThan(this))     return that.minus(this).opposite();
    LargeInteger li=ARRAY_FACTORY.array(this._size);
    li._size=Calculus.subtract(_words,_size,that._words,that._size,li._words);
 ...
 */
  public LargeInteger times(  LargeInteger that){
    if (that._size > this._size)     return that.times(this);
    if (that._size <= 1)     return this.times(that.longValue());
    if (that._size < 10) {
  ...
  private LargeInteger shiftWordLeft(  int w){
    if (_size == 0)     return LargeInteger.ZERO;
 ...
 */
  public LargeInteger times(  long multiplier){
    if ((this._size == 0) || (multiplier == 0))     return LargeInteger.ZERO;
    if (multiplier == Long.MIN_VALUE)     return times(LONG_MIN_VALUE);
 ...
 */
  public LargeInteger divide(  LargeInteger that){
    if ((that._size <= 1) && ((that._words[0] >> 31) == 0))     return divide(that.intValue());
    LargeInteger result;
    ...
    if (precision <= 0) {
      ...
      if (remainder.compareTo(thatAbs) >= 0) {
        ...
        if (remainder.compareTo(thatAbs) >= 0)         throw new Error("Verification error for " + this + "/"+ that+ ", please submit a bug report.");
      }
 else       if (remainder.isNegative()) {
        remainder=remainder.plus(thatAbs);
        result=result.minus(ONE);
        if (remainder.isNegative())         throw new Error("Verification error for " + this + "/"+ that+ ", please submit a bug report.");
      }
    }
    LargeInteger li=NO_ARRAY_FACTORY.object();
 ...
 */
  public LargeInteger divide(  int divisor){
    ...
    long rem=Calculus.divide(_words,_size,MathLib.abs(divisor),li._words);
    li._size=(_size > 0) && (li._words[_size - 1] == 0L) ? _size - 1 : _size;
 ...
 */
  public LargeInteger inverseScaled(  int precision){
    if (precision <= 30) {
      ...
      LargeInteger x=inverseScaled(precision / 2 + 1);
 ...
 */
  public LargeInteger modInverse(  LargeInteger m){
    if (!m.isPositive())     throw new ArithmeticException("Modulus is not a positive number");
    StackContext.enter();
    try {
      ...
      if (!a.abs().equals(ONE))       throw new ArithmeticException("GCD(" + this + ", "+ m+ ") = "+ a);
 ...
 */
  ...
  private void subtract(  LargeInteger that){
    _size=Calculus.subtract(_words,_size,that._words,that._size,_words);
  }
  /** 
 ...
 */
  public LargeInteger shiftLeft(  int n){
    ...
    li._isNegative=_isNegative;
    li._size=Calculus.shiftLeft(wordShift,bitShift,_words,_size,li._words);
    return li;
  }
  /** 
 * Returns the value of this large integer after performing a binary
 ...
 */
  public LargeInteger times2pow(  int n){
    ...
    int wordShift=n < 63 ? 0 : n / 63;
    int bitShift=n - ((wordShift << 6) - wordShift);
    if (_size <= wordShift)     return LargeInteger.ZERO;
    LargeInteger li=ARRAY_FACTORY.array(_size - wordShift);
    li._size=Calculus.shiftRight(wordShift,bitShift,_words,_size,li._words);
    li._isNegative=_isNegative && (li._size != 0);
    return li;
  }
  /** 
 ...
 */
  public LargeInteger times10pow(  int n){
    if (this._size == 0)     return LargeInteger.ZERO;
    if (n >= 0) {
      int bitLength=(int)(n * DIGITS_TO_BITS);
      LargeInteger li=ARRAY_FACTORY.array(_size + (bitLength / 63) + 1);
      li._isNegative=_isNegative;
      int i=(n >= LONG_POW_5.length) ? LONG_POW_5.length - 1 : n;
      li._size=Calculus.multiply(_words,_size,LONG_POW_5[i],li._words);
      ...
      final int wordShift=n < 63 ? 0 : n / 63;
      final int bitShift=n - ((wordShift << 6) - wordShift);
  ...
  private static final int[] INT_POW_5=new int[]{1,5,25,125,625,3125,15625,78125,390625,1953125,9765625,48828125,244140625,1220703125};
  private static final long[] LONG_POW_5=new long[]{1L,5L,25L,125L,625L,3125L,15625L,78125L,390625L,1953125L,9765625L,48828125L,244140625L,1220703125L,6103515625L,30517578125L,152587890625L,762939453125L,3814697265625L,19073486328125L,95367431640625L,476837158203125L,2384185791015625L,11920928955078125L,59604644775390625L,298023223876953125L,1490116119384765625L,7450580596923828125L};
  /** 
 ...
 */
  public boolean equals(  Object that){
    if (!(that instanceof LargeInteger))     return false;
    LargeInteger li=(LargeInteger)that;
    return (_size == li._size) && (_isNegative == li._isNegative) && (compare(_words,li._words,_size) == 0);
  }
  /** 
 ...
 */
  public boolean equals(  long value){
    ...
    return ((_size <= 1) && (_isNegative ? -_words[0] == value : _words[0] == value)) || ((value == Long.MIN_VALUE) && (_isNegative) && (_size == 2)&& (_words[1] == 1)&& (_words[0] == 0));
  }
  /** 
 ...
 */
  public long longValue(){
    if (_size == 0)     return 0;
    return (_size <= 1) ? (_isNegative ? -_words[0] : _words[0]) : (_isNegative ? -((_words[1] << 63) | _words[0]) : (_words[1] << 63) | _words[0]);
  }
  /** 
 ...
 */
  public double doubleValue(){
    if (_size == 0)     return 0;
    if (_size <= 1)     return _isNegative ? -_words[0] : _words[0];
    final int n=_size - 1;
    final int bitLength=MathLib.bitLength(_words[n]) + (n << 6) - n;
 ...
 */
  public int compareTo(  LargeInteger that){
    if (_isNegative && !that._isNegative)     return -1;
    if (!_isNegative && that._isNegative)     return 1;
    if (_size > that._size)     return _isNegative ? -1 : 1;
    if (that._size > _size)     return _isNegative ? 1 : -1;
    return _isNegative ? compare(that._words,_words,_size) : compare(_words,that._words,_size);
  }
  /** 
 ...
 */
  public int compareTo(  long value){
    if (_size > 1)     return (value == Long.MIN_VALUE) && (this.equals(Long.MIN_VALUE)) ? 0 : (_isNegative ? -1 : 1);
    long thisValue=_isNegative ? -_words[0] : _words[0];
    return thisValue < value ? -1 : ((thisValue == value) ? 0 : 1);
  }
  @Override public LargeInteger copy(){
    LargeInteger li=ARRAY_FACTORY.array(_size);
    li._isNegative=_isNegative;
    li._size=_size;
    if (_size <= 1) {
 ...
 */
  ...
  private static long pow(  int radix,  int n){
    if (radix == 10)     return LONG_POW_10[n];
    if (radix == 16)     return LONG_POW_16[n];
    long l=1;
    for (int i=0; i < n; i++) {
      l*=radix;
    }
  ...
  private static final long[] LONG_POW_10=new long[]{1,10,100,1000,10000,100000,1000000,10000000,100000000,1000000000,10000000000L,100000000000L,1000000000000L,10000000000000L,100000000000000L,1000000000000000L,10000000000000000L,100000000000000000L,1000000000000000000L};
  private static final long[] LONG_POW_16=new long[]{0x1,0x10,0x100,0x1000,0x10000,0x100000,0x1000000,0x10000000,0x100000000L,0x1000000000L,0x10000000000L,0x100000000000L,0x1000000000000L,0x10000000000000L,0x100000000000000L,0x1000000000000000L};
  /** 
 ...
 */
  ...
  private static Appendable write(  LargeInteger li,  int radix,  int divisor,  Appendable out) throws IOException {
    if (li._size <= 1)     return TypeFormat.format(li._size == 0 ? 0 : li._words[0],radix,out);
    int rem=(int)Calculus.divide(li._words,li._size,divisor,li._words);
    if (li._words[li._size - 1] == 0L) {
      li._size--;
    }
    write(li,radix,divisor,out);
    divisor/=radix;
    while (rem < divisor) {
      out.append('0');
      divisor/=radix;
    ...
    if (0 != rem) {
      return TypeFormat.format(rem,radix,out);
    }
 else {
      return out;
    }
