 ...
 */
  static final XMLFormat<Real> XML=new XMLFormat<Real>(Real.class){
    @Override public Real newInstance(    Class<Real> cls,    InputElement xml) throws XMLStreamException {
 ...
 */
  private static final LocalContext.Reference<Integer> EXACT_PRECISION=new LocalContext.Reference<Integer>(new Integer(19));
  /** 
 ...
 * @param precision the minimum number of digits assumed exact for {@link #isExact exact} numbers.
 */
 ...
 */
  public static Real valueOf(  double doubleValue){
    if (doubleValue == 0.0)     return Real.ZERO;
    if (Double.isNaN(doubleValue) || Double.isInfinite(doubleValue))     return Real.NaN;
    int e=MathLib.floorLog10(MathLib.abs(doubleValue)) - 18 + 1;
 ...
 */
  public static Real valueOf(  CharSequence chars) throws NumberFormatException {
    ...
    Text txt=Text.valueOf(chars);
    if ((txt.length() == 3) && (txt.indexOf("NaN",0) == 0))     return NaN;
    if (txt.equals("0"))     return ZERO;
    int exponentIndex=txt.indexOf("E",0);
    if (exponentIndex >= 0) {
      int exponent=TypeFormat.parseInt(txt.subtext(exponentIndex + 1,txt.length()));
      Real r=valueOf(txt.subtext(0,exponentIndex));
      if (r == ZERO)       return valueOf(LargeInteger.ZERO,1,exponent);
    ...
    if (errorIndex >= 0) {
      ...
      real._error=LargeInteger.valueOf(txt.subtext(errorIndex + 1,txt.length()));
      if (real._error.isNegative())       throw new NumberFormatException(chars + " not parsable (error cannot be negative)");
      real._exponent=0;
      return real;
    }
    int decimalPointIndex=txt.indexOf(".",0);
    if (decimalPointIndex >= 0) {
      LargeInteger integer=LargeInteger.valueOf(txt.subtext(0,decimalPointIndex));
      LargeInteger fraction=LargeInteger.valueOf(txt.subtext(decimalPointIndex + 1,txt.length()));
 ...
 */
  public int getAccuracy(){
    if (_error.isZero())     return Integer.MAX_VALUE;
 ...
 */
  public LargeInteger round(){
    if (this == NaN)     throw new ArithmeticException("Cannot convert NaN to integer value");
 ...
 */
  public Real plus(  Real that){
    ...
    if (this._exponent > that._exponent)     return that.plus(this);
 ...
 */
  public Real times(  Real that){
    ...
    if (exp > Integer.MAX_VALUE || (exp < Integer.MIN_VALUE))     return NaN;
 ...
 */
  public Real inverse(){
    if ((this == NaN) || (this == ZERO))     return NaN;
    if (this.isExact())     return this.toInexact().inverse();
    LargeInteger thisMin=this._significand.minus(this._error);
    LargeInteger thisMax=this._significand.plus(this._error);
    if (thisMin.isNegative() && thisMax.isPositive())     return NaN;
 ...
 */
  public Real sqrt(){
    ...
    if (this.isExact())     return this.toInexact().sqrt();
    LargeInteger thisMin=this._significand.minus(this._error);
    LargeInteger thisMax=this._significand.plus(this._error);
 ...
 */
  public Text toText(){
    ...
    if (isExact()) {
      return (_exponent == 0) ? _significand.toText() : _significand.toText().plus("E").plus(Text.valueOf(_exponent));
    }
    int errorDigits=_error.digitLength();
    LargeInteger m=(_significand.isPositive()) ? _significand.plus(FIVE.times10pow(errorDigits - 1)) : _significand.plus(MINUS_FIVE.times10pow(errorDigits - 1));
