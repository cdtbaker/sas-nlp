 ...
 */
public final class Term implements Serializable, Comparable<Term>, ValueType, Realtime {
  /** 
 ...
 */
  private static final ArrayFactory<Term> FACTORY=new ArrayFactory<Term>(){
    @Override protected Term create(    int size){
 ...
 */
  public static Term valueOf(  Variable<?> v,  int n){
    if (n == 0)     return ONE;
    if (n < 0)     throw new IllegalArgumentException("n: " + n + " negative values are not allowed");
 ...
 */
  public Variable<?> getVariable(  int index){
    if (index > _size)     throw new IllegalArgumentException();
 ...
 */
  public int getPower(  Variable<?> v){
    for (int i=0; i < _size; i++) {
      if (_variables[i] == v)       return _powers[i];
    }
    return 0;
  }
  /** 
  ...
  public Term divide(  Term that){
    ...
    for (int i=0, j=0; ; ) {
      ...
      if (right == left) {
        ...
        if (power < 0)         throw new UnsupportedOperationException(this + "/" + that+ " would result in a negative power");
        if (power > 0) {
 ...
 */
  public boolean equals(  Object obj){
    ...
    for (int i=0; i < _size; i++) {
      if ((!this._variables[i].equals(that._variables[i])) || (this._powers[i] != that._powers[i]))       return false;
    }
    return true;
  }
  /** 
 ...
 */
@SuppressWarnings("unchecked") Ring evaluate(){
Ring result=null;
for (int i=0; i < _size; i++) {
Ring pow2=(Ring)_variables[i].get();
if (pow2 == null) throw new FunctionException("Variable: " + _variables[i] + " is not set");
int n=_powers[i];
while (n >= 1) {
if ((n & 1) == 1) {
result=(result == null) ? pow2 : (Ring)result.times(pow2);
}
