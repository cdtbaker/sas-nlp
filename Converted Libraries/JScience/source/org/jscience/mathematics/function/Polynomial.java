 ...
 */
  public Polynomial<R> plus(  Polynomial<R> that){
    ...
    for (FastMap.Entry<Term,R> e=result._termToCoef.head(), end=result._termToCoef.tail(); (e=e.getNext()) != end; ) {
      Term term=e.getKey();
 ...
 */
  public Polynomial<R> opposite(){
    Polynomial<R> result=Polynomial.newInstance();
    for (FastMap.Entry<Term,R> e=_termToCoef.head(), end=_termToCoef.tail(); (e=e.getNext()) != end; ) {
 ...
 */
  public Polynomial<R> compose(  Polynomial<R> that){
    ...
    if (getVariables().size() != 1)     throw new FunctionException("This polynomial is not monovariate");
    ...
    for (    Map.Entry<Term,R> entry : this._termToCoef.entrySet()) {
      ...
      if (power > 0) {
        ...
        result=(result != null) ? result.plus(cst.times(fn)) : cst.times(fn);
      }
 else {
  ...
  @SuppressWarnings("unchecked") @Override public <Z>Function<Z,R> compose(  Function<Z,R> that){
    return (Function<Z,R>)((that instanceof Polynomial) ? compose((Polynomial)that) : super.compose(that));
  }
  @Override public Polynomial<R> differentiate(  Variable<R> v){
  ...
  @SuppressWarnings("unchecked") @Override public Polynomial<R> integrate(  Variable<R> v){
    Polynomial<R> result=null;
    for (    Map.Entry<Term,R> entry : this._termToCoef.entrySet()) {
      Term term=entry.getKey();
      R coef=entry.getValue();
  ...
  @SuppressWarnings("unchecked") @Override public Function<R,R> plus(  Function<R,R> that){
    return (that instanceof Polynomial) ? this.plus((Polynomial<R>)that) : super.plus(that);
  }
  @SuppressWarnings("unchecked") @Override public Function<R,R> minus(  Function<R,R> that){
    return (that instanceof Polynomial) ? this.minus((Polynomial<R>)that) : super.minus(that);
  }
  @SuppressWarnings("unchecked") @Override public Function<R,R> times(  Function<R,R> that){
    return (that instanceof Polynomial) ? this.times((Polynomial<R>)that) : super.times(that);
  }
  @SuppressWarnings("unchecked") @Override public Polynomial<R> pow(  int n){
    return (Polynomial<R>)super.pow(n);
  }
  @SuppressWarnings("unchecked") @Override public List<Variable<R>> getVariables(){
    Term product=_termToCoef.head().getNext().getKey();
    for (FastMap.Entry<Term,R> e=_termToCoef.head().getNext(), end=_termToCoef.tail(); (e=e.getNext()) != end; ) {
  ...
  @Override @SuppressWarnings("unchecked") public R evaluate(){
    R sum=null;
 ...
 */
  ...
  @SuppressWarnings("unchecked") private static <R extends Ring<R>>Polynomial<R> newInstance(){
    Polynomial p=FACTORY.object();
    p._termToCoef.clear();
    return p;
  }
  @SuppressWarnings("unchecked") private static final ObjectFactory<Polynomial> FACTORY=new ObjectFactory<Polynomial>(){
