 ...
 */
  @SuppressWarnings("unchecked") public static <F extends Field<F>>RationalFunction<F> valueOf(  Polynomial<F> dividend,  Polynomial<F> divisor){
    RationalFunction<F> rf=FACTORY.object();
    rf._dividend=dividend;
    rf._divisor=divisor;
    return rf;
  }
  @SuppressWarnings("unchecked") private static final ObjectFactory<RationalFunction> FACTORY=new ObjectFactory<RationalFunction>(){
    protected RationalFunction create(){
      return new RationalFunction();
    }
    @SuppressWarnings("unchecked") protected void cleanup(    RationalFunction rf){
      rf._dividend=null;
      rf._divisor=null;
    }
  }
 ...
 */
  public RationalFunction<F> plus(  RationalFunction<F> that){
    return valueOf(this._dividend.times(that._divisor).plus(this._divisor.times(that._dividend)),this._divisor.times(that._divisor));
  }
  /** 
 ...
 */
  public RationalFunction<F> times(  RationalFunction<F> that){
    return valueOf(this._dividend.times(that._dividend),this._divisor.times(that._divisor));
  }
  /** 
 ...
 */
  ...
  @SuppressWarnings("unchecked") @Override public List<Variable<F>> getVariables(){
    return merge(_dividend.getVariables(),_divisor.getVariables());
  }
  @SuppressWarnings("unchecked") @Override public F evaluate(){
  ...
  @Override public RationalFunction<F> differentiate(  Variable<F> v){
    return valueOf(_divisor.times(_dividend.differentiate(v)).plus(_dividend.times(_divisor.differentiate(v)).opposite()),_dividend.pow(2));
  }
  @SuppressWarnings("unchecked") @Override public Function<F,F> plus(  Function<F,F> that){
    return (that instanceof RationalFunction) ? this.plus((RationalFunction<F>)that) : super.plus(that);
  }
  @SuppressWarnings("unchecked") @Override public Function<F,F> minus(  Function<F,F> that){
    return (that instanceof RationalFunction) ? this.minus((RationalFunction<F>)that) : super.minus(that);
  }
  @SuppressWarnings("unchecked") @Override public Function<F,F> times(  Function<F,F> that){
    return (that instanceof RationalFunction) ? this.times((RationalFunction<F>)that) : super.times(that);
  }
  @SuppressWarnings("unchecked") @Override public Function<F,F> divide(  Function<F,F> that){
    return (that instanceof RationalFunction) ? this.divide((RationalFunction<F>)that) : super.divide(that);
  }
  @SuppressWarnings("unchecked") @Override public RationalFunction<F> pow(  int n){
