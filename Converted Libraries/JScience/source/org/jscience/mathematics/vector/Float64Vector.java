 ...
 */
  protected static final XMLFormat<Float64Vector> XML=new XMLFormat<Float64Vector>(Float64Vector.class){
    @Override public Float64Vector newInstance(    Class<Float64Vector> cls,    InputElement xml) throws XMLStreamException {
      int dimension=xml.getAttribute("dimension",0);
      Float64Vector V=FACTORY.array(dimension);
      V._dimension=dimension;
      return V;
    }
    @SuppressWarnings("unchecked") @Override public void read(    InputElement xml,    Float64Vector V) throws XMLStreamException {
      for (int i=0, n=V._dimension; i < n; ) {
        V._values[i++]=((Float64)xml.getNext()).doubleValue();
      }
      if (xml.hasNext())       throw new XMLStreamException("Too many elements");
    }
    @Override public void write(    Float64Vector V,    OutputElement xml) throws XMLStreamException {
 ...
 */
  private static final ArrayFactory<Float64Vector> FACTORY=new ArrayFactory<Float64Vector>(){
    @Override protected Float64Vector create(    int capacity){
 ...
 */
  public static Float64Vector valueOf(  Vector<Float64> that){
    if (that instanceof Float64Vector)     return (Float64Vector)that;
 ...
 */
  public double getValue(  int i){
    if (i >= _dimension)     throw new ArrayIndexOutOfBoundsException();
    return _values[i];
  }
  /** 
 ...
 */
  ...
  @Override public Float64 get(  int i){
    if (i >= _dimension)     throw new IndexOutOfBoundsException();
    return Float64.valueOf(_values[i]);
  }
  @Override public Float64Vector opposite(){
  ...
  @Override public Float64Vector plus(  Vector<Float64> that){
  ...
  @Override public Float64Vector minus(  Vector<Float64> that){
    Float64Vector T=Float64Vector.valueOf(that);
    if (T._dimension != _dimension)     throw new DimensionException();
    Float64Vector V=FACTORY.array(_dimension);
    V._dimension=_dimension;
  ...
  @Override public Float64Vector times(  Float64 k){
    Float64Vector V=FACTORY.array(_dimension);
    V._dimension=_dimension;
 ...
 */
  ...
  @Override public Float64 times(  Vector<Float64> that){
    Float64Vector T=Float64Vector.valueOf(that);
    if (T._dimension != _dimension)     throw new DimensionException();
  ...
  @Override public Float64Vector cross(  Vector<Float64> that){
    Float64Vector T=Float64Vector.valueOf(that);
    if ((this._dimension != 3) || (T._dimension != 3))     throw new DimensionException("The cross product of two vectors requires " + "3-dimensional vectors");
  ...
  @Override public Float64Vector copy(){
    Float64Vector V=FACTORY.array(_dimension);
    V._dimension=_dimension;
    for (int i=0; i < _dimension; i++) {
      V._values[i]=_values[i];
