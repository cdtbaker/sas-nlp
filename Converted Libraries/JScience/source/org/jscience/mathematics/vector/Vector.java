 ...
 */
  public Vector<F> cross(  Vector<F> that){
    if ((this.getDimension() != 3) || (that.getDimension() != 3))     throw new DimensionException("The cross product of two vectors requires " + "3-dimensional vectors");
    FastTable<F> elements=FastTable.newInstance();
    elements.add((this.get(1).times(that.get(2))).plus((this.get(2).times(that.get(1))).opposite()));
    elements.add((this.get(2).times(that.get(0))).plus((this.get(0).times(that.get(2))).opposite()));
    elements.add((this.get(0).times(that.get(1))).plus((this.get(1).times(that.get(0))).opposite()));
 ...
 */
  public boolean equals(  Vector<F> that,  Comparator<F> cmp){
    ...
    final int dimension=this.getDimension();
    if (that.getDimension() != dimension)     return false;
    for (int i=dimension; --i >= 0; ) {
      if (cmp.compare(this.get(i),that.get(i)) != 0)       return false;
    }
    return true;
  }
  /** 
 ...
 */
  public boolean equals(  Object that){
    ...
    if (v.getDimension() != dimension)     return false;
    for (int i=dimension; --i >= 0; ) {
