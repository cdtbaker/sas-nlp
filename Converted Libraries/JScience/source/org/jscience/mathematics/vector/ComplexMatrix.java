 ...
 */
  ...
  @Override public Complex get(  int i,  int j){
    return _transposed ? _rows.get(j).get(i) : _rows.get(i).get(j);
  }
  @Override public ComplexVector getRow(  int i){
  ...
  @Override public ComplexVector getColumn(  int j){
  ...
  @Override public ComplexVector getDiagonal(){
  ...
  @Override public ComplexMatrix opposite(){
  ...
  @Override public ComplexMatrix plus(  Matrix<Complex> that){
  ...
  @Override public ComplexMatrix minus(  Matrix<Complex> that){
    return this.plus(that.opposite());
  }
  @Override public ComplexMatrix times(  Complex k){
    ComplexMatrix M=ComplexMatrix.newInstance(_n,_transposed);
    for (int i=0, p=_rows.size(); i < p; i++) {
  ...
  @Override public ComplexVector times(  Vector<Complex> v){
  ...
  @Override public ComplexMatrix times(  Matrix<Complex> that){
...
;
  ...
  @Override public ComplexMatrix inverse(){
  ...
  @Override public Complex determinant(){
    return LUDecomposition.valueOf(this).determinant();
  }
  @Override public ComplexMatrix transpose(){
    ComplexMatrix M=ComplexMatrix.newInstance(_n,!_transposed);
    M._rows.addAll(this._rows);
    return M;
  }
  @Override public Complex cofactor(  int i,  int j){
    if (_transposed) {
      int k=i;
      i=j;
      j=k;
  ...
  @Override public ComplexMatrix adjoint(){
    ComplexMatrix M=ComplexMatrix.newInstance(_n,_transposed);
    int m=_rows.size();
    for (int i=0; i < m; i++) {
  ...
  @Override public ComplexMatrix tensor(  Matrix<Complex> that){
    return ComplexMatrix.valueOf(DenseMatrix.valueOf(this).tensor(that));
  }
  @Override public ComplexVector vectorization(){
    return ComplexVector.valueOf(DenseMatrix.valueOf(this).vectorization());
  }
  @Override public ComplexMatrix copy(){
    ComplexMatrix M=ComplexMatrix.newInstance(_n,_transposed);
  ...
  private static ObjectFactory<ComplexMatrix> FACTORY=new ObjectFactory<ComplexMatrix>(){
    @Override protected ComplexMatrix create(){
