 ...
 */
  public static <F extends Field<F>>SparseMatrix<F> valueOf(  SparseVector<F>... rows){
    ...
    for (int i=0, m=rows.length; i < m; i++) {
      ...
      if (rowi._dimension != n)       throw new DimensionException("All vectors must have the same dimension.");
      if (!zero.equals(rowi._zero))       throw new DimensionException("All vectors must have the same zero element.");
      M._rows.add(rowi);
    }
    return M;
  }
 ...
 */
  public static <F extends Field<F>>SparseMatrix<F> valueOf(  List<SparseVector<F>> rows){
    ...
    for (int i=0, m=rows.size(); i < m; i++) {
      ...
      if (rowi.getDimension() != n)       throw new DimensionException("All vectors must have the same dimension.");
      if (!zero.equals(rowi._zero))       throw new DimensionException("All vectors must have the same zero element.");
      M._rows.add(rowi);
    }
    return M;
 ...
 */
  public static <F extends Field<F>>SparseMatrix<F> valueOf(  Matrix<F> that,  F zero,  FastComparator<? super F> comparator){
    if (that instanceof SparseMatrix)     return (SparseMatrix<F>)that;
    int n=that.getNumberOfColumns();
    int m=that.getNumberOfRows();
    SparseMatrix<F> M=SparseMatrix.newInstance(n,zero,false);
    for (int i=0; i < m; i++) {
      SparseVector<F> rowi=SparseVector.valueOf(that.getRow(i),zero,comparator);
      M._rows.add(rowi);
    }
    return M;
  }
 ...
 */
  ...
  @Override public F get(  int i,  int j){
    return _transposed ? _rows.get(j).get(i) : _rows.get(i).get(j);
  }
  @Override public SparseVector<F> getRow(  int i){
  ...
  @Override public SparseVector<F> getColumn(  int j){
  ...
  @Override public SparseVector<F> getDiagonal(){
  ...
  @Override public SparseMatrix<F> opposite(){
  ...
  @Override public SparseMatrix<F> plus(  Matrix<F> that){
    if (this.getNumberOfRows() != that.getNumberOfRows())     throw new DimensionException();
    SparseMatrix<F> M=SparseMatrix.newInstance(_n,_zero,_transposed);
    for (int i=0, p=_rows.size(); i < p; i++) {
  ...
  @Override public SparseMatrix<F> minus(  Matrix<F> that){
    return this.plus(that.opposite());
  }
  @Override public SparseMatrix<F> times(  F k){
    SparseMatrix<F> M=SparseMatrix.newInstance(_n,_zero,_transposed);
    for (int i=0, p=_rows.size(); i < p; i++) {
  ...
  @Override public SparseVector<F> times(  Vector<F> v){
    if (v.getDimension() != this.getNumberOfColumns())     throw new DimensionException();
  ...
  @Override public SparseMatrix<F> times(  Matrix<F> that){
    final int m=this.getNumberOfRows();
    final int n=this.getNumberOfColumns();
    final int p=that.getNumberOfColumns();
    if (that.getNumberOfRows() != n)     throw new DimensionException();
  ...
  @Override public SparseMatrix<F> inverse(){
    if (!isSquare())     throw new DimensionException("Matrix not square");
    ...
    for (int i=0, m=A._rows.size(); i < m; i++) {
      ...
      for (FastMap.Entry<Index,F> e=row._elements.head(), end=row._elements.tail(); (e=e.getNext()) != end; ) {
  ...
  @Override public F determinant(){
    if (!isSquare())     throw new DimensionException("Matrix not square");
    if (_n == 1)     return this.get(0,0);
    SparseVector<F> row0=this.getRow(0);
    F det=null;
    for (FastMap.Entry<Index,F> e=row0._elements.head(), end=row0._elements.tail(); (e=e.getNext()) != end; ) {
  ...
  @Override public Matrix<F> solve(  Matrix<F> y){
    return this.inverse().times(y);
  }
  @Override public SparseMatrix<F> transpose(){
    SparseMatrix<F> M=SparseMatrix.newInstance(_n,_zero,!_transposed);
    M._rows.addAll(this._rows);
    return M;
  }
  @Override public F cofactor(  int i,  int j){
    if (_transposed) {
      int k=i;
      i=j;
      j=k;
  ...
  @Override public SparseMatrix<F> adjoint(){
    SparseMatrix<F> M=SparseMatrix.newInstance(_n,_zero,_transposed);
    int m=_rows.size();
    for (int i=0; i < m; i++) {
      ...
      for (int j=0; j < _n; j++) {
        ...
        if (!_zero.equals(cofactor)) {
          row._elements.put(Index.valueOf(j),((i + j) % 2 == 0) ? cofactor : cofactor.opposite());
        }
      }
    }
    return M.transpose();
  }
  @Override public SparseMatrix<F> tensor(  Matrix<F> that){
    ...
    for (int i=0; i < m; i++) {
      ...
      for (FastMap.Entry<Index,F> e=thisRow._elements.head(), end=thisRow._elements.tail(); (e=e.getNext()) != end; ) {
        F a=e.getValue();
  ...
  @Override public SparseVector<F> vectorization(){
    SparseVector<F> V=SparseVector.newInstance(_n * this.getNumberOfRows(),_zero);
    int offset=0;
    for (int j=0, n=this.getNumberOfColumns(); j < n; j++) {
      SparseVector<F> column=this.getColumn(j);
      for (FastMap.Entry<Index,F> e=column._elements.head(), end=column._elements.tail(); (e=e.getNext()) != end; ) {
        V._elements.put(Index.valueOf(e.getKey().intValue() + offset),e.getValue());
      }
      offset+=this.getNumberOfRows();
    }
    return V;
  }
  @SuppressWarnings("unchecked") @Override public SparseMatrix<F> copy(){
  ...
  @SuppressWarnings("unchecked") static <F extends Field<F>>SparseMatrix<F> newInstance(  int n,  F zero,  boolean transposed){
  ...
  private static final ObjectFactory<SparseMatrix> FACTORY=new ObjectFactory<SparseMatrix>(){
    @Override protected SparseMatrix create(){
      return new SparseMatrix();
    }
    @Override protected void cleanup(    SparseMatrix matrix){
      matrix._rows.reset();
    }
  }
;
