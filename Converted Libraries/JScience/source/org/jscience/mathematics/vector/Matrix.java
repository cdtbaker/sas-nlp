 ...
 * </DenseMatrix>[/code]
 */
  @SuppressWarnings("unchecked") protected static final XMLFormat<Matrix> XML=new XMLFormat<Matrix>(Matrix.class){
    @Override public void read(    InputElement xml,    Matrix M) throws XMLStreamException {
    }
    @Override public void write(    Matrix M,    OutputElement xml) throws XMLStreamException {
 ...
 */
  public boolean equals(  Matrix<F> that,  Comparator<F> cmp){
    ...
    final int m=this.getNumberOfRows();
    final int n=this.getNumberOfColumns();
    if ((that.getNumberOfRows() != m) || (that.getNumberOfColumns() != n))     return false;
    for (int i=m; --i >= 0; ) {
      for (int j=n; --j >= 0; ) {
        if (cmp.compare(this.get(i,j),that.get(i,j)) != 0)         return false;
      }
    }
    return true;
  }
 ...
 */
  public boolean equals(  Object that){
    ...
    if ((M.getNumberOfRows() != m) || (M.getNumberOfColumns() != n))     return false;
    for (int i=m; --i >= 0; ) {
      for (int j=n; --j >= 0; ) {
        if (!this.get(i,j).equals(M.get(i,j)))         return false;
      }
    }
    return true;
  }
