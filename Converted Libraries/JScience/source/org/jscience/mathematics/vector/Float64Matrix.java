 ...
 */
  ...
  @Override public Float64 get(  int i,  int j){
    return _transposed ? _rows.get(j).get(i) : _rows.get(i).get(j);
  }
  @Override public Float64Vector getRow(  int i){
  ...
  @Override public Float64Vector getColumn(  int j){
  ...
  @Override public Float64Vector getDiagonal(){
  ...
  @Override public Float64Matrix opposite(){
  ...
  @Override public Float64Matrix plus(  Matrix<Float64> that){
  ...
  @Override public Float64Matrix minus(  Matrix<Float64> that){
    return this.plus(that.opposite());
  }
  @Override public Float64Matrix times(  Float64 k){
    Float64Matrix M=Float64Matrix.newInstance(_n,_transposed);
    for (int i=0, p=_rows.size(); i < p; i++) {
  ...
  @Override public Float64Vector times(  Vector<Float64> v){
  ...
  @Override public Float64Matrix times(  Matrix<Float64> that){
...
;
    ...
    public void run(){
      if (_rightColumnEnd - _rightColumnStart < 32) {
        ...
        Multiply firstHalf=Multiply.valueOf(_left,_right,_rightColumnStart,halfIndex,_columnsResult);
        Multiply secondHalf=Multiply.valueOf(_left,_right,halfIndex,_rightColumnEnd,_columnsResult);
  ...
  @Override public Float64Matrix inverse(){
  ...
  @Override public Float64 determinant(){
    return LUDecomposition.valueOf(this).determinant();
  }
  @Override public Float64Matrix transpose(){
    Float64Matrix M=Float64Matrix.newInstance(_n,!_transposed);
    M._rows.addAll(this._rows);
    return M;
  }
  @Override public Float64 cofactor(  int i,  int j){
    if (_transposed) {
      int k=i;
      i=j;
      j=k;
  ...
  @Override public Float64Matrix adjoint(){
    Float64Matrix M=Float64Matrix.newInstance(_n,_transposed);
    int m=_rows.size();
    for (int i=0; i < m; i++) {
  ...
  @Override public Float64Matrix tensor(  Matrix<Float64> that){
    return Float64Matrix.valueOf(DenseMatrix.valueOf(this).tensor(that));
  }
  @Override public Float64Vector vectorization(){
    return Float64Vector.valueOf(DenseMatrix.valueOf(this).vectorization());
  }
  @Override public Float64Matrix copy(){
    Float64Matrix M=Float64Matrix.newInstance(_n,_transposed);
  ...
  private static ObjectFactory<Float64Matrix> FACTORY=new ObjectFactory<Float64Matrix>(){
    @Override protected Float64Matrix create(){
      return new Float64Matrix();
    }
    @Override protected void cleanup(    Float64Matrix matrix){
      matrix._rows.reset();
    }
  }
;
