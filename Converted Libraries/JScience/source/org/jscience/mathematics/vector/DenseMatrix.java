 ...
 */
  public static <F extends Field<F>>DenseMatrix<F> valueOf(  F[][] elements){
    ...
    for (int i=0; i < m; i++) {
      ...
      if (row.getDimension() != n)       throw new DimensionException();
      M._rows.add(row);
    }
    return M;
  }
 ...
 */
  public static <F extends Field<F>>DenseMatrix<F> valueOf(  Matrix<F> that){
    if (that instanceof DenseMatrix)     return (DenseMatrix<F>)that;
  ...
  @Override public DenseVector<F> getRow(  int i){
  ...
  @Override public DenseVector<F> getColumn(  int j){
  ...
  @Override public DenseVector<F> getDiagonal(){
  ...
  @Override public DenseMatrix<F> opposite(){
  ...
  @Override public DenseMatrix<F> plus(  Matrix<F> that){
  ...
  @Override public DenseMatrix<F> minus(  Matrix<F> that){
    return this.plus(that.opposite());
  }
  @Override public DenseMatrix<F> times(  F k){
    DenseMatrix<F> M=DenseMatrix.newInstance(_n,_transposed);
    for (int i=0, p=_rows.size(); i < p; i++) {
  ...
  @Override public DenseVector<F> times(  Vector<F> v){
  ...
  @Override public DenseMatrix<F> times(  Matrix<F> that){
...
private static class Multiply<F extends Field<F>> implements Runnable {
    private static final ObjectFactory<Multiply> FACTORY=new ObjectFactory<Multiply>(){
      @Override protected Multiply create(){
        return new Multiply();
      }
    }
;
    ...
    @SuppressWarnings("unchecked") static <F extends Field<F>>Multiply<F> valueOf(    DenseMatrix<F> left,    Matrix<F> right,    int rightColumnStart,    int rightColumnEnd,    FastTable<DenseVector<F>> columnsResult){
    ...
    public void run(){
      if (_rightColumnEnd - _rightColumnStart < 32) {
        ...
        Multiply<F> firstHalf=Multiply.valueOf(_left,_right,_rightColumnStart,halfIndex,_columnsResult);
        Multiply<F> secondHalf=Multiply.valueOf(_left,_right,halfIndex,_rightColumnEnd,_columnsResult);
  ...
  @Override public DenseMatrix<F> inverse(){
  ...
  @Override public DenseMatrix<F> transpose(){
    DenseMatrix<F> M=DenseMatrix.newInstance(_n,!_transposed);
  ...
  @Override public DenseMatrix<F> adjoint(){
    DenseMatrix<F> M=DenseMatrix.newInstance(_n,_transposed);
    int m=_rows.size();
    for (int i=0; i < m; i++) {
      ...
      for (int j=0; j < _n; j++) {
        ...
        row._elements.add(((i + j) % 2 == 0) ? cofactor : cofactor.opposite());
      }
    }
    return M.transpose();
  }
  @Override public Matrix<F> tensor(  Matrix<F> that){
  ...
  @Override public Vector<F> vectorization(){
    DenseVector<F> V=DenseVector.newInstance();
  ...
  @Override public DenseMatrix<F> copy(){
    DenseMatrix<F> M=DenseMatrix.newInstance(_n,_transposed);
  ...
  @SuppressWarnings("unchecked") static <F extends Field<F>>DenseMatrix<F> newInstance(  int n,  boolean transposed){
  ...
  private static ObjectFactory<DenseMatrix> FACTORY=new ObjectFactory<DenseMatrix>(){
    @Override protected DenseMatrix create(){
      return new DenseMatrix();
    }
    @Override protected void cleanup(    DenseMatrix matrix){
      matrix._rows.reset();
    }
  }
;
