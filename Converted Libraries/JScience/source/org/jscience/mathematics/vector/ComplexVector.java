 ...
 */
  protected static final XMLFormat<ComplexVector> XML=new XMLFormat<ComplexVector>(ComplexVector.class){
    @Override public ComplexVector newInstance(    Class<ComplexVector> cls,    InputElement xml) throws XMLStreamException {
      int dimension=xml.getAttribute("dimension",0);
      ComplexVector V=FACTORY.array(dimension);
      V._dimension=dimension;
      return V;
    }
    @SuppressWarnings("unchecked") @Override public void read(    InputElement xml,    ComplexVector V) throws XMLStreamException {
    ...
    @Override public void write(    ComplexVector V,    OutputElement xml) throws XMLStreamException {
 ...
 */
  private static final ArrayFactory<ComplexVector> FACTORY=new ArrayFactory<ComplexVector>(){
    @Override protected ComplexVector create(    int capacity){
 ...
 */
  public static ComplexVector valueOf(  Vector<Complex> that){
    if (that instanceof ComplexVector)     return (ComplexVector)that;
 ...
 */
  ...
  @Override public ComplexVector plus(  Vector<Complex> that){
  ...
  @Override public ComplexVector minus(  Vector<Complex> that){
    ComplexVector T=ComplexVector.valueOf(that);
    if (T._dimension != _dimension)     throw new DimensionException();
    ComplexVector V=FACTORY.array(_dimension);
    V._dimension=_dimension;
  ...
  @Override public ComplexVector times(  Complex k){
    ComplexVector V=FACTORY.array(_dimension);
    V._dimension=_dimension;
    for (int i=0; i < _dimension; i++) {
      double real=_reals[i];
  ...
  @Override public Complex times(  Vector<Complex> that){
    ComplexVector T=ComplexVector.valueOf(that);
    if (T._dimension != _dimension)     throw new DimensionException();
