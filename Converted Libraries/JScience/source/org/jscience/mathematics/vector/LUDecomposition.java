 ...
 */
  public static final Comparator<Field> NUMERIC_COMPARATOR=new Comparator<Field>(){
    @SuppressWarnings("unchecked") public int compare(    Field left,    Field right){
      if ((left instanceof Number) && (right instanceof Number))       return ((Number)left).isLargerThan((Number)right) ? 1 : -1;
      if (left.equals(left.plus(left)))       return -1;
      if (right.equals(right.plus(right)))       return 1;
      return 0;
    }
  }
;
 ...
 */
  @SuppressWarnings("unchecked") public static <F extends Field<F>>LUDecomposition<F> valueOf(  Matrix<F> source){
    if (!source.isSquare())     throw new DimensionException("Matrix is not square");
 ...
 */
  private void construct(  Matrix<F> source){
    _LU=source instanceof DenseMatrix ? ((DenseMatrix<F>)source).copy() : DenseMatrix.valueOf(source);
    _pivots.clear();
    for (int i=0; i < _n; i++) {
    ...
    for (int k=0; k < _n; k++) {
      ...
      for (int i=k + 1; i < n; i++) {
        ...
        for (int j=k + 1; j < n; j++) {
          _LU.set(i,j,_LU.get(i,j).plus(_LU.get(i,k).times(_LU.get(k,j).opposite())));
        }
      }
    }
  }
 ...
 */
  public DenseMatrix<F> solve(  Matrix<F> B){
    if (_n != B.getNumberOfRows())     throw new DimensionException("Input vector has " + B.getNumberOfRows() + " rows instead of "+ _n);
    ...
    for (int k=0; k < _n; k++) {
      for (int i=k + 1; i < _n; i++) {
        F luik=_LU.get(i,k);
        for (int j=0; j < n; j++) {
          X.set(i,j,X.get(i,j).plus(luik.times(X.get(k,j).opposite())));
        }
      }
    }
    ...
    for (int i=0; i < n; i++) {
      for (int j=n - 2; j >= 0; j--) {
        for (int k=j + 1; k < n; k++) {
          F lukj=_LU.get(k,j);
          if (R.get(i,j) != null) {
            R.set(i,j,R.get(i,j).plus(R.get(i,k).times(lukj.opposite())));
          }
 else {
 ...
 */
  public SparseMatrix<F> getPermutation(  F zero,  F one){
    ...
    for (int i=0; i < _n; i++) {
      P.getRow(_pivots.get(i).intValue())._elements.put(Index.valueOf(i),one);
    }
    return P;
  }
  /** 
 ...
 */
  ...
  private static final ObjectFactory<LUDecomposition> FACTORY=new ObjectFactory<LUDecomposition>(){
    ...
    @SuppressWarnings("unchecked") protected void cleanup(    LUDecomposition lu){
      lu._LU=null;
    }
  }
;
