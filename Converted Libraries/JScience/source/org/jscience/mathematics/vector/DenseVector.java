 ...
 */
  protected static final XMLFormat<DenseVector> XML=new XMLFormat<DenseVector>(DenseVector.class){
    @Override public DenseVector newInstance(    Class<DenseVector> cls,    InputElement xml) throws XMLStreamException {
      return FACTORY.object();
    }
    @SuppressWarnings("unchecked") @Override public void read(    InputElement xml,    DenseVector V) throws XMLStreamException {
    ...
    @Override public void write(    DenseVector V,    OutputElement xml) throws XMLStreamException {
 ...
 */
  ...
  @Override public DenseVector<F> opposite(){
    DenseVector<F> V=DenseVector.newInstance();
  ...
  @Override public DenseVector<F> plus(  Vector<F> that){
  ...
  @Override public DenseVector<F> minus(  Vector<F> that){
    return this.plus(that.opposite());
  }
  @Override public DenseVector<F> times(  F k){
    DenseVector<F> V=DenseVector.newInstance();
    for (int i=0, n=_elements.size(); i < n; ) {
  ...
  @SuppressWarnings("unchecked") @Override public DenseVector<F> copy(){
    DenseVector<F> V=DenseVector.newInstance();
    for (    F e : _elements) {
  ...
  @SuppressWarnings("unchecked") static <F extends Field<F>>DenseVector<F> newInstance(){
    return FACTORY.object();
  }
  private static final ObjectFactory<DenseVector> FACTORY=new ObjectFactory<DenseVector>(){
    @Override protected DenseVector create(){
      return new DenseVector();
    }
    @Override protected void cleanup(    DenseVector vector){
