 ...
 */
  protected static final XMLFormat<SparseVector> XML=new XMLFormat<SparseVector>(SparseVector.class){
    @Override public SparseVector newInstance(    Class<SparseVector> cls,    InputElement xml) throws XMLStreamException {
      return FACTORY.object();
    }
    @SuppressWarnings("unchecked") @Override public void read(    InputElement xml,    SparseVector V) throws XMLStreamException {
      V._dimension=xml.getAttribute("dimension",0);
      V._zero=xml.get("Zero");
      V._elements.putAll(xml.get("Elements",FastMap.class));
    }
    @Override public void write(    SparseVector V,    OutputElement xml) throws XMLStreamException {
 ...
 */
  public static <F extends Field<F>>SparseVector<F> valueOf(  Vector<F> that,  F zero,  FastComparator<? super F> comparator){
    if (that instanceof SparseVector)     return SparseVector.valueOf((SparseVector<F>)that,zero,comparator);
  ...
  private static <F extends Field<F>>SparseVector<F> valueOf(  SparseVector<F> that,  F zero,  FastComparator<? super F> comparator){
    ...
    for (FastMap.Entry<Index,F> e=that._elements.head(), n=that._elements.tail(); (e=e.getNext()) != n; ) {
 ...
 */
  ...
  @Override public F get(  int i){
    if ((i < 0) || (i >= _dimension))     throw new IndexOutOfBoundsException();
    F element=_elements.get(Index.valueOf(i));
    return (element == null) ? _zero : element;
  }
  @Override public SparseVector<F> opposite(){
  ...
  @Override public SparseVector<F> plus(  Vector<F> that){
    if (that instanceof SparseVector)     return plus((SparseVector<F>)that);
  ...
  @Override public SparseVector<F> times(  F k){
    SparseVector<F> V=SparseVector.newInstance(_dimension,_zero);
    for (FastMap.Entry<Index,F> e=_elements.head(), n=_elements.tail(); (e=e.getNext()) != n; ) {
  ...
  @Override public F times(  Vector<F> that){
    if (that.getDimension() != _dimension)     throw new DimensionException();
    F sum=null;
    for (FastMap.Entry<Index,F> e=_elements.head(), n=_elements.tail(); (e=e.getNext()) != n; ) {
  ...
  @SuppressWarnings("unchecked") @Override public SparseVector<F> copy(){
  ...
  @SuppressWarnings("unchecked") static <F extends Field<F>>SparseVector<F> newInstance(  int dimension,  F zero){
  ...
  private static final ObjectFactory<SparseVector> FACTORY=new ObjectFactory<SparseVector>(){
    @Override protected SparseVector create(){
      return new SparseVector();
    }
    @Override protected void cleanup(    SparseVector vector){
