 ...
 */
  @SuppressWarnings("unchecked") protected static final XMLFormat<Amount> XML=new XMLFormat<Amount>(Amount.class){
    @Override public Amount newInstance(    Class<Amount> cls,    InputElement xml) throws XMLStreamException {
      Unit unit=Unit.valueOf(xml.getAttribute("unit"));
      Amount<?> m=Amount.newInstance(unit);
      if (xml.getAttribute("error") == null)       return m.setExact(xml.getAttribute("value",0L));
      m._isExact=false;
    ...
    @Override public void read(    javolution.xml.XMLFormat.InputElement arg0,    Amount arg1) throws XMLStreamException {
    }
    @Override public void write(    Amount m,    OutputElement xml) throws XMLStreamException {
      if (m._isExact) {
 ...
 */
  public static <Q extends Quantity>Amount<Q> valueOf(  double value,  double error,  Unit<Q> unit){
    if (error < 0)     throw new IllegalArgumentException("error: " + error + " is negative");
    Amount<Q> m=Amount.newInstance(unit);
 ...
 */
  public static <Q extends Quantity>Amount<Q> rangeOf(  double minimum,  double maximum,  Unit<Q> unit){
    if (minimum > maximum)     throw new IllegalArgumentException("minimum: " + minimum + " greater than maximum: "+ maximum);
    Amount<Q> m=Amount.newInstance(unit);
    m._isExact=false;
 ...
 */
  public long getExactValue() throws AmountException {
    if (!_isExact)     throw new AmountException("Inexact measures don't have exact values");
    return _exactValue;
  }
  /** 
 ...
 */
  @SuppressWarnings("unchecked") public <R extends Quantity>Amount<R> to(  Unit<R> unit){
    if ((_unit == unit) || this._unit.equals(unit))     return (Amount<R>)this;
 ...
 */
  public Amount<Q> opposite(){
    ...
    if ((_isExact) && (_exactValue != Long.MAX_VALUE))     return m.setExact(-_exactValue);
    m._isExact=false;
    m._minimum=-_maximum;
    m._maximum=-_minimum;
    return m;
 ...
 */
  @SuppressWarnings("unchecked") public Amount<Q> plus(  Amount that) throws ConversionException {
    ...
    if (this._isExact && thatToUnit._isExact) {
      ...
      double sumDouble=((double)this._exactValue) + ((double)thatToUnit._exactValue);
      if (sumLong == sumDouble)       return m.setExact(sumLong);
    }
 ...
 */
  @SuppressWarnings("unchecked") public Amount<Q> minus(  Amount that) throws ConversionException {
    final Amount thatToUnit=that.to(_unit);
    Amount<Q> m=Amount.newInstance(_unit);
    if (this._isExact && thatToUnit._isExact) {
      long diffLong=this._exactValue - thatToUnit._exactValue;
      double diffDouble=((double)this._exactValue) - ((double)thatToUnit._exactValue);
      if (diffLong == diffDouble)       return m.setExact(diffLong);
    }
 ...
 */
  public Amount<Q> times(  long factor){
    ...
    if (this._isExact) {
      ...
      if (productLong == productDouble)       return m.setExact(productLong);
    }
    m._isExact=false;
 ...
 * @return <code>this · that</code>.
 */
  @SuppressWarnings("unchecked") public Amount<? extends Quantity> times(  Amount that){
 ...
 */
  public Amount<Q> divide(  long divisor){
    ...
    if (this._isExact) {
      ...
      if (quotientLong == quotientDouble)       return m.setExact(quotientLong);
    }
 ...
 * @return <code>this / that</code>.
 */
  @SuppressWarnings("unchecked") public Amount<? extends Quantity> divide(  Amount that){
    if (that._isExact) {
 ...
 */
  public Amount<Q> abs(){
    return (_isExact) ? ((_exactValue < 0) ? this.opposite() : this) : (_minimum >= -_maximum) ? this : this.opposite();
  }
  /** 
 ...
 */
  public Amount<? extends Quantity> sqrt(){
    ...
    if (this._isExact) {
      ...
      if (sqrtLong * sqrtLong == _exactValue)       return m.setExact(sqrtLong);
    }
 ...
 */
  public Amount<? extends Quantity> root(  int n){
    ...
    if (this._isExact) {
      ...
      if (thisLong == _exactValue)       return m.setExact(rootLong);
    }
 ...
 */
  @SuppressWarnings("unchecked") public int compareTo(  Measurable<Q> that){
 ...
 */
  public boolean equals(  Object that){
    ...
    if (_isExact != m._isExact)     return false;
    if (_isExact && (this._exactValue == m._exactValue))     return true;
    if (_minimum != m._minimum)     return false;
    if (_maximum != m._maximum)     return false;
    return true;
  }
  /** 
 ...
 * @return <code>this ≅ that</code>
 */
  @SuppressWarnings("unchecked") public boolean approximates(  Amount that){
 ...
 */
  ...
  public double doubleValue(  Unit<Q> unit){
    return ((_unit == unit) || _unit.equals(unit)) ? this.getEstimatedValue() : this.to(unit).getEstimatedValue();
  }
  public final long longValue(  Unit<Q> unit){
    if (!_unit.equals(unit))     return this.to(unit).longValue(unit);
    if (_isExact)     return _exactValue;
    ...
    throw new ArithmeticException(doubleValue + " " + _unit+ " cannot be represented as long");
  }
...
synchronized (leftTable) {
  ...
  @SuppressWarnings("unchecked") private static <Q extends Quantity>Amount<Q> newInstance(  Unit<?> unit){
    Amount<Q> measure=FACTORY.object();
    measure._unit=(Unit<Q>)unit;
    return measure;
  }
  @SuppressWarnings("unchecked") private static <Q extends Quantity>Amount<Q> copyOf(  Amount original){
    Amount<Q> measure=FACTORY.object();
  ...
  @SuppressWarnings("unchecked") private static final ObjectFactory<Amount> FACTORY=new ObjectFactory<Amount>(){
    @Override protected Amount create(){
      return new Amount();
    }
  }
;
