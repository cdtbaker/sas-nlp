    ...
    System.out.println("Testing...");
{
      ...
      Amount<Frequency> m10=m8.divide(3).times(3);
      Amount<Frequency> m11=m8.divide(7).times(7);
      ...
      System.out.println("Numeric Errors");
{
        ...
        AmountFormat.setInstance(AmountFormat.getExactDigitsInstance());
        System.out.println(x + " (" + Amount.valueOf(ns,0.5,NANO(SECOND)).to(MILLI(SECOND))+ ")");
      }
{
        double x=1.0;
        double v=0.01;
    ...
    }
{
      ...
      UnitFormat.getInstance().label(USD,"$");
      UnitFormat.getInstance().label(EUR,"€");
      Currency.setReferenceCurrency(USD);
      EUR.setExchangeRate(1.17);
      Amount<?> carMileage=Amount.valueOf(20,MILE.divide(GALLON_LIQUID_US));
      Amount<?> gazPrice=Amount.valueOf(1.2,EUR.divide(LITRE));
    ...
    }
{
      ...
      Amount<ElectricResistance> R2=Amount.valueOf(300,3,OHM);
      Amount<ElectricPotential> U0=Amount.valueOf(28,0.01,VOLT);
      DenseMatrix<Amount<?>> A=DenseMatrix.valueOf(new Amount<?>[][]{{Amount.ONE,Amount.ONE,Amount.valueOf(0,OHM)},{Amount.ONE.opposite(),Amount.ZERO,R1},{Amount.ZERO,Amount.ONE.opposite(),R2}});
      DenseVector<Amount<?>> B=DenseVector.valueOf(new Amount<?>[]{U0,Amount.valueOf(0,VOLT),Amount.valueOf(0,VOLT)});
    ...
    }
{
      ...
      CoordinatesConverter<LatLong,XYZ> latLongToXYZ=LatLong.CRS.getConverterTo(XYZ.CRS);
 ...
 */
  ...
  private static void multiplyMatrices(  double[][] values){
    ...
    for (int i=0; i < 5; i++)     FM.times(FM);
    startTime();
    Matrix<Float64> R2=FM.times(FM);
    endTime(1);
    if (!R2.equals(Float64Matrix.valueOf(R1.o)))     throw new Error("Error in matrix multiplication");
    System.out.print("Matrix<Complex> 500x500 multiplication: ");
    Complex[][] complexes=new Complex[size][size];
    for (int i=0; i < size; i++) {
      for (int j=0; j < size; j++) {
        complexes[i][j]=Complex.valueOf(MathLib.random(),MathLib.random());
      }
    }
    Matrix<Complex> CM=ComplexMatrix.valueOf(complexes);
    for (int i=0; i < 5; i++)     CM.times(CM);
    startTime();
    CM.times(CM);
    endTime(1);
    ...
    for (int i=0; i < size; i++) {
      for (int j=0; j < size; j++) {
        measures[i][j]=Amount.valueOf(MathLib.random(Long.MIN_VALUE,Long.MAX_VALUE),Unit.ONE);
      }
    }
...
private static final class MatrixDouble {
    ...
    MatrixDouble times(    MatrixDouble that){
      if (that.m != this.n)       throw new Error("Wrong dimensions");
 ...
 * @param iterationsthe number iterations performed since {@link #startTime}.
 */
  public static void endTime(  int iterations){
    ...
    long divisor;
    String unit;
    if (picoDuration > 1000 * 1000 * 1000* 1000L) {
      unit=" s";
