 ...
 */
  @SuppressWarnings("unchecked") public final <T extends Quantity>Unit<T> asType(  Class<T> type) throws ClassCastException {
    ...
    if (!dim1.equals(dim2))     throw new ClassCastException();
    return (Unit<T>)this;
  }
  /** 
 ...
 */
  public final Dimension getDimension(){
    ...
    if (systemUnit instanceof BaseUnit)     return Dimension.getModel().getDimension((BaseUnit<?>)systemUnit);
    if (systemUnit instanceof AlternateUnit)     return ((AlternateUnit<?>)systemUnit).getParent().getDimension();
 ...
 */
  public final UnitConverter getConverterTo(  Unit<?> that) throws ConversionException {
    ...
    if (thisSystemUnit.equals(thatSystemUnit))     return that.toStandardUnit().inverse().concatenate(this.toStandardUnit());
    if (!thisSystemUnit.getDimension().equals(thatSystemUnit.getDimension()))     throw new ConversionException(this + " is not compatible with " + that);
  ...
  private Unit<?> getBaseUnits(){
    ...
    if (systemUnit instanceof AlternateUnit)     return ((AlternateUnit<?>)systemUnit).getParent().getBaseUnits();
  ...
  private static UnitConverter transformOf(  Unit<?> baseUnits){
    if (baseUnits instanceof BaseUnit)     return Dimension.getModel().getTransform((BaseUnit<?>)baseUnits);
    ...
    for (int i=0; i < productUnit.getUnitCount(); i++) {
      ...
      if (!cvtr.isLinear())       throw new ConversionException(baseUnits + " is non-linear, cannot convert");
      if (productUnit.getUnitRoot(i) != 1)       throw new ConversionException(productUnit + " holds a base unit with fractional exponent");
