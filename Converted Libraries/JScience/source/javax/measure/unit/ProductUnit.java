 ...
 */
  @SuppressWarnings("unchecked") private static Unit<? extends Quantity> getInstance(  Element[] leftElems,  Element[] rightElems){
 ...
 */
  @SuppressWarnings("unchecked") public Unit<? extends Quantity> getUnit(  int index){
 ...
 */
  ...
  @Override @SuppressWarnings("unchecked") public Unit<? super Q> getStandardUnit(){
    if (hasOnlyStandardUnit())     return this;
    Unit systemUnit=ONE;
    for (int i=0; i < _elements.length; i++) {
  ...
  @Override public UnitConverter toStandardUnit(){
    if (hasOnlyStandardUnit())     return UnitConverter.IDENTITY;
    UnitConverter converter=UnitConverter.IDENTITY;
    for (int i=0; i < _elements.length; i++) {
      UnitConverter cvtr=_elements[i]._unit.toStandardUnit();
      if (!cvtr.isLinear())       throw new ConversionException(_elements[i]._unit + " is non-linear, cannot convert");
      if (_elements[i]._root != 1)       throw new ConversionException(_elements[i]._unit + " holds a base unit with fractional exponent");
 ...
 */
  private boolean hasOnlyStandardUnit(){
    for (int i=0; i < _elements.length; i++) {
      ...
      if (!u.isStandardUnit())       return false;
    }
    return true;
  }
  /** 
