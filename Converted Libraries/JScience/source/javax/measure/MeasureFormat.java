...
static final class NumberUnit extends MeasureFormat {
    ...
    @Override public StringBuffer format(    Object obj,    StringBuffer toAppendTo,    FieldPosition pos){
      ...
      if (value instanceof Number) {
        if (unit instanceof CompoundUnit)         return formatCompound(((Number)value).doubleValue(),(CompoundUnit<?>)unit,toAppendTo,pos);
    ...
    @Override public Object parseObject(    String source,    ParsePosition pos){
      ...
      try {
        ...
        if (i == pos.getIndex())         return null;
        i=pos.getIndex();
        if (i >= source.length())         return measureOf(value,Unit.ONE);
        boolean isCompound=!Character.isWhitespace(source.charAt(i));
        if (isCompound)         return parseCompound(value,source,pos);
        if (++i >= source.length())         return measureOf(value,Unit.ONE);
        pos.setIndex(i);
    ...
    @SuppressWarnings("unchecked") private Object parseCompound(    Number highValue,    String source,    ParsePosition pos) throws ParseException {
      Unit high=_unitFormat.parseSingleUnit(source,pos);
      int i=pos.getIndex();
      if (i >= source.length() || Character.isWhitespace(source.charAt(i)))       return measureOf(highValue,high);
    ...
    @SuppressWarnings("unchecked") private static Measure measureOf(    Number value,    Unit unit){
