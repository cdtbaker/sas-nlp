...
private static class TwoDimensional<Q extends Quantity> extends VectorMeasure<Q> {
    ...
    @Override public double doubleValue(    Unit<Q> unit){
      double norm=Math.sqrt(_x * _x + _y * _y);
      if ((unit == _unit) || (unit.equals(_unit)))       return norm;
    ...
    @Override public TwoDimensional<Q> to(    Unit<Q> unit){
...
private static class ThreeDimensional<Q extends Quantity> extends VectorMeasure<Q> {
    ...
    @Override public ThreeDimensional<Q> to(    Unit<Q> unit){
      if ((unit == _unit) || (unit.equals(_unit)))       return this;
      UnitConverter cvtr=_unit.getConverterTo(unit);
...
private static class MultiDimensional<Q extends Quantity> extends VectorMeasure<Q> {
    ...
    @Override public double doubleValue(    Unit<Q> unit){
      ...
      if ((unit == _unit) || (unit.equals(_unit)))       return Math.sqrt(normSquare);
    ...
    @Override public MultiDimensional<Q> to(    Unit<Q> unit){
      if ((unit == _unit) || (unit.equals(_unit)))       return this;
      UnitConverter cvtr=_unit.getConverterTo(unit);
