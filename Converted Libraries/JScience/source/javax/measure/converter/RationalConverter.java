 ...
 */
  public RationalConverter(  long dividend,  long divisor){
    if (divisor < 0)     throw new IllegalArgumentException("Negative divisor");
    if (dividend == divisor)     throw new IllegalArgumentException("Identity converter not allowed");
    _dividend=dividend;
    _divisor=divisor;
  }
  /** 
 ...
 */
  ...
  @Override public UnitConverter inverse(){
    return _dividend < 0 ? new RationalConverter(-_divisor,-_dividend) : new RationalConverter(_divisor,_dividend);
  }
  ...
  @Override public UnitConverter concatenate(  UnitConverter converter){
