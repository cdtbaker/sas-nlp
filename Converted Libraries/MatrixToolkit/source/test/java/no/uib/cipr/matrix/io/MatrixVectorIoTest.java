...
public class MatrixVectorIoTest extends TestCase {
  ...
  public void testSparseWriteRead() throws Exception {
    CompRowMatrix mat=new CompRowMatrix(3,3,new int[][]{{1,2},{0,2},{1}});
    mat.set(0,1,1);
    mat.set(0,2,2);
    mat.set(1,0,3);
    mat.set(1,2,4);
  ...
  private int[] buildRowArray(  CompRowMatrix mat){
    ...
    while (true) {
      ...
      if (nextValidRow >= mat.numRows()) {
        while (curIndex < mat.getData().length)         rows[curIndex++]=curRow;
        break;
      }
      int nextRowIndex=rowPs[nextValidRow];
      while (curIndex < nextRowIndex)       rows[curIndex++]=curRow;
    }
    return rows;
  }
}
