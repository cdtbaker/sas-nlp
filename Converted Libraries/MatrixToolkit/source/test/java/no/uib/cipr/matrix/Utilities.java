 ...
 * @param numNumber of entries on each symmetry band
 ...
 */
  public static double[][] symmetryPopulate(  Matrix A,  int num){
    ...
    if (m != n)     throw new IllegalArgumentException("m != n");
    double[][] values=new double[n][m];
    if (n == 0)     return values;
    for (int j=0; j < m; ++j)     for (int i=0; i < num; ++i) {
 ...
 * @param AMatrix to populate
 * @param numNumber of entries on each column
 * @return The matrix data in dense format
 */
 ...
 * @param AMatrix to populate
 * @param numNumber of entries on each row
 * @return The matrix data in dense format
 */
  public static double[][] rowPopulate(  Matrix A,  int num){
    ...
    for (int i=0; i < n; ++i)     for (int j=0; j < num; ++j) {
      double value=Math.random();
 ...
 * matrices
 * @param nNumber of rows
 * @param mNumber of columns
 * @param bNumber of entries on each row
 */
  public static int[][] getRowPattern(  int n,  int m,  int b){
    int[][] nz=new int[n][];
    for (int i=0; i < n; ++i) {
      Set<Integer> row=new HashSet<Integer>();
      for (int j=0; j < b; ++j)       row.add(getInt(m));
      nz[i]=new int[row.size()];
      int j=0;
      for (      Integer colind : row)       nz[i][j++]=colind;
    }
    return nz;
  }
  /** 
 ...
 * @param nNumber of rows
 * @param mNumber of columns
 * @param bNumber of entries on each column
 */
  public static int[][] getColumnPattern(  int n,  int m,  int b){
    int[][] nz=new int[m][];
    for (int i=0; i < m; ++i) {
      Set<Integer> column=new HashSet<Integer>();
      for (int j=0; j < b; ++j)       column.add(getInt(n));
      nz[i]=new int[column.size()];
      int j=0;
      for (      Integer rowind : column)       nz[i][j++]=rowind;
    }
    return nz;
  }
  /** 
 ...
 * @param AMatrix to populate
 * @param nzColumn indices on each row
 * @return The matrix data in dense format
 */
  public static double[][] rowPopulate(  Matrix A,  int[][] nz){
    ...
    double[][] values=new double[n][m];
    if (m == 0)     return values;
    for (int i=0; i < n; ++i)     for (int j=0; j < nz[i].length; ++j) {
      double value=Math.random();
      int k=nz[i][j];
      values[i][k]=value;
      A.set(i,k,value);
 ...
 * Populates the matrix, using the given non-zero pattern
 * @param AMatrix to populate
 * @param nzRow indices on each column
 * @return The matrix data in dense format
 */
  public static double[][] columnPopulate(  Matrix A,  int[][] nz){
    ...
    double[][] values=new double[n][m];
    if (n == 0)     return values;
    for (int j=0; j < m; ++j)     for (int i=0; i < nz[j].length; ++i) {
      double value=Math.random();
      int k=nz[j][i];
      values[k][j]=value;
      A.set(k,j,value);
 ...
 * @param AMatrix to populate
 * @param klNumber of subdiagonls
 * @param kuNumber of superdiagonals
 * @return The matrix data in dense format
 */
  public static double[][] bandPopulate(  Matrix A,  int kl,  int ku){
    int n=A.numRows(), m=A.numColumns();
    double[][] values=new double[n][m];
    for (int i=0; i < n; ++i)     for (int j=0; j < m; ++j)     if (j - ku <= i && i <= j + kl) {
      double value=Math.random();
      values[i][j]=value;
      A.set(i,j,value);
    }
 ...
 */
  public static double[][] unitBandPopulate(  Matrix A,  int kl,  int ku){
    int n=A.numRows(), m=A.numColumns();
    double[][] values=new double[n][m];
    for (int i=0; i < n; ++i)     for (int j=0; j < m; ++j)     if (i != j && j - ku <= i && i <= j + kl) {
      double value=Math.random();
      values[i][j]=value;
      A.set(i,j,value);
    }
 ...
 */
  public static double[][] lowerPopulate(  Matrix A){
    int n=A.numRows(), m=A.numColumns();
    double[][] values=new double[n][m];
    for (int i=0; i < n; ++i)     for (int j=0; j < m; ++j)     if (j <= i) {
      double value=Math.random();
      values[i][j]=value;
      A.set(i,j,value);
    }
 ...
 */
  ...
  public static void upperPopulateGauss(  Matrix A){
    Random random=new Random();
    for (int i=0; i < A.numRows(); i++)     for (int j=0; j <= i; j++)     A.set(i,j,random.nextGaussian());
  }
  /** 
 ...
 */
  public static void lowerSymmetrice(  double[][] Ad){
    int n=Ad.length;
    for (int i=0; i < n; ++i)     for (int j=0; j < i; ++j)     Ad[j][i]=Ad[i][j];
  }
  /** 
 ...
 */
  public static void upperSymmetrice(  double[][] Ad){
    ...
    for (int i=0; i < n; ++i) {
      int m=Ad[0].length;
      for (int j=i + 1; j < m; ++j)       Ad[j][i]=Ad[i][j];
    }
  }
  /** 
 ...
 */
  public static double[][] unitSet(  double[][] Ad){
    for (int i=0; i < Ad.length; ++i)     Ad[i][i]=1;
    return Ad;
  }
 ...
 */
  public static boolean singular(  Matrix A) throws NotConvergedException {
    ...
    double[] S=svd.getS();
    for (int i=0; i < S.length; ++i)     if (S[i] == 0)     return true;
    return false;
  }
  /** 
    ...
    }
{
      ...
      for (int i=0; i < S.length; ++i)       if (Math.abs(S[i]) > 1e-10)       return false;
    }
    return true;
  }
  /** 
 ...
 */
  public static void addDiagonal(  Matrix A,  double shift){
    int n=A.numRows(), m=A.numColumns();
    for (int i=0; i < Math.min(m,n); ++i)     A.add(i,i,shift);
  }
}
