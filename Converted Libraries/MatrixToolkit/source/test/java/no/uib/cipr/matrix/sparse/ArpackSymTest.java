 ...
 * @author Sam Halliday
 */
@Log public class ArpackSymTest extends TestCase {
    ...
    for (int i=100; i <= 500; i=i + 100) {
      ...
      for (      Map.Entry<Double,DenseVectorSub> e : results.entrySet()) {
        ...
        for (        Map.Entry<Double,DenseVector> evdEntry : evd.entrySet()) {
          if (e.getKey() - evdEntry.getKey() < 0.0001)           value=true;
          if (e.getValue().dot(evdEntry.getValue()) < 0.9999)           vector=true;
        }
        if (!value)         fail("no matching eigenvalue for " + e.getKey() + " : "+ evd.keySet());
        if (!vector)         fail("no matching eigenvector for " + e.getKey() + " : "+ evd.keySet());
      }
    }
  }
