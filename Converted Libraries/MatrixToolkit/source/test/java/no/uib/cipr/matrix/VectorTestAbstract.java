 ...
 */
  ...
  protected double[] scale(  double alpha){
    for (int i=0; i < xd.length; ++i)     xd[i]*=alpha;
    return xd;
  }
  ...
  protected double[] set(  double alpha,  double[] yd,  double beta,  double[] zd){
    for (int i=0; i < xd.length; ++i)     xd[i]=alpha * yd[i] + beta * zd[i];
    return xd;
  }
  ...
  protected double[] add(  double alpha,  double[] yd){
    for (int i=0; i < xd.length; ++i)     xd[i]+=alpha * yd[i];
    return xd;
  }
  ...
  protected double dot(  double[] yd){
    double dot=0;
    for (int i=0; i < yd.length; ++i)     dot+=xd[i] * yd[i];
    return dot;
  }
  ...
  protected double norm1(){
    double norm=0;
    for (    double d : xd)     norm+=Math.abs(d);
    return norm;
  }
  protected double norm2(){
    double norm=0;
    for (    double d : xd)     norm+=d * d;
    return Math.sqrt(norm);
  }
  protected double normInf(){
    double norm=0;
    for (    double d : xd)     norm=Math.max(Math.abs(d),norm);
    return norm;
  }
  ...
  public void testIteratorGet(){
    double[] data=new double[x.size()];
    for (    VectorEntry e : x)     data[e.index()]=e.get();
    assertEquals(xd,x);
    assertEquals(xd,data);
  }
  public void testIteratorSet(){
    double alpha=Math.random();
    for (    VectorEntry e : x)     e.set(alpha * e.get());
