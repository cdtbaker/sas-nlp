 ...
 * @author Sam Halliday
 */
@Log public class LinkedSparseMatrixTest extends SparseMatrixTestAbstract {
  ...
  public void ignoredTimedMult(){
    ...
    for (    Matrix m : Lists.newArrayList(dense,sparse)) {
      ...
      for (int i=0; i < 10; i++)       m.mult(t,o);
      log.info("starting " + m.getClass() + " "+ o.getClass());
      watch.start();
      for (int i=0; i < 100; i++)       m.mult(t,o);
      watch.stop();
  ...
  public void ignoredTimedTransMult(){
    ...
    for (    Matrix m : Lists.newArrayList(dense,sparse)) {
      ...
      Matrix o=new DenseMatrix(dense.numRows(),dense.numColumns());
      log.info("warming up " + m.getClass() + " "+ o.getClass());
      for (int i=0; i < 10; i++)       m.transAmult(t,o);
      log.info("starting " + m.getClass() + " "+ o.getClass());
      watch.start();
      for (int i=0; i < 100; i++)       m.transAmult(t,o);
      watch.stop();
      log.info(m.getClass() + " " + o.getClass()+ " "+ watch);
    }
  }
          ...
          }
{
          ...
          String[] line=new String[]{Integer.toString(n),Integer.toString(m),Long.toString(denseMem),Long.toString(denseInitTime),Long.toString(denseMultTime),Long.toString(sparseMem),Long.toString(sparseInitTime),Long.toString(sparseMultTime)};
