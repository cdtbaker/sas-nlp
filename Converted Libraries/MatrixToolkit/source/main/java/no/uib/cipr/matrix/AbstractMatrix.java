 ...
 */
  protected AbstractMatrix(  int numRows,  int numColumns){
    if (numRows < 0 || numColumns < 0)     throw new IndexOutOfBoundsException("Matrix size cannot be negative");
 ...
 */
  protected void check(  int row,  int column){
    if (row < 0)     throw new IndexOutOfBoundsException("row index is negative (" + row + ")");
    if (column < 0)     throw new IndexOutOfBoundsException("column index is negative (" + column + ")");
    if (row >= numRows)     throw new IndexOutOfBoundsException("row index >= numRows (" + row + " >= "+ numRows+ ")");
    if (column >= numColumns)     throw new IndexOutOfBoundsException("column index >= numColumns (" + column + " >= "+ numColumns+ ")");
  }
  public Matrix copy(){
    throw new UnsupportedOperationException();
  }
  public Matrix zero(){
    for (    MatrixEntry e : this)     e.set(0);
    return this;
  }
  ...
  public Vector multAdd(  double alpha,  Vector x,  Vector y){
    checkMultAdd(x,y);
    if (alpha != 0)     for (    MatrixEntry e : this)     y.add(e.row(),alpha * e.get() * x.get(e.column()));
    return y;
  }
  /** 
 ...
 */
  protected void checkMultAdd(  Vector x,  Vector y){
    if (numColumns != x.size())     throw new IndexOutOfBoundsException("A.numColumns != x.size (" + numColumns + " != "+ x.size()+ ")");
    if (numRows != y.size())     throw new IndexOutOfBoundsException("A.numRows != y.size (" + numRows + " != "+ y.size()+ ")");
  }
  ...
  public Vector transMultAdd(  double alpha,  Vector x,  Vector y){
    ...
    if (alpha != 0)     for (    MatrixEntry e : this)     y.add(e.column(),alpha * e.get() * x.get(e.row()));
    return y;
  }
  /** 
 ...
 */
  protected void checkTransMultAdd(  Vector x,  Vector y){
    if (numRows != x.size())     throw new IndexOutOfBoundsException("A.numRows != x.size (" + numRows + " != "+ x.size()+ ")");
    if (numColumns != y.size())     throw new IndexOutOfBoundsException("A.numColumns != y.size (" + numColumns + " != "+ y.size()+ ")");
  }
 ...
 */
  protected void checkSolve(  Vector b,  Vector x){
    if (!isSquare())     throw new IndexOutOfBoundsException("!A.isSquare");
    if (numRows != b.size())     throw new IndexOutOfBoundsException("numRows != b.size (" + numRows + " != "+ b.size()+ ")");
    if (numColumns != x.size())     throw new IndexOutOfBoundsException("numColumns != x.size (" + numColumns + " != "+ x.size()+ ")");
  }
  ...
  public Matrix rank1(  double alpha,  Vector x,  Vector y){
    ...
    for (    VectorEntry ei : x)     if (ei.get() != 0)     for (    VectorEntry ej : y)     if (ej.get() != 0)     add(ei.index(),ej.index(),alpha * ei.get() * ej.get());
    return this;
  }
  /** 
 ...
 */
  protected void checkRank1(  Vector x,  Vector y){
    if (!isSquare())     throw new IndexOutOfBoundsException("!A.isSquare");
    if (x.size() != numRows)     throw new IndexOutOfBoundsException("x.size != A.numRows (" + x.size() + " != "+ numRows+ ")");
    if (y.size() != numColumns)     throw new IndexOutOfBoundsException("y.size != A.numColumns (" + y.size() + " != "+ numColumns+ ")");
  }
  ...
  public Matrix rank2(  double alpha,  Vector x,  Vector y){
    checkRank2(x,y);
    if (alpha == 0)     return this;
    for (    VectorEntry ei : x)     for (    VectorEntry ej : y) {
 ...
 */
  protected void checkRank2(  Vector x,  Vector y){
    if (!isSquare())     throw new IndexOutOfBoundsException("!A.isSquare");
    if (x.size() != numRows)     throw new IndexOutOfBoundsException("x.size != A.numRows (" + x.size() + " != "+ numRows+ ")");
    if (y.size() != numRows)     throw new IndexOutOfBoundsException("y.size != A.numRows (" + y.size() + " != "+ numRows+ ")");
  }
  ...
  public Matrix multAdd(  double alpha,  Matrix B,  Matrix C){
    checkMultAdd(B,C);
    if (alpha != 0)     for (int i=0; i < numRows; ++i)     for (int j=0; j < C.numColumns(); ++j) {
      double dot=0;
      for (int k=0; k < numColumns; ++k)       dot+=get(i,k) * B.get(k,j);
      C.add(i,j,alpha * dot);
    }
    return C;
  }
 ...
 */
  protected void checkMultAdd(  Matrix B,  Matrix C){
    if (numRows != C.numRows())     throw new IndexOutOfBoundsException("A.numRows != C.numRows (" + numRows + " != "+ C.numRows()+ ")");
    if (numColumns != B.numRows())     throw new IndexOutOfBoundsException("A.numColumns != B.numRows (" + numColumns + " != "+ B.numRows()+ ")");
    if (B.numColumns() != C.numColumns())     throw new IndexOutOfBoundsException("B.numColumns != C.numColumns (" + B.numRows() + " != "+ C.numColumns()+ ")");
  }
  ...
  public Matrix transAmultAdd(  double alpha,  Matrix B,  Matrix C){
    ...
    if (alpha != 0)     for (int i=0; i < numColumns; ++i)     for (int j=0; j < C.numColumns(); ++j) {
      double dot=0;
      for (int k=0; k < numRows; ++k)       dot+=get(k,i) * B.get(k,j);
      C.add(i,j,alpha * dot);
    }
    return C;
  }
 ...
 */
  protected void checkTransAmultAdd(  Matrix B,  Matrix C){
    if (numRows != B.numRows())     throw new IndexOutOfBoundsException("A.numRows != B.numRows (" + numRows + " != "+ B.numRows()+ ")");
    if (numColumns != C.numRows())     throw new IndexOutOfBoundsException("A.numColumns != C.numRows (" + numColumns + " != "+ C.numRows()+ ")");
    if (B.numColumns() != C.numColumns())     throw new IndexOutOfBoundsException("B.numColumns != C.numColumns (" + B.numColumns() + " != "+ C.numColumns()+ ")");
  }
  ...
  public Matrix transBmultAdd(  double alpha,  Matrix B,  Matrix C){
    ...
    if (alpha != 0)     for (int i=0; i < numRows; ++i)     for (int j=0; j < C.numColumns(); ++j) {
      double dot=0;
      for (int k=0; k < numColumns; ++k)       dot+=get(i,k) * B.get(j,k);
      C.add(i,j,alpha * dot);
    }
    return C;
  }
 ...
 */
  protected void checkTransBmultAdd(  Matrix B,  Matrix C){
    if (numColumns != B.numColumns())     throw new IndexOutOfBoundsException("A.numColumns != B.numColumns (" + numColumns + " != "+ B.numColumns()+ ")");
    if (numRows != C.numRows())     throw new IndexOutOfBoundsException("A.numRows != C.numRows (" + numRows + " != "+ C.numRows()+ ")");
    if (B.numRows() != C.numColumns())     throw new IndexOutOfBoundsException("B.numRows != C.numColumns (" + B.numRows() + " != "+ C.numColumns()+ ")");
  }
  ...
  public Matrix transABmultAdd(  double alpha,  Matrix B,  Matrix C){
    ...
    if (alpha != 0)     for (int i=0; i < numColumns; ++i)     for (int j=0; j < C.numColumns(); ++j) {
      double dot=0;
      for (int k=0; k < numRows; ++k)       dot+=get(k,i) * B.get(j,k);
      C.add(i,j,alpha * dot);
    }
    return C;
  }
 ...
 */
  protected void checkTransABmultAdd(  Matrix B,  Matrix C){
    if (numRows != B.numColumns())     throw new IndexOutOfBoundsException("A.numRows != B.numColumns (" + numRows + " != "+ B.numColumns()+ ")");
    if (numColumns != C.numRows())     throw new IndexOutOfBoundsException("A.numColumns != C.numRows (" + numColumns + " != "+ C.numRows()+ ")");
    if (B.numRows() != C.numColumns())     throw new IndexOutOfBoundsException("B.numRows != C.numColumns (" + B.numRows() + " != "+ C.numColumns()+ ")");
  }
 ...
 */
  protected void checkSolve(  Matrix B,  Matrix X){
    if (!isSquare())     throw new IndexOutOfBoundsException("!A.isSquare");
    if (B.numRows() != numRows)     throw new IndexOutOfBoundsException("B.numRows != A.numRows (" + B.numRows() + " != "+ numRows+ ")");
    if (B.numColumns() != X.numColumns())     throw new IndexOutOfBoundsException("B.numColumns != X.numColumns (" + B.numColumns() + " != "+ X.numColumns()+ ")");
    if (X.numRows() != numColumns)     throw new IndexOutOfBoundsException("X.numRows != A.numColumns (" + X.numRows() + " != "+ numColumns+ ")");
  }
 ...
 */
  protected void checkTransRank1(  Matrix C){
    if (!isSquare())     throw new IndexOutOfBoundsException("!A.isSquare");
    if (numRows != C.numColumns())     throw new IndexOutOfBoundsException("A.numRows != C.numColumns (" + numRows + " != "+ C.numColumns()+ ")");
  }
 ...
 */
  protected void checkRank2(  Matrix B,  Matrix C){
    if (!isSquare())     throw new IndexOutOfBoundsException("!A.isSquare");
    if (B.numRows() != C.numRows())     throw new IndexOutOfBoundsException("B.numRows != C.numRows (" + B.numRows() + " != "+ C.numRows()+ ")");
    if (B.numColumns() != C.numColumns())     throw new IndexOutOfBoundsException("B.numColumns != C.numColumns (" + B.numColumns() + " != "+ C.numColumns()+ ")");
  }
 ...
 */
 ...
 else     if (alpha == 0)     return zero();
    for (    MatrixEntry e : this)     e.set(alpha * e.get());
    return this;
  }
  ...
  public Matrix set(  double alpha,  Matrix B){
    ...
    if (B == this)     return scale(alpha);
    zero();
    for (    MatrixEntry e : B)     if (e.get() != 0)     set(e.row(),e.column(),alpha * e.get());
    return this;
  }
  ...
  public Matrix add(  double alpha,  Matrix B){
    checkSize(B);
    if (alpha != 0)     for (    MatrixEntry e : B)     add(e.row(),e.column(),alpha * e.get());
    return this;
  }
  /** 
 ...
 */
  ...
  public Matrix transpose(){
    checkTranspose();
    for (int j=0; j < numColumns; ++j)     for (int i=j + 1; i < numRows; ++i) {
 ...
 */
  ...
  public Matrix transpose(  Matrix B){
    checkTranspose(B);
    if (B == this)     return transpose();
    B.zero();
    for (    MatrixEntry e : this)     B.set(e.column(),e.row(),e.get());
    return B;
  }
  /** 
 ...
 */
 ...
 else     if (type == Norm.Frobenius)     return normF();
 else     if (type == Norm.Infinity)     return normInf();
 else     return max();
  }
  /** 
 * Computes the 1 norm
 */
  protected double norm1(){
    ...
    for (    MatrixEntry e : this)     rowSum[e.row()]+=Math.abs(e.get());
    return max(rowSum);
  }
  /** 
 ...
 */
  protected double normInf(){
    ...
    for (    MatrixEntry e : this)     columnSum[e.column()]+=Math.abs(e.get());
 ...
 */
  protected double max(){
    double max=0;
    for (    MatrixEntry e : this)     max=Math.max(Math.abs(e.get()),max);
    return max;
  }
  /** 
 ...
 */
  protected double max(  double[] x){
    double max=0;
    for (int i=0; i < x.length; ++i)     max=Math.max(x[i],max);
    return max;
  }
  @Override public String toString(){
    Formatter out=new Formatter();
    out.format("%10d %10d %19d\n",numRows,numColumns,Matrices.cardinality(this));
    int i=0;
    for (    MatrixEntry e : this) {
      if (e.get() != 0)       out.format("%10d %10d % .12e\n",e.row() + 1,e.column() + 1,e.get());
      if (++i == 100) {
        out.format("...\n");
        break;
      }
