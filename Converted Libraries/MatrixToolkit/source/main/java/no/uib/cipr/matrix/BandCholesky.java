 ...
 * @param nMatrix size
 * @param kdNumber of matrix bands
 ...
 */
  public BandCholesky(  int n,  int kd,  boolean upper){
    ...
    this.kd=kd;
    this.upper=upper;
    if (upper)     Cu=new UpperTriangBandMatrix(n,kd);
 else     Cl=new LowerTriangBandMatrix(n,kd);
  }
  /** 
 ...
 */
  ...
  private BandCholesky decompose(  AbstractBandMatrix A){
    ...
    if (upper && A.ku != kd)     throw new IllegalArgumentException("A.ku != kd");
    if (!upper && A.kl != kd)     throw new IllegalArgumentException("A.kl != kd");
    notspd=false;
    intW info=new intW(0);
    if (upper)     LAPACK.getInstance().dpbtrf(UpLo.Upper.netlib(),n,kd,A.getData(),Matrices.ld(kd + 1),info);
 else     LAPACK.getInstance().dpbtrf(UpLo.Lower.netlib(),n,kd,A.getData(),Matrices.ld(kd + 1),info);
 ...
 */
  public double rcond(  Matrix A){
    ...
    if (upper)     LAPACK.getInstance().dpbcon(UpLo.Upper.netlib(),n,kd,Cu.getData(),Matrices.ld(kd + 1),anorm,rcond,work,lwork,info);
 else     LAPACK.getInstance().dpbcon(UpLo.Lower.netlib(),n,kd,Cl.getData(),Matrices.ld(kd + 1),anorm,rcond,work,lwork,info);
 ...
 */
  public DenseMatrix solve(  DenseMatrix B) throws MatrixNotSPDException {
    ...
    intW info=new intW(0);
    if (upper)     LAPACK.getInstance().dpbtrs(UpLo.Upper.netlib(),n,kd,B.numColumns(),Cu.getData(),Matrices.ld(kd + 1),B.getData(),Matrices.ld(n),info);
 else     LAPACK.getInstance().dpbtrs(UpLo.Lower.netlib(),n,kd,B.numColumns(),Cl.getData(),Matrices.ld(kd + 1),B.getData(),Matrices.ld(n),info);
    if (info.val < 0)     throw new IllegalArgumentException();
    return B;
  }
}
