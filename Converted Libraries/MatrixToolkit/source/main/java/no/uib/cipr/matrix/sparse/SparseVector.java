 ...
 * @param nzInitial number of non-zeros
 */
 ...
 * @param xVector to copy from. A deep copy is made
 */
 ...
 * Constructor for SparseVector
 * @param sizeSize of the vector
 * @param indexIndices of the vector
 * @param dataEntries of the vector
 ...
 */
  public SparseVector(  int size,  int[] index,  double[] data,  boolean deep){
    super(size);
    if (index.length != data.length)     throw new IllegalArgumentException("index.length != data.length");
    if (deep) {
      used=index.length;
 ...
 * Constructor for SparseVector
 * @param sizeSize of the vector
 * @param indexThe vector indices are copies from this array
 * @param dataThe vector entries are copies from this array
 */
 ...
 */
  private int getIndex(  int ind){
    ...
    if (i < used && index[i] == ind)     return i;
    int[] newIndex=index;
    double[] newData=data;
    if (++used > data.length) {
  ...
  @Override public SparseVector copy(){
    return new SparseVector(this);
  }
  @Override public SparseVector zero(){
    java.util.Arrays.fill(data,0);
    used=0;
    return this;
  }
  @Override public SparseVector scale(  double alpha){
    if (alpha == 0)     return zero();
 else     if (alpha == 1)     return this;
    for (int i=0; i < used; ++i)     data[i]*=alpha;
    return this;
  }
  @Override public double dot(  Vector y){
    ...
    double ret=0;
    for (int i=0; i < used; ++i)     ret+=data[i] * yd[index[i]];
    return ret;
  }
  @Override protected double norm1(){
    double sum=0;
    for (int i=0; i < used; ++i)     sum+=Math.abs(data[i]);
    return sum;
  }
  @Override protected double norm2(){
    double norm=0;
    for (int i=0; i < used; ++i)     norm+=data[i] * data[i];
  ...
  @Override protected double normInf(){
    double max=0;
    for (int i=0; i < used; ++i)     max=Math.max(Math.abs(data[i]),max);
    return max;
  }
  /** 
 ...
 */
  public int[] getIndex(){
    if (used == index.length)     return index;
 ...
 */
  public void compact(){
    ...
    if (nz < data.length) {
      ...
      for (int i=0, j=0; i < data.length; ++i)       if (data[i] != 0.) {
        newIndex[j]=index[i];
        newData[j]=data[i];
        j++;
      }
  ...
  @Override public Iterator<VectorEntry> iterator(){
    return new SparseVectorIterator();
  }
  @Override public Vector set(  Vector y){
    if (!(y instanceof SparseVector))     return super.set(y);
    checkSize(y);
