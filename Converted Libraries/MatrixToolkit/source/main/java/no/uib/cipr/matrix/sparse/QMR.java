 ...
 */
  private Vector r, y, z, v, w, p, q, d, s, v_tld, w_tld, y_tld, z_tld, p_tld;
  /** 
 ...
 * @param templateVector to use as template for the work vectors needed in the
 * solution process
 * @param M1Left preconditioner
 * @param M2Right preconditioner
 */
  ...
  public Vector solve(  Matrix A,  Vector b,  Vector x) throws IterativeSolverNotConvergedException {
    ...
    for (iter.setFirst(); !iter.converged(r,x); iter.next()) {
      if (rho == 0)       throw new IterativeSolverNotConvergedException(NotConvergedException.Reason.Breakdown,"rho",iter);
      if (xi == 0)       throw new IterativeSolverNotConvergedException(NotConvergedException.Reason.Breakdown,"xi",iter);
      v.set(1 / rho,v_tld);
      y.scale(1 / rho);
      w.set(1 / xi,w_tld);
      z.scale(1 / xi);
      delta=z.dot(y);
      if (delta == 0)       throw new IterativeSolverNotConvergedException(NotConvergedException.Reason.Breakdown,"delta",iter);
      M2.apply(y,y_tld);
      ...
      A.mult(p,p_tld);
      ep=q.dot(p_tld);
      if (ep == 0)       throw new IterativeSolverNotConvergedException(NotConvergedException.Reason.Breakdown,"ep",iter);
      beta=ep / delta;
      if (beta == 0)       throw new IterativeSolverNotConvergedException(NotConvergedException.Reason.Breakdown,"beta",iter);
      ...
      if (gamma == 0)       throw new IterativeSolverNotConvergedException(NotConvergedException.Reason.Breakdown,"gamma",iter);
  ...
  @Override public void setPreconditioner(  Preconditioner M){
