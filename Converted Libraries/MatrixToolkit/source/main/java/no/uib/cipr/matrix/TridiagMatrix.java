 ...
 */
  ...
  /** 
 * Constructor for TridiagMatrix
 * @param AMatrix to copy from. Only the tridiagonal part is copied
 */
 ...
 */
  public TridiagMatrix(  Matrix A,  boolean deep){
    super(A);
    if (!isSquare())     throw new IllegalArgumentException("Tridiagonal matrix must be square");
    ...
    if (deep) {
      ...
      superDiag=new double[n - 1];
      subDiag=new double[n - 1];
      for (      MatrixEntry e : A)       if (e.row() == e.column() || e.row() == e.column() - 1 || e.row() == e.column() + 1)       set(e.row(),e.column(),e.get());
    }
 else {
 ...
 */
 ...
 else     if (row == column + 1)     subDiag[column]+=value;
 else     if (row == column - 1)     superDiag[row]+=value;
 else     throw new IndexOutOfBoundsException("Insertion index outside of band");
  }
  @Override public double get(  int row,  int column){
    check(row,column);
    if (row == column)     return diag[row];
 else     if (row == column + 1)     return subDiag[column];
 else     if (row == column - 1)     return superDiag[row];
 else     return 0;
  }
  ...
  @Override public TridiagMatrix copy(){
    return new TridiagMatrix(this);
  }
  @Override public TridiagMatrix zero(){
    Arrays.fill(diag,0);
  ...
  @Override public Matrix solve(  Matrix B,  Matrix X){
    ...
    LAPACK.getInstance().dgtsv(numRows,X.numColumns(),subDiag.clone(),diag.clone(),superDiag.clone(),Xd,Matrices.ld(numRows),info);
 ...
 */
    ...
    @Override public MatrixEntry next(){
      ...
      if (bandIndex < band.length - 1)       bandIndex++;
        ...
        bandIndex=0;
        whichBand++;
        if (whichBand == 1)         band=subDiag;
 else         if (whichBand == 2)         band=superDiag;
