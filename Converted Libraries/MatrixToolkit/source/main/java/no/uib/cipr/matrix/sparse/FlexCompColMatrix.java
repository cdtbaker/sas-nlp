 ...
 * @param numColumnsNumber of column
 */
  public FlexCompColMatrix(  int numRows,  int numColumns){
    super(numRows,numColumns);
    colD=new SparseVector[numColumns];
    for (int i=0; i < numColumns; ++i)     colD[i]=new SparseVector(numRows);
  }
  /** 
 * Constructor for FlexCompColMatrix
 * @param AMatrix to copy contents from
 * @param deepTrue for a deep copy, false for a reference copy. A reference
 ...
 */
  public FlexCompColMatrix(  Matrix A,  boolean deep){
    ...
    if (deep) {
      ...
      for (int i=0; i < numColumns; ++i)       colD[i]=Ar.getColumn(i);
    }
  }
  /** 
 * Constructor for FlexCompColMatrix
 * @param AMatrix to copy contents from. The copy will be deep
 */
 ...
 */
  public void setColumn(  int i,  SparseVector x){
    if (x.size() != numRows)     throw new IllegalArgumentException("New column must be of the same size as existing column");
    colD[i]=x;
  }
  ...
  @Override public Vector transMultAdd(  final double alpha,  final Vector x,  final Vector y){
    checkTransMultAdd(x,y);
    for (int i=0; i < numColumns; ++i)     y.add(i,alpha * colD[i].dot(x));
    return y;
  }
  ...
  @Override public FlexCompColMatrix copy(){
    return new FlexCompColMatrix(this);
  }
  @Override public FlexCompColMatrix zero(){
    for (int i=0; i < numColumns; ++i)     colD[i].zero();
    return this;
  }
  /** 
 ...
 */
  public void compact(){
    for (    Vector v : colD)     ((SparseVector)v).compact();
  }
  /** 
 ...
 */
    private SuperIterator<SparseVector,VectorEntry> iterator=new SuperIterator<SparseVector,VectorEntry>(Arrays.asList(colD));
    /** 
 * Entry returned
 */
