 ...
 */
  ...
  private void factor(){
    ...
    for (int i=1; i < n; ++i) {
      ...
      for (int k=0; k < i; ++k) {
        ...
        if (rowData[diagind[k]] == 0)         throw new RuntimeException("Zero diagonal entry on row " + (k + 1) + " during ILU process");
        double LUik=LUi[k] / rowData[diagind[k]];
        if (Math.abs(LUik) <= taui)         continue;
        for (int j=diagind[k] + 1; j < rowUsed; ++j)         LUi[rowIndex[j]]-=LUik * rowData[j];
        LUi[k]=LUik;
      }
      gather(LUi,rowi,taui,i);
      if (rowi.getIndex()[diagind[i]] != i) {
        diagind[i]=findDiagonalIndex(rowi,i);
        if (diagind[i] < 0)         throw new RuntimeException("Missing diagonal entry on row " + (i + 1) + " during ILU process");
      }
    }
  ...
  private int findDiagonalIndex(  SparseVector v,  int k){
    return no.uib.cipr.matrix.sparse.Arrays.binarySearch(v.getIndex(),k,0,v.getUsed());
  }
  /** 
 ...
 */
  private void scatter(  SparseVector v,  double[] z){
    ...
    Arrays.fill(z,0);
    for (int i=0; i < used; ++i)     z[index[i]]=data[i];
  }
  /** 
 ...
 */
  private void gather(  double[] z,  SparseVector v,  double taui,  int d){
    ...
    for (    VectorEntry e : v) {
      if (e.index() < d)       nl++;
 else       if (e.index() > d)       nu++;
    }
    v.zero();
    lower.clear();
    for (int i=0; i < d; ++i)     if (Math.abs(z[i]) > taui)     lower.add(new IntDoubleEntry(i,z[i]));
    upper.clear();
    for (int i=d + 1; i < z.length; ++i)     if (Math.abs(z[i]) > taui)     upper.add(new IntDoubleEntry(i,z[i]));
...
private static class IntDoubleEntry implements Comparable<IntDoubleEntry> {
    ...
    public int compareTo(    IntDoubleEntry o){
      if (Math.abs(value) < Math.abs(o.value))       return 1;
 else       if (Math.abs(value) == Math.abs(o.value))       return 0;
 else       return -1;
    }
...
private static class UnitLowerFlexCompRowMatrix extends AbstractMatrix {
    ...
    @Override public Vector solve(    Vector b,    Vector x){
      ...
      for (int i=0; i < numRows; ++i) {
        ...
        double sum=0;
        for (int j=0; j < diagind[i]; ++j)         sum+=data[j] * xd[index[j]];
        xd[i]=bd[i] - sum;
      }
      return x;
    }
    @Override public Vector transSolve(    Vector b,    Vector x){
      ...
      for (int i=numRows - 1; i >= 0; --i) {
        ...
        int[] index=row.getIndex();
        double[] data=row.getData();
        for (int j=0; j < diagind[i]; ++j)         xd[index[j]]-=data[j] * xd[i];
      }
      return x;
    }
  }
...
private static class UpperFlexCompRowMatrix extends AbstractMatrix {
    ...
    @Override public Vector solve(    Vector b,    Vector x){
      ...
      for (int i=numRows - 1; i >= 0; --i) {
        ...
        double[] data=row.getData();
        double sum=0;
        for (int j=diagind[i] + 1; j < used; ++j)         sum+=data[j] * xd[index[j]];
    ...
    @Override public Vector transSolve(    Vector b,    Vector x){
      ...
      for (int i=0; i < numRows; ++i) {
        ...
        double[] data=row.getData();
        xd[i]/=data[diagind[i]];
        for (int j=diagind[i] + 1; j < used; ++j)         xd[index[j]]-=data[j] * xd[i];
      }
      return x;
    }
  }
