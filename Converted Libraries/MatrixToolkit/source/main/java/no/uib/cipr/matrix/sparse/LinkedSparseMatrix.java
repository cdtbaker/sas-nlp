 ...
 * @author Sam Halliday
 */
@Log public class LinkedSparseMatrix extends AbstractMatrix {
@AllArgsConstructor @ToString(exclude={"rowTail","colTail"}) static class Node {
    final int row, col;
    double val;
...
class Linked {
    ...
    public double get(    int row,    int col){
      if (isHead(row,col))       return head.val;
      if (col <= row) {
        Node node=findPreceedingByRow(row,col);
        if (isNextByRow(node,row,col))         return node.rowTail.val;
      }
 else {
        Node node=findPreceedingByCol(row,col);
        if (isNextByCol(node,row,col))         return node.colTail.val;
      }
      return 0;
    }
    public void set(    int row,    int col,    double val){
      ...
      if (isHead(row,col))       head.val=val;
 else {
        Node prevRow=findPreceedingByRow(row,col);
        if (isNextByRow(prevRow,row,col))         prevRow.rowTail.val=val;
 else {
    ...
    private void updateCache(    Node inserted){
      if (rows[inserted.row] == null || inserted.col > rows[inserted.row].col)       rows[inserted.row]=inserted;
      if (cols[inserted.col] == null || inserted.row > cols[inserted.col].row)       cols[inserted.col]=inserted;
    }
    private void delete(    int row,    int col){
      ...
      if (isNextByRow(precRow,row,col)) {
        if (rows[row] == precRow.rowTail)         rows[row]=precRow.row == row ? precRow : null;
        precRow.rowTail=precRow.rowTail.rowTail;
      }
      if (isNextByCol(precCol,row,col)) {
        if (cols[col] == precCol.colTail)         cols[col]=precCol.col == col ? precCol : null;
    ...
    private Node cachedByRow(    int row){
      for (int i=row; i >= 0; i--)       if (rows[i] != null)       return rows[i];
      return head;
    }
    ...
    private Node cachedByCol(    int col){
      for (int i=col; i >= 0; i--)       if (cols[i] != null)       return cols[i];
      return head;
    }
    Node startOfRow(    int row){
      if (row == 0)       return head;
      Node prec=findPreceedingByRow(row,0);
      if (prec.rowTail != null && prec.rowTail.row == row)       return prec.rowTail;
      return null;
    }
    Node startOfCol(    int col){
      if (col == 0)       return head;
      Node prec=findPreceedingByCol(0,col);
      if (prec != null && prec.colTail != null && prec.colTail.col == col)       return prec.colTail;
      return null;
    }
  }
  Linked links;
...
;
  ...
  @Override public Matrix scale(  double alpha){
    if (alpha == 0)     zero();
 else     if (alpha != 1)     for (    MatrixEntry e : this)     set(e.row(),e.column(),e.get() * alpha);
    return this;
  }
  ...
  @Override public Matrix multAdd(  double alpha,  Matrix B,  Matrix C){
    ...
    for (int i=0; i < numRows; i++) {
      ...
      if (row != null)       for (int j=0; j < B.numColumns(); j++) {
        Node node=row;
        double v=0;
  ...
  @Override public Matrix transBmultAdd(  double alpha,  Matrix B,  Matrix C){
    ...
    for (int i=0; i < numRows; i++) {
      Node row=links.startOfRow(i);
      if (row != null)       for (int j=0; j < B.numRows(); j++) {
        Node node=row;
        double v=0;
        while (node != null && node.row == i) {
