 ...
 */
  public static int cardinality(  Vector x){
    int nz=0;
    for (    VectorEntry e : x)     if (e.get() != 0)     nz++;
    return nz;
  }
  /** 
 ...
 */
  public static int cardinality(  Matrix A){
    int nz=0;
    for (    MatrixEntry e : A)     if (e.get() != 0)     nz++;
    return nz;
  }
  /** 
 ...
 */
  public static double[][] getArray(  Matrix A){
    ...
    for (    MatrixEntry e : A)     Ad[e.row()][e.column()]=e.get();
    return Ad;
  }
  /** 
 ...
 */
  public static double[] getArray(  Vector x){
    ...
    for (    VectorEntry e : x)     xd[e.index()]=e.get();
    return xd;
  }
  /** 
 * Returns the identity matrix of the given size
 * @param sizeNumber of rows/columns of the matrix
 ...
 */
  public static DenseMatrix identity(  int size){
    ...
    for (int i=0; i < size; ++i)     A.set(i,i,1);
    return A;
  }
  /** 
 ...
 * between 0 and 1
 * @param xVector to populate
 */
  public static Vector random(  Vector x){
    for (int i=0; i < x.size(); ++i)     x.set(i,Math.random());
    return x;
  }
  /** 
 ...
 * between 0 and 1
 * @param AMatrix to populate
 */
  public static Matrix random(  Matrix A){
    for (int j=0; j < A.numColumns(); ++j)     for (int i=0; i < A.numRows(); ++i)     A.set(i,j,Math.random());
    return A;
  }
  /** 
 ...
 * @param AMatrix to create view on
 * @param rowRows to access. Must be within the bounds of <code>A</code>
 * @param columnColumns to access. Must be within the bounds of <code>A</code>
 ...
 * @param xVector to create view on
 * @param indexIndices to access. Must be within the bounds of <code>x</code>
...
private static class SynchronizedVector extends AbstractVector {
    ...
    @Override public synchronized void add(    int index,    double value){
      x.add(index,value);
    }
    @Override public synchronized void set(    int index,    double value){
      x.set(index,value);
    }
    @Override public synchronized double get(    int index){
      return x.get(index);
    }
...
private static class SynchronizedMatrix extends AbstractMatrix {
    ...
    @Override public synchronized void add(    int row,    int column,    double value){
      A.add(row,column,value);
    }
    @Override public synchronized void set(    int row,    int column,    double value){
      A.set(row,column,value);
    }
    @Override public synchronized double get(    int row,    int column){
...
private static class SynchronizedRowMatrix extends AbstractMatrix {
    ...
    public SynchronizedRowMatrix(    Matrix A){
      ...
      for (int i=0; i < lock.length; ++i)       lock[i]=new Object();
    }
    @Override public void add(    int row,    int column,    double value){
 ...
 * @param fromStart, inclusive
 * @param toStop, exclusive
 */
  public static int[] index(  int from,  int to){
    int length=to - from;
    if (length < 0)     length=0;
    int[] index=new int[length];
    for (int i=from, j=0; j < length; ++i, ++j)     index[j]=i;
    return index;
  }
  /** 
 ...
 */
  public static int[] index(  int from,  int stride,  int to){
    if (stride == 1)     return index(from,to);
 else     if (stride == 0)     return new int[0];
    if (to <= from && stride > 0)     return new int[0];
    if (from <= to && stride < 0)     return new int[0];
    int length=Math.abs((to - from) / stride);
    if (Math.abs((to - from) % stride) > 0)     length++;
    if (length < 0)     length=0;
    int[] index=new int[length];
    for (int i=from, j=0; j < length; i+=stride, ++j)     index[j]=i;
    return index;
  }
  /** 
 ...
 */
  public static int[] columnBandwidth(  Matrix A){
    ...
    for (    MatrixEntry e : A)     nz[e.column()]++;
    return nz;
  }
  /** 
 ...
 */
  public static int getNumSubDiagonals(  Matrix A){
    int kl=0;
    for (    MatrixEntry e : A)     kl=Math.max(kl,e.row() - e.column());
    return kl;
  }
  /** 
 ...
 */
  public static int getNumSuperDiagonals(  Matrix A){
    int ku=0;
    for (    MatrixEntry e : A)     ku=Math.max(ku,e.column() - e.row());
    return ku;
  }
  /** 
 ...
 */
  public static void zeroRows(  Matrix A,  double diagonal,  int... row){
    ...
    for (    MatrixEntry e : A) {
      ...
      if (j >= 0) {
        if (e.row() == e.column())         e.set(diagonal);
 else         e.set(0);
      }
    }
    if (diagonal != 0)     for (    int rowI : row)     A.set(rowI,rowI,diagonal);
  }
  /** 
 ...
 */
  public static void zeroColumns(  Matrix A,  double diagonal,  int... column){
    ...
    if (diagonal != 0)     for (    int columnI : column)     A.set(columnI,columnI,diagonal);
  }
