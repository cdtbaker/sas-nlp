 ...
 */
  ...
  @Override public Matrix solve(  Matrix B,  Matrix X){
    ...
    LAPACK.getInstance().dspsv(uplo.netlib(),numRows,X.numColumns(),data.clone(),ipiv,Xd,Matrices.ld(numRows),info);
  ...
  Matrix SPDsolve(  Matrix B,  Matrix X){
    ...
    X.set(B);
    intW info=new intW(0);
    LAPACK.getInstance().dppsv(uplo.netlib(),numRows,X.numColumns(),data.clone(),Xd,Matrices.ld(numRows),info);
