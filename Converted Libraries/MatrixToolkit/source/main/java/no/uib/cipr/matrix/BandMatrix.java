 ...
 * @param klNumber of bands above the main diagonal (superdiagonals)
 * @param kuNumber of bands below the main diagonal (subdiagonals)
 */
 ...
 */
  ...
  @Override public BandMatrix copy(){
  ...
  @Override public Vector multAdd(  double alpha,  Vector x,  Vector y){
    ...
    BLAS.getInstance().dgbmv(Transpose.NoTranspose.netlib(),numRows,numColumns,kl,ku,alpha,data,kl + ku + 1,xd,1,1,yd,1);
    return y;
  }
  @Override public Vector transMultAdd(  double alpha,  Vector x,  Vector y){
    ...
    double[] xd=((DenseVector)x).getData(), yd=((DenseVector)y).getData();
    BLAS.getInstance().dgbmv(Transpose.Transpose.netlib(),numRows,numColumns,kl,ku,alpha,data,kl + ku + 1,xd,1,1,yd,1);
    return y;
  }
  @Override public Matrix solve(  Matrix B,  Matrix X){
    ...
    LAPACK.getInstance().dgbsv(numRows,kl,ku,X.numColumns(),Af.getData(),Matrices.ld(2 * kl + ku + 1),ipiv,Xd,Matrices.ld(numRows),info);
  ...
  @Override public Matrix transpose(){
    checkTranspose();
    if (kl != ku)     throw new IllegalArgumentException("kl != ku");
    for (int j=0; j < numColumns; ++j)     for (int i=j + 1; i < Math.min(j + kl + 1,numRows); ++i) {
