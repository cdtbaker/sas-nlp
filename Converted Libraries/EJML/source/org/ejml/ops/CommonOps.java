 ...
 * @author Peter Abeles
 */
@SuppressWarnings({"ForLoopReplaceableByForEach"}) public class CommonOps {
  /** 
 ...
 */
  public static void multInner(  RowD1Matrix64F a,  RowD1Matrix64F c){
    if (a.numCols != c.numCols || a.numCols != c.numRows)     throw new IllegalArgumentException("Rows and columns of 'c' must be the same as the columns in 'a'");
 ...
 */
  public static void multOuter(  RowD1Matrix64F a,  RowD1Matrix64F c){
    if (a.numRows != c.numCols || a.numRows != c.numRows)     throw new IllegalArgumentException("Rows and columns of 'c' must be the same as the rows in 'a'");
 ...
 */
  public static DenseMatrix64F transpose(  DenseMatrix64F A,  DenseMatrix64F A_tran){
    ...
    if (A.numRows > EjmlParameters.TRANSPOSE_SWITCH && A.numCols > EjmlParameters.TRANSPOSE_SWITCH)     TransposeAlgs.block(A,A_tran,EjmlParameters.BLOCK_WIDTH);
 else     TransposeAlgs.standard(A,A_tran);
    return A_tran;
  }
  /** 
 * <p>
 ...
 */
  public static double det(  DenseMatrix64F mat){
    ...
    if (numCol != numRow) {
      ...
      if (!alg.decompose(mat))       return 0.0;
 ...
 */
  public static boolean invert(  DenseMatrix64F mat,  DenseMatrix64F result){
    if (mat.numCols <= UnrolledInverseFromMinor.MAX) {
      ...
      if (!solver.setA(mat))       return false;
 ...
 */
  public static void pinv(  DenseMatrix64F A,  DenseMatrix64F invA){
    ...
    if (!solver.setA(A))     throw new IllegalArgumentException("Invert failed, maybe a bug?");
    solver.invert(invA);
  }
  /** 
 ...
 */
  ...
  public static DenseMatrix64F diag(  DenseMatrix64F ret,  int width,  double... diagEl){
    if (ret == null) {
      ...
      if (ret.numRows != width || ret.numCols != width)       throw new IllegalArgumentException("Unexpected matrix size");
 ...
 */
  public static void extract(  ReshapeMatrix64F src,  int srcY0,  int srcY1,  int srcX0,  int srcX1,  ReshapeMatrix64F dst,  int dstY0,  int dstX0){
    if (srcY1 < srcY0 || srcY0 < 0 || srcY1 > src.numRows)     throw new IllegalArgumentException("srcY1 < srcY0 || srcY0 < 0 || srcY1 > src.numRows");
    if (srcX1 < srcX0 || srcX0 < 0 || srcX1 > src.numCols)     throw new IllegalArgumentException("srcX1 < srcX0 || srcX0 < 0 || srcX1 > src.numCols");
    int w=srcX1 - srcX0;
    int h=srcY1 - srcY0;
    if (dstY0 + h > dst.numRows)     throw new IllegalArgumentException("dst is too small in rows");
    if (dstX0 + w > dst.numCols)     throw new IllegalArgumentException("dst is too small in columns");
 ...
 */
 ...
 else     if (output.getNumElements() != input.numRows)     throw new IllegalArgumentException("Output does not have enough elements to store the results");
 ...
 */
 ...
 else     if (output.getNumElements() != input.numCols)     throw new IllegalArgumentException("Output does not have enough elements to store the results");
 ...
 */
  public static void scale(  double alpha,  D1Matrix64F a,  D1Matrix64F b){
    if (a.numRows != b.numRows || a.numCols != b.numCols)     throw new IllegalArgumentException("Matrices must have the same shape");
    final int size=a.getNumElements();
    for (int i=0; i < size; i++) {
 ...
 */
 ...
 else     if (reduced.numCols != A.numCols || reduced.numRows != A.numRows)     throw new IllegalArgumentException("'re' must have the same shape as the original input matrix");
