 ...
 */
  public static DenseMatrix64F createReflector(  RowD1Matrix64F u){
    if (!MatrixFeatures.isVector(u))     throw new IllegalArgumentException("u must be a vector");
 ...
 */
  public static void addIdentity(  RowD1Matrix64F A,  RowD1Matrix64F B,  double alpha){
    ...
    if (B.numCols != A.numCols || B.numRows != A.numRows)     throw new IllegalArgumentException("B must be the same shape as A");
    int n=A.numCols;
    int index=0;
    for (int i=0; i < n; i++) {
 ...
 */
  public static DenseMatrix64F[] splitIntoVectors(  RowD1Matrix64F A,  boolean column){
    ...
    for (int i=0; i < w; i++) {
      ...
      if (column)       subvector(A,0,i,o,false,0,a);
 else       subvector(A,i,0,o,true,0,a);
      ret[i]=a;
    }
    return ret;
  }
 ...
 */
  public static DenseMatrix64F pivotMatrix(  DenseMatrix64F ret,  int pivots[],  int numPivots,  boolean transposed){
    if (ret == null) {
      ...
      if (ret.numCols != numPivots || ret.numRows != numPivots)       throw new IllegalArgumentException("Unexpected matrix dimension");
