 ...
 */
  public static boolean hasNaN(  D1Matrix64F m){
    ...
    for (int i=0; i < length; i++) {
      if (Double.isNaN(m.get(i)))       return true;
    }
    return false;
  }
  /** 
 ...
 */
  public static boolean hasUncountable(  D1Matrix64F m){
    ...
    for (int i=0; i < length; i++) {
      double a=m.get(i);
      if (Double.isNaN(a) || Double.isInfinite(a))       return true;
    }
    return false;
  }
  /** 
 ...
 */
  public static boolean isEquals(  D1Matrix64F a,  D1Matrix64F b,  double tol){
    ...
    if (tol == 0.0)     return isEquals(a,b);
 ...
 */
  public static boolean isIdentical(  D1Matrix64F a,  D1Matrix64F b,  double tol){
    ...
    if (tol < 0)     throw new IllegalArgumentException("Tolerance must be greater than or equal to zero.");
    final int length=a.getNumElements();
    for (int i=0; i < length; i++) {
      double valA=a.get(i);
      double valB=b.get(i);
  ...
  public static boolean isIdentity(  DenseMatrix64F mat,  double tol){
    ...
    for (int i=0; i < mat.numRows; i++) {
      for (int j=0; j < mat.numCols; j++) {
        if (i == j) {
          if (!(Math.abs(mat.get(index++) - 1) <= tol))           return false;
        }
 else {
          if (!(Math.abs(mat.get(index++)) <= tol))           return false;
        }
      }
    }
    return true;
 ...
 */
  public static boolean isConstantVal(  DenseMatrix64F mat,  double val,  double tol){
    ...
    for (int i=0; i < mat.numRows; i++) {
      for (int j=0; j < mat.numCols; j++) {
        if (!(Math.abs(mat.get(index++) - val) <= tol))         return false;
      }
    }
    return true;
  }
 ...
 */
  public static boolean isNegative(  D1Matrix64F a,  D1Matrix64F b,  double tol){
    if (a.numRows != b.numRows || a.numCols != b.numCols)     throw new IllegalArgumentException("Matrix dimensions must match");
    int length=a.getNumElements();
    for (int i=0; i < length; i++) {
      if (!(Math.abs(a.get(i) + b.get(i)) <= tol))       return false;
    }
    return true;
  }
  /** 
