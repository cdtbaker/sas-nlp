 ...
 */
  protected BaseLinearSolverQrp(  QRPDecomposition<DenseMatrix64F> decomposition,  boolean norm2Solution){
    ...
    if (internalSolver.modifiesA())     internalSolver=new LinearSolverSafe<DenseMatrix64F>(internalSolver);
  }
  @Override public boolean setA(  DenseMatrix64F A){
    _setA(A);
    decomposition.setSingularThreshold(CommonOps.elementMaxAbs(A) * UtilEjml.EPS);
    if (!decomposition.decompose(A))     return false;
    ...
    if (norm2Solution && rank < numCols) {
      ...
      for (int i=0; i < numCols - rank; i++) {
        for (int j=0; j < numCols - rank; j++) {
          if (i == j)           W.set(i + rank,j,-1);
 else           W.set(i + rank,j,0);
        }
      }
    }
    return true;
 ...
 */
  protected void upgradeSolution(  DenseMatrix64F X){
    DenseMatrix64F z=Y;
    if (!internalSolver.setA(W))     throw new RuntimeException("This should never happen.  Is input NaN?");
  ...
  @Override public void invert(  DenseMatrix64F A_inv){
    if (A_inv.numCols != numRows || A_inv.numRows != numCols)     throw new IllegalArgumentException("Unexpected dimensions for A_inv");
