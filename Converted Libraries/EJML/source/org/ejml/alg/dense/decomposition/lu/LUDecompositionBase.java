 ...
 */
  @Override public DenseMatrix64F getLower(  DenseMatrix64F lower){
    ...
    if (lower == null) {
      ...
      if (lower.numCols != numCols || lower.numRows != numRows)       throw new IllegalArgumentException("Unexpected matrix dimension");
 ...
 */
  @Override public DenseMatrix64F getUpper(  DenseMatrix64F upper){
    ...
    if (upper == null) {
      ...
      if (upper.numCols != numCols || upper.numRows != numRows)       throw new IllegalArgumentException("Unexpected matrix dimension");
 ...
 */
  @Override public boolean isSingular(){
    for (int i=0; i < m; i++) {
      if (Math.abs(dataLU[i * n + i]) < UtilEjml.EPS)       return true;
    }
    return false;
  }
  /** 
 ...
 */
  @Override public double computeDeterminant(){
    if (m != n)     throw new IllegalArgumentException("Must be a square matrix.");
    double ret=pivsign;
    int total=m * n;
 ...
 */
  public void _solveVectorInternal(  double[] vv){
    ...
    for (int i=0; i < n; i++) {
      ...
      if (ii != 0) {
        int index=i * n + ii - 1;
        for (int j=ii - 1; j < i; j++)         sum-=dataLU[index++] * vv[j];
      }
 else       if (sum != 0.0) {
        ii=i + 1;
      }
