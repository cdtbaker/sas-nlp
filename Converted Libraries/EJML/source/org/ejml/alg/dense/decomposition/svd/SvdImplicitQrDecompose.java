 ...
 */
  public SvdImplicitQrDecompose(  boolean compact,  boolean computeU,  boolean computeV,  boolean canUseTallBidiagonal){
  ...
  @Override public DenseMatrix64F getU(  DenseMatrix64F U,  boolean transpose){
    if (!prefComputeU)     throw new IllegalArgumentException("As requested U was not computed.");
    if (transpose) {
      if (U == null)       return Ut;
 else       if (U.numRows != Ut.numRows || U.numCols != Ut.numCols)       throw new IllegalArgumentException("Unexpected shape of U");
      U.set(Ut);
    }
 else {
      if (U == null)       U=new DenseMatrix64F(Ut.numCols,Ut.numRows);
 else       if (U.numRows != Ut.numCols || U.numCols != Ut.numRows)       throw new IllegalArgumentException("Unexpected shape of U");
      CommonOps.transpose(Ut,U);
    }
    return U;
  }
  @Override public DenseMatrix64F getV(  DenseMatrix64F V,  boolean transpose){
    if (!prefComputeV)     throw new IllegalArgumentException("As requested V was not computed.");
    if (transpose) {
      if (V == null)       return Vt;
 else       if (V.numRows != Vt.numRows || V.numCols != Vt.numCols)       throw new IllegalArgumentException("Unexpected shape of V");
      V.set(Vt);
    }
 else {
      if (V == null)       V=new DenseMatrix64F(Vt.numCols,Vt.numRows);
 else       if (V.numRows != Vt.numCols || V.numCols != Vt.numRows)       throw new IllegalArgumentException("Unexpected shape of V");
  ...
  @Override public DenseMatrix64F getW(  DenseMatrix64F W){
    ...
    if (W == null)     W=new DenseMatrix64F(m,n);
 else {
      W.reshape(m,n,false);
      W.zero();
    }
  ...
  @Override public boolean decompose(  DenseMatrix64F orig){
    setup(orig);
    if (bidiagonalization(orig))     return false;
    if (computeUWV())     return false;
 ...
 */
  private boolean computeUWV(){
    ...
    if (computeU)     Ut=bidiag.getU(Ut,true,compact);
    if (computeV)     Vt=bidiag.getV(Vt,true,compact);
    qralg.setFastValues(false);
    if (computeU)     qralg.setUt(Ut);
 else     qralg.setUt(null);
    if (computeV)     qralg.setVt(Vt);
 else     qralg.setVt(null);
