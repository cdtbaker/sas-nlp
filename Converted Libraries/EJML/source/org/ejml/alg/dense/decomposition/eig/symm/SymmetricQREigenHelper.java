 ...
 */
  ...
  protected void performImplicitSingleStep(  double lambda,  boolean byAngle){
    if (x2 - x1 == 1) {
      ...
      if (bulge != 0.0)       removeBulgeEnd(x2 - 2);
    }
  }
 ...
 */
  ...
  protected void removeBulge(  int x1){
    ...
    off[x1 + 2]=c * a34;
    bulge=s * a34;
    if (Q != null)     updateQ(x1 + 1,x1 + 2,c,s);
  }
  /** 
 ...
 */
  public boolean nextSplit(){
    if (numSplits == 0)     return false;
    x2=splits[--numSplits];
    if (numSplits > 0)     x1=splits[numSplits - 1] + 1;
 else     x1=0;
    return true;
  }
  public double computeShift(){
    if (x2 - x1 >= 1)     return computeWilkinsonShift();
 else     return diag[x2];
  }
  public double computeWilkinsonShift(){
    ...
    if (diff0 < diff1)     return scale * eigenSmall.value0.real;
 else     return scale * eigenSmall.value1.real;
  }
