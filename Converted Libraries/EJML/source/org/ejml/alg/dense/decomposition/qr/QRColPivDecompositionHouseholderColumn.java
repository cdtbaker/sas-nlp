 ...
 * @author Peter Abeles
 */
public class QRColPivDecompositionHouseholderColumn extends QRDecompositionHouseholderColumn implements QRPDecomposition<DenseMatrix64F> {
 ...
 */
  ...
  @Override public void setSingularThreshold(  double threshold){
 ...
 */
  @Override public boolean decompose(  DenseMatrix64F A){
    ...
    for (int j=0; j < minLength; j++) {
      if (j > 0)       updateNorms(j);
      swapColumns(j);
      if (!householderPivot(j))       break;
      updateA(j);
      rank=j + 1;
    }
    return true;
 ...
 */
  ...
  @Override public int[] getPivots(){
    return pivots;
  }
  @Override public DenseMatrix64F getPivotMatrix(  DenseMatrix64F P){
    if (P == null)     P=new DenseMatrix64F(numCols,numCols);
 else     if (P.numRows != numCols)     throw new IllegalArgumentException("Number of rows must be " + numCols);
 else     if (P.numCols != numCols)     throw new IllegalArgumentException("Number of columns must be " + numCols);
 else {
      P.zero();
    }
