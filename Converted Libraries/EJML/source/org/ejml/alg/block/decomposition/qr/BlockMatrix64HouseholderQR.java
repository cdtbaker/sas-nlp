 ...
 * @inheritDoc
 */
  @Override public BlockMatrix64F getQ(  BlockMatrix64F Q,  boolean compact){
 ...
 */
  public void applyQ(  BlockMatrix64F B,  boolean isIdentity){
    ...
    if (start == minDimen)     start-=blockLength;
    if (start < 0)     start=0;
    for (int i=start; i >= 0; i-=blockLength) {
      ...
      if (!saveW)       BlockHouseHolder.computeW_Column(blockLength,Y,W,temp,gammas,Y.col0);
 ...
 * @inheritDoc
 */
  @Override public BlockMatrix64F getR(  BlockMatrix64F R,  boolean compact){
 ...
 */
  private void setup(  BlockMatrix64F orig){
    ...
    if (temp.length < blockLength)     temp=new double[blockLength];
    if (gammas.length < orig.numCols)     gammas=new double[orig.numCols];
    if (saveW) {
