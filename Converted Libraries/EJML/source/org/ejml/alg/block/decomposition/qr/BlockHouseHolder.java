 ...
 */
  public static boolean decomposeQR_block_col(  final int blockLength,  final D1Submatrix64F Y,  final double gamma[]){
    ...
    for (int i=0; i < min; i++) {
      if (!computeHouseHolderCol(blockLength,Y,gamma,i))       return false;
 ...
 */
  public static void multTransA_vecCol(  final int blockLength,  D1Submatrix64F A,  D1Submatrix64F B,  D1Submatrix64F C){
    ...
    if (widthA > blockLength)     throw new IllegalArgumentException("A is expected to be at most one block wide.");
    for (int j=B.col0; j < B.col1; j+=blockLength) {
      int widthB=Math.min(blockLength,B.col1 - j);
      ...
      for (int k=A.row0; k < A.row1; k+=blockLength) {
        ...
        if (k == A.row0)         multTransABlockSet_lowerTriag(A.original.data,B.original.data,C.original.data,indexA,indexB,indexC,heightA,widthA,widthB);
 else         BlockInnerMultiplication.blockMultPlusTransA(A.original.data,B.original.data,C.original.data,indexA,indexB,indexC,heightA,widthA,widthB);
      }
    }
  }
  /** 
