 ...
 */
  public static void invert(  final int blockLength,  final boolean upper,  final D1Submatrix64F T,  final D1Submatrix64F T_inv,  final double temp[]){
    if (upper)     throw new IllegalArgumentException("Upper triangular matrices not supported yet");
    if (temp.length < blockLength * blockLength)     throw new IllegalArgumentException("Temp must be at least blockLength*blockLength long.");
    if (T.row0 != T_inv.row0 || T.row1 != T_inv.row1 || T.col0 != T_inv.col0 || T.col1 != T_inv.col1)     throw new IllegalArgumentException("T and T_inv must be at the same elements in the matrix");
 ...
 */
  public static void solveBlock(  final int blockLength,  final boolean upper,  final D1Submatrix64F T,  final D1Submatrix64F B,  final boolean transT,  final boolean transB){
    ...
    if (Trows > blockLength)     throw new IllegalArgumentException("T can be at most the size of a block");
    ...
    if (transB) {
      ...
      if (Trows != B.row1 - B.row0)       throw new IllegalArgumentException("T and B must have the same number of rows.");
      if (upper) {
        if (transT) {
