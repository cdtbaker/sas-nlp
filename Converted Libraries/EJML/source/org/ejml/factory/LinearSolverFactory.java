 ...
 */
  public static LinearSolver<DenseMatrix64F> general(  int numRows,  int numCols){
    if (numRows == numCols)     return linear(numRows);
 else     return leastSquares(numRows,numCols);
  }
  /** 
 ...
 */
  public static LinearSolver<DenseMatrix64F> leastSquares(  int numRows,  int numCols){
    if (numCols < EjmlParameters.SWITCH_BLOCK64_QR) {
      ...
      if (EjmlParameters.MEMORY == EjmlParameters.MemoryUsage.FASTER)       return new LinearSolverQrBlock64();
 else       return new LinearSolverQrHouseCol();
    }
  }
  /** 
 ...
 */
  public static LinearSolver<DenseMatrix64F> symmPosDef(  int matrixWidth){
    if (matrixWidth < EjmlParameters.SWITCH_BLOCK64_CHOLESKY) {
      ...
      if (EjmlParameters.MEMORY == EjmlParameters.MemoryUsage.FASTER)       return new LinearSolverCholBlock64();
 else {
        CholeskyDecompositionCommon decomp=new CholeskyDecompositionInner(true);
        return new LinearSolverChol(decomp);
      }
 ...
 */
  public static LinearSolver<DenseMatrix64F> leastSquaresQrPivot(  boolean computeNorm2,  boolean computeQ){
    QRColPivDecompositionHouseholderColumn decomposition=new QRColPivDecompositionHouseholderColumn();
    if (computeQ)     return new SolvePseudoInverseQrp(decomposition,computeNorm2);
 else     return new LinearSolverQrpHouseCol(decomposition,computeNorm2);
  }
  /** 
 * <p>
 ...
 */
  public static LinearSolver<DenseMatrix64F> pseudoInverse(  boolean useSVD){
    if (useSVD)     return new SolvePseudoInverseSvd();
 else     return leastSquaresQrPivot(true,false);
  }
  /** 
