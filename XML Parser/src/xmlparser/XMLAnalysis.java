...
public class XMLAnalysis extends AbstractCrystalMethodAnalysis {
	...
	public void beforeAllMethods(ITypeRoot rootNode,
		 ...
		 * elements = new Stack<Element>(); methodDec = null; String source =
		 * null; try { source = rootNode.getSource(); } catch
		 * (JavaModelException e) { // TODO: what is the best way of terminating
		 * the analysis? System.exit(-1); }
		 * 
		 * if (source == null) { // TODO: what is the best way of terminating
		 * the analysis? System.exit(-1); }
		 * 
		 * sourceS = new Element("source"); String packageName= "";
		 * if(compilationUnit.getPackage() != null){ packageName =
		 * compilationUnit .getPackage().toString(); packageName =
		 * packageName.substring(8,packageName.length()-2); }
		 * sourceS.setAttribute("package", packageName); elements.push(sourceS);
		 * 
		 * List<ImportDeclaration> ids = (List<ImportDeclaration>)
		 * compilationUnit .imports(); for (ImportDeclaration id : ids) {
		 * Element el = new Element("import"); el.setAttribute("name",
		 * id.toString().substring(7,id.getLength()-1)); sourceS.addContent(el);
		 * } // Property: At this point, source exists and is not null.
		 * 
		 * // initialize the cvisitor with the source parser = new
		 * XMLParser(compilationUnit, source.split("\n"));
		 * 
		 * d = new Document();
		 * 
		 * d.setRootElement(sourceS); compilationUnit.accept(parser);
		 * 
		 * // To visite LineComment and BlockComment nodes, we need to call the
		 * // accept method on them // (see documentation of getCommentList()
		 * for more details) List<Comment> l = compilationUnit.getCommentList();
		 * for (Comment c : l) { if (c instanceof Javadoc && c.getParent() ==
		 * null) { c.accept(parser); } if (!(c instanceof Javadoc)) {
		 * c.accept(parser);
		 * 
		 * } }
		 * 
		 * // records start time of analysis startTime =
		 * System.currentTimeMillis(); //System.out.println("Running: " +
		 * this.getName()); super.beforeAllMethods(rootNode, compilationUnit);
		 */
		...
		try {
			...
			JMLElement r = com.jml.builder.XMLFromSource.createXML(
					compilationUnit, rootNode.getSource(), true);
		 ...
		 * com.jml.builder.XMLFromSource.createXML(compilationUnit,
		 * rootNode.getSource(), true); } catch (JavaModelException e) {
		 * e.printStackTrace(); } com.jml.output.XMLOutputter o = new
		 * com.jml.output.XMLOutputter(r); System.out.println(o.getString());
		 */
	...
	public void afterAllMethods(ITypeRoot compUnit, CompilationUnit rootNode) {
		...
		/*
		 * // records end time endTime = System.currentTimeMillis();
		 * 
		 * XMLOutputter xml = new XMLOutputter();
		 * xml.setFormat(Format.getPrettyFormat());
		 * //System.out.println(StringEscapeUtils
		 * .unescapeXml(xml.outputString(d))); String path =
		 * compUnit.getPath().toString(); path =
		 * path.substring(path.lastIndexOf("src/"), path.indexOf(".java"));
		 * 
		 * File dir = new File(path.substring(0, path.lastIndexOf('/')));
		 * 
		 ...
		 * 
		 * //System.out.println("\nEnd of " + getName() + ", \nTime Taken: " //
		 * + (endTime - startTime) + "ms"); // create and run a test analysis
	...
	//
	// for (int lineCount = startLineNumber - 1; lineCount <= endLineNumber - 1;
	// lineCount++) {
	//
