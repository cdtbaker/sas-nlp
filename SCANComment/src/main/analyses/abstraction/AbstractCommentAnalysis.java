 ...
 * @param <CommentType>
 */
public abstract class AbstractCommentAnalysis<LaticeType extends ILattice, CommentType extends AnnotationComment<LaticeType>>
	...
	public void analyse(ASTNode node, Expression varName, Expression exp) {
		...
		if (toReport(varName.toString(),
			...
			}else if (element != current.get(varName.toString()).getExpected()) {
				getReporter().reportUserProblem(
						"The variable " + varName.toString() + getWarning(element),
						node, getName(), SEVERITY.ERROR);
			}
		}
